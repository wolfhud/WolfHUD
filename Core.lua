if not _G.WolfHUD then
    ...
    WolfHUD.identifier = string.match(WolfHUD.mod_path,
                                      "[\\/]([%w_%-%.]+)[\\/]$") or "WolfHUD"
    ...
    function WolfHUD:Reset()
        ...
        for _, filename in pairs(file.GetFiles(self.mod_path .. "/loc/")) do
            ...
            if str and Idstring(str) and Idstring(str):key() ==
        ...
        WolfHUD.settings = {
            ...
            CustomHUD = {
                ...
                PLAYER = {
                    ...
                    WEAPON = {ICON = 4, NAME = 1, AMMO = 4, FIREMODE = 2},
                    KILLCOUNTER = {
                        HIDE = false,
                ...
                TEAMMATE = {
                    ...
                    CARRY = true,
                    CONDITION_ICON_COLOR = "white",
                    BUILD = {HIDE = false, DURATION = 15},
                    AI_COLOR = {USE = false, COLOR = "white"},
                    WEAPON = {ICON = 4, NAME = 1, AMMO = 4, FIREMODE = 1},
                    INTERACTION = {
            ...
            HUDList = {
                ...
                BUFF_LIST = {
                    ...
                    TECHNICIAN_BUFFS = {lock_n_load = true},
                    GHOST_BUFFS = {
                        dire_need = true,
                        second_wind = true,
                        sixth_sense = true,
    ...
    function WolfHUD:print_log(...)
        ...
        local params = {...}
        local msg_type, text = table.remove(params, #params),
        ...
        if msg_type and LOG_MODES[tostring(msg_type)] then
            if type(text) == "table" or type(text) == "userdata" then
                ...
                if not text[1] or type(text[1]) ~= "string" then
                    log(string.format("[WolfHUD] %s:",
                                      string.upper(type(msg_type))))
                    log_table(text)
                    return
                else
            ...
            text = string.format("[WolfHUD] %s: %s", string.upper(msg_type),
                                 text)
            log(text)
            if LOG_MODES.to_console and con and con.print and con.error then
                local t = Application:time()
                text = string.format("%02d:%06.3f\t>\t%s", math.floor(t / 60),
                                     t % 60, text)
    ...
    function WolfHUD:Load()
        ...
        if file then
            local function parse_settings(table_dst, table_src, setting_path)
                for k, v in pairs(table_src) do
                    if type(table_dst[k]) == type(v) then
                        ...
                        self:print_log(
                            "Error while loading, Setting types don't match (%s->%s)",
                            self:SafeTableConcat(setting_path, "->") or "",
                            k or "N/A", "error")
            ...
            self:print_log(
                "Error while loading, settings file could not be opened (" ..
                    self.settings_path .. ")", "error")
        end
        if corrupted then
            self:Save()
            self:print_log("Settings file appears to be corrupted, resaving...",
                           "error")
        end
    end
    ...
    function WolfHUD:Save()
        if table.size(self.settings or {}) > 0 then
            ...
            if file then
                ...
                self:print_log(
                    "Error while saving, settings file could not be opened (" ..
                        self.settings_path .. ")", "error")
            end
        else
            self:print_log(
                "Error while saving, settings table appears to be empty...",
                "error")
        end
    end
    ...
    function WolfHUD:getSetting(id_table, default)
        if type(id_table) == "table" then
            ...
            for i = 1, #id_table do
                ...
                if entry == nil then
                    self:print_log(
                        "Requested setting doesn't exists!  (id='" ..
                            self:SafeTableConcat(id_table, "->") .. "', type='" ..
                            (default and type(default) or "n/a") .. "') ",
                        "error")
                    return default
                end
            end
    ...
    function WolfHUD:getColorSetting(id_table, default, ...)
        ...
        return self:getColor(color_name, ...) or default and
                   self:getColor(default, ...)
    end
    ...
    function WolfHUD:getColorID(name)
        if self.tweak_data and type(name) == "string" then
            for i, data in ipairs(self:getTweakEntry("color_table", "table")) do
                if name == data.name then return i end
            end
        end
    end
        ...
        if self.tweak_data and type(name) == "string" then
            for i, data in ipairs(self:getTweakEntry("color_table", "table")) do
                if name == data.name then
                    return
                        data.color and Color(data.color) or data.color_func and
                            data.color_func(...) or nil
                end
            end
        end
    end
    ...
    function WolfHUD:getTweakEntry(id, val_type, default)
        ...
        if value ~= nil and (not val_type or type(value) == val_type) then
            ...
            self:print_log(
                "Requested tweak_entry doesn't exists!  (id='" .. id ..
                    "', type='" .. tostring(val_type) .. "') ", "error")
            return default
        end
    end
    ...
    function WolfHUD:getCharacterName(character_id, to_upper)
        ...
        if name_table then
            local level_id = managers.job and managers.job:current_level_id() or
                                 "default"
            local name_id = name_table[level_id] or name_table.default
            name = to_upper and managers.localization:to_upper_text(name_id) or
                       managers.localization:text(name_id)
        end
    ...
    function WolfHUD:truncateNameTag(name)
        local truncated_name = name:gsub('^%b[]', ''):gsub('^%b==', ''):gsub(
                                   '^%s*(.-)%s*$', '%1')
    ...
    if not WolfHUD.tweak_path then -- Populate tweak data
        local tweak_path = string.format("%s%s", WolfHUD.save_path,
                                         WolfHUD.tweak_file)
        if not io.file_is_readable(tweak_path) then
            tweak_path = string.format("%s%s", WolfHUD.mod_path,
                                       WolfHUD.tweak_file)
        end
        if io.file_is_readable(tweak_path) then
            ...
            WolfHUD:print_log(string.format(
                                  "Tweak Data file couldn't be found! (%s)",
                                  tweak_path), "error")
        end
    end
        ...
        WolfHUD.apply_settings_clbk = {
            ["CustomHUD"] = function(setting, value)
                if managers.hud and managers.hud.change_hud_setting then
                    ...
                    managers.hud:change_hud_setting(type, setting,
                                                    WolfHUD:getColor(value) or
                                                        value)
                end
            end,
            ["HUDList"] = function(setting, value)
                if managers.hud and HUDListManager and setting then
                    ...
                    if list == "BUFF_LIST" and category ~= "show_buffs" then
                        managers.hud:change_bufflist_setting(option,
                                                             WolfHUD:getColor(
                                                                 value) or value)
                    elseif list == "RIGHT_LIST" and category ==
                        "SHOW_PICKUP_CATEGORIES" then
                        managers.hud:change_pickuplist_setting(option,
                                                               WolfHUD:getColor(
                                                                   value) or
                                                                   value)
                    else
                        managers.hud:change_list_setting(option,
                                                         WolfHUD:getColor(value) or
                                                             value)
                    end
                end
            ...
            ["AssaultBanner"] = function(setting, value)
                if managers.hud and managers.hud.change_assaultbanner_setting then
                    managers.hud:change_assaultbanner_setting(
                        tostring(setting[#setting]), value)
                end
            end,
            ["TabStats"] = function(setting, value)
                if managers.hud and managers.hud.change_tabstats_setting then
                    managers.hud:change_tabstats_setting(
                        tostring(setting[#setting]), value)
                end
            end,
            ["DrivingHUD"] = function(setting, value)
                if managers.hud and managers.hud.change_drivinghud_setting then
                    managers.hud:change_drivinghud_setting(
                        tostring(setting[#setting]),
                        WolfHUD:getColor(value) or value)
                end
            end,
            ["GADGETS"] = function(setting, value)
                if WeaponGadgetBase and WeaponGadgetBase.update_theme_setting and
                    #setting >= 4 then
                    WeaponGadgetBase.update_theme_setting(setting[1],
                                                          setting[2],
                                                          setting[3],
                                                          setting[4],
                                                          WolfHUD:getColor(value) or
            ...
            ["MOD_OVERRIDES"] = function(setting, value)
                if FedInv then FedInv.loadAssetsCallback() end
            end
        }
    end

    if not WolfHUD:DirectoryExists(WolfHUD.assets_path) then
        WolfHUD:print_log("Folder '%s' doesn't exist, creating....\t%s",
                          WolfHUD.assets_path, tostring(
                              WolfHUD:createDirectory(WolfHUD.assets_path)),
                          "warining")
    end
    ...
    do -- Romove Disabled Updates, so they don't show up in the download manager.
        ...
        for i, update in pairs(mod:GetUpdates()) do
            if update:GetInstallFolder() ~= WolfHUD.identifier then
                local directory = Application:nice_path(
                                      update:GetInstallDirectory() .. "/" ..
                                          update:GetInstallFolder(), true)
    ...
    do
        ...
        Hooks:Add("MenuManagerSetupCustomMenus",
                  "MenuManagerSetupCustomMenus_WolfHUD",
            ...
            create_menu({menu_options}, BLT and
                            BLT.Mods.Constants:LuaModOptionsMenuID() or
                            "blt_options")
        end)

        -- Populate options menus
        Hooks:Add("MenuManagerPopulateCustomMenus",
                  "MenuManagerPopulateCustomMenus_WolfHUD",
                  function(menu_manager, nodes)
            -- Called on setting change
            local function change_setting(setting, value)
                if WolfHUD:getSetting(setting, nil) ~= value and
                    WolfHUD:setSetting(setting, value) then
                    WolfHUD:print_log(string.format("Change setting: %s = %s",
                                                    WolfHUD:SafeTableConcat(
                                                        setting, "->"),
                                                    tostring(value)), "info") -- Change type back!
            ...
            local function add_visible_reqs(menu_id, id, data)
                ...
                -- Add visual callback
                MenuCallbackHandler[visual_clbk_id] =
                    function(self, item)
                ...
                -- Add enable callback
                MenuCallbackHandler[enabled_clbk_id] =
                    function(self, item)
            ...
            MenuCallbackHandler[update_visible_clbks] =
                function(self, item)
                    ...
                    if gui_node then
                        ...
                        for _, row_item in pairs(gui_node.row_items) do
                            ...
                            if option_item._type ~= "divider" and
                                option_item:parameters().name ~=
                                item:parameters().name then
                                local enabled = true

                                for _, clbk in
                                    ipairs(
                                        option_item._enabled_callback_name_list or
                                            {}) do
                                    enabled =
                                        enabled and
                                            self[clbk](self, option_item)
                                end
            ...
            local create_item_handlers =
                {
                    ...
                    slider = function(menu_id, offset, data, value)
                        local id = string.format("%s_%s_slider", menu_id,
                                                 data.name_id)
                        ...
                        MenuHelper:AddSlider(
                            {
                                ...
                                title = data.name_id,
                                desc = data.desc_id,
                                callback = string.format("%s %s", clbk_id,
                                                         update_visible_clbks),
                                value = value or 0,
                                min = data.min_value,
                                max = data.max_value,
                                step = data.step_size,
                    ...
                    toggle = function(menu_id, offset, data, value)
                        local id = string.format("%s_%s_toggle", menu_id,
                                                 data.name_id)
                        local clbk_id = id .. "_clbk"
                        ...
                        MenuCallbackHandler[clbk_id] =
                            function(self, item)
                                local value =
                                    (item:value() == "on") and true or false
                    ...
                    multi_choice = function(menu_id, offset, data, value)
                        local id = string.format("%s_%s_multi", menu_id,
                                                 data.name_id)
                        local clbk_id = id .. "_clbk"
                        ...
                        if data.add_color_options then
                            ...
                            for i, item in pairs(menu._items_list) do
                                if item:parameters().name == id then
                                    ...
                                    for k, v in
                                        ipairs(
                                            WolfHUD:getTweakEntry("color_table",
                                                                  "table") or {}) do
                                        if data.add_rainbow or v.name ~=
                                            "rainbow" then
                                            local color_name =
                                                managers.localization:text(
                                                    "wolfhud_colors_" .. v.name)
                                            color_name =
                                                not color_name:lower():find(
                                                    "error") and color_name or
                                                    string.upper(v.name)
                                            local params =
                                                {
                                                    _meta = "option",
                                    ...
                                    for __, clbk in
                                        pairs(item:parameters().callback) do
                                        clbk(item)
                                    end
                                    break
                                end
                    ...
                    input = function(menu_id, offset, data)
                        local id = string.format("%s_%s_input", menu_id,
                                                 data.name_id)
                        local clbk_id = id .. "_clbk"
                    ...
                    button = function(menu_id, offset, data)
                        local id = string.format("%s_%s_button", menu_id,
                                                 data.name_id)
                        ...
                        MenuCallbackHandler[clbk_id] =
                            MenuCallbackHandler[clbk_id] or
                                function(self, item) end
                    ...
                    keybind = function(menu_id, offset, data)
                        local id = string.format("%s_%s_keybind", menu_id,
                                                 data.name_id)
                        local clbk_id = data.clbk or (id .. "_clbk")
                    ...
                    divider = function(menu_id, offset, data)
                        local id = string.format("%s_divider_%d", menu_id,
                                                 offset)

                        local item_data = {type = "MenuItemDivider"}
                        local params = {
                            name = id,
            ...
            local function populate_menu(menu_table, parent_id)
                ...
                for i, data in ipairs(menu_table) do
                    local value = data.value and
                                      WolfHUD:getSetting(data.value, nil)
                    create_item_handlers[data.type](data.parent_id or parent_id,
                                                    item_amount - i, data, value)
            ...
            populate_menu({menu_options}, BLT and
                              BLT.Mods.Constants:LuaModOptionsMenuID() or
                              "blt_options")
        end)
        ...
        Hooks:Add("MenuManagerBuildCustomMenus",
                  "MenuManagerBuildCustomMenus_WolfHUD",
                  function(menu_manager, nodes)
            ...
            -- Add focus callback
            MenuCallbackHandler[focus_clbk] =
                function(node, focus)
                    if focus then
                        for _, option_item in pairs(node._items) do
                            if option_item._type ~= "divider" then
                                ...
                                for _, clbk in
                                    ...
                                    enabled =
                                        enabled and
                                            MenuCallbackHandler[clbk](
                                                node.callback_handler,
                                                option_item)
                                end
            ...
            MenuCallbackHandler[reset_clbk] =
                function(self, item)
                    local menu_title = managers.localization:text(
                                           "wolfhud_reset_options_title")
                    local menu_message =
                        managers.localization:text(
                            "wolfhud_reset_options_confirm")
                    local menu_buttons =
                        {
                            [1] = {
                                ...
                                callback = function(self, item)
                                    ...
                                    for i, menu_id in ipairs(WolfHUD.menu_ids) do
                                        ...
                                        if menu then
                                            for __, menu_item in
                                                ipairs(menu._items_list) do
                                                local setting =
                                                    menu_item._create_data and
                                                        clone(
                                                            menu_item._create_data
                                                                .value)
                                                if menu_item.set_value and
                                                    setting then
                                                    local value =
                                                        WolfHUD:getSetting(
                                                            setting, nil)
                                                    if value ~= nil then
                                                        local script =
                                                            table.remove(
                                                                setting, 1)
                                                        if WolfHUD.apply_settings_clbk[script] then
                                                            WolfHUD.apply_settings_clbk[script](
                                                                setting, value)
                                                        end

                                                        if menu_item._type ==
                                                            "toggle" then
                                                            if menu_item._create_data
                                                                .invert_value then
                                                                value =
                                                                    not value
                                                            end
                                    ...
                                    WolfHUD.settings_changed = true
                                    WolfHUD:print_log("Settings resetted!",
                                                      "info")
                                end
                            },
                            [2] = {
            ...
            local function finalize_menu(menu_table, parent_id)
                for i, data in ipairs(menu_table) do
                    if data.type == "menu" then
                        nodes[data.menu_id] =
                            MenuHelper:BuildMenu(data.menu_id, {
                                back_callback = back_clbk,
                                focus_changed_callback = focus_clbk
                            })
                        MenuHelper:AddMenuItem(
                            nodes[data.parent_id or parent_id], data.menu_id,
                            data.name_id, data.desc_id, data.position or i)
            ...
            finalize_menu({menu_options}, BLT and
                              BLT.Mods.Constants:LuaModOptionsMenuID() or
                              "blt_options")
        end)
    end
    ...
    Hooks:Add("LocalizationManagerPostInit",
              "LocalizationManagerPostInit_WolfHUD", function(loc)
        local loc_path = WolfHUD.mod_path .. "loc/"
        if file.DirectoryExists(loc_path) then
            loc:load_localization_file(string.format("%s/%s.json", loc_path,
                                                     WolfHUD:getSetting(
                                                         {"LANGUAGE"}, "english")))
            loc:load_localization_file(
                string.format("%s/english.json", loc_path), false)

            if WolfHUD:getSetting({"INVENTORY", "USE_REAL_WEAPON_NAMES"}, false) then
                loc:load_localization_file(
                    string.format("%s/RealWeaponNames.json", loc_path))
            end
        else
            WolfHUD:print_log("Localization folder seems to be missing!",
                              "error")
        end

        local localized_strings = {}
        localized_strings["cash_sign"] =
            WolfHUD:getTweakEntry("CASH_SIGN", "string", "$")
        ...
        for interact_id, data in pairs(tweak_data.interaction) do
            if type(data) == "table" and data.text_id and not data.verify_owner then
                localized_strings[data.text_id] =
                    loc:text(data.text_id, {BTN_INTERACT = "$BTN_INTERACT"}) ..
                        "$VALUE"
            end
        end
    ...
    Hooks:Add("MenuManagerOnOpenMenu", "MenuManagerOnOpenMenu_WolfHUDCore",
              function(menu_manager, menu_name, position)
        if menu_name == "menu_main" then
            local mod = BLT and BLT.Mods:GetMod(WolfHUD.identifier or "")

            if mod and not mod.supermod then
                QuickMenu:new(managers.localization:text(
                                  "wolfhud_dialog_requires_superblt_title"),
                              managers.localization:text(
                                  "wolfhud_dialog_requires_superblt_desc_long",
                ...
                if BLT.Notifications then
                    local params = {
                        ...
                        text = managers.localization:text(
                            "wolfhud_dialog_requires_superblt_desc_short",
                            {URL = "superblt.znix.xyz"}),
            ...
            elseif mod:AreUpdatesEnabled() and WolfHUD.identifier ~=
                "WolfHUD-master" then
                if BLT.Notifications then
                    local params = {
                        title = managers.localization:text(
                            "wolfhud_dialog_update_install_folder_title"),
                        text = managers.localization:text(
                            "wolfhud_dialog_update_install_folder_desc",
                            {FOLDER = WolfHUD.identifier}),
                        icon = mod:GetModImage(),
                        -- icon_texture_rect = {0, 0, 1 , 1},
                        color = Color(1, 0.2, 0),
                        priority = 10
