local print_info = print_info or
                       function(...)
...
if RequiredScript == "lib/units/weapons/weapongadgetbase" then
    ...
    WeaponGadgetBase.SPAWNED_UNITS = {laser = {}, flashlight = {}}

    function WeaponGadgetBase.update_theme_setting(gadget, theme, feature,
                                                   setting, value)
    ...
    function WeaponGadgetBase:init(...)
        ...
        if WeaponGadgetBase.SPAWNED_UNITS[self.GADGET_TYPE] then
            WeaponGadgetBase.SPAWNED_UNITS[self.GADGET_TYPE][self._unit:key()] =
                self._unit
        end
    end
    ...
    function WeaponGadgetBase:set_owner_unit(owner, is_akimbo)
        if alive(owner) then
            print_info("Setting %s unit %s owner (slot %d)",
                       tostring(self.GADGET_TYPE), tostring(self._unit:key()),
                       owner:slot())
    ...
    function WeaponGadgetBase:refresh_themes() end
    ...
    function WeaponGadgetBase:get_theme(theme)
        if self._themes then
            if theme then
                ...
                return self._theme_type and self._themes[self._theme_type] or
                           self._themes.default
            end
        end
    end

    function WeaponGadgetBase:_set_theme(theme) end

    function WeaponGadgetBase:_modify_color(color, intensity) end
    ...
    function WeaponGadgetBase:_update_effects(data, t, dt)
        if data then
            ...
            if data.rainbow then
                ...
                color = Vector3((1 + math.sin(r + 0)) / 2,
                                (1 + math.sin(r + 120)) / 2,
                                (1 + math.sin(r + 240)) / 2)
            end
...
elseif RequiredScript == "lib/units/weapons/weaponlaser" then
    ...
    function WeaponLaser:color() -- OVERWRITE
        ...
        if theme and theme.brush then return Color(theme.brush:unpack()) end
    ...
    function WeaponLaser:set_color_by_theme(type)
        if not self._themes[type] then
            print_info(
                "ERROR (WeaponLaser:set_color_by_theme): Attempting to set missing theme %s",
                tostring(type))
        end
    ...
    function WeaponLaser:refresh_themes()
        for theme, data in pairs(WeaponGadgetBase.THEME_SETTINGS.laser or {}) do
            local beam = data.beam.enabled and
                             Vector3(data.beam.r, data.beam.g, data.beam.b) or
                             self._custom_color or
                             Vector3(
                                 tweak_data.custom_colors.defaults.laser:unpack())

            self._themes[theme] = {
                light = data.dot.match_beam and beam or
                    Vector3(data.dot.r, data.dot.g, data.dot.b),
                glow = data.glow.match_beam and beam or
                    Vector3(data.glow.r, data.glow.g, data.glow.b),
                brush = beam,
                alpha = {
                    dot = data.dot.a,
                    glow = data.glow.a,
    ...
    function WeaponLaser:_set_colors(light, glow, brush)
        local theme = self._themes[self._theme_type] or self._themes.default
        local alpha = theme and theme.alpha or
                          {dot = 1, glow = 0.02, beam = 0.15}

        mvector3.set(self._light_color,
                     (light or theme.light) * 10 * alpha.dot *
                         (self._current_intensity or 1))
        mvector3.set(self._light_glow_color, (glow or theme.glow) * 10 *
                         alpha.glow * (self._current_intensity or 1))
        self._brush_color = Color((brush or theme.brush):unpack()):with_alpha(
                                alpha.beam * (self._current_intensity or 1))
...
elseif RequiredScript == "lib/units/weapons/weaponflashlight" then
    ...
    function WeaponFlashLight:color() -- OVERWRITE
        ...
        if theme and theme.light then return Color(theme.light:unpack()) end
    end
    ...
    function WeaponFlashLight:refresh_themes()
        for theme, data in pairs(WeaponGadgetBase.THEME_SETTINGS.flashlight or
                                     {}) do
            self._themes[theme] = {
                brightness = data.light.brightness,
                light = data.light.enabled and
                    Vector3(data.light.r, data.light.g, data.light.b) or
                    self._custom_color or
                    Vector3(
                        tweak_data.custom_colors.defaults.flashlight:unpack()),
    ...
    function WeaponFlashLight:_set_colors(light)
        ...
        if light then
            ...
            local light_color = Color(light.x, light.y, light.z) *
                                    (theme.brightness or 1) *
                                    (self._current_intensity or 1) *
                                    (self._intensity_modifier or 1)
    ...
    function RaycastWeaponBase:_update_gadget_owner(gadgets)
        if gadgets then
            for _, id in ipairs(gadgets) do
                local base = self._parts[id] and alive(self._parts[id].unit) and
                                 self._parts[id].unit:base()
...
elseif RequiredScript == "lib/units/weapons/newnpcraycastweaponbase" then
    ...
    function NewNPCRaycastWeaponBase:setup(...)
        setup_original(self, ...)
        self:_update_gadget_owner(
            managers.weapon_factory:get_parts_from_weapon_by_type_or_perk(
                "gadget", self._factory_id, self._blueprint))
    end
...
if string.lower(RequiredScript) == "lib/units/weapons/newraycastweaponbase" then
    function NewRaycastWeaponBase:_setup_laser()
        if self._gadgets then
            for i, part_id in ipairs(self._gadgets) do
                ...
                if base and base.GADGET_TYPE and base.GADGET_TYPE ==
                    (WeaponLaser.GADGET_TYPE or "") then
                    ...
                    if owner then
                        ...
                        if owner and owner:network() then -- Sync gadget state and laser color to other players
                            owner:network():send("set_weapon_gadget_state",
                                                 self._gadget_on)
                            if base.color then
                                local col = base:color() or Color(1, 0, 1, 0)
                                owner:network():send("set_weapon_gadget_color",
                                                     col.r * 255, col.g * 255,
                                                     col.b * 255)
                            end
                        end
    ...
    function NewRaycastWeaponBase:on_enabled(...)
        ...
        if not self._init_laser_state and not self:is_npc() and
            self._assembly_complete and managers.player:current_state() ==
            "standard" and
            WolfHUD:getSetting({"GADGETS", "LASER_AUTO_ON"}, true) then
            self:_setup_laser()
...
if string.lower(RequiredScript) == "lib/units/cameras/fpcameraplayerbase" then
    ...
    function FPCameraPlayerBase:clbk_stance_entered(new_shoulder_stance,
                                                    new_head_stance,
                                                    new_vel_overshot, new_fov,
                                                    new_shakers, stance_mod, ...)
        if self._want_rotated then
            ...
            stance_mod = {
                rotation = FPCameraPlayerBase.angled_sight_rotation[self._sight_id] or
                    Rotation(0, 0, -45),
                translation = FPCameraPlayerBase.angled_sight_translation[self._weapon_name] or
                    Vector3(-13, 7, -12)
            }
        ...
        return clbk_stance_entered_original(self, new_shoulder_stance,
                                            new_head_stance, new_vel_overshot,
                                            new_fov, new_shakers, stance_mod,
                                            ...)
    end
...
elseif string.lower(RequiredScript) ==
    ...
    local _check_action_weapon_gadget_original =
        PlayerStandard._check_action_weapon_gadget
    ...
    function PlayerStandard:_stance_entered(...)
        ...
        local sight_id = weapon_base and weapon_base._second_sight_data and
                             weapon_base._second_sight_data.part_id
        local rotate_weapon = WolfHUD:getSetting(
                                  {"GADGETS", "SHOW_ANGELED_SIGHT"}, true) and
                                  sight_id and
                                  PlayerStandard.ANGELED_SIGHTS[sight_id]
        self._camera_unit:base():set_want_rotated(
            not self._state_data.in_steelsight and
                self._equipped_unit:base():is_second_sight_on() and
                not self:_is_reloading() and rotate_weapon)
        self._camera_unit:base():set_want_restored(
            not self._state_data.in_steelsight and
                (not self._equipped_unit:base():is_second_sight_on() or
                    self:_is_reloading()) and rotate_weapon)
        self._camera_unit:base():set_weapon_name(
            weapon_base and weapon_base._name_id, sight_id)
    ...
    function PlayerStandard:_check_action_weapon_gadget(t, input, ...)
        ...
        local sight_id = weapon_base and weapon_base._second_sight_data and
                             weapon_base._second_sight_data.part_id
        if input.btn_weapon_gadget_press and sight_id and
            PlayerStandard.ANGELED_SIGHTS[sight_id] then
