...
function HUDSuspicion:_is_detected()
    local detected_text = self._suspicion_panel and
                              self._suspicion_panel:child("suspicion_detected")
    return
        self._discovered or self._suspicion_value and self._suspicion_value >= 1 or
            detected_text and detected_text:visible() and detected_text:alpha() >
            0
end
...
function HUDSuspicion:_animate_text(suspicion_panel, suspicion_text)
    while true do
        local detection = self:_is_detected() and 1 or self._suspicion_value or
                              0
...
function HUDSuspicion:animate_eye(...)
    ...
    if not was_animating and self._eye_animation then
        ...
        if WolfHUD:getSetting({"HUDSuspicion", "SHOW_PERCENTAGE"}, true) and
            not self._text_animation then
            self._suspicion_text:set_outlines_visible(
                WolfHUD:getSetting({"HUDSuspicion", "SHOW_PERCENTAGE_OUTLINE"},
                                   true))
            self._text_animation = self._suspicion_text_panel:animate(
                                       callback(self, self, "_animate_text"),
...
function HUDSuspicion:hide(...)
    ...
    if self._suspicion_panel then self._suspicion_panel:set_visible(false) end
...
function HUDSuspicion:rescale()
    ...
    if self._scale ~= scale then
        ...
        local hud_stealthmeter_bg =
            self._misc_panel:child("hud_stealthmeter_bg")
        local suspicion_detected = self._suspicion_panel:child(
                                       "suspicion_detected")
        ...
        hud_stealthmeter_bg:set_size((hud_stealthmeter_bg:w() / self._scale) *
                                         scale, (hud_stealthmeter_bg:h() /
                                         self._scale) * scale)
        suspicion_detected:set_font_size(
            (suspicion_detected:font_size() / self._scale) * scale)
        local fontSize = (self._suspicion_text:font_size() / self._scale) *
                             scale
        ...
        hud_stealth_exclam:set_size((hud_stealth_exclam:w() / self._scale) *
                                        scale, (hud_stealth_exclam:h() /
                                        self._scale) * scale)
        ...
        hud_stealth_eye:set_center(suspicion_left:center_x(),
                                   suspicion_left:bottom() - 4)
        hud_stealth_exclam:set_center(suspicion_left:center_x(),
                                      suspicion_left:top() - 4)
        self._suspicion_text:set_y(suspicion_left:top() +
                                       (suspicion_left:center_y() -
                                           suspicion_left:top()) / 2 -
