...
if RequiredScript == "lib/managers/menumanager" then
    local function create_profile_menu_node(nodes, menu_id)
        local arugements = {
            ...
            menu_components = nodes.kit and "mission_briefing" or
                managers.menu:is_pc_controller() and "inventory" or "",
            ...
            {["_meta"] = "legend", ["name"] = "menu_legend_select"},
            {["_meta"] = "legend", ["name"] = "menu_legend_back"}
        }
        ...
        if node_class then
            ...
            local callback_handler = CoreSerialize.string_to_classtable(
                                         "MenuCallbackHandler")
    ...
    Hooks:Add("MenuManagerBuildCustomMenus",
              "WolfHUD_MenuManager_BuildProfileMenu",
    ...
    local LobbyOptionInitiator_modify_node_orig =
        LobbyOptionInitiator.modify_node
    function LobbyOptionInitiator:modify_node(node, ...)
        local active_menu = managers.menu:active_menu()
        local briefing_nodes = active_menu and active_menu.logic and
                                   active_menu.logic._data._nodes
        if briefing_nodes and briefing_nodes.kit and
            not briefing_nodes[PROFILE_MENU_ID] then
            create_profile_menu_node(briefing_nodes, PROFILE_MENU_ID, true)
        end
        return LobbyOptionInitiator_modify_node_orig(self, node, ...)
    end

    ProfileSwitchInitiator = ProfileSwitchInitiator or
                                 class(SkillSwitchInitiator)
    function ProfileSwitchInitiator:modify_node(node, data)
        node:clean_items()
        self:create_divider(node, "title",
                            "wolfhud_profile_switch_title_profiles", nil,
...
elseif RequiredScript == "lib/managers/menu/renderers/menunodeskillswitchgui" then
    MenuNodeProfileSwitchGui = MenuNodeProfileSwitchGui or
                                   class(MenuNodeSkillSwitchGui)
    ...
    function MenuNodeProfileSwitchGui:_setup_item_panel(...)
        ...
        if alive(self.title_text) then
            self.title_text:set_text(managers.localization:to_upper_text(
                                         "wolfhud_profile_switch_title"))
        end
        ...
        if alive(self.item_panel) and LoadoutPanel then
            ...
            self.profile_preview = LoadoutPanel:new(self.item_panel:parent(),
                                                    self, 0,
                                                    self.PROFILE_PREVIEW_W,
                                                    math.floor(
                                                        self.item_panel:h()) -
                                                        offset_h, {
                component_layout = {
                    {"skills"}, {"perk"}, {"primary", "secondary"},
                    {"grenade", "armor"}, {"deployable", "secondary_deployable"}
                },
                default = {alpha = 0.9},
                margin = 5,
                borders = {1, 1, 1, 1},
            ...
            local outfit = managers.multi_profile:get_profile_outfit(
                               managers.multi_profile:current_profile_id())
    ...
    function MenuNodeProfileSwitchGui:_create_menu_item(row_item, ...)
        ...
        if row_item.type ~= "divider" and row_item.name ~= "back" then
            ...
            skill_name = profile.skillset and
                             managers.skilltree:get_skill_switch_name(
                                 profile.skillset, false) or
                             managers.localization:to_upper_text(
                                 "menu_st_locked_skill_switch")
            if profile.perk_deck then
                local data = tweak_data.skilltree.specializations[tonumber(
                                 profile.perk_deck)]
        ...
        elseif row_item.type == "divider" and row_item.name == "divider_title" then
            ...
            if alive(row_item.skill_points_gui) then
                row_item.skill_points_gui:set_text(
                    string.format("%s-%s", managers.localization:to_upper_text(
                                      "menu_st_skilltree", {}),
                                  managers.localization:to_upper_text(
                                      "menu_st_skill_switch_title_name", {})))
    ...
    function MenuNodeProfileSwitchGui:_highlight_row_item(row_item, mouse_over, ...)
        MenuNodeProfileSwitchGui.super._highlight_row_item(self, row_item,
                                                           mouse_over, ...)

        local mpm = managers.multi_profile
        local profile_id = row_item.name and mpm:is_valid_id(row_item.name) and
                               row_item.name or mpm:current_profile_id()
    ...
    function MenuNodeProfileSwitchGui:_align_marker(row_item, ...)
        ...
        if self.profile_preview then
            if row_item.name ~= "back" then
                self._marker_data.marker:set_w(
                    self.item_panel:w() - self.profile_preview:w() - 10)
            else
    ...
    function MenuNodeProfileSwitchGui:_clear_gui(...)
        if self.profile_preview then self.profile_preview:destroy() end

        MenuNodeProfileSwitchGui.super._clear_gui(self, ...)
    end

    function MenuNodeProfileSwitchGui:mouse_moved(o, x, y, ...)
        local used, icon = MenuNodeProfileSwitchGui.super.mouse_moved(self, o,
                                                                      x, y, ...)
        return true, icon
    end
    ...
    function MenuNodeProfileSwitchGui:change_outfit_preview(profile_id)
        local mpm = managers.multi_profile
        if self.profile_preview and mpm:is_valid_id(profile_id) and
            (self._previewing_outfit or 0) ~= profile_id then
...
elseif RequiredScript == "lib/managers/multiprofilemanager" then
    ...
    function MultiProfileManager:is_valid_id(profile_id)
        return
            profile_id and type(profile_id) == "number" and profile_id > 0 and
                profile_id <= self:profile_count() and true or false
    end
    ...
    function MultiProfileManager:get_profile_outfit(profile_id)
        if profile_id ~= self:current_profile_id() then
            ...
            local outfit = {
                ...
                player_style = profile.player_style or
                    managers.blackmarket:get_default_player_style(),
                suit_variations = (profile.suit_variations or {}),
                deployable = profile.deployable,
                secondary_deployable = profile.deployable_secondary,
                deployable_amount = self:get_deployable_amount(
                    profile.deployable, gd.skills),
                secondary_deployable_amount = self:get_deployable_amount(
                    profile.deployable_secondary, gd.skills)
            }
            return outfit
        else
    ...
    function MultiProfileManager:get_deployable_amount(deployable, skills)
        ...
        if deployable then
            ...
            amount = tweak_data.equipments[deployable] and
                         tweak_data.equipments[deployable].quantity[1] or 0
            local upgrade_def, upgrade_values, skill_tweak =
                tweak_data.upgrades.definitions, tweak_data.upgrades.values,
                tweak_data.skilltree.skills
            if skills and upgrade_values[deployable] and
                upgrade_values[deployable]["quantity"] then
                local value_index = 0
                for skill, data in pairs(skills) do
                    local skill_data = skill_tweak[skill]
                    for i = 1, data.unlocked do
                        local upgrade_ids =
                            skill_data and skill_data[i].upgrades
                        for _, upgrade_id in ipairs(upgrade_ids) do
                            local u_data =
                                upgrade_def[upgrade_id] and
                                    upgrade_def[upgrade_id].upgrade
                            if u_data and u_data.category == deployable and
                                u_data.upgrade == "quantity" then
                                if upgrade_def[upgrade_id].incremental then
                                    value_index =
                                        value_index + (u_data.value or 0)
                                else
                ...
                if value_index > 0 then
                    amount = amount +
                                 (upgrade_values[deployable]["quantity"][math.floor(
                                     value_index)] or 0)
                end
            end
        end
...
elseif RequiredScript == "lib/managers/menu/multiprofileitemgui" then
    ...
    function MultiProfileItemGui:init(...)
        ...
        self._max_length = WolfHUD:getTweakEntry("MAX_PROFILE_NAME_LENGTH",
                                                 "number", 20)
    end
elseif RequiredScript == "lib/managers/menu/missionbriefinggui" then
    ...
    function MissionBriefingGui:special_btn_pressed(button, ...)
        if self._enabled and button == Idstring("menu_change_profile_right") and
            managers.menu:get_controller():get_input_bool(
                "menu_change_profile_left") then
            managers.menu:open_node(PROFILE_MENU_ID, {})
            return
        end
    ...
    function MissionBriefingGui:input_focus(...)
        ...
        if focus then
            local active_menu = managers.menu:active_menu()
            local selected_node = active_menu and
                                      active_menu.logic:selected_node()
            if selected_node and selected_node:parameters().name ==
                PROFILE_MENU_ID then focus = false end
        end
        return focus
    end
    ...
    if CoreClass then CoreClass.override_class(JukeboxItem, JukeboxItemNew) end
elseif RequiredScript == "lib/managers/menu/playerinventorygui" then
    ...
    function PlayerInventoryGui:special_btn_pressed(button, ...)
        if button == Idstring("menu_change_profile_right") and
            managers.menu:get_controller():get_input_bool(
                "menu_change_profile_left") then
            managers.menu:open_node(PROFILE_MENU_ID, {})
            return
