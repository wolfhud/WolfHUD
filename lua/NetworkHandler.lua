...
if WolfHUD and not WolfHUD.Sync then
    ...
    function WolfHUD.Sync.string_to_table(str)
        ...
        for k, v in pairs(tbl) do tbl[k] = self.to_original_type(v) end
    ...
    function WolfHUD.Sync:send_to_peer(peer_id, messageType, data)
        if peer_id and peer_id ~= Net:LocalPeerID() and messageType then
            local tags = {id = self.msg_id, event = messageType}
    ...
    function WolfHUD.Sync:send_to_host(messageType, data)
        self:send_to_peer(managers.network:session():server_peer():id(),
                          messageType, data)
    end
    ...
    function WolfHUD.Sync:receive_message(peer_id, event, data)
        if peer_id and event then
            ...
            if event == events.discover_wolfhud then
                ...
                managers.chat:feed_system_message(ChatManager.GAME, "Client " ..
                                                      tostring(peer_id) ..
                                                      " is using WolfHUD ;)") -- TEST
            elseif event == events.confirm_wolfhud then
                WolfHUD.Sync.peers[peer_id] = true
                managers.chat:feed_system_message(ChatManager.GAME,
                                                  "The Host is using WolfHUD ;)") -- TEST
    ...
    Hooks:Add("NetworkReceivedData", "NetworkReceivedData_WolfHUDSync",
              function(sender, messageType, data)
        sender = tonumber(sender)
        if sender then
            local tags = WolfHUD.Sync.string_to_table(messageType)
            if WolfHUD.Sync and tags.id and tags.id == WolfHUD.Sync.msg_id and
                not string.is_nil_or_empty(tags.event) then
                if tags.table then
    ...
    Hooks:Add("BaseNetworkSessionOnPeerRemoved",
              "BaseNetworkSessionOnPeerRemoved_WolfHUDSync",
              function(self, peer, peer_id, reason)
        WolfHUD.Sync:receive_message(peer_id,
                                     WolfHUD.Sync.events.peer_disconnect, "")
    end)

    Hooks:Add("BaseNetworkSessionOnLoadComplete",
              "BaseNetworkSessionOnLoadComplete_WolfHUDSync",
    ...
    function WolfHUD.Sync:endless_assault_status(status)
        if Network:is_server() then
            self:send_to_all_discovered_peers(
                WolfHUD.Sync.events.locked_assault_status, tostring(status))
        end
    end
end
