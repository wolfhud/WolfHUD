if string.lower(RequiredScript) == "lib/managers/hud/newhudstatsscreen" then
    ...
    function HUDStatsScreen:recreate_left(...)
        if WolfHUD:getSetting({"TabStats", "ENABLED"}, true) then
            ...
            for i, data in pairs(managers.objectives:get_active_objectives()) do
                ...
                placer:new_row(8)
                local item = placer:add_bottom(
                                 self._left:fine_text(
                                     {
                            word_wrap = true,
                            wrap = true,
            ...
            local ext_inv_panel = ExtendedPanel:new(self._left, {
                x = self._leftpos[2],
                y = self._left:h() - self._leftpos[2] - medium_font_size * 2 -
                    10,
                w = self._left:w() - 2 * self._leftpos[2],
                h = medium_font_size * 2 + 5
            })

            local body_text = placer:add_row(
                                  ext_inv_panel:fine_text(
                                      {
                        keep_w = true,
            ...
            local body_texture, body_rect =
                tweak_data.hud_icons:get_icon_data("equipment_body_bag")
            local body_icon = placer:add_left(
                                  ext_inv_panel:fit_bitmap(
                                      {
                        w = 17,
                        h = 17,
            ...
            local track_text = placer:add_bottom(
                                   ext_inv_panel:fine_text(
                                       {
                        text = managers.localization:to_upper_text(
                            "menu_es_playing_track") .. " " ..
            ...
            local list_panel = ExtendedPanel:new(self._left, {
                y = y + self._leftpos[2],
                w = self._left:w(),
                h = self._left:h() - y - ext_inv_panel:h() - 2 *
                    self._leftpos[2]
            })
    ...
    function HUDStatsScreen:recreate_right(...)
        if WolfHUD:getSetting({"TabStats", "ENABLED"}, true) then
            ...
            local stats_panel = ExtendedPanel:new(self._right, {
                w = self._right:w(),
                h = self._right:h()
            })
    ...
    function HUDStatsScreen:_create_clock(panel)
        local clock_panel = ExtendedPanel:new(panel, {
            w = panel:w() * 0.5,
            h = tweak_data.hud_stats.objectives_font
        })
        local placer = UiPlacer:new(0, 0)
    ...
    function HUDStatsScreen:_create_sidejobs_list(panel, ...)
        ...
        for _, category in pairs(categories) do
            ...
            if challenges and table.size(challenges or {}) >
                (challenges.category_id and 1 or 0) then
                placer:new_row(10, 8)
                local t = placer:add_bottom(
                              panel:fine_text(
                                  {
                            wrap = true,
                            word_wrap = true,
                            text_id = challenges.category_id or
                                string.format("menu_challenge_div_cat_%s",
                                              category),
                ...
                if t:bottom() > panel:h() * 0.85 then return end

                local with_bg = true
                for _, challenge in pairs(challenges) do
                    if type(challenge) == "table" and
                        self:is_challenge_completable(challenge) then
                        local s = placer:add_row(
                                      HudSidejob:new(panel, challenge.data,
                                                     with_bg), 0, 0)
    ...
    function HUDStatsScreen:get_active_challenges()
        local categories = {"safehouse_daily", "daily", "weekly", "monthly"}

        local challenges = {
            safehouse_daily = {category_id = "menu_cs_div_safehouse_daily"}
        }
        ...
        if not managers.custom_safehouse:has_rewarded_daily(current_daily.id) then
            local daily_data = deep_clone(
                                   tweak_data.safehouse:get_daily_data(
                                       current_daily.id))
            daily_data.name_id = daily_data.id
            daily_data.category = "safehouse_daily"
            daily_data.completed =
                managers.custom_safehouse:has_completed_daily(current_daily.id)
            daily_data.rewarded = false
            daily_data.timestamp = current_daily.timestamp
            daily_data.interval =
                managers.custom_safehouse:interval_til_new_daily()
            if current_daily.trophy and current_daily.trophy.objectives then
                daily_data.objectives = deep_clone(
                                            current_daily.trophy.objectives) -- Fix missing objective progress for safehouse daily
            end
        ...
        if managers.dlc:has_dlc("tango") then
            ...
            for _, tango_data in ipairs(managers.tango:challenges() or {}) do
                if not tango_data.rewarded then
                    ...
                    data.category = "tango"
                    local tango_challenge = {id = tango_data.id, data = data}
        ...
        for _, challenge in
            pairs(managers.challenge:get_all_active_challenges()) do
    ...
    function HUDStatsScreen:is_challenge_completable(challenge)
        ...
        for _, obj_data in ipairs(objectives or {}) do
            if obj_data and not obj_data.completed then
                ...
                if (obj_data.achievement_id or obj_data.progress_id) and
                    managers.custom_safehouse:can_progress_trophies(
                        obj_data.achievement_id or obj_data.progress_id) then
                    -- Try to find requirements for objective
                    for category, data in pairs(tweak_data.achievement) do
                        if type(data) == "table" and category ~= "visual" and
                            data[obj_data.achievement_id or obj_data.progress_id] then
                            requirements =
                                data[obj_data.achievement_id or
                                    obj_data.progress_id]
                            break
                        end
                    end
                end
                ...
                if requirements and table.size(requirements) > 0 then
                    ...
                    if (requirements.need_full_job or not requirements.is_dropin) and
                        managers.statistics:is_dropin() then
                        is_completeable = false
                    end
                    if is_completeable and requirements.crime_spree and
                        managers.crime_spree and
                        not managers.crime_spree:is_active() then
                        if type(requirements.crime_spree) ~= "number" or
                            requirements.crime_spree >
                            managers.crime_spree:spree_level() then
                            is_completeable = false
                        end
                    end
                    if is_completeable and requirements.difficulty then
                        local difficulty_stars =
                            managers.job:current_difficulty_stars()
                        local difficulty =
                            tweak_data.difficulties[difficulty_stars + 2] or
                                "unknown"
                        if not table.contains(requirements.difficulty,
                                              difficulty) then
                            is_completeable = false
                        end
                    end
                    if is_completeable and
                        (requirements.job or requirements.jobs) then
                        local job_id = managers.job:current_job_id()
                        if type(requirements.job) == "string" and job_id ~=
                            requirements.job or
                            not table.contains(requirements.jobs or {}, job_id) then
                            is_completeable = false
                        end
                    end

                    if is_completeable and
                        not (managers.challenge:check_equipped_team(requirements) and
                            managers.challenge:check_equipped(requirements)) then
                        is_completeable = false
                    end
                end
    ...
    HUDStatsScreen.STAT_ITEMS = {
        {
            name = "accuracy",
            text_id = "victory_hit_accuracy",
            color = Color.white,
            update = {
                func = "session_hit_accuracy",
                func_alltime = "",
                params = {},
                suffix = "%"
            },
            ...
            name = "total_damage",
            text_id = "wolfhud_tabstats_total_damage",
            color = Color(1, 0.69, 0.19, 0.38),
            update = {func = "session_damage", func_alltime = "", params = {}},
            no_alltime = true
        }, {
            name = "tanks_killed",
            text_id = "wolfhud_tabstats_tanks_killed",
            color = Color.red,
            update = {
                func = "session_total_tanks_killed",
                func_alltime = "total_tanks_killed",
                params = {}
            }
        }, {
            name = "cloakers_killed",
            text_id = "wolfhud_tabstats_cloakers_killed",
            color = Color.green,
            update = {
                func = "session_enemy_killed_by_type",
                func_alltime = "enemy_killed_by_type",
                params = {"spooc", "count"}
            }
        }, {
            name = "shields_killed",
            text_id = "wolfhud_tabstats_shields_killed",
            color = Color.yellow,
            update = {
                func = "session_enemy_killed_by_type",
                func_alltime = "enemy_killed_by_type",
            ...
            name = "snipers_killed",
            text_id = "wolfhud_tabstats_snipers_killed",
            ...
            name = "tasers_killed",
            text_id = "wolfhud_tabstats_tasers_killed",
            ...
            name = "medic_killed",
            text_id = "wolfhud_tabstats_medic_killed",
            color = Color(1, 1, 0.55, 0.24),
            update = {
                func = "session_enemy_killed_by_type",
                func_alltime = "enemy_killed_by_type",
                params = {"medic", "count"}
            ...
            name = "gensec_killed",
            text_id = "wolfhud_tabstats_gensec_killed",
            ...
            name = "melee_killed",
            text_id = "wolfhud_tabstats_melee_kills",
            ...
            name = "explosion_killed",
            text_id = "wolfhud_tabstats_explosion_kills",
            color = Color(1, 1, 0.5, 0),
            update = {
                func = "session_enemy_killed_by_type",
                func_alltime = "enemy_killed_by_type",
                params = {"total", "explosion"}
            }
        }, {
            name = "nonspecials_killed",
            text_id = "wolfhud_tabstats_nonspecial_kills",
            color = Color(1, 0.78, 0.15, 0.21),
            update = {
                func = "session_enemy_killed_by_type",
                func_alltime = "enemy_killed_by_type",
                params = {"non_special", "count"}
            }
        }, {
            name = "total_killed",
            text_id = "wolfhud_tabstats_total_kills",
            color = Color(1, 0.5, 0.5, 0.21),
            update = {
                func = "session_enemy_killed_by_type",
                func_alltime = "enemy_killed_by_type",
                params = {"allbois", "count"}
            }
        }, {
            name = "total_downs",
            text_id = "victory_total_downed",
            color = Color(1, 0.5, 0.5, 0.5),
            update = {
                func = "total_downed",
                func_alltime = "total_downed_alltime",
                params = {}
            }
        }, {
            name = "total_revives",
            text_id = "wolfhud_tabstats_total_revives",
            color = Color(1, 1, 0, 0.4),
            update = {
                func = "session_total_revives",
                func_alltime = "total_revives",
                params = {}
            }
        }
    }
    ...
    function HUDStatsScreen.getMaskImage()
        local outfit = managers.blackmarket:unpack_outfit_from_string(
                           managers.blackmarket:outfit_string())
        local mask_id = outfit and outfit.mask and outfit.mask.mask_id or
                            "alienware"

        local mask_icon = "guis/textures/pd2/blackmarket/icons/masks/alienware"
        if mask_id then
            local guis_catalog = "guis/"
            local bundle_folder = tweak_data.blackmarket.masks[mask_id] and
                                      tweak_data.blackmarket.masks[mask_id]
                                          .texture_bundle_folder
            if bundle_folder then
                guis_catalog = guis_catalog .. "dlcs/" ..
                                   tostring(bundle_folder) .. "/"
            end
            mask_icon = tweak_data.blackmarket.masks[mask_id].custom_texture or
                            guis_catalog ..
                            "textures/pd2/blackmarket/icons/masks/" .. mask_id
        end
    ...
    function HUDStatsScreen:_create_stat_list(panel, ...)
        ...
        if is_crime_spree then
            ...
            local mission = managers.crime_spree:get_mission(
                                managers.crime_spree:current_played_mission())

            if mission then
                local level_str = managers.localization:to_upper_text(
                                      tweak_data.levels[mission.level.level_id]
                                          .name_id) or ""
                ...
                placer:add_right(panel:fine_text(
                        ...
                        font = large_font,
                        font_size = tweak_data.hud_stats.objectives_title_size,
                        text = string.format("+%s", managers.localization:text(
                                                 "menu_cs_level",
            ...
            difficulty_text = managers.localization:text("menu_cs_level", {
                level = managers.experience:cash_string(
                    managers.crime_spree:server_spree_level(), "")
            })
            difficulty_color = tweak_data.screen_colors.crime_spree_risk
        elseif is_skirmish then
            local day_title = placer:add_bottom(
                                  panel:fine_text(
                                      {
                        font = tweak_data.hud_stats.objectives_font,
                        font_size = tweak_data.hud_stats.loot_size,
                        text = managers.localization:to_upper_text(
                            managers.skirmish:is_weekly_skirmish() and
                                "hud_weekly_skirmish" or "hud_skirmish"),
            ...
            if level_data then
                placer:add_bottom(panel:fine_text(
                        ...
                        font_size = tweak_data.hud_stats.objectives_title_size,
                        text = managers.localization:to_upper_text(
                            tostring(level_data.name_id)) or "Unknown"
                    }))
            end

            difficulty_text = managers.localization:to_upper_text(
                                  "hud_assault_waves", {
                    current = managers.skirmish:current_wave_number(),
                    max = #tweak_data.skirmish.ransom_amounts
                })
            difficulty_color = tweak_data.screen_colors.skirmish_color
        else
            local job_chain = managers.job:current_job_chain_data()
            local is_ghostable = managers.job:is_level_ghostable(
            ...
            local day_title = placer:add_bottom(
                        ...
                        font_size = tweak_data.hud_stats.loot_size,
                        text = managers.localization:to_upper_text(
                            "hud_days_title", {DAY = day, DAYS = days})
                    }))

            if managers.job:is_level_ghostable(managers.job:current_level_id()) then
                local is_whisper_mode = managers.groupai and
                                            managers.groupai:state()
                                                :whisper_mode()
                local ghost_color = is_whisper_mode and Color.white or
                                        tweak_data.screen_colors.important_1
                local ghost = placer:add_right(
                                  panel:bitmap(
                                      {
            ...
            if level_data then
                placer:add_bottom(panel:fine_text(
                        ...
                        text = managers.localization:to_upper_text(
                            tostring(level_data.name_id)) or "Unknown",
                        color = (managers.job:is_current_job_professional() and
                            tweak_data.screen_colors.important_1 or
                            tweak_data.screen_colors.text)
                    }))
            end

            local difficulty_stars = managers.job:current_difficulty_stars()
            local difficulty = tweak_data.difficulties[difficulty_stars + 2] or
                                   1
            local difficulty_string_id =
                tweak_data.difficulty_name_ids[difficulty]
            difficulty_text = managers.localization:to_upper_text(
                                  difficulty_string_id)
            difficulty_color = Global.game_settings.one_down and
                                   tweak_data.screen_colors.one_down or
                                   difficulty_stars > 0 and
                                   tweak_data.screen_colors.risk or
                                   tweak_data.screen_colors.text
        end
        ...
        if self._tabstats_settings.SHOW_MASK then
            ...
            local char_data =
                HUDStatsScreen.CHARACTERS[managers.criminals:local_character_name()]
            if char_data then
                mask_icon = char_data.texture or mask_icon
                mask_color =
                    WolfHUD:getSetting({"TabStats", "COLOR"}, "rainbow") ==
                        "rainbow" and char_data.color or mask_color
            end
        end
        ...
        placer:new_row(0, 12)
        local _, paygrade_text = self:_create_stat_list_entry(placer, panel,
                                                              self._rightpos[2],
                                                              list_w,
                                                              "paygrade", {
            text = managers.localization:to_upper_text(
                "menu_lobby_difficulty_title"),
            font_size = self._tabstats_settings.FONT_SIZE or 18
        }, {
        ...
        placer:new_row()
        self:_create_stat_list_entry(placer, panel, self._rightpos[2], list_w,
                                     "offshore_payout", {
        ...
        if not is_skirmish then
            placer:new_row()
            self:_create_stat_list_entry(placer, panel, self._rightpos[2],
                                         list_w, "cleaner_costs", {
                text = managers.localization:to_upper_text(
                    "victory_civilians_killed_penalty"),
                font_size = self._tabstats_settings.FONT_SIZE or 18
            }, {
                text = difficulty_text,
                font_size = self._tabstats_settings.FONT_SIZE or 18
        ...
        if not (is_crime_spree or is_skirmish) then
            ...
            self:_create_stat_list_entry(placer, panel, self._rightpos[2],
                                         list_w, "bag_cash", {
                text = utf8.to_upper("Secured Bags value:"),
                font_size = self._tabstats_settings.FONT_SIZE or 18
            }, {
                text = difficulty_text,
                font_size = self._tabstats_settings.FONT_SIZE or 18
        ...
        local items_color = self._tabstats_settings.COLOR ~= "rainbow" and
                                WolfHUD:getColor(
                                    self._tabstats_settings.COLOR or "red") or
                                false
        for i, data in ipairs(HUDStatsScreen.STAT_ITEMS) do
            ...
            local text_params = {
                color = color,
                font_size = self._tabstats_settings.FONT_SIZE or 18,
                text = string.format("0%s", data.suffix or "")
            }
            self:_create_stat_list_entry(placer, panel, self._rightpos[2],
                                         (data.no_alltime and small_list_w or
                                             list_w), name, title_params,
                                         text_params, not data.no_alltime and
                                             text_params or nil, nil)
            placer:new_row()
        end
    ...
    local add_bg = false
    function HUDStatsScreen:_create_stat_list_entry(placer, panel, offset, w,
                                                    name, title_params,
                                                    text_params, alltime_params,
                                                    force_bg_state)
        if placer and panel then
            local session_w = w * 0.6
            local alltime_w = w - session_w
            local total_w = session_w
            if force_bg_state ~= nil then add_bg = force_bg_state end

            local title = placer:add_right(
                              panel:text(
                                  {
                        name = name and string.format("%s_title", name) or nil,
                        color = title_params.color or
                            tweak_data.screen_colors.text,
            ...
            if alltime_params then
                local text = placer:add_right(
                                 panel:text(
                                     {
                            name = name and
                                string.format("%s_alltime_text", name) or nil,
                            color = alltime_params.color or
                                tweak_data.screen_colors.text,
    ...
    function HUDStatsScreen:_update_stats_list(panel, item)
        ...
        if managers.money and managers.statistics and managers.experience and
            not item then
            local money_current_stage = managers.crime_spree:is_active() and
                                            managers.crime_spree:get_potential_payout_from_current_stage(
                                                "cash") or
                                            managers.skirmish:is_skirmish() and
                                            managers.skirmish:current_ransom_amount() or
                                            managers.money:get_potential_payout_from_current_stage() or
                                            0
            local offshore_rate = managers.money:get_tweak_value(
                                      "money_manager", "offshore_rate") or 0
            local civilian_kills =
                managers.statistics:session_total_civilian_kills() or 0
            local cleaner_costs =
                (managers.money:get_civilian_deduction() or 0) * civilian_kills
            local offshore_money = money_current_stage -
                                       math.round(
                                           money_current_stage * offshore_rate)
            local spending_cash = money_current_stage * offshore_rate -
                                      cleaner_costs
            panel:child("offshore_payout_text"):set_text(
                managers.experience:cash_string(offshore_money))
            panel:child("spending_cash_text"):set_text(
                managers.experience:cash_string(spending_cash))

            if panel:child("cleaner_costs_text") then
                if spending_cash > 0 then
                    panel:child("spending_cash_text"):set_color(
                        tweak_data.screen_colors.friend_color)
                elseif spending_cash == 0 then
                    panel:child("spending_cash_text"):set_color(
                        tweak_data.screen_colors.text)
                else
                    panel:child("spending_cash_text"):set_color(
                        tweak_data.screen_colors.heat_cold_color)
                end
                panel:child("cleaner_costs_text"):set_text(
                    managers.experience:cash_string(cleaner_costs) .. " (" ..
                        tostring(civilian_kills) .. ")")
            end

            if panel:child("bag_amount_text") then
                local mandatory_bags_data =
                    managers.loot:get_mandatory_bags_data()
                local mandatory_amount =
                    mandatory_bags_data and mandatory_bags_data.amount or 0
                local secured_amount =
                    managers.loot:get_secured_mandatory_bags_amount() or 0
                local bonus_amount =
                    managers.loot:get_secured_bonus_bags_amount() or 0
                local bags_amount_str = tostring(bonus_amount or 0)
                if mandatory_amount > 0 then
                    bags_amount_str = string.format("%i / %i%s", secured_amount,
                                                    mandatory_amount,
                                                    bonus_amount > 0 and
                                                        string.format(" + %i",
                                                                      bonus_amount) or
            ...
            if panel:child("bag_cash_text") and panel:child("instant_cash_text") then
                panel:child("bag_cash_text"):set_text(
                    managers.experience:cash_string(
                        (managers.money:get_secured_mandatory_bags_money() or 0) +
                            (managers.money:get_secured_bonus_bags_money() or 0)))
                panel:child("instant_cash_text"):set_text(
                    managers.experience:cash_string(
            ...
            if not data.manual_update and (not item or item == data.name) then
                ...
                if update_data then
                    ...
                    if not data.no_alltime then
                        suffix_table.func_alltime = "_alltime_text"
                    end
                    for func, suffix in pairs(suffix_table) do
                        local item = panel:child(data.name .. suffix)
                        if item and update_data[func] and
                            managers.statistics[update_data[func]] then
                            local value =
                                managers.statistics[update_data[func]](
                                    managers.statistics,
                                    unpack(update_data.params or {}))
                            local value_str = ""
                            if type(value) == "table" then
                                for i, count in ipairs(value) do
                                    value_str =
                                        value_str ..
                                            managers.money:add_decimal_marks_to_string(
                                                tostring(count)) ..
                                            (value[i + 1] and " / " or "")
                                end
                            else
                            ...
                            item:set_text(
                                value_str .. (update_data.suffix or ""))
                        end
                    end
                end
            end
    ...
    function HudSidejob:init(parent, data, black_bg)
        HudSidejob.super.init(self, parent,
                              {border = 10, padding = 4, fixed_w = parent:w()})
        ...
        local title = placer:add_bottom(self:fine_text(
                ...
                font_size = medium_font_size,
                color = completed and
                    tweak_data.screen_colors.challenge_completed_color or
                    tweak_data.screen_colors.text,
                w = parent:w(),
                keep_w = true
            }))
        ...
        local texture = rewards and rewards[#rewards].texture_path or
                            "guis/textures/pd2/feature_crimenet_heat"
        local bitmap = placer:add_row(self:bitmap(
                                          {w = 50, h = 50, texture = texture}))
        ...
        if self._objectives then
            for i, objective in ipairs(self._objectives or {}) do
                if objective.display ~= false then
                    local desc = self:text(
                            ...
                            text = managers.localization:text(
                                objective.desc_id or self._info.objective_id or
                                    self._info.desc_id or "N/A", {
                                    max_progress = objective.max_progress or "?",
                                    progress = objective.progress or "?"
                                }),
                            font = tiny_font,
                            font_size = tiny_font_size,
                ...
                if objective.max_progress > 1 and objective.show_progress ~=
                    false then
                    local bar = placer:add_bottom(
                                    TextProgressBar:new(self, {
                            w = 300,
                            h = 10,
                            back_color = Color(255, 60, 60, 65) / 255,
                            max = objective.max_progress
                        }, {font_size = 12, font = tiny_font},
                                                        objective.progress))
                end
            end
        end
        ...
        if black_bg then
            self:rect({layer = -1, color = Color.black:with_alpha(0.6)})
        end
    end
    ...
    function HudSidejob:_create_time_string(timestamp)
        if timestamp >= 0 then
            ...
            if timestamp > 24 then
                expire_string = managers.localization:text(
                                    "cn_menu_contract_length",
                                    {stages = math.floor(timestamp / 24)})
            end
            expire_string = string.format("%s %02d:%02d:%02d", expire_string,
                                          timestamp % 24, minutes, seconds)
            return expire_string
        else
            return managers.localization:text(
                       "menu_challenge_about_to_expire_extended")
        end
    end
...
elseif string.lower(RequiredScript) == "lib/managers/statisticsmanager" then
    ...
    local session_enemy_killed_by_type_original =
        StatisticsManager.session_enemy_killed_by_type
    ...
    function StatisticsManager:enemy_killed_by_type(enemy, type)
        ...
        return self._global.killed and self._global.killed[enemy] and
                   self._global.killed[enemy][type] or 0
    end
    ...
    local TANK_IDs = {
        "tank", "tank_green", "tank_black", "tank_skull", "tank_medic",
        "tank_mini", "tank_hw"
    }
    ...
    function StatisticsManager:session_total_revives()
        return {
            self._global.session.revives.player_count,
            self._global.session.revives.npc_count
        }
    end

    function StatisticsManager:total_revives()
        return {
            self._global.revives.player_count, self._global.revives.npc_count
        }
    end
    ...
    function StatisticsManager:add_session_damage(damage, peer_id)
        ...
        local peer_uid = peer and peer:user_id() or Steam:userid()
        self._session_damage = self._session_damage or {}
        self._session_damage[peer_uid] =
            (self._session_damage[peer_uid] or 0) + (damage * 10)
    end
    ...
    function StatisticsManager:most_session_damage()
        ...
        local peer_name = user_id and Steam:username(user_id) or
                              managers.localization:text("debug_undecided")
        return string.format("%s (%s)", peer_name,
                             managers.money:add_decimal_marks_to_string(
                                 tostring(max_damage)))
    end
elseif string.lower(RequiredScript) == "lib/units/enemies/cop/copdamage" then
    ...
    function CopDamage:_process_damage(damage_info)
        local attacker = alive(damage_info.attacker_unit) and
                             damage_info.attacker_unit
        ...
        if attacker and damage >= 0.1 then
            ...
            if alive(killer) then
                if killer:in_slot(2) then
                    ...
                    local peer_id =
                        managers.criminals:character_peer_id_by_unit(killer)
                    if peer_id then
...
elseif string.lower(RequiredScript) == "lib/managers/menu/stageendscreengui" then
    ...
    function StatsTabItem:set_stats(stats_data, ...)
        ...
        elseif table.contains(stats_data, "favourite_weapon") then
            local total_objectives = managers.objectives:total_objectives(
                                         Global.level_data and
    ...
    function StageEndScreenGui:feed_statistics(data, ...)
        ...
        new_data.most_damage = tostring(
                                   managers.statistics:most_session_damage())
        new_data.session_damage = tostring(
                                      managers.statistics:session_damage_string())
...
elseif string.lower(RequiredScript) == "lib/managers/crimespreemanager" then
    ...
    function CrimeSpreeManager:get_current_reward_multiplier()
        if self:has_failed() then return 0 end
        ...
        if not self:_is_host() and reward_add > 0 then
            if self:server_spree_level() > self:spree_level() then
                ...
                local catch_up = math.floor(
                                     diff * tweak_data.crime_spree.catchup_bonus)
        ...
        if self:server_spree_level() >= self:spree_level() and reward_add > 0 then
            local streak_bonus =
                (self._global.winning_streak or 1) + reward_add *
                    tweak_data.crime_spree.winning_streak
            if 1 > streak_bonus then streak_bonus = streak_bonus + 1 end
...
elseif string.lower(RequiredScript) == "lib/managers/hudmanagerpd2" then
    ...
    function HUDManager:_setup_stats_screen() -- Tmp: Currently not much useful on left skirmish stats screen...
        if not self:alive(self.STATS_SCREEN_FULLSCREEN) then return end
