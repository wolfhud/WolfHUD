if WolfHUD:getSetting({"CustomHUD", "ENABLED"}, true) then return end
...
if RequiredScript == "lib/managers/hudmanagerpd2" then
    ...
    function HUDManager:teammate_progress(peer_id, type_index, enabled,
        ...
        if character_data then
            local teammate_panel =
                self._teammate_panels[character_data.panel_id]
            local name_label = self:_name_label_by_peer_id(peer_id)
            if name_label then
                teammate_panel:set_interact_text(
                    name_label.panel:child("action"):text())
            end
            teammate_panel:set_interact_visibility(
                enabled and timer and
                    WolfHUD:getSetting({
                        "CustomHUD", "TEAMMATE", "INTERACTION", "MIN_DURATION"
                    }, 1) <= timer and
                    not WolfHUD:getSetting(
                        {"CustomHUD", "TEAMMATE", "INTERACTION", "HIDE"}, false) and
                    WolfHUD:getSetting({
...
elseif RequiredScript == "lib/managers/hud/hudteammate" then
    ...
    function HUDTeammate:init(...)
        ...
        self._condition_icon:set_color(WolfHUD:getColorSetting(
                                           {
                "CustomHUD", self._setting_prefix, "CONDITION_ICON_COLOR"
            }, "white"))
    ...
    function HUDTeammate:set_name(name, ...)
        if not self._ai then
            if WolfHUD:getSetting({
                "CustomHUD", self._setting_prefix, "TRUNCATE_TAGS"
            }, true) then name = WolfHUD:truncateNameTag(name) end
            if WolfHUD:getSetting({"CustomHUD", self._setting_prefix, "RANK"},
                                  true) then
                local peer = self:peer_id() and
                                 managers.network:session():peer(self:peer_id())
                local infamy, level = peer and peer:rank() or
                                          managers.experience:current_rank(),
                                      peer and peer:level() or
    ...
    function HUDTeammate:_truncate_name()
        ...
        while (name_panel:x() + w) > self._max_name_panel_width do
            if name_panel:font_size() > 15.1 then
                ...
                name_panel:set_text(
                    teammate_name:sub(1, teammate_name:len() - 1))
            end
            teammate_name = name_panel:text()
            _, _, w, h = name_panel:text_rect()
        end
        if not self._ai then
            name_panel:set_range_color((self._color_pos or 0) + 1,
                                       name_panel:text():len() + 1,
                                       self._panel:child("callsign"):color()
                                           :with_alpha(1))
        else
            name_panel:set_color(WolfHUD:getSetting(
                                     {
                    ...
                    }, "white") or tweak_data.chat_colors[5])
        end
    ...
    function HUDTeammate:update(t, dt) self:update_latency(t, dt) end
    ...
    function HUDTeammate:_create_stamina_circle()
        local radial_health_panel = self._panel:child("player"):child(
                                        "radial_health_panel")
        self._stamina_bar = radial_health_panel:bitmap(
                ...
                layer = 2,
                visible = WolfHUD:getSetting({"CustomHUD", "PLAYER", "STAMINA"},
                                             true)
            })
        self._stamina_bar:set_color(Color(1, 1, 0, 0))
        self._stamina_bar:set_center(radial_health_panel:child("radial_health")
                                         :center())
        ...
        self._stamina_line:set_center(radial_health_panel:child("radial_health")
                                          :center())
    end
    ...
    function HUDTeammate:set_max_stamina(value)
        if not self._max_stamina or self._max_stamina ~= value then
            ...
            local threshold = tweak_data.player.movement_state.stamina
                                  .MIN_STAMINA_THRESHOLD
            local angle = 360 * (threshold / self._max_stamina) - 90
            local x = 0.48 * w * math.cos(angle) + w * 0.5 +
                          self._stamina_bar:x()
            local y = 0.48 * w * math.sin(angle) + w * 0.5 +
                          self._stamina_bar:y()
    ...
    function HUDTeammate:set_current_stamina(value)
        ...
        self:set_stamina_meter_visibility(
            WolfHUD:getSetting({"CustomHUD", "PLAYER", "STAMINA"}, true) and
                not self._condition_icon:visible())
    end
    ...
    function HUDTeammate:teammate_progress(enabled, tweak_data_id, timer,
                                           success, ...)
        local show = timer and
                         WolfHUD:getSetting(
                             {
                    "CustomHUD", "TEAMMATE", "INTERACTION", "MIN_DURATION"
                }, 1) <= timer and
                         not WolfHUD:getSetting(
                             {"CustomHUD", "TEAMMATE", "INTERACTION", "HIDE"},
                             false)
        teammate_progress_original(self, enabled and show, tweak_data_id, timer,
                                   success and show, ...)
        if enabled and show and
            WolfHUD:getSetting({
                "CustomHUD", "TEAMMATE", "INTERACTION", "NUMBER"
            }, true) then
    ...
    function HUDTeammate:_start_interact_timer(interaction_time)
        ...
        condition_timer:animate(callback(self, self, "_animate_interact_timer"),
                                interaction_time)
    end
    ...
    function HUDTeammate:_animate_interact_timer(condition_timer, total_time)
        ...
        while t >= 0 do
            ...
            condition_timer:set_color(math.lerp(Color('00FF00'), Color.white,
                                                t / total_time))
        end
