local _upd_criminal_suspicion_progress_original =
    ...
    if self._ai_enabled then
        for obs_key, obs_susp_data in pairs(self._suspicion_hud_data or {}) do
            ...
            if managers.enemy:is_civilian(unit) then
                ...
                local waypoint = managers.hud and
                                     managers.hud._hud.waypoints[waypoint_id]
                if waypoint then
                    local color, arrow_color
                    if unit:anim_data().drop and
                        WolfHUD:getSetting(
                            {"HUDSuspicion", "SHOW_PACIFIED_CIVILIANS"}, true) then
                        if not obs_susp_data._subdued_civ then
                            ...
                            waypoint.bitmap:set_image(
                                "guis/textures/menu_singletick")
                        end
