...
if RequiredScript == "lib/managers/hud/hudchat" then
    ...
    HUDChat.MAX_OUTPUT_LINES = WolfHUD:getSetting(
                                   {"HUDChat", "MAX_OUTPUT_LINES"}, 8) -- Number of chat lines to show
    HUDChat.MAX_INPUT_LINES = WolfHUD:getSetting({"HUDChat", "MAX_INPUT_LINES"},
                                                 5) -- Number of lines of text you can type
    ...
    HUDChat.SCROLLBAR_ALIGN = WolfHUD:getSetting({"HUDChat", "SCROLLBAR_ALIGN"},
                                                 2) -- Alignment of the scroll bar (1 = left, 2 = right)
    ...
    function HUDChat:init(ws, hud)
        local fullscreen = managers.hud:script(
    ...
    function HUDChat:_create_input_panel()
        ...
        local gradient = self._input_panel:gradient(
                             { -- TODO: Why won't this POS behave?
                name = "input_bg",
                visible = false, -- TODO: Remove
                alpha = 0, -- TODO: Remove
                gradient_points = {
                    0, Color.white:with_alpha(0), 0.2,
                    Color.white:with_alpha(0.25), 1, Color.white:with_alpha(0)
                },
                layer = -1,
                valign = "grow",
                blend_mode = "sub"
        ...
        local input_prompt = self._input_panel:text(
                ...
                name = "input_prompt",
                text = utf8.to_upper(
                    managers.localization:text("debug_chat_say")),
    ...
    function HUDChat:_create_output_panel()
        ...
        if HUDChat.COLORED_BG then
            output_panel:gradient({
                ...
                gradient_points = {
                    0, Color.white:with_alpha(0.3), 0.3,
                    Color.white:with_alpha(0.1), 0.5,
                    Color.white:with_alpha(0.2), 0.7,
                    Color.white:with_alpha(0.1), 1, Color.white:with_alpha(0.3)
                },
                layer = -1,
                valign = "grow",
                blend_mode = "sub",
    ...
    function HUDChat:_layout_output_panel()
        ...
        output_panel:set_h(HUDChat.LINE_HEIGHT *
                               math.min(HUDChat.MAX_OUTPUT_LINES,
                                        self._total_message_lines))
        if self._total_message_lines > HUDChat.MAX_OUTPUT_LINES then
            ...
            local scroll_bar_position =
                output_panel:child("scroll_bar_position")
            ...
            local positon_height_area = scroll_bar_bg:h() - scroll_bar_up:h() -
                                            scroll_bar_down:h() - 4
            scroll_bar_position:set_h(math.max(
                                          (HUDChat.MAX_OUTPUT_LINES /
                                              self._total_message_lines) *
                                              positon_height_area, 3))
            scroll_bar_position:set_center_y(
                (1 - self._current_line_offset / self._total_message_lines) *
                    positon_height_area + scroll_bar_up:h() + 2 -
                    scroll_bar_position:h() / 2)
        end
    ...
    function HUDChat:receive_message(name, message, color, icon)
        local output_panel = self._panel:child("output_panel")
        local scroll_bar_bg = output_panel and
                                  output_panel:child("scroll_bar_bg")
        ...
        local heisttime = managers.game_play_central and
                              managers.game_play_central:get_heist_timer() or 0
        ...
        if hours > 0 then
            time_format_text = string.format("%d:%02d:%02d", hours, minutes,
                                             seconds)
        else
        ...
        table.insert(self._messages,
                     {panel = msg_panel, name = name, lines = no_lines})
    ...
    function HUDChat:_set_input_lines(no_lines)
        if no_lines ~= self._current_input_lines then
            ...
            self._input_panel:child("input_text"):set_h(
                no_lines * HUDChat.LINE_HEIGHT)
            self._input_panel:set_bottom(self._panel:h())
            self._panel:child("output_panel")
                :set_bottom(self._input_panel:top())
        end
    end
    ...
    function HUDChat:update_key_down(o, k)
        ...
        repeat
            ...
            if self._key_pressed == Idstring("backspace") then
                ...
                if not (utf8.len(text:text()) < 1) or type(self._esc_callback) ~=
                    "number" then end
            ...
            elseif self._key_pressed == Idstring("page up") then
                self:_change_line_offset(
                    HUDChat.MAX_OUTPUT_LINES - self._current_input_lines)
    ...
    function HUDChat:_change_line_offset(diff)
        if diff ~= 0 then
            self:_set_line_offset(math.clamp(self._current_line_offset + diff,
                                             0,
                                             math.max(
                                                 self._total_message_lines -
                                                     HUDChat.MAX_OUTPUT_LINES +
                                                     self._current_input_lines -
                                                     1, 0)))
        end
    end
    ...
    function HUDChat:_on_focus(...)
        if HUDChat.MOUSE_SUPPORT then self:connect_mouse() end

        return _on_focus_original(self, ...)
    end

    function HUDChat:_loose_focus(...)
        if HUDChat.MOUSE_SUPPORT then self:disconnect_mouse() end
    ...
    function HUDChat:_mouse_move(o, x, y)
        if self._mouse_state then
            ...
            local output_panel = self._panel:child("output_panel")
            self:_move_scroll_bar_position_center(
                y - self._panel:y() - output_panel:y())
            self._mouse_state = y
        end
    end
    ...
    function HUDChat:_mouse_press(o, button, x, y)
        ...
        elseif button == Idstring("0") then
            local scroll_bar_position = self._panel:child("output_panel"):child(
                                            "scroll_bar_position")
    ...
    function HUDChat:_mouse_release(o, button, x, y)
        ...
        if button == Idstring("0") then self._mouse_state = nil end
    end
    ...
    function HUDChat:_move_scroll_bar_position_center(y)
        ...
        local positon_height_area = scroll_bar_bg:h() - scroll_bar_up:h() -
                                        scroll_bar_down:h() - 4
        local new_line_offset = math.round(
                                    (1 -
                                        ((y - scroll_bar_up:h() - 2) /
                                            positon_height_area)) *
                                        self._total_message_lines)
