...
function LoadoutBaseItem:destroyed() return self._to_destroy end

function LoadoutBaseItem:name() return self._name end

function LoadoutBaseItem:enabled() return next(self._disable_reason) == nil end
...
function LoadoutBaseItem:set_w(w) return self:set_size(w, nil) end

function LoadoutBaseItem:set_h(h) return self:set_size(nil, h) end

function LoadoutBaseItem:update(t, dt) end

function LoadoutBaseItem:arrange() end

function LoadoutBaseItem:set_outfit(outfit) return table.size(outfit) > 0 end
...
function LoadoutBaseItem:text_rect()
    return self:x(), self:y(), self:w(), self:h()
end

function LoadoutBaseItem:set_alpha(v)
    if not self:destroyed() then self._panel:set_alpha(v) end
end
function LoadoutBaseItem:set_x(v)
    if not self:destroyed() then self._panel:set_x(v) end
end
function LoadoutBaseItem:set_y(v)
    if not self:destroyed() then self._panel:set_y(v) end
end
function LoadoutBaseItem:set_left(v)
    if not self:destroyed() then self._panel:set_left(v) end
end
function LoadoutBaseItem:set_right(v)
    if not self:destroyed() then self._panel:set_right(v) end
end
function LoadoutBaseItem:set_top(v)
    if not self:destroyed() then self._panel:set_top(v) end
end
function LoadoutBaseItem:set_bottom(v)
    if not self:destroyed() then self._panel:set_bottom(v) end
end
function LoadoutBaseItem:set_center(x, y)
    if not self:destroyed() then self._panel:set_center(x, y) end
end
function LoadoutBaseItem:set_center_x(v)
    if not self:destroyed() then self._panel:set_center_x(v) end
end
function LoadoutBaseItem:set_center_y(v)
    if not self:destroyed() then self._panel:set_center_y(v) end
end
function LoadoutBaseItem:set_layer(v)
    if not self:destroyed() then self._panel:set_layer(v) end
end

LoadoutPanel = LoadoutPanel or class(LoadoutBaseItem)
LoadoutPanel.NAME_TO_CLASS = {
    name = {
        class = "LoadoutNameItem",
        params = {
            font_size = tweak_data.menu.pd2_medium_font_size,
            height = tweak_data.menu.pd2_medium_font_size,
            margin = 0,
            use_peer_color = true
        }
    },
    level = {
        class = "LoadoutLevelItem",
        params = {
            font_size = tweak_data.menu.pd2_medium_font_size,
            height = tweak_data.menu.pd2_medium_font_size,
            margin = 0,
    ...
    skills = {
        class = "LoadoutSkillsItem",
        params = {height = tweak_data.menu.pd2_small_font_size, margin = 2}
    },
    perk = {
        class = "LoadoutPerkItem",
        params = {height = tweak_data.menu.pd2_small_font_size, margin = 2}
    },
    ...
    secondary_deployable = {
        class = "LoadoutDeployableItem",
        params = {margin = 5}
    }
}
function LoadoutPanel:init(base_panel, owner, peer_id, width, height, params)

    LoadoutPanel.super.init(self, base_panel, owner,
                            string.format("Peer_%d", peer_id), width, height,
                            params)

    self._peer_id = peer_id
    self._peer_color = tweak_data.chat_colors[peer_id]
    self._local_peer = not managers.network:session() or
                           managers.network:session():local_peer():id() ==
                           peer_id

    -- Setup sub components
    self._components = {}
    self._component_layout = params.component_layout or {
        {"name"}, {"character"}, {"skills"}, {"perk"}, {"primary"},
        {"secondary"}, {"melee_weapon", "grenade"}, {"armor", "mask"},
    ...
    local comp_h =
        (self._panel:h() - 2 * self._margin - math.ceil(fixed_comp_h)) /
            (table.size(self._component_layout) - fixed_comp_amnt)
        ...
        for i, name in ipairs(data) do
            ...
            if class then
                ...
                self._components[name] =
                    _G[class]:new(self._panel, self, name, sub_comp_w,
                                  component_params and component_params.height or
                                      math.ceil(comp_h), component_params or {})
            end
        end
    end

    self._border_config = params.borders or {1, 1, 1, 1}
    self._borders = BoxGuiObject:new(self._panel, {sides = self._border_config})
    ...
    if self._panel:visible() ~= visible then
        ...
        if self._borders then
            self._borders:create_sides(self._panel, {
                sides = (visible and self._border_config or {0, 0, 0, 0})
            })
        end
        return true
    end
...
function LoadoutPanel:update(t, dt)
    for name, component in pairs(self._components) do component:update(t, dt) end
end
...
function LoadoutPanel:arrange()
    ...
    for i, components in ipairs(active_comps) do
        local new_w = math.floor((self._panel:w() - (2 * self._margin)) /
...
function LoadoutPanel:set_outfit(outfit)
    ...
    if enabled then self:arrange() end
...
function LoadoutPanel:get_peer_id() return self._peer_id end
...
function LoadoutPanel:local_peer() return self._local_peer end
...
function LoadoutPanel:mark_cheater(status, reason)
    for _, item_name in ipairs({"name", "character", "level"}) do
        ...
        if item then
            item:set_color(status and tweak_data.screen_colors.pro_color or
                               item._default_color or Color.white)
        end
    end
    --[[
...
function LoadoutTextItem:init(base_panel, owner, name, width, height, params)
    ...
    LoadoutTextItem.super.init(self, base_panel, owner, name, width, height,
                               params)

    self._font_size = math.min(params.font_size or
                                   tweak_data.menu.pd2_medium_font_size,
                               self._panel:h() - 2 * self._margin)
    ...
    if self:enabled() and alive(self._text) then
        ...
        while w > math.max(self._panel:w() - 2 * self._margin, 1) do
            if self._text:font_size() > 11 then
                ...
                self._text:set_text(self._text:text():sub(1, self._text:text()
                                                              :len() - 1))
            end
    ...
    if text and alive(self._text) then
        ...
        if color_range then
            ...
            for _, range in ipairs(color_range) do
                self._text:set_range_color(range[1], range[2],
                                           color:with_alpha(alpha * 0.75))
            end
        end
    end
end
...
function LoadoutTextItem:set_color(color)
    if color then self._text:set_color(color) end
end
...
function LoadoutImageItem:init(base_panel, owner, name, width, height, params)
    ...
    LoadoutImageItem.super.init(self, base_panel, owner, name, width, height,
                                params)
    ...
    if params.amount then self:set_amount(params.text) end
...
function LoadoutImageItem:arrange()
    if self:enabled() then
        ...
        self._text:set_left(self._margin)
        self._text:set_w(self._panel:w() - 2 * self._margin)
        local max_w, max_h = self._panel:w() - 2 * self._margin,
                             (self._panel:h() -
                                 (self._text:visible() and self._text:h() or 0)) -
                                 2 * self._margin
        local texture_w, texture_h = self._icon:texture_width() or 1,
                                     self._icon:texture_height() or 1
        ...
        self._icon:set_center_y(self._panel:h() / 2 +
                                    ((self._text:visible() and self._text:h() or
                                        0) * 0.5))

        self._amount_text:set_font_size(tweak_data.menu.pd2_small_font_size *
                                            0.9)
        local _, _, w, h = self._amount_text:text_rect()
        while w > ((self._panel:w() - 2 * self._margin) / 2) do
            if self._amount_text:font_size() > 10 then
                self._amount_text:set_font_size(
                    self._amount_text:font_size() - 0.1)
            else
                break
            end
...
function LoadoutImageItem:set_amount(amount, color_range)
    if amount then
        ...
        if color_range then
            local color, alpha = self._text:color(), self._text:alpha()
            for _, range in ipairs(color_range) do
                self._amount_text:set_range_color(range[1], range[2],
                                                  color:with_alpha(alpha * 0.65))
            end
        end
    end
end
...
function LoadoutImageItem:set_outfit(outfit)
    if outfit[self._name] then
        ...
        if self._loadout ~= outfit[self._name] then
            ...
            local texture, name =
                self:get_outfit_data(self._name, self._loadout)
            local amount = (self._name == "grenade") and
                               tweak_data.blackmarket.projectiles[self._loadout] and
                               tweak_data.blackmarket.projectiles[self._loadout]
                                   .max_amount or 0
...
function LoadoutImageItem:get_outfit_data(type, id)
    ...
    local texture_name = tweak_entry[type][id] and
                             tweak_entry[type][id].texture_name or tostring(id)
    local texture = string.format("%s%s%s", guis_catalog, texture_path[type],
                                  texture_name)

    local rarity_texture
    if type == "weapon_skin" then
        local rarity = tweak_entry[type][id] and tweak_entry[type][id].rarity
        rarity_texture = tweak_data.economy.rarities[rarity] and
                             tweak_data.economy.rarities[rarity].bg_texture

        id = tweak_entry[type][id] and tweak_entry[type][id].weapon_id or id
        type = "weapon"
    end

    local name_id = tweak_entry[type][id] and tweak_entry[type][id].name_id or
                        tostring(id)
...
function LoadoutNameItem:init(base_panel, owner, name, width, height, params)
    ...
    LoadoutNameItem.super.init(self, base_panel, owner, name, width, height,
                               params)
...
function LoadoutNameItem:set_outfit(outfit)
    ...
    if peer_uid then
        ...
        if peer_uid ~= self._loadout then
            ...
            local level, infamy = self._owner:local_peer() and
                                      managers.experience:current_level() or
                                      peer:level(), self._owner:local_peer() and
                                      managers.experience:current_rank() or
                                      peer:rank()
            local level_str = string.format(" [%s%s]", (infamy or 0) > 0 and
                                                string.format("%s-",
                                                              managers.experience:rank_string(
                                                                  infamy)) or "",
                                            tostring(level or 0))
            self:set_text(string.format("%s%s", name,
                                        self._show_level and level_str or ""))

            if name and level then self._loadout = peer_uid end
...
function LoadoutLevelItem:set_outfit(outfit)
    ...
    if peer_uid then
        ...
        if peer_uid ~= self._loadout then
            ...
            if level then self._loadout = peer_uid end
...
function LoadoutCSLevelItem:init(base_panel, owner, name, width, height, params)
    ...
    LoadoutCSLevelItem.super
        .init(base_panel, owner, name, width, height, params)
end
...
function LoadoutCSLevelItem:set_outfit(outfit)
    ...
    if level >= 0 then
        ...
        if tostring(level) ~= self._loadout then
            local level_str = managers.localization:text("menu_cs_level", {
...
function LoadoutDetectionItem:set_outfit(outfit)
    ...
    if peer then
        self:set_enabled("peer", true)
        local detection = math.round(
                              managers.blackmarket:get_suspicion_offset_of_peer(
                                  peer,
                                  tweak_data.player.SUSPICION_OFFSET_LERP or
                                      0.75) * 100)
        if tostring(detection) ~= self._loadout then
            self:set_text(utf8.char(57363) .. detection, detection < 50 and
                              Color(1, 0, 0.8, 1) or Color(1, 1, 0.2, 0))
...
function LoadoutPingItem:init(base_panel, owner, name, width, height, params)
    ...
    LoadoutPingItem.super.init(self, base_panel, owner, name, width, height,
                               params)
    ...
    if not self._owner:local_peer() then
        if (self._next_update_t or 0) <= t then
            ...
            if peer then
                ...
                self:set_text(string.format("%.0fms", latency),
                              latency < 75 and Color('C2FC97') or latency < 150 and
...
function LoadoutPingItem:set_text(text, color)
    ...
    if alive(self._text) then self._text:set_color(color) end
end
...
function LoadoutPlaytimeItem:set_outfit(outfit)
    ...
    if steam_id then
        ...
        if steam_id ~= self._loadout then
            ...
            local all_games_url = string.format(
                                      "http://steamcommunity.com/profiles/%s/games/?xml=1",
                                      tostring(steam_id))
            Steam:http_request(all_games_url,
                               callback(self, self, "set_playtime_clbk"))
        end
    else
        self:set_enabled("peer", false)
    end
        ...
        if gameStart then
            ...
            if hoursStart and hoursEnd and ((hoursEnd - hoursStart) > 2) then
                ...
                text = managers.localization:text(
                           "wolfhud_loadout_hours_played", {HOURS = playtime})
            end
        else
...
function LoadoutCharacterItem:set_outfit(outfit)
    ...
    if peer then
        ...
        if character ~= self._loadout then
            ...
            self:set_text(managers.localization:text(
                              "menu_" ..
...
function LoadoutSkillsItem:init(base_panel, owner, name, width, height, params)
    ...
    LoadoutSkillsItem.super.init(self, base_panel, owner, name, width, height,
                                 params)
    ...
    for i, tree in ipairs(tweak_data.skilltree.skill_pages_order) do
        ...
        if tree then
            table.insert(self._tree_names,
                         tree.name_id and
                             utf8.sub(managers.localization:text(tree.name_id),
                                      1, 1) or "?")
        end
    end
end
        ...
        if skill_data and skill_str ~= self._loadout then
            ...
            for tree = 1, #self._tree_names, 1 do
                ...
                for sub_tree = 1, subtree_amt, 1 do
                    local skills = skill_data[(tree - 1) * subtree_amt +
                                       sub_tree] or 0
                    ...
                    tree_detailed = string.format("%s%02d ", tree_detailed,
                                                  skills)
                ...
                if self._detailed then
                    ...
                    text =
                        string.format("%s%s:%02d ", text, tree_name, tree_sum)
                end
                points_total = points_total + tree_sum
            end

            local is_cheating = points_total >
                                    (self:get_max_skillpoints() or points_total)
            self:set_color(is_cheating and tweak_data.screen_colors.pro_color or
                               self._default_color)
            self._owner:mark_cheater(is_cheating, is_cheating and
                                         "Too many skillpoints used." or "")
...
LoadoutSkillsItem.POINTS_MAP = {{1, 1}, {10, 2}}
...
function LoadoutPerkItem:set_outfit(outfit)
    if outfit.skills and outfit.skills.specializations then
        self:set_enabled("outfit", true)
        local deck_index, deck_level = unpack(
                                           outfit.skills.specializations or {})
        local data =
            tweak_data.skilltree.specializations[tonumber(deck_index) or 0]
        local name_id = data and data.name_id
        local deck_id = string.format("%s_%s", name_id or "",
                                      tostring(deck_level))
...
function LoadoutWeaponItem:init(base_panel, owner, name, width, height, params)
    ...
    LoadoutWeaponItem.super.init(self, base_panel, owner, name, width, height,
                                 params)
...
function LoadoutWeaponItem:arrange()
    ...
    if alive(self._rarity) then
        local max_w, max_h = self._panel:w() - 2 * self._margin,
                             (self._panel:h() -
                                 (self._text:visible() and self._text:h() or 0)) -
                                 2 * self._margin
        local texture_w, texture_h = self._rarity:texture_width() or 1,
                                     self._rarity:texture_height() or 1
        ...
        self._rarity:set_center_y(self._panel:h() / 2 +
                                      ((self._text:visible() and self._text:h() or
                                          0) * 0.5))
    end
    ...
    for i, perk in ipairs(self._perks or {}) do
        if alive(perk) and perk:visible() then
            local size = math.min(self._panel:h() / 4,
                                  self._panel:w() / #self._perks, 16)
            perk:set_w(size)
            perk:set_h(size)
            perk:set_rightbottom(math.round(
                                     self._panel:w() - self._margin -
                                         ((i - 1) * (perk:w() + 1))),
                                 math.round(self._panel:h() - self._margin))
        end
    end
end
...
function LoadoutWeaponItem:set_outfit(outfit)
    if outfit[self._name] then
        ...
        if weapon_changed or perks_changed then self:arrange() end
    else
        self:set_enabled("outfit", false)
    end
end
...
function LoadoutWeaponItem:update_weapon(outfit)
    local weapon_id = managers.weapon_factory:get_weapon_id_by_factory_id(
                          outfit[self._name].factory_id)
    if weapon_id then
        self:set_enabled("outfit", true)
        if self._loadout ~= weapon_id then
            self._loadout = weapon_id
            local skin_id = outfit[self._name].cosmetics and
                                outfit[self._name].cosmetics.id
            local skin_tweak = tweak_data.blackmarket.weapon_skins[skin_id]
            local weapon_skin =
                skin_tweak and not skin_tweak.is_a_color_skin and
                    (table.contains(skin_tweak.weapon_ids or {}, weapon_id) or
                        (skin_tweak.weapon_id and skin_tweak.weapon_id ==
                            weapon_id) ~= (skin_tweak.use_blacklist or false)) or
                    false
            local texture, name, rarity =
                self:get_outfit_data(weapon_skin and "weapon_skin" or "weapon",
                                     weapon_skin and skin_id or weapon_id)
...
function LoadoutWeaponItem:update_perks(outfit)
    local perks = {}
    if outfit[self._name] and outfit[self._name].factory_id and
        outfit[self._name].blueprint then
        perks = managers.blackmarket:get_perks_from_weapon_blueprint(
                    outfit[self._name].factory_id, outfit[self._name].blueprint)

        local factory = tweak_data.weapon.factory.parts
        local parts =
            managers.weapon_factory:get_parts_from_weapon_by_type_or_perk(
                "bonus", outfit[self._name].factory_id,
                outfit[self._name].blueprint) or {}
        local stats, custom_stats, has_stat_boost, has_team_boost, has_stat,
              has_team
        for _, part_id in ipairs(parts) do
            stats = factory[part_id] and factory[part_id].stats or false
            custom_stats = factory[part_id] and factory[part_id].custom_stats or
                               false
            has_stat_boost = stats and 1 < table.size(stats) and true or false
            has_team_boost = custom_stats and
                                 (custom_stats.exp_multiplier or
                                     custom_stats.money_multiplier) and true or
                                 false
            if has_stat_boost then perks.bonus_stats = stats end
            if has_team_boost then perks.bonus_team = custom_stats end
        end
        if outfit[self._name].cosmetics and outfit[self._name].cosmetics.bonus and
            not managers.job:is_current_job_competitive() then
            local bonus_data = tweak_data.economy.bonuses[tweak_data.blackmarket
                                   .weapon_skins[outfit[self._name].cosmetics.id]
                                   .bonus]
            has_stat = bonus_data and bonus_data.stats and true or false
            has_team = bonus_data and
                           (bonus_data.exp_multiplier or
                               bonus_data.money_multiplier and true) or false
    ...
    if perk_str ~= self._perk_str then
        for i, perk in ipairs(self._perks or {}) do
            if alive(perk) then
                ...
                if alive(parent) then parent:remove(perk) end
            end
        end
        self._perks = {}
        local perk_size = math.min(self._panel:h() / 4,
                                   self._panel:w() / #self._perks, 16)
...
function LoadoutMeleeItem:init(base_panel, owner, name, width, height, params)
    ...
    LoadoutMeleeItem.super.init(self, base_panel, owner, name, width, height,
                                params)

    local stock_img_height = (self._panel:h() -
                                 (self._text:visible() and self._text:h() or 0)) /
                                 2
...
function LoadoutMeleeItem:arrange()
    ...
    local max_w, max_h = self._panel:w() - 2 * self._margin,
                         ((self._panel:h() -
                             (self._text:visible() and self._text:h() or 0)) - 2 *
                             self._margin) / 2
    for i, panel in ipairs(self._weapon_stock or {}) do
        if alive(panel) and panel:visible() then
            local texture_w, texture_h = panel:texture_width() or 1,
                                         panel:texture_height() or 1
    ...
    if outfit.melee_weapon then
        if outfit.melee_weapon == "weapon" then
            ...
            for i, name in ipairs({"primary", "secondary"}) do
                local weapon_id = outfit[name].cosmetics and
                                      outfit[name].cosmetics.id or
                                      managers.weapon_factory:get_weapon_id_by_factory_id(
                                          outfit[name].factory_id)
                local skinned =
                    tweak_data.blackmarket.weapon_skins[weapon_id] and true
                local texture, name, rarity =
                    self:get_outfit_data(skinned and "weapon_skin" or "weapon",
                                         weapon_id)
            ...
            if loadout_id ~= self._loadout then
                ...
                local _, name, _ = self:get_outfit_data("melee_weapon",
                                                        outfit.melee_weapon)
                self:set_text(name)
            ...
            for i, panel in ipairs(self._weapon_stock or {}) do
                if alive(panel) then panel:set_visible(false) end
            end
...
function LoadoutDeployableItem:set_outfit(outfit)
    if outfit[self._name] and tostring(outfit[self._name]) ~= "nil" and
        (self._name ~= "secondary_deployable" or
            (outfit.skills and outfit.skills.skills and
                (tonumber(outfit.skills.skills[7]) or 0) >= 12)) then
        self:set_enabled("outfit", true)
        if self._loadout ~= outfit[self._name] then
            self._loadout = outfit[self._name]
            local texture, name = self:get_outfit_data("deployables",
                                                       self._loadout)
