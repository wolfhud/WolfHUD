...
if requiresScript == "lib/managers/systemmenumanager" then
    ...
    function GenericSystemMenuManager:show_input(data)
        local success = self:_show_class(data, self.GENERIC_INPUT_DIALOG_CLASS,
                                         self.INPUT_DIALOG_CLASS, data.force)
...
elseif requiresScript == "lib/managers/dialogs/specializationdialog" then
    ...
    function TextInputDialog:init(manager, data, ...)
        ...
        local text_config = {
            ...
            font_size = data.font_size or
                _G.tweak_data.menu.pd2_medium_font_size,
            w = data.w or 500,
            h = data.h or 400,
        ...
        self._panel_script = _G.TextInputBoxGui:new(self._ws,
                                                    self._data.title or "",
                                                    self._data.text or "",
                                                    self._data.user_text or "",
                                                    self._data, text_config)
        ...
        self._resolution_changed_callback =
            callback(self, self, "resolution_changed_callback")
        managers.viewport:add_resolution_changed_func(
            self._resolution_changed_callback)
    ...
    function TextInputDialog:set_input_enabled(enabled)
        ...
        if managers.controller:get_default_wrapper_type() == "pc" or
            managers.controller:get_default_wrapper_type() == "steam" then
        ...
        if enabled then
            local dialog_panel = self._panel_script and
                                     self._panel_script._panel
            if not self._kb_connected and self._ws and dialog_panel then
                local kb = Input:keyboard()
                self._ws:connect_keyboard(kb)

                dialog_panel:enter_text(function(that, char)
                    self:on_enter_text(char)
                end)
                dialog_panel:key_press(function(that, key)
                    self:on_key_press(key)
                end)
                dialog_panel:key_release(
                    function(that, key)
    ...
    function TextInputDialog:mouse_moved(o, x, y)
        if self._panel_script and alive(self._panel_script._text_input_panel) then
            ...
            if status then return true, "link" end
        end
    ...
    function TextInputDialog:mouse_pressed(o, button, x, y)
        if button == Idstring("0") then
            if self._panel_script and
                alive(self._panel_script._text_input_panel) then
                local x, y = managers.mouse_pointer:convert_1280_mouse_pos(x, y)
                local status = self._panel_script._text_input_panel:inside(x, y)
    ...
    function TextInputDialog:on_key_press(key)
        ...
        if key == Idstring("backspace") then
            if self._data.text_input_focus then
                ...
                self._next_key_send_t = Application:time() +
                                            TextInputDialog.KEY_INIT_DELAY
            end
    ...
    function TextInputDialog:on_key_release(key) self._last_key_pressed = nil end
    ...
    function TextInputDialog:update_input(t, dt)
        if self._data.text_input_focus then
            if self._last_key_pressed and self._next_key_send_t < t then
                ...
                self._panel_script:update_user_text(text)
                self._next_key_send_t = self._next_key_send_t +
                                            TextInputDialog.KEY_DELAY

                self._data.user_text = text
            end

            local scroll = self._controller:get_input_axis("menu_scroll")
            if scroll.y > self.MOVE_AXIS_LIMIT or scroll.y <
                -self.MOVE_AXIS_LIMIT then
                self._panel_script:set_textinput_selected(false)
                self._data.text_input_focus = false
            end
        end
    ...
    function TextInputDialog:chk_mouse_pointer_status()
        ...
        elseif managers.controller:get_default_wrapper_type() == "pc" or
            managers.controller:get_default_wrapper_type() == "steam" then
    ...
    function TextInputDialog:button_pressed(button_index)
        ...
        if callback_func then callback_func(button_index, self._data) end
    end
elseif requiresScript == "lib/managers/menu/specializationboxgui" then
    ...
    function TextInputBoxGui:_create_text_box(ws, title, text, user_text,
                                              content_data, config, ...)
        text = text .. "\n\n."

        local panel = TextInputBoxGui.super._create_text_box(self, ws, title,
                                                             text, content_data,
                                                             config, ...)
    ...
    function TextInputBoxGui:update_user_text(text)
        ...
        if text_box then
            ...
            if cursor then cursor:set_x(text_box:right()) end
        end
    end
