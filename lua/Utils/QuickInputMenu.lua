...
function QuickInputMenu:new(...) return self:init(...) end

function QuickInputMenu:init(title, text, user_text, options, show_immediately,
                             config)

    QuickInputMenu._menu_id_index = QuickInputMenu._menu_id_index + 1
    self.dialog_data = {
        id = QuickInputMenu._menu_id_key ..
            tostring(QuickInputMenu._menu_id_index),
        title = title,
        text = text,
    ...
    if (not options) or (options ~= nil and type(options) ~= "table") or
        (options ~= nil and type(options) == "table" and #options == 0) then
        add_default = true
    end
    if add_default then
        local tbl = {text = "OK", is_cancel_button = true}
    ...
    for _, option in ipairs(options) do
        ...
        local button = {}
        local callback_data = {data = option.data, callback = option.callback}
    ...
    if show_immediately then self:show() end
...
function QuickInputMenu:show() self:Show() end
...
function QuickInputMenu:hide() self:Hide() end
