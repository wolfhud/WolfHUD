...
function OutlinedText:set_outlines_color(color)
    if color then self:_call_func_bgs("set_color", color:with_alpha(0.5)) end
end

function OutlinedText:remove()
    self._parent:remove(self._text)
    for _, bg in pairs(self._bgs) do self._parent:remove(bg) end
end
...
function OutlinedText:set_center(...)
    return self:_call_func_text("set_center", ...)
end
function OutlinedText:set_center_x(...)
    return self:_call_func_text("set_center_x", ...)
end
function OutlinedText:set_center_y(...)
    return self:_call_func_text("set_center_y", ...)
end
...
function OutlinedText:set_font_size(...)
    return self:_call_func_all("set_font_size", ...)
end
function OutlinedText:set_visible(...)
    return self:_call_func_text("set_visible", ...)
end
...
function OutlinedText:set_color(...)
    return self:_call_func_text("set_color", ...)
end
function OutlinedText:set_alpha(...) return
    self:_call_func_all("set_alpha", ...) end
...
function OutlinedText:_call_func_all(func, ...)
    local results
    if self._text[func] then results = {self._text[func](self._text, ...)} end
...
function OutlinedText:_call_func_bgs(func, ...)
    for _, bg in ipairs(self._bgs) do if bg[func] then bg[func](bg, ...) end end
