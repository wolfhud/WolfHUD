if string.lower(RequiredScript) == "lib/managers/hud/hudobjectives" then
    ...
    function HUDObjectives:init(hud)
        if alive(self._panel) then hud.panel:remove(self._panel) end
        ...
        self._panel = hud.panel:panel({
            ...
            h = 130,
            w = 400,
            x = (WolfHUD:getSetting({"TabStats", "CLOCK_MODE"}, 3) == 4) and 0 or
                80,
            valign = "top"
        })

        self._bg_box = HUDBGBox_create(self._panel, {w = 400, h = 38})
    ...
    function HUDObjectives:activate_objective(data)
        ...
        local width, height, wrapped_text =
            self:_get_wrapped_text_dimensions(utf8.to_upper(data.text))
        ...
        if not self._active_move then
            self._bg_box:stop()
            self._bg_box:animate(callback(self, self,
                                          "_animate_update_objective"))
        end
    ...
    function HUDObjectives:update_amount_objective(data, hide_animation)
        if data.id ~= self._active_objective_id then return end
        local amount = (data.current_amount or 0)
        self._amount_text:set_text(amount .. "/" .. data.amount)
        self._amount_text:set_left(self._objective_text:right() +
                                       HUDObjectives._TEXT_MARGIN)
        self._amount_text:set_bottom(self._objective_text:h() +
                                         HUDObjectives._TEXT_MARGIN)
        self._bg_box:set_w(HUDObjectives._TEXT_MARGIN * 3 +
                               self._objective_text:w() +
                               self:_get_text_dimensions(
                                   self._amount_text:text()).w)
        self._amount_text:set_visible(true)
        self._amount_text:stop()
        if not hide_animation and amount > 0 then
            self._amount_text:animate(
                callback(self, self, "_animate_new_amount"))
        else
    ...
    function HUDObjectives:remind_objective(id)
        if id ~= self._active_objective_id then return end
        if not self._active_move then
            self._bg_box:stop()
            self._bg_box:animate(callback(self, self,
                                          "_animate_update_objective"))
    ...
    function HUDObjectives:_animate_new_amount(object)
        ...
        while t > 0 do
            ...
            t = t - dt
            object:set_color(Color(1, 1, 1,
                                   1 - (0.5 * math.sin(t * 360 * 2) + 0.5)))
        end
        object:set_color(Color(1, 1, 1, 1))
    end
    ...
    function HUDObjectives:_animate_update_objective(object)
        ...
        while t > 0 do
            ...
            object:set_y((self._offset_y or 0) +
                             math.round(
                                 (1 + math.sin((TOTAL_T - t) * 450 * 2)) *
                                     (HUDObjectives._BOUNCE * (t / TOTAL_T))))
        end
        object:set_y(self._offset_y or 0)
    end

    function HUDObjectives:_get_text_dimensions(text_string)
        local string_width_measure_text_field =
            self._panel:child("string_dimensions") or self._panel:text(
                {
    ...
    function HUDObjectives:_get_wrapped_text_dimensions(text_string)
        local layout_text_field = self._panel:child("layout") or
                                      self._panel:text(
                                          {
                    name = "layout",
        ...
        local line_breaks = table.collect(layout_text_field:line_breaks(),
                                          function(index)
            return index + 1
        end)
        local wrapped_lines = {}
        for line = 1, #line_breaks do
            ...
            local string_range = utf8.sub(text_string, range_start,
                                          (range_end or 0) - 1)
            table.insert(wrapped_lines, string.trim(string_range))
        end
        local wrapped_text = ""
        local w, h = 0, layout_text_field:font_size() *
                         math.max(#wrapped_lines, 1)
    ...
    function HUDObjectives:apply_offset(offset)
        if offset and offset ~= self._offset_y then
            ...
            if alive(self._bg_box) then
                self._bg_box:stop()
                self._bg_box:animate(callback(self, self, "_animate_move"),
                                     self._bg_box:x(), self._offset_y, true)
                self._panel:animate(callback(self, self, "_animate_move"),
                                    (self._offset_y > 40 or
                                        WolfHUD:getSetting(
                                            {"TabStats", "CLOCK_MODE"}, 3) == 4) and
                                        0 or 80, self._panel:y(), true)
            end
        end
        if managers.hud and managers.hud.change_list_setting then
            managers.hud:change_list_setting("left_list_height_offset",
                                             self._offset_y +
                                                 (self._bg_box:w() > 0 and
                                                     (self._bg_box:h() +
                                                         HUDObjectives._BOUNCE) or
                                                     40) +
                                                 HUDObjectives._TEXT_MARGIN)
        end
    end
...
elseif string.lower(RequiredScript) == "lib/managers/hud/hudheisttimer" then
    ...
    function HUDHeistTimer:init(hud, tweak_hud)
        ...
        self._enabled =
            (WolfHUD:getSetting({"TabStats", "CLOCK_MODE"}, 3) ~= 4) and
                not (tweak_hud and tweak_hud.no_timer)
...
elseif string.lower(RequiredScript) ==
    "core/lib/managers/subtitle/coresubtitlepresenter" then
    core:module("CoreSubtitlePresenter")
    local _on_resolution_changed_original =
        OverlayPresenter._on_resolution_changed
    ...
    function OverlayPresenter:apply_bottom_offset()
        if self.__subtitle_panel and self._bottom_off then
            self.__subtitle_panel:set_height(
                self._bottom_off or self.__subtitle_panel:h())
