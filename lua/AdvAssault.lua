if string.lower(RequiredScript) == "lib/managers/hud/hudassaultcorner" then
    ...
    local _set_hostage_offseted_original =
        HUDAssaultCorner._set_hostage_offseted
    local set_buff_enabled_original = HUDAssaultCorner.set_buff_enabled
    local show_point_of_no_return_timer_original =
        HUDAssaultCorner.show_point_of_no_return_timer
    local hide_point_of_no_return_timer_original =
        HUDAssaultCorner.hide_point_of_no_return_timer
    local show_casing_original = HUDAssaultCorner.show_casing
    local hide_casing_original = HUDAssaultCorner.hide_casing
    local set_assault_wave_number_original =
        HUDAssaultCorner.set_assault_wave_number
    local _animate_wave_started_original =
        HUDAssaultCorner._animate_wave_started
    local _animate_wave_completed_original =
        HUDAssaultCorner._animate_wave_completed
    ...
    function HUDAssaultCorner:init(...)
        ...
        if self:should_display_waves() then
            ...
            if alive(wave_panel) then wave_panel:set_alpha(0) end
            ...
            if alive(assault_panel) then
                self._wave_text = assault_panel:text(
                        ...
                        font_size = tweak_data.hud
                            .active_objective_title_font_size * 0.9
                    })
                self._wave_text:set_top(
                    self._bg_box and self._bg_box:bottom() or 40)
                self._wave_text:set_right(
                    self._bg_box and self._bg_box:right() or 575)
            end
        end
    ...
    function HUDAssaultCorner:update_banner_pos()
        ...
        local banner_pos = math.clamp(WolfHUD:getSetting(
                                          {"AssaultBanner", "POSITION"}, 2), 1,
                                      3)
        local assault_panel = self._hud_panel:child("assault_panel")
        local buffs_panel = self._hud_panel:child("buffs_panel")
        local point_of_no_return_panel =
            self._hud_panel:child("point_of_no_return_panel")
        local casing_panel = self._hud_panel:child("casing_panel")
        if alive(assault_panel) and alive(buffs_panel) and
            alive(point_of_no_return_panel) and alive(casing_panel) then
            if banner_pos < 2 then -- Quite messy, but all the panels in this class are far wider than they would need to be, giving "false information" on their w() function...
                ...
                assault_panel:set_right((buffs_panel:visible() and
                                            buffs_panel:right() or 80) +
                                            self._bg_box:w() + 6 +
                                            assault_panel:child(
                                                "icon_assaultbox"):w())
                point_of_no_return_panel:set_right(
                    80 + self._bg_box:w() + 3 +
                        point_of_no_return_panel:child("icon_noreturnbox"):w())
                casing_panel:set_right(80 + self._bg_box:w() + 3 +
                                           casing_panel:child("icon_casingbox")
                                               :w())
            elseif banner_pos == 2 then
                assault_panel:set_right(hud_w / 2 + self._bg_box:w() / 2 +
                                            assault_panel:child(
                                                "icon_assaultbox"):w() + 3)
                buffs_panel:set_x(
                    assault_panel:left() + self._bg_box:left() - 3 -
                        buffs_panel:w())
                point_of_no_return_panel:set_right(
                    hud_w / 2 +
                        (self._bg_box:w() +
                            point_of_no_return_panel:child("icon_noreturnbox")
                                :w()) / 2)
                casing_panel:set_right(hud_w / 2 +
                                           (self._bg_box:w() +
                                               casing_panel:child(
                                                   "icon_casingbox"):w()) / 2)
            else
    ...
    function HUDAssaultCorner:update_hudlist_offset(banner_visible)
        banner_visible = banner_visible or banner_visible == nil and
                             (self._assault or self._point_of_no_return or
                                 self._casing)
        local banner_pos = math.clamp(WolfHUD:getSetting(
                                          {"AssaultBanner", "POSITION"}, 2), 1,
                                      3)
        if managers.hud and banner_pos ~= 2 then
            local offset = banner_visible and
                               ((self._bg_box and self._bg_box:bottom() or 0) +
                                   (self:should_display_waves() and
                                       self._wave_text:h() or 0) + 12) or 0
            if banner_pos > 2 and HUDListManager then
                managers.hud:change_list_setting("right_list_height_offset",
                                                 offset)
            elseif banner_pos < 2 then
                if managers.hud._hud_objectives and
                    managers.hud._hud_objectives.apply_offset then
    ...
    function HUDAssaultCorner:locked_assault(status)
        local assault_panel = self._hud_panel:child("assault_panel")
        local icon_assaultbox = assault_panel and
                                    assault_panel:child("icon_assaultbox")
        local image
        if status then
...
elseif string.lower(RequiredScript) == "lib/managers/hudmanagerpd2" then
    ...
    function HUDManager:sync_start_assault(...)
        ...
        if Network:is_server() then self:_locked_assault() end
    end
    ...
    function HUDManager:_create_downed_hud(...)
        ...
        if banner_pos == 2 and self._hud_player_downed then
            ...
            local timer_msg = downed_panel and
                                  downed_panel:child("downed_panel"):child(
                                      "timer_msg")
    ...
    function HUDManager:_create_custody_hud(...)
        ...
        if banner_pos == 2 and self._hud_player_custody then
            ...
            local timer_msg = custody_panel and
                                  custody_panel:child("custody_panel") and
                                  custody_panel:child("custody_panel"):child(
                                      "timer_msg")
    ...
    function LocalizationManager:hud_adv_assault()
        if WolfHUD:getSetting({"AssaultBanner", "USE_ADV_ASSAULT"}, true) then
            if managers.hud and managers.hud:is_assault_locked() then
                ...
                local assault_data = Network:is_server() and gai_state and
                                         gai_state._task_data.assault
                if tweak and gai_state and assault_data and assault_data.active then
                    local get_value =
                        gai_state._get_difficulty_dependent_value or
                            function() return 0 end
                    local get_mult = gai_state._get_balancing_multiplier or
                                         function()
                            return 0
                        end
                    local phase = self:text("wolfhud_advassault_phase_title") ..
                                      "  " ..
                                      self:text(
                                          "wolfhud_advassault_phase_" ..
                                              assault_data.phase)

                    local spawns = get_value(gai_state, tweak.force_pool) *
                                       get_mult(gai_state,
                                                tweak.force_pool_balance_mul)
                    local spawns_left = self:text(
                                            "wolfhud_advassault_spawns_title") ..
                                            "  " ..
                                            math.round(
                                                math.max(
                                                    spawns -
                                                        assault_data.force_spawned,
                                                    0))

                    local time_left = assault_data.phase_end_t - gai_state._t +
                                          350
                    if assault_data.phase == "build" then
                        local sustain_duration =
                            math.lerp(get_value(gai_state,
                                                tweak.sustain_duration_min),
                                      get_value(gai_state,
                                                tweak.sustain_duration_max), 0.5) *
                                get_mult(gai_state,
                                         tweak.sustain_duration_balance_mul)
                        time_left = time_left + sustain_duration +
                                        tweak.fade_duration
                    ...
                    if time_left < 0 then
                        ...
                        time_left =
                            self:text("wolfhud_advassault_time_title") .. "  " ..
                                string.format("%.2f", time_left)
                    end

                    local spacer = string.rep(" ", 10)
                    local sep = string.format("%s%s%s", spacer, self:text(
                                                  "hud_assault_end_line"),
                                              spacer)
                    return string.format("%s%s%s%s%s", phase, sep, spawns_left,
                                         sep, time_left)
                end
            end
        end
