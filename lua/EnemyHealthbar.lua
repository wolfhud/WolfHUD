if string.lower(RequiredScript) == "lib/managers/hudmanager" then
    ...
    Hooks:PostHook(HUDManager, "_player_hud_layout",
                   "WolfHUDPostHUDManagerPlayerInfoHUDLayout", function(self)
        ...
        local unit_health_main = managers.hud:script(
                                     PlayerBase.PLAYER_INFO_HUD_PD2).panel:panel(
                                     {
        ...
        self._unit_health_enemy_location =
            self._unit_health_panel:text(
                {
                    name = "unit_health_enemy_location",
                    text = "^",
                    blend_mode = "normal",
                    visible = WolfHUD:getSetting(
                        {"EnemyHealthbar", "SHOW_POINTER"}, false),
                    alpha = 0.75,
                    halign = "center",
                    font = "fonts/font_medium_shadow_mf",
                    font_size = 20,
        ...
        self._unit_bar:set_center(self._unit_health_panel:center_x(),
                                  self._unit_health_panel:center_y() - 190)
        self._unit_bar_bg:set_center(self._unit_health_panel:center_x(),
                                     self._unit_health_panel:center_y() - 190)
        ...
        self._unit_health_enemy_location:set_center_x(
            self._unit_bar_bg:center_x())
    ...
    function HUDManager:set_unit_health_visible(visible, shield)
        if visible and self._shield ~= shield then
            ...
            self._bar_text_rect = self._shield and self._shield_text_rect or
                                      self._health_text_rect
        end

        if visible == true and not self._unit_health_visible and
            WolfHUD:getSetting({"EnemyHealthbar", "ENABLED"}, true) then

            self._unit_health_visible = true
            self._unit_health_enemy_location:set_visible(
                WolfHUD:getSetting({"EnemyHealthbar", "SHOW_POINTER"}, false))
            self._unit_health_panel:stop()
            self._unit_health_panel:animate(
                function(p)
                    self._unit_health_panel:set_visible(true)

                    over(0.25, function(o)
                        self._unit_health_panel:set_alpha(
                            math.lerp(self._unit_health_panel:alpha(), 1, o))
                    end)
                end)
        ...
        elseif visible == false and self._unit_health_visible then
            ...
            self._unit_health_panel:animate(
                function(p)
                    ...
                    over(1.5, function(o)
                        self._unit_health_panel:set_alpha(
                            math.lerp(self._unit_health_panel:alpha(), 0, o))
                    end)
    ...
    function HUDManager:set_unit_health(current, total, tweak_table)
        ...
        if total > 0 then
            self._unit_health_text:set_text(
                string.format("%s/%s",
            ...
            self._unit_health_text:set_text(
                string.format("%s", managers.money:add_decimal_marks_to_string(
                                  tostring(current))))
        end
        ...
        self._unit_health_text:set_color(
            _r <= 0.1 and Color.red or _r <= 0.25 and Color.yellow or
                Color.white)
        ...
        self._unit_bar:animate(function(p)
            if rn < r then
                over(0.2, function(o)
                    ...
                    self._unit_bar:set_texture_rect(self._bar_text_rect[1],
                                                    self._bar_text_rect[2],
                                                    math.lerp(r, rn, o),
                                                    self._bar_text_rect[4])
                end)
            end
...
elseif string.lower(RequiredScript) ==
    ...
    Hooks:PostHook(PlayerStandard, "_update_fwd_ray",
                   "WolfHUDPostPlayerStandardUpdate", function(self, t, dt)
        if self._fwd_ray and self._fwd_ray.unit and type(self._fwd_ray.unit) ==
            "userdata" then
            ...
            if alive(unit) then
                if unit:in_slot(25) and not unit:character_damage():dead() and
                    (table.contains(managers.groupai:state():turrets() or {},
                                    unit) or
                        WolfHUD:getSetting({"EnemyHealthbar", "SHOW_CIVILIAN"},
                                           true) and Network:is_server()) then
                    self._last_unit = nil
                    visible = true
                    name_id = unit:base():get_name_id() or "TURRET"
                    if not unit:character_damage():needs_repair() then
                        shield = true
                        health = unit:character_damage()._shield_health * 10 or
                                     0
                        max_health =
                            unit:character_damage()._SHIELD_HEALTH_INIT * 10 or
                                0
                    else
                        health = unit:character_damage()._health * 10 or 0
                        max_health =
                            unit:character_damage()._HEALTH_INIT * 10 or 0
                    end
                elseif alive(unit) and (unit:in_slot(12) or
                    WolfHUD:getSetting({"EnemyHealthbar", "SHOW_CIVILIAN"},
                                       false) and
                    (unit:in_slot(21) or unit:in_slot(22)) or unit:in_slot(16) and
                    Network:is_server()) and not unit:character_damage():dead() then
                    ...
                    if name_id == "robbers_safehouse" and unit:interaction() then
                        name_id =
                            CriminalsManager.convert_new_to_old_character_workname(
                                unit:interaction().character or name_id)
                    end
                elseif alive(unit) and unit:in_slot(39) and
                    WolfHUD:getSetting({"EnemyHealthbar", "SHOW_VEHICLE"}, true) and
                    unit:vehicle_driving() and not self._seat then
                    self._last_unit = nil
                    visible = true
                    health = unit:character_damage()._health or 0
                    max_health = unit:character_damage()._current_max_health or
                                     0
            ...
            if not visible and self._last_unit and alive(self._last_unit) then
                ...
                max_health = self._last_unit:character_damage()._HEALTH_INIT *
                                 10 or 0
                name_id = self._last_unit:base()._tweak_table or "ENEMY"

                if name_id == "robbers_safehouse" and
                    self._last_unit:interaction() then
                    name_id =
                        CriminalsManager.convert_new_to_old_character_workname(
                            self._last_unit:interaction().character or name_id)
                end

                local angle = (self:getUnitRotation(self._last_unit) + 360) %
                                  360
                if self._last_unit:character_damage():dead() or
                    (angle < 350 and angle > 10) then
                    visible = false
                    self._last_unit = nil
                else
                    visible = true
            ...
            if health and (name or name_id) then
                managers.hud:set_unit_health(math.floor(health or 0),
                                             math.floor(max_health or 0),
                                             name_id or
                                                 string.upper(name or "UNKNOWN"))
            end
        else
    ...
    function PlayerStandard:getUnitRotation(unit)
        ...
        local rotation = math.floor(vector:to_polar_with_reference(forward,
                                                                   math.UP).spin)
...
elseif string.lower(RequiredScript) == "lib/states/ingamearrested" then
    Hooks:PostHook(IngameArrestedState, "at_enter",
                   "WolfHUDPostIngameArrestedAtEnter", function(self)
