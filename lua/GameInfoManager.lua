...
if string.lower(RequiredScript) == "lib/setups/setup" then
    ...
    GameInfoManager._TIMER_CALLBACKS = {
        default = {
            ...
            set = function(timers, key, timer)
                GameInfoManager._TIMER_CALLBACKS.default.update(timers, key,
                                                                Application:time(),
                                                                timer)
            end,
            start_count_up = function(timers, key)
                if timers[key] and timers[key].ext._visible then
                    GameInfoManager._TIMER_CALLBACKS.default.set_active(timers,
                                                                        key,
                                                                        true)
                    GameInfoManager._TIMER_CALLBACKS.default.set_jammed(timers,
                                                                        key,
                                                                        false)
                end
            end,
            ...
            pause = function(timers, key)
                GameInfoManager._TIMER_CALLBACKS.default.set_jammed(timers, key,
                                                                    true)
            end,
            ...
            stop = function(timers, key)
                GameInfoManager._TIMER_CALLBACKS.default.set_active(timers, key,
                                                                    false)
            end,
            ...
            update = function(timers, key, t, timer, timer_ratio)
                if timers[key] then
                    ...
                    managers.gameinfo:_listener_callback("timer", "update", key,
                                                         timers[key])
                end
            end,
            set_active = function(timers, key, status)
                if timers[key] and timers[key].active ~= status then
                    ...
                    managers.gameinfo:_listener_callback("timer", "set_active",
                                                         key, timers[key])
                end
            end,
            set_jammed = function(timers, key, status)
                if timers[key] and timers[key].jammed ~= status then
                    ...
                    managers.gameinfo:_listener_callback("timer", "set_jammed",
                                                         key, timers[key])
                end
            end,
            set_powered = function(timers, key, status)
                if timers[key] and timers[key].powered ~= status then
                    ...
                    managers.gameinfo:_listener_callback("timer", "set_powered",
                                                         key, timers[key])
                end
            end,
            -- Only securitylocks
            set_current_bar = function(timers, key, current_bar)
                if timers[key] and timers[key].current_bar ~= current_bar then
                    ...
                    managers.gameinfo:_listener_callback("timer",
                                                         "set_current_bar", key,
                                                         timers[key])
                end
            end,
        ...
        overrides = {
            ...
            [132864] = { -- Meltdown vault temperature
                set = function(timers, key, timer)
                    if timer > 0 then
                        GameInfoManager._TIMER_CALLBACKS.default.set_active(
                            timers, key, true)
                    end
                    GameInfoManager._TIMER_CALLBACKS.default.set(timers, key,
                                                                 timer)
                end,
                start_count_down = function(timers, key)
                    GameInfoManager._TIMER_CALLBACKS.default.set_active(timers,
            ...
            [101936] = {
                pause = function(...)
                    GameInfoManager._TIMER_CALLBACKS.overrides
                        .stop_on_pause(...)
                end
            }, -- GO Bank time lock
            [139706] = {
            ...
            }, -- Hoxton Revenge alarm	(UNTESTED)
            [132675] = {
                pause = function(...)
                    GameInfoManager._TIMER_CALLBACKS.overrides
                        .stop_on_loud_pause(...)
                end
            }, -- Hoxton Revenge panic room time lock	(UNTESTED)
            [133922] = {
                pause = function(...)
                    GameInfoManager._TIMER_CALLBACKS.overrides
                        .stop_on_loud_pause(...)
                end
            }, -- The Diamond pressure plates timer
            [130022] = {
                pause = function(...)
                    GameInfoManager._TIMER_CALLBACKS.overrides
                        .stop_on_loud_pause(...)
    ...
    GameInfoManager._INTERACTIONS = {
        ...
        COMPOSITE_LOOT_UNITS = {
            ...
            [100777] = 3,
            [100773] = 2,
            [100771] = 1 -- Slaughterhouse alt 2.
        },
        CONDITIONAL_COMPOSITE_LOOT_UNITS = {
            disassemble_turret = function(unit) -- Turret - Armored Transport: Train
                return
                    unit:body("body_01") and unit:body("body_01"):enabled() and
                        3 or unit:body("body_02") and
                        unit:body("body_02"):enabled() and 2 or
                        unit:body("body_03") and unit:body("body_03"):enabled() and
                        1 or 0
            end
        },
...
lounge		100421		100448			102049
            ...
            -- FF3 lounge vault
            [100548] = function()
                return GameInfoManager._INTERACTIONS.CONDITIONAL_IGNORE_IDS
                           .ff3_vault(100448, 102049)
            end,
            [100549] = function()
                return GameInfoManager._INTERACTIONS.CONDITIONAL_IGNORE_IDS
                           .ff3_vault(100448, 102049)
    ...
    GameInfoManager._INTERACTIONS.IGNORE_IDS.watchdogs_2_day =
        table.deep_map_copy(GameInfoManager._INTERACTIONS.IGNORE_IDS.watchdogs_2)
    GameInfoManager._INTERACTIONS.IGNORE_IDS.welcome_to_the_jungle_1_night =
        table.deep_map_copy(GameInfoManager._INTERACTIONS.IGNORE_IDS
    ...
    GameInfoManager._UNITS = {
        TWEAK_ID_BY_NAME = {
            [tostring(Idstring(
                          "units/pd2_dlc_born/characters/npc_male_mechanic/npc_male_mechanic"))] = "mechanic",
            [tostring(Idstring(
                          "units/pd2_dlc_born/characters/npc_male_mechanic/npc_male_mechanic_husk"))] = "mechanic",
            [tostring(Idstring(
                          "units/pd2_skirmish/characters/civ_male_bank_manager_hostage/civ_male_bank_manager_hostage"))] = "civ_hostage",
            [tostring(Idstring(
                          "units/pd2_skirmish/characters/civ_male_bank_manager_hostage/civ_male_bank_manager_hostage_husk"))] = "civ_hostage",
            [tostring(Idstring(
                          "units/pd2_skirmish/characters/civ_female_museum_curator_hostage/civ_female_museum_curator_hostage"))] = "civ_hostage",
            [tostring(Idstring(
                          "units/pd2_skirmish/characters/civ_female_museum_curator_hostage/civ_female_museum_curator_hostage_husk"))] = "civ_hostage",
            [tostring(Idstring(
                          "units/pd2_skirmish/characters/civ_female_drug_lord_hostage/civ_female_drug_lord_hostage"))] = "civ_hostage",
            [tostring(Idstring(
                          "units/pd2_skirmish/characters/civ_female_drug_lord_hostage/civ_female_drug_lord_hostage_husk"))] = "civ_hostage",
            [tostring(Idstring(
                          "units/pd2_skirmish/characters/civ_male_prisoner_hostage/civ_male_prisoner_hostage"))] = "civ_hostage",
            [tostring(Idstring(
                          "units/pd2_skirmish/characters/civ_male_prisoner_hostage/civ_male_prisoner_hostage_husk"))] = "civ_hostage"
        }
    }
    ...
    GameInfoManager._BUFFS = {
        on_activate = {
            armor_break_invulnerable_debuff = function(id, data)
                local upgrade_value = managers.player:upgrade_value("temporary",
                                                                    "armor_break_invulnerable")
                managers.gameinfo:event("timed_buff", "activate",
                                        "armor_break_invulnerable", {
                    t = data.t,
                    duration = upgrade_value and upgrade_value[1] or 0
                })
            end
        },
        on_set_duration = {
        ...
        on_set_value = {
            overkill = function(id, data)
                if managers.player:has_category_upgrade("player",
                                                        "overkill_all_weapons") then
                    managers.gameinfo:event("buff", "set_value",
                                            "overkill_aced",
                                            {value = data.value})
                end
            end
        ...
        cooldown = {long_dis_revive = "inspire_revive_debuff"},
        -- Team upgrades
        damage_dampener = {
        ...
        damage = {hostage_absorption = {id = "forced_friendship", level = 0}}
        --[[
		weapon = {
    ...
    function GameInfoManager:init()
        ...
        self._auto_expire_timers = {on_expire = {}, expire_t = {}}
        self._timed_buff_expire_clbk = callback(self, self,
                                                "_on_timed_buff_expired")
        self._timed_stack_expire_clbk = callback(self, self,
                                                 "_on_timed_stack_expired")
        self._player_actions_expire_clbk =
            callback(self, self, "_on_player_action_expired")
    end
    ...
    function GameInfoManager:update(t, dt)
        ...
        while self._scheduled_callbacks[1] and self._scheduled_callbacks[1].t <=
            t do
    ...
    function GameInfoManager:add_scheduled_callback(id, delay, clbk, ...)
        ...
        table.insert(self._scheduled_callbacks, pos,
                     {id = id, t = t, clbk = clbk, args = {...}})
    ...
    function GameInfoManager:get_bodybag_amount() return self._bodybag_amount end
    ...
    function GameInfoManager:_timer_event(event, key, ...)
        if event == "create" then
            if not self._timers[key] then
                ...
                self:_listener_callback("timer", "create", key,
                                        self._timers[key])
            end
        elseif event == "destroy" then
            if self._timers[key] then
                GameInfoManager._TIMER_CALLBACKS.default.set_active(
                    self._timers, key, false)
                self:_listener_callback("timer", "destroy", key,
                                        self._timers[key])
                self._timers[key] = nil
            end
        elseif self._timers[key] then
            local timer_id = self._timers[key].id
            local timer_override =
                GameInfoManager._TIMER_CALLBACKS.overrides[timer_id]

            if timer_override and timer_override[event] then
                timer_override[event](self._timers, key, ...)
            else
                GameInfoManager._TIMER_CALLBACKS.default[event](self._timers,
                                                                key, ...)
            end
        end
    end
    ...
    function GameInfoManager:_unit_event(event, key, data)
        if event == "add" then
            if not self._units[key] then
                ...
                local unit_type = lookup[tostring(data.unit:name())] or
    ...
    function GameInfoManager:_unit_count_event(event, unit_type, value)
        if event == "change" then
            if value ~= 0 then
                self._unit_count[unit_type] =
                    (self._unit_count[unit_type] or 0) + value
                self:_listener_callback("unit_count", "change", unit_type, value)
            end
        elseif event == "set" then
            self:_unit_count_event("change", unit_type,
                                   value - (self._unit_count[unit_type] or 0))
        end
    end
    ...
    function GameInfoManager:_minion_event(event, key, data)
        if event == "add" then
            if not self._minions[key] then
                ...
                if self._minions[key].unit:base()._tweak_table:find("security") or
                    self._minions[key].unit:base()._tweak_table:find("gensec") then
        ...
        elseif self._minions[key] then
            if event == "remove" then
                self:_listener_callback("minion", "remove", key,
                                        self._minions[key])
                if self._minions[key].unit:base()._tweak_table:find("security") or
                    self._minions[key].unit:base()._tweak_table:find("gensec") then
                    self:_unit_count_event("change", "sec_minion", -1)
                ...
                elseif event == "set_damage_resistance" then
                    self._minions[key].damage_resistance =
                        data.damage_resistance
                elseif event == "set_damage_multiplier" then
                    self._minions[key].damage_multiplier =
    ...
    function GameInfoManager:_interactive_unit_event(event, key, data)
        ...
        if lookup.IGNORE_IDS[level_id] and
            (lookup.IGNORE_IDS[level_id][data.editor_id] or
                lookup.IGNORE_IDS[level_id][data.interact_id]) then
            return
        end
        ...
        if interact_clbk then
            ...
            local carry_id = data.unit:carry_data() and
                                 data.unit:carry_data():carry_id() or
                                 lookup.INTERACTION_TO_CARRY[data.interact_id] or
                                 (self._loot[key] and self._loot[key].carry_id)

            if carry_id then
                print_debug("Loot Interaction: %s - %s (%d)", event, carry_id,
                            data.editor_id, "info")
                data.carry_id = carry_id
                self:_loot_interaction_handler(event, key, data)
            else
                self:_listener_callback("interactable_unit", event, key,
                                        data.unit, data.interact_id, carry_id)
            end
        end
    end
    ...
    function GameInfoManager:_camera_event(event, key, data)
        if event == "create" then
            if not self._cameras[key] then
                local type = GameInfoManager._CAMERAS[tostring(
                                 data.unit:name():key())]
                ...
                if not type then
                    print_debug("UNKNOWN CAMERA UNIT TYPE: %s",
                                tostring(data.unit:name():key()), "warning")
                end
            end
        elseif self._cameras[key] then
            ...
            elseif event == "set_enabled" then
                self._cameras[key].enabled =
                    self._cameras[key].is_drone or data.enabled
            ...
            elseif event == "set_tape_loop_restart_active" then
                self._cameras[key].tape_loop_restart_active =
                    data.tape_loop_restart_active
            ...
            if not self._upd_camera_count then
                ...
                self:add_scheduled_callback("camera_count", 0.1, callback(self,
                                                                          self,
                                                                          "_recount_active_cameras"))
            end
        end
    end
    ...
    function GameInfoManager:_special_equipment_interaction_handler(event, key,
                                                                    data)
        if event == "add" then
            if not self._special_equipment[key] then
                self._special_equipment[key] =
                    {unit = data.unit, interact_id = data.interact_id}
                self:_listener_callback("special_equipment", "add", key,
                                        self._special_equipment[key])
                self:_special_equipment_count_event("change", data.interact_id,
                                                    1,
                                                    self._special_equipment[key])
            end
        elseif event == "remove" then
            if self._special_equipment[key] then
                self:_listener_callback("special_equipment", "remove", key,
                                        self._special_equipment[key])
                self:_special_equipment_count_event("change", data.interact_id,
                                                    -1,
                                                    self._special_equipment[key])
    ...
    function GameInfoManager:_special_equipment_count_event(event, interact_id,
                                                            value, data)
        if event == "change" then
            if value ~= 0 then
                self:_listener_callback("special_equipment_count", "change",
                                        interact_id, value, data)
            end
        end
    end
    ...
    function GameInfoManager:_deployable_interaction_handler(event, key, data)
        local type =
            GameInfoManager._EQUIPMENT.INTERACTION_ID_TO_TYPE[data.interact_id]
        local active = event == "add" or event == "interact"

        if not self._deployables[type][key] and
            GameInfoManager._EQUIPMENT.INTERACTON_ID_ALLOW_CREATE[data.interact_id] then
            if active then
                self:_bag_deployable_event("create", key, {unit = data.unit},
                                           type)
            else
        ...
        if self._deployables[type][key] then
            local offset =
                GameInfoManager._EQUIPMENT.AMOUNT_OFFSETS[data.unit:editor_id()] or
                    GameInfoManager._EQUIPMENT.AMOUNT_OFFSETS[data.interact_id]

            self:_bag_deployable_event("set_active", key, {active = active},
                                       type)

            if active and offset then
                self:_bag_deployable_event("set_amount_offset", key,
                                           {amount_offset = offset}, type)
            end
        end
    end
    ...
    function GameInfoManager:_loot_interaction_handler(event, key, data)
        if event == "add" then
            if not self._loot[key] then
                local composite_lookup =
                    GameInfoManager._INTERACTIONS.COMPOSITE_LOOT_UNITS
                local conditional_lookup =
                    GameInfoManager._INTERACTIONS
                        .CONDITIONAL_COMPOSITE_LOOT_UNITS
                local count_clbk = conditional_lookup[data.editor_id] or
                                       conditional_lookup[data.interact_id]
                local count = count_clbk and count_clbk(data.unit) or
                                  composite_lookup[data.editor_id] or
                                  composite_lookup[data.interact_id] or 1
                local bagged =
                    GameInfoManager._INTERACTIONS.BAGGED_IDS[data.interact_id] and
                        true or false
                ...
                self:_loot_count_event("change", data.carry_id, bagged, count,
                                       self._loot[key])
            end
        elseif self._loot[key] then
            if event == "remove" then
                self:_listener_callback("loot", "remove", key, self._loot[key])
                self:_loot_count_event("change", data.carry_id,
                                       self._loot[key].bagged,
                                       -self._loot[key].count, self._loot[key])
                self._loot[key] = nil
            elseif event == "interact" then
                ...
                self:_loot_count_event("change", data.carry_id, bagged, change,
                                       self._loot[key])
            end
        end
    end

    function GameInfoManager:_loot_count_event(event, carry_id, bagged, value,
                                               data)
        if event == "change" then
            if value ~= 0 then
                self:_listener_callback("loot_count", "change", carry_id,
                                        bagged, value, data)
            end
        end
    end
    ...
    function GameInfoManager:_bag_deployable_event(event, key, data, type)
        if event == "create" then
            if not self._deployables[type][key] then
                ...
                self:_listener_callback(type, "create", key,
                                        self._deployables[type][key])
            end
        elseif self._deployables[type][key] then
            local function update_aggregate_attribute(aggregate_key, attr)
                if not self._deployables[type][aggregate_key] then
                    return
                end

                local total = 0
                for k, v in pairs(self._deployables[type][aggregate_key]
                                      .aggregate_members or {}) do
                ...
                self:_listener_callback(type, "set_" .. attr, aggregate_key,
                                        self._deployables[type][aggregate_key])
                print_debug("UPDATE AGGREGATE %s: %s", tostring(attr),
                            tostring(total), "info")
            end
            ...
            local aggregate_key = lookup[type] or lookup[level_id] and
                                      lookup[level_id][editor_id]
            print_debug("_bag_deployable_event: %s | %s", type,
                        self._deployables[type][key].unit:editor_id(), "info")
            if event == "destroy" then
                self:_listener_callback(type, "destroy", key,
                                        self._deployables[type][key])
                self._deployables[type][key] = nil

                if aggregate_key and self._deployables[type][aggregate_key] then
                    self._deployables[type][aggregate_key].aggregate_members[key] =
                        nil

                    if next(self._deployables[type][aggregate_key]
                                .aggregate_members or {}) == nil then
                        self:_listener_callback(type, "destroy", aggregate_key,
                                                self._deployables[type][aggregate_key])
            ...
            elseif event == "set_active" then
                ...
                if self._deployables[type][key].active ~= data.active then
                    ...
                    self:_listener_callback(type, "set_active", key,
                                            self._deployables[type][key])
                end

                if aggregate_key then
                    self._deployables[type][aggregate_key] =
                        self._deployables[type][aggregate_key] or {
                            position = self._deployables[type][key].unit:interaction()
                                :interact_position(),
                            aggregate_members = {}
                        }
                    self._deployables[type][aggregate_key].aggregate_members[key] =
                    ...
                    if self._deployables[type][aggregate_key].active ~=
                        aggregate_active then
                        self._deployables[type][aggregate_key].active =
                            aggregate_active
                        self:_listener_callback(type, "set_active",
                                                aggregate_key,
                                                self._deployables[type][aggregate_key])
                    end
            ...
            elseif event == "set_owner" then
                ...
                self:_listener_callback(type, "set_owner", key,
                                        self._deployables[type][key])
            ...
            elseif event == "set_upgrades" then
                ...
                self:_listener_callback(type, "set_upgrades", key,
                                        self._deployables[type][key])
            elseif event == "set_max_amount" then
                self._deployables[type][key].max_amount = data.max_amount
                self:_listener_callback(type, "set_max_amount", key,
                                        self._deployables[type][key])
            ...
            elseif event == "set_amount_offset" then
                ...
                self:_listener_callback(type, "set_amount_offset", key,
                                        self._deployables[type][key])
            ...
            elseif event == "set_amount" then
                ...
                self:_listener_callback(type, "set_amount", key,
                                        self._deployables[type][key])
    ...
    function GameInfoManager:_sentry_event(event, key, data)
        ...
        if event == "create" then
            local sentry_type =
                data.unit:base() and data.unit:base():get_type() or ""

            if not self._sentries[key] and
                (sentry_type == "sentry_gun" or sentry_type ==
                    "sentry_gun_silent") then -- GameInfoManager._EQUIPMENT.SENTRY_KEYS[tostring(data.unit:name():key())] then
                self._sentries[key] = {unit = data.unit, kills = 0}
                self:_listener_callback("sentry", event, key,
                                        self._sentries[key])
            end
        elseif self._sentries[key] then
            if event == "set_active" then
                if self._sentries[key].active == data.active then
                    return
                end
    ...
    function GameInfoManager:_temporary_buff_event(event, data)
        ...
        local id = data.level and type(buff_data) == "table" and
                       buff_data[data.level] or buff_data
        ...
        if id then
            ...
            print_debug("Unknown temporary buff event: %s, %s, %s",
                        tostring(event), tostring(data.category),
                        tostring(data.upgrade), "warning")
        end
    end
    ...
    function GameInfoManager:_timed_buff_event(event, id, data)
        ...
        if event == "activate" then
            self:_buff_event("set_duration", id, {
                t = data.t,
                duration = data.duration,
                expire_t = data.expire_t
            })
            if data.expire_t or (data.t and data.duration) then
                self:_add_player_timer_expiration(id, id, data.expire_t or
                                                      (data.t + data.duration),
                                                  self._timed_buff_expire_clbk)
            end
        ...
        elseif self._buffs[id] then
            if event == "set_expire" then
                self:_remove_player_timer_expiration(id)
                self:_add_player_timer_expiration(id, id,
                                                  self._buffs[id].expire_t,
                                                  self._timed_buff_expire_clbk)
    ...
    function GameInfoManager:_timed_stack_buff_event(event, id, data)
        print_debug("_timed_stack_buff_event(%s, %s)", tostring(event),
                    tostring(id), "info")
        ...
        if event == "add_stack" then
            ...
            local expire_t = data.expire_t or data.duration and
                                 (data.duration + t) or t
            local key = string.format("%s_%f_%f", id, t, math.random())

            local i = #self._buffs[id].stacks
            while self._buffs[id].stacks[i] and
                self._buffs[id].stacks[i].expire_t > expire_t do
                i = i - 1
            end
            table.insert(self._buffs[id].stacks, i + 1,
                         {key = key, t = t, expire_t = expire_t})

            self:_listener_callback("buff", "add_timed_stack", id,
                                    self._buffs[id])
            self:_add_player_timer_expiration(key, id, expire_t,
                                              self._timed_stack_expire_clbk)
        end
    end
    ...
    function GameInfoManager:_buff_event(event, id, data)
        ...
        elseif self._buffs[id] then
            ...
            elseif event == "change_stack_count" then
                return self:_buff_event("set_stack_count", id, {
                    stack_count = (self._buffs[id].stack_count or 0) +
                        data.difference
                })
            elseif event == "increment_stack_count" then
                return self:_buff_event("set_stack_count", id, {
                    stack_count = (self._buffs[id].stack_count or 0) + 1
                })
            ...
            elseif event == "set_expire" then
                local expire_t = data.duration and
                                     (data.duration + Application:time()) or
                                     data.expire_t
                return self:_buff_event("set_duration", id, {
                    t = self._buffs[id].t,
                    expire_t = expire_t
                })
            elseif event == "change_expire" then
                local expire_t = data.difference and
                                     (self._buffs[id].expire_t + data.difference) or
                                     data.expire_t
                return self:_buff_event("set_duration", id, {
                    t = self._buffs[id].t,
                    expire_t = expire_t
        ...
        if GameInfoManager._BUFFS[clbk_name] and
            GameInfoManager._BUFFS[clbk_name][id] then
            GameInfoManager._BUFFS[clbk_name][id](id, self._buffs[id])
        end
    end

    function GameInfoManager:_team_buff_event(event, data)
        local buff_data = GameInfoManager._BUFFS[data.category] and
                              GameInfoManager._BUFFS[data.category][data.upgrade]
        ...
        if id then
            ...
            print_debug("Unknown team buff event: %s, %s, %s", event,
                        data.category, data.upgrade, "warning")
        end
    end

    function GameInfoManager:_player_action_event(event, id, data)
        print_debug("_player_action_event(%s, %s)", tostring(event),
                    tostring(id), "info")

        if event == "activate" then
            if not self._player_actions[id] then
                self._player_actions[id] = {}
                self:_listener_callback("player_action", "activate", id,
                                        self._player_actions[id])
            end
        ...
        elseif self._player_actions[id] then
            ...
            elseif event == "set_expire" then
                local expire_t = data.duration and
                                     (data.duration + Application:time()) or
                                     data.expire_t
                return self:_player_action_event("set_duration", id, {
                    t = self._player_actions[id].t,
                    expire_t = expire_t
                })
            elseif event == "change_expire" then
                local expire_t = data.difference and
                                     (self._player_actions[id].expire_t +
                                         data.difference) or data.expire_t
                return self:_player_action_event("set_duration", id, {
                    t = self._player_actions[id].t,
                    expire_t = expire_t
                })
            elseif event == "set_data" then
                self._player_actions[id].data =
                    self._player_actions[id].data or {}
                for k, v in pairs(data) do
                    self._player_actions[id].data[k] = v
                end
            end

            self:_listener_callback("player_action", event, id,
                                    self._player_actions[id])
        end
    end

    function GameInfoManager:register_listener(listener_id, source_type, event,
                                               clbk, keys, data_only)
        local listener_keys

        if keys then
            listener_keys = {}
            for _, key in ipairs(keys) do listener_keys[key] = true end
        end

        self._listeners[source_type] = self._listeners[source_type] or {}
        self._listeners[source_type][event] =
            self._listeners[source_type][event] or {}
        self._listeners[source_type][event][listener_id] =
            {clbk = clbk, keys = listener_keys, data_only = data_only}
    end
    ...
    function GameInfoManager:_listener_callback(source, event, key, ...)
        for listener_id, data in pairs(self._listeners[source] and
                                           self._listeners[source][event] or {}) do
    ...
    function GameInfoManager:_add_player_timer_expiration(key, id, expire_t,
                                                          expire_clbk)
    ...
    function GameInfoManager:_update_player_timer_expiration(ut, udt)
        ...
        while self._auto_expire_timers.expire_t[1] and
            self._auto_expire_timers.expire_t[1].expire_t < t do
    ...
    function GameInfoManager:_on_timed_stack_expired(t, key, id)
        if self._buffs[id].stacks[1] then
            ...
            self:_listener_callback("buff", "remove_timed_stack", id,
                                    self._buffs[id])
    ...
    function GameInfoManager:_recount_active_cameras()
        ...
        for key, cam_data in pairs(self._cameras) do
            if cam_data.enabled ~= false and
                (cam_data.active or Network:is_client()) then
                local id = cam_data.unit and cam_data.unit:editor_id() or key
                print_debug(
                    "Camera (%s): Drone:%s Enabled:%s Active:%s Broken:%s Loop:%s",
                    id, tostring(cam_data.is_drone), tostring(cam_data.enabled),
                    tostring(cam_data.active), tostring(cam_data.broken),
                    tostring(cam_data.tape_loop_expire_t and
                                 cam_data.tape_loop_expire_t >
                                 Application:time() and
                                 string.format("%.2f",
                                               cam_data.tape_loop_expire_t -
                                                   Application:time()) or false),
                    "info")

                if not cam_data.broken then count = count + 1 end
            end
        end
...
if string.lower(RequiredScript) == "lib/units/props/digitalgui" then
    ...
    function DigitalGui:load(data, ...)
        ...
        if state.timer then self:_do_timer_callback("set", state.timer) end
        ...
        if state.timer_paused then self:_do_timer_callback("pause") end
    end
...
if string.lower(RequiredScript) == "lib/units/props/drill" then
    ...
    function Drill:set_autorepair(...)
        ...
        if timer_gui and timer_gui._info_key then
            managers.gameinfo:event("timer", "set_autorepair",
                                    timer_gui._info_key, self._autorepair)
        end
    end
...
if string.lower(RequiredScript) == "lib/units/props/timergui" then
    ...
    function TimerGui:init(unit, ...)
        self._info_key = tostring(unit:key())
        local device_type = unit:base().is_drill and "drill" or
                                unit:base().is_hacking_device and "hack" or
                                unit:base().is_saw and "saw" or "timer"
        if (unit:base().is_drill or unit:base().is_saw) and
            unit:base()._disable_upgrades then
            device_type = string.format("%s_noupgrade", device_type)
        end
        managers.gameinfo:event("timer", "create", self._info_key, unit, self,
                                device_type)
    ...
    function TimerGui:set_background_icons(...)
        if not self._unit:base()._disable_upgrades then
            local skills = self._unit:base().get_skill_upgrades and
                               self._unit:base():get_skill_upgrades()
            ...
            local function player_can_upgrade(drill_upgrades, player_upgrades)
                ...
                return player_upgrades.auto_repair_level_1 >
                           drill_upgrades.auto_repair_level_1 or
                           player_upgrades.auto_repair_level_2 >
                           drill_upgrades.auto_repair_level_2 or
                           player_upgrades.speed_upgrade_level >
                           drill_upgrades.speed_upgrade_level or
                           player_upgrades.silent_drill and
                           not drill_upgrades.silent_drill or
                           player_upgrades.reduced_alert and
                           not drill_upgrades.reduced_alert
            end
            local can_upgrade = player_skills and
                                    player_can_upgrade(skills, player_skills) or
                                    false
            local upgrade_table = {
                restarter = (skills.auto_repair_level_1 or 0) +
                    (skills.auto_repair_level_2 or 0),
                faster = (skills.speed_upgrade_level or 0),
                silent = (skills.reduced_alert and 1 or 0) +
                    (skills.silent_drill and 1 or 0)
            }
            managers.gameinfo:event("timer", "set_upgradable", self._info_key,
                                    can_upgrade)
            managers.gameinfo:event("timer", "set_upgrades", self._info_key,
                                    upgrade_table)
        end
    ...
    function TimerGui:update(unit, t, dt, ...)
        ...
        managers.gameinfo:event("timer", "update", self._info_key, t,
                                self._time_left,
                                self._current_timer / self._timer)
    end
    ...
    function TimerGui:_set_jammed(jammed, ...)
        managers.gameinfo:event("timer", "set_jammed", self._info_key,
                                jammed and true or false)
        return _set_jammed_original(self, jammed, ...)
    end

    function TimerGui:_set_powered(powered, ...)
        managers.gameinfo:event("timer", "set_powered", self._info_key,
                                powered and true or false)
...
if string.lower(RequiredScript) == "lib/units/props/securitylockgui" then
    ...
    function SecurityLockGui:init(unit, ...)
        ...
        managers.gameinfo:event("timer", "create", self._info_key, unit, self,
                                "securitylock")
        managers.gameinfo:event("timer", "set_total_bars", self._info_key,
                                self._bars)
        init_original(self, unit, ...)
    end
...
if string.lower(RequiredScript) == "lib/managers/enemymanager" then
    ...
    function EnemyManager:on_enemy_registered(unit, ...)
        managers.gameinfo:event("unit", "add", tostring(unit:key()),
                                {unit = unit})
...
if string.lower(RequiredScript) ==
    "lib/managers/group_ai_states/groupaistatebase" then
    ...
    local sync_hostage_headcount_original =
        GroupAIStateBase.sync_hostage_headcount
    local convert_hostage_to_criminal_original =
        GroupAIStateBase.convert_hostage_to_criminal
    ...
    function GroupAIStateBase:on_hostage_state(...)
        ...
        managers.enemy:add_delayed_clbk("hostage_count_update", callback(self,
                                                                         self,
                                                                         "_update_hostage_count"),
                                        Application:time() + 0.05)
    end
    ...
    function GroupAIStateBase:convert_hostage_to_criminal(unit, peer_unit, ...)
        ...
        if unit:brain()._logic_data.is_converted then
            ...
            local peer_id = peer_unit and
                                managers.network:session():peer_by_unit(
                                    peer_unit):id() or
                                managers.network:session():local_peer():id()
            local owner_base = peer_id and peer_id ~=
                                   managers.network:session():local_peer():id() and
                                   peer_unit:base() or managers.player
            local damage_mult = (owner_base:upgrade_value("player",
                                                          "convert_enemies_damage_multiplier",
                                                          0) or 0)

            managers.gameinfo:event("minion", "add", key, {unit = unit})
            managers.gameinfo:event("minion", "set_owner", key,
                                    {owner = peer_id})
            if damage_mult > 0 then
                managers.gameinfo:event("minion", "set_damage_multiplier", key,
    ...
    function GroupAIStateBase:sync_converted_enemy(converted_enemy, ...)
        if self._police[converted_enemy:key()] then
            managers.gameinfo:event("minion", "add",
                                    tostring(converted_enemy:key()),
                                    {unit = converted_enemy})
        end
    ...
    function GroupAIStateBase:_update_hostage_count()
        ...
        for u_key, u_data in pairs(managers.enemy:all_enemies()) do
            if alive(u_data.unit) and
                (u_data.unit:brain():surrendered() or
                    u_data.unit:base().mic_is_being_moved) then
                if u_data.unit:base()._tweak_table:find("security") or
                    u_data.unit:base()._tweak_table:find("gensec") then
        ...
        civilian_hostages = civilian_hostages - police_hostages -
                                security_hostages
        managers.gameinfo:event("unit_count", "set", "civ_hostage",
                                civilian_hostages)
        managers.gameinfo:event("unit_count", "set", "cop_hostage",
                                police_hostages)
        managers.gameinfo:event("unit_count", "set", "sec_hostage",
                                security_hostages)
    end
...
if string.lower(RequiredScript) == "lib/network/handlers/unitnetworkhandler" then
    ...
    local interaction_set_active_original =
        UnitNetworkHandler.interaction_set_active
    local alarm_pager_interaction_original =
        UnitNetworkHandler.alarm_pager_interaction
    ...
    function UnitNetworkHandler:mark_minion(unit, owner_id, ...)
        ...
        if self._verify_character(unit) then
            local key = tostring(unit:key())
            local peer = owner_id and owner_id ~=
                             managers.network:session():local_peer():id() and
                             managers.network:session():peer(owner_id)
            local owner_base = peer and peer:unit() and peer:unit():base() or
                                   managers.player
            local damage_mult = (owner_base:upgrade_value("player",
                                                          "convert_enemies_damage_multiplier",
                                                          1) or 1) -- * (owner_base:upgrade_value("player", "passive_convert_enemies_damage_multiplier", 1) or 1)
    ...
    function UnitNetworkHandler:hostage_trade(unit, ...)
        if self._verify_gamestate(self._gamestate_filter.any_ingame) and
            self._verify_character(unit) then
    ...
    function UnitNetworkHandler:interaction_set_active(unit, u_id, active,
                                                       tweak_data, flash, sender, ...)
        if self._verify_gamestate(self._gamestate_filter.any_ingame) and
            self._verify_sender(sender) then
            if tweak_data == "corpse_alarm_pager" then
                if not alive(unit) then
                    local u_data = managers.enemy:get_corpse_unit_data_from_id(
                                       u_id)
                ...
                if alive(unit) then
                    ...
                    elseif not flash then
                        managers.gameinfo:event("pager", "set_answered",
                                                tostring(unit:key()))
                    end
                end
            end
        ...
        return interaction_set_active_original(self, unit, u_id, active,
                                               tweak_data, flash, sender, ...)
    end

    function UnitNetworkHandler:alarm_pager_interaction(u_id, tweak_table,
                                                        status, sender, ...)
        if self._verify_gamestate(self._gamestate_filter.any_ingame) then
            ...
            if unit_data and unit_data.unit:interaction():active() and
                unit_data.unit:interaction().tweak_data == tweak_table and
                self._verify_sender(sender) then
                if status == 1 then
                    managers.gameinfo:event("pager", "set_answered",
                                            tostring(unit_data.unit:key()))
                else
        ...
        return alarm_pager_interaction_original(self, u_id, tweak_table, status,
                                                sender, ...)
    end
...
if string.lower(RequiredScript) == "lib/units/enemies/cop/copdamage" then
    ...
    function CopDamage:_update_minion_dmg_resist(data)
        if alive(self._unit) then
            managers.gameinfo:event("minion", "set_damage_resistance",
                                    tostring(self._unit:key()), data)
        end
    end
    ...
    function CopDamage:convert_to_criminal(...)
        ...
        if self._damage_reduction_multiplier < 1 then
            ...
            managers.enemy:add_delayed_clbk(key .. "_update_minion_dmg_resist",
                                            callback(self, self,
                                                     "_update_minion_dmg_resist",
                                                     data), 0)
        end
    end
    ...
    function CopDamage:_on_damage_received(damage_info, ...)
        if self._unit:in_slot(16) then
            managers.gameinfo:event("minion", "set_health_ratio",
    ...
    function CopDamage:chk_killshot(attacker_unit, ...)
        if alive(attacker_unit) then
            ...
            elseif attacker_unit:in_slot(25) and
                managers.gameinfo:get_sentries(key) then
...
if string.lower(RequiredScript) == "lib/managers/objectinteractionmanager" then
    ...
    local end_action_interact_original =
        ObjectInteractionManager.end_action_interact
    ...
    function ObjectInteractionManager:interact(...)
        if alive(self._active_unit) and self._active_unit:interaction() then
            if self._active_unit:interaction().tweak_data ==
                "corpse_alarm_pager" then
                managers.gameinfo:event("pager", "set_answered",
                                        tostring(self._active_unit:key()))
            end
        end
    ...
    function ObjectInteractionManager:end_action_interact(...)
        ...
        if alive(self._active_unit) and self._active_unit:interaction() then
            ...
            managers.gameinfo:event("interactive_unit", "interact",
                                    tostring(self._active_unit:key()), {
    ...
    function ObjectInteractionManager:remove_unit_clbk(unit, interact_id)
        ...
        if self._queued_units[key] then
            ...
            managers.gameinfo:event("interactive_unit", "remove", key, {
                unit = unit,
                editor_id = editor_id,
                interact_id = id
            })
    ...
    function ObjectInteractionManager:_process_queued_units(t)
        for key, unit in pairs(self._queued_units) do
            if alive(unit) then
                ...
                local editor_id = unit:editor_id()
                managers.gameinfo:event("interactive_unit", "add", key, {
                    unit = unit,
                    editor_id = editor_id,
...
if string.lower(RequiredScript) ==
    "lib/units/equipment/ecm_jammer/ecmjammerbase" then
    ...
    function ECMJammerBase:init(unit, ...)
        ...
        managers.gameinfo:event("ecm", "create", self._ecm_unit_key,
                                {unit = unit})
        return init_original(self, unit, ...)
    end
    ...
    function ECMJammerBase:setup(upgrade_lvl, owner, ...)
        ...
        managers.gameinfo:event("ecm", "set_owner", self._ecm_unit_key,
                                {owner = self._owner_id})
        managers.gameinfo:event("ecm", "set_upgrade_level", self._ecm_unit_key,
                                {upgrade_level = upgrade_lvl})
    end
    ...
    function ECMJammerBase:set_active(active, ...)
        if self._jammer_active ~= active then
            managers.gameinfo:event("ecm", "set_jammer_active",
                                    self._ecm_unit_key, {jammer_active = active})
        end
    ...
    function ECMJammerBase:_set_feedback_active(state, ...)
        if not state and self._feedback_active and managers.network and
            managers.network:session() and
            managers.network:session():local_peer() and
            managers.network:session():local_peer():id() then
            local peer_id = managers.network:session():local_peer():id()

            if self._owner_id == peer_id and
                managers.player:has_category_upgrade("ecm_jammer",
                                                     "can_retrigger") then
                self._retrigger_delay = tweak_data.upgrades
                                            .ecm_feedback_retrigger_interval or
                                            60
                managers.gameinfo:event("ecm", "set_retrigger_active",
                                        self._ecm_unit_key,
                                        {retrigger_active = true})
            end
        end
        managers.gameinfo:event("ecm", "set_feedback_active",
                                self._ecm_unit_key, {feedback_active = state})
        local val = _set_feedback_active_original(self, state, ...)
        if Network:is_server() then
            managers.gameinfo:event("ecm", "set_feedback_duration",
                                    self._ecm_unit_key, {
                feedback_duration = self._feedback_duration,
                feedback_expire_t = self._feedback_expire_t
            })
        end
        return val
    end
    ...
    function ECMJammerBase:clbk_feedback(...)
        ...
        if self._feedback_expire_t - TimerManager:game():time() <
            self._feedback_duration * 0.1 then
            managers.gameinfo:event("ecm", "set_feedback_low",
                                    self._ecm_unit_key, {feedback_low = true})
        end
    end
    ...
    function ECMJammerBase:update(unit, t, dt, ...)
        ...
        if not self._battery_empty then
            managers.gameinfo:event("ecm", "set_jammer_battery",
                                    self._ecm_unit_key,
                                    {jammer_battery = self._battery_life})
        end

        if self._retrigger_delay then
            self._retrigger_delay = self._retrigger_delay - dt
            managers.gameinfo:event("ecm", "set_retrigger_delay",
                                    self._ecm_unit_key,
                                    {retrigger_delay = self._retrigger_delay})
        ...
        if self._feedback_active and self._feedback_expire_t and
            self._feedback_expire_t < t then
            managers.gameinfo:event("ecm", "set_feedback_active",
                                    self._ecm_unit_key,
    ...
    function ECMJammerBase:_set_battery_low(...)
        ...
        managers.gameinfo:event("ecm", "set_battery_low", self._ecm_unit_key,
                                {battery_low = true})
    end

    function ECMJammerBase:contour_interaction(...)
        if managers.network:session() and self._owner_id ==
            managers.network:session():local_peer():id() and
            managers.player:has_category_upgrade("ecm_jammer",
                                                 "can_activate_feedback") then
            self._retrigger_delay = nil
            managers.gameinfo:event("ecm", "set_retrigger_active",
                                    self._ecm_unit_key,
                                    {retrigger_active = false})
        end
        managers.gameinfo:event("ecm", "set_feedback_low", self._ecm_unit_key,
                                {feedback_low = false})
    ...
    function ECMJammerBase:destroy(...)
        managers.gameinfo:event("ecm", "set_feedback_active",
                                self._ecm_unit_key, {feedback_active = false})
        managers.gameinfo:event("ecm", "set_retrigger_active",
                                self._ecm_unit_key, {retrigger_active = false})
...
if string.lower(RequiredScript) ==
    "lib/units/equipment/doctor_bag/doctorbagbase" then
    ...
    function DoctorBagBase.spawn(pos, rot, amount_upgrade_lvl, peer_id, ...)
        ...
        if alive(unit) then
            ...
            managers.gameinfo:event("doc_bag", "set_owner", key,
                                    {owner = peer_id})
        end
        return unit
    end
    ...
    function DoctorBagBase:init(unit, ...)
        local key = tostring(unit:key())
        managers.gameinfo:event("doc_bag", "create", key, {unit = unit})
        managers.gameinfo:event("doc_bag", "set_max_amount", key,
    ...
    function DoctorBagBase:setup(...)
        ...
        managers.gameinfo:event("doc_bag", "set_max_amount",
                                tostring(self._unit:key()),
                                {max_amount = self._amount})
        return value
    end

    function DoctorBagBase:sync_setup(amount_upgrade_lvl, peer_id, ...)
        managers.gameinfo:event("doc_bag", "set_owner",
                                tostring(self._unit:key()), {owner = peer_id})
        return sync_setup_original(self, amount_upgrade_lvl, peer_id, ...)
    end

    function DoctorBagBase:_set_visual_stage(...)
        managers.gameinfo:event("doc_bag", "set_amount",
                                tostring(self._unit:key()),
                                {amount = self._amount})
    ...
    function DoctorBagBase:_get_upgrade_levels(...)
        ...
        managers.gameinfo:event("doc_bag", "set_upgrades",
                                tostring(self._unit:key()), {
            upgrades = {damage_reduction = upgrade_values[2]}
        })
...
if string.lower(RequiredScript) ==
    "lib/units/equipment/first_aid_kit/firstaidkitbase" then
    ...
    function FirstAidKitBase.spawn(pos, rot, bits, peer_id, ...)
        ...
        if alive(unit) then
            local key = tostring(unit:key())
            managers.gameinfo:event("first_aid_kit", "create", key,
                                    {unit = unit})
            managers.gameinfo:event("first_aid_kit", "set_owner", key,
                                    {owner = peer_id})
        end
        return unit
    end
    ...
    function FirstAidKitBase:sync_setup(bits, peer_id, ...)
        managers.gameinfo:event("first_aid_kit", "set_owner",
                                tostring(self._unit:key()), {owner = peer_id})
    ...
    function FirstAidKitBase:_get_upgrade_levels(...)
        local upgrade_values = {_get_upgrade_levels_original(self, ...)}
        managers.gameinfo:event("first_aid_kit", "set_upgrades",
                                tostring(self._unit:key()), {
            upgrades = {
                damage_reduction = upgrade_values[1],
                auto_recovery = upgrade_values[2]
            }
        })
        return unpack(upgrade_values)
    end
    ...
    function FirstAidKitBase:destroy(...)
        managers.gameinfo:event("first_aid_kit", "destroy",
                                tostring(self._unit:key()))
        return destroy_original(self, ...)
    end
...
if string.lower(RequiredScript) == "lib/units/equipment/ammo_bag/ammobagbase" then
    ...
    function AmmoBagBase.spawn(pos, rot, ammo_upgrade_lvl, peer_id,
        ...
        if alive(unit) then
            ...
            managers.gameinfo:event("ammo_bag", "set_owner", key,
                                    {owner = peer_id})
            managers.gameinfo:event("ammo_bag", "set_upgrades", key, {
                upgrades = {bullet_storm = bullet_storm_level}
            })
        end
        return unit
    end
    ...
    function AmmoBagBase:init(unit, ...)
        local key = tostring(unit:key())
        managers.gameinfo:event("ammo_bag", "create", key, {unit = unit})
        managers.gameinfo:event("ammo_bag", "set_max_amount", key,
                                {max_amount = self._max_ammo_amount})
        init_original(self, unit, ...)
    end

    function AmmoBagBase:setup(...)
        local value = setup_original(self, ...)
        managers.gameinfo:event("ammo_bag", "set_max_amount",
                                tostring(self._unit:key()),
                                {max_amount = self._ammo_amount})
        return value
    end
    ...
    function AmmoBagBase:_set_visual_stage(...)
        managers.gameinfo:event("ammo_bag", "set_amount",
                                tostring(self._unit:key()),
                                {amount = self._ammo_amount})
        return _set_visual_stage_original(self, ...)
    end

    function AmmoBagBase:destroy(...)
        managers.gameinfo:event("ammo_bag", "destroy",
                                tostring(self._unit:key()))
        return destroy_original(self, ...)
    end
...
if string.lower(RequiredScript) ==
    "lib/units/equipment/bodybags_bag/bodybagsbagbase" then
    ...
    function BodyBagsBagBase.spawn(pos, rot, upgrade_lvl, peer_id, ...)
        ...
        if alive(unit) then
            ...
            managers.gameinfo:event("body_bag", "set_owner", key,
                                    {owner = peer_id})
        end
        return unit
    end
    ...
    function BodyBagsBagBase:init(unit, ...)
        ...
        managers.gameinfo:event("body_bag", "create", key, {unit = unit})
        init_original(self, unit, ...)
        managers.gameinfo:event("body_bag", "set_max_amount", key,
                                {max_amount = self._max_bodybag_amount})
    end

    function BodyBagsBagBase:sync_setup(upgrade_lvl, peer_id, ...)
        managers.gameinfo:event("body_bag", "set_owner",
                                tostring(self._unit:key()), {owner = peer_id})
        return sync_setup_original(self, upgrade_lvl, peer_id, ...)
    end

    function BodyBagsBagBase:_set_visual_stage(...)
        managers.gameinfo:event("body_bag", "set_amount",
                                tostring(self._unit:key()),
                                {amount = self._bodybag_amount})
        return _set_visual_stage_original(self, ...)
    end

    function BodyBagsBagBase:destroy(...)
        managers.gameinfo:event("body_bag", "destroy",
                                tostring(self._unit:key()))
        return destroy_original(self, ...)
    end
...
if string.lower(RequiredScript) ==
    "lib/units/equipment/grenade_crate/grenadecratebase" then
    ...
    function GrenadeCrateBase:init(unit, ...)
        ...
        init_original(self, unit, ...)
        managers.gameinfo:event("grenade_crate", "set_max_amount", key,
                                {max_amount = self._max_grenade_amount})
    end

    function GrenadeCrateBase:_set_visual_stage(...)
        managers.gameinfo:event("grenade_crate", "set_amount",
                                tostring(self._unit:key()),
                                {amount = self._grenade_amount})
        return _set_visual_stage_original(self, ...)
    end

    function GrenadeCrateBase:destroy(...)
        managers.gameinfo:event("grenade_crate", "destroy",
                                tostring(self._unit:key()))
        return destroy_original(self, ...)
    end
...
if string.lower(RequiredScript) == "lib/units/props/securitycamera" then
    ...
    local _activate_tape_loop_restart_original =
        SecurityCamera._activate_tape_loop_restart
    local _deactivate_tape_loop_restart_original =
        SecurityCamera._deactivate_tape_loop_restart
    ...
    function SecurityCamera:init(unit)
        managers.gameinfo:event("camera", "create", tostring(unit:key()),
                                {unit = unit})
        return init_original(self, unit)
    end
    ...
    function SecurityCamera:_start_tape_loop(...)
        ...
        managers.gameinfo:event("camera", "set_tape_loop_active",
                                tostring(self._unit:key()),
                                {tape_loop_active = true})
        managers.gameinfo:event("camera", "set_tape_loop_expire_t",
                                tostring(self._unit:key()), {
            tape_loop_expire_t = self._tape_loop_end_t + 5
        })
    end

    function SecurityCamera:_activate_tape_loop_restart(...)
        _activate_tape_loop_restart_original(self, ...)
        managers.gameinfo:event("camera", "set_tape_loop_restart_active",
                                tostring(self._unit:key()),
                                {tape_loop_restart_active = true})
    end

    function SecurityCamera:_deactivate_tape_loop_restart(...)
        managers.gameinfo:event("camera", "set_tape_loop_restart_active",
                                tostring(self._unit:key()),
                                {tape_loop_restart_active = false})
        if not self._tape_loop_end_t then
            managers.gameinfo:event("camera", "set_tape_loop_active",
                                    tostring(self._unit:key()),
                                    {tape_loop_active = false})
        end
    ...
    function SecurityCamera:on_unit_set_enabled(enabled, ...)
        managers.gameinfo:event("camera", "set_enabled",
                                tostring(self._unit:key()), {enabled = enabled})

        on_unit_set_enabled_original(self, enabled, ...)
    end

    function SecurityCamera:set_update_enabled(state, ...)
        managers.gameinfo:event("camera", "set_active",
                                tostring(self._unit:key()), {active = state})

        return set_update_enabled_original(self, state, ...)
    end

    function SecurityCamera:generate_cooldown(...)
        managers.gameinfo:event("camera", "set_broken",
                                tostring(self._unit:key()), {broken = true})
...
if string.lower(RequiredScript) ==
    "lib/units/equipment/sentry_gun/sentrygunbase" then
    ...
    function SentryGunBase.spawn(owner, pos, rot, peer_id, ...)
        ...
        if alive(unit) then
            managers.gameinfo:event("sentry", "create", tostring(unit:key()),
                                    {unit = unit})
            managers.gameinfo:event("sentry", "set_owner", tostring(unit:key()),
                                    {owner = peer_id})
        end
        return unit
    end
    ...
    function SentryGunBase:sync_setup(upgrade_lvl, peer_id, ...)
        managers.gameinfo:event("sentry", "set_owner",
                                tostring(self._unit:key()), {owner = peer_id})
        return sync_setup_original(self, upgrade_lvl, peer_id, ...)
    end
...
if string.lower(RequiredScript) ==
    "lib/units/equipment/sentry_gun/sentrygundamage" then
    ...
    function SentryGunDamage:init(...)
        init_original(self, ...)
        managers.gameinfo:event("sentry", "set_active",
                                tostring(self._unit:key()), {active = true})
        managers.gameinfo:event("sentry", "set_health_ratio",
                                tostring(self._unit:key()),
                                {health_ratio = self:health_ratio()})
    end
    ...
    function SentryGunDamage:die(...)
        managers.gameinfo:event("sentry", "set_active",
                                tostring(self._unit:key()), {active = false})
...
if string.lower(RequiredScript) == "lib/units/weapons/sentrygunweapon" then
    ...
    function SentryGunWeapon:init(...)
        init_original(self, ...)
        managers.gameinfo:event("sentry", "set_ammo_ratio",
                                tostring(self._unit:key()),
...
if string.lower(RequiredScript) == "lib/managers/playermanager" then
    ...
    local disable_cooldown_upgrade_original =
        PlayerManager.disable_cooldown_upgrade
    local activate_temporary_upgrade_original =
        PlayerManager.activate_temporary_upgrade
    local activate_temporary_upgrade_by_level_original =
        PlayerManager.activate_temporary_upgrade_by_level
    local deactivate_temporary_upgrade_original =
        PlayerManager.deactivate_temporary_upgrade
    local count_up_player_minions_original =
        PlayerManager.count_up_player_minions
    local count_down_player_minions_original =
        PlayerManager.count_down_player_minions
    local set_melee_dmg_multiplier_original =
        PlayerManager.set_melee_dmg_multiplier
    local mul_to_accuracy_multiplier_original =
        PlayerManager.mul_to_accuracy_multiplier
    ...
    local add_synced_team_upgrade_original =
        PlayerManager.add_synced_team_upgrade
    ...
    local _on_messiah_recharge_event_original =
        PlayerManager._on_messiah_recharge_event
    ...
    local set_synced_cocaine_stacks_original =
        PlayerManager.set_synced_cocaine_stacks
    ...
    local _on_grenade_cooldown_end_original =
        PlayerManager._on_grenade_cooldown_end
    local speed_up_grenade_cooldown_original =
        PlayerManager.speed_up_grenade_cooldown
    ...
    function PlayerManager:spawned_player(id, ...)
        ...
        if id == 1 then
            if not PLAYER_HAS_SPAWNED then
                ...
                for category, data in pairs(self._global.team_upgrades) do
                    for upgrade, value in pairs(data) do
                        local value = self:team_upgrade_value(category, upgrade,
                                                              0)
                        managers.gameinfo:event("team_buff", "activate", {
                            peer = 0,
                            category = category,
                            upgrade = upgrade,
                            value = value
            ...
            if self:has_category_upgrade("player",
                                         "messiah_revive_from_bleed_out") and
                (self._messiah_charges or 0) > 0 then
                managers.gameinfo:event("buff", "activate", "messiah")
                managers.gameinfo:event("buff", "set_stack_count", "messiah", {
                    stack_count = (self._messiah_charges or 0)
                })
            end

            if self:has_category_upgrade("player", "health_damage_reduction") then
                local value = (1 -
                                  self:upgrade_value("player",
                                                     "health_damage_reduction",
                                                     1))
                managers.gameinfo:event("buff", "activate", "frenzy")
                managers.gameinfo:event("buff", "set_value", "frenzy",
                                        {value = value})
            end
            ...
            if self:has_category_upgrade("player", "headshot_regen_armor_bonus") then
                local function on_headshot()
                    managers.gameinfo:event("timed_buff", "activate",
                                            "bullseye_debuff", {
                        duration = tweak_data.upgrades
                            .on_headshot_dealt_cooldown or 0
                    })
                end

                self._message_system:register(Message.OnHeadShot,
                                              "bullseye_debuff_listener",
                                              on_headshot)
            end

            self._is_sociopath = self:has_category_upgrade("player",
                                                           "killshot_regen_armor_bonus") or
                                     self:has_category_upgrade("player",
                                                               "killshot_close_regen_armor_bonus") or
                                     self:has_category_upgrade("player",
                                                               "killshot_close_panic_chance") or
                                     self:has_category_upgrade("player",
                                                               "melee_kill_life_leech")
        end
    end
    ...
    function PlayerManager:activate_temporary_upgrade(category, upgrade, ...)
        ...
        if self._temporary_upgrades[category] and
            self._temporary_upgrades[category][upgrade] then
            local t = Application:time()
            local expire_t = self._temporary_upgrades[category][upgrade]
                                 .expire_time
            local level
            local upgrade_level = self:upgrade_level(category, upgrade, 0)
            if upgrade_level > 0 then level = upgrade_level end
            local value = self:temporary_upgrade_value(category, upgrade, 0)

            if expire_t > t then
                managers.gameinfo:event("temporary_buff", "activate", {
                    t = t,
                    expire_t = expire_t,
                    category = category,
                    upgrade = upgrade,
    ...
    function PlayerManager:disable_cooldown_upgrade(category, upgrade, ...)
        ...
        if self._global.cooldown_upgrades[category] and
            self._global.cooldown_upgrades[category][upgrade] then
            local t = Application:time()
            local expire_t = self._global.cooldown_upgrades[category][upgrade]
                                 .cooldown_time
    ...
    function PlayerManager:activate_temporary_upgrade_by_level(category,
                                                               upgrade, level, ...)
        activate_temporary_upgrade_by_level_original(self, category, upgrade,
                                                     level, ...)
    ...
    function PlayerManager:deactivate_temporary_upgrade(category, upgrade, ...)
        if self._temporary_upgrades[category] and
            self._temporary_upgrades[category][upgrade] then
            managers.gameinfo:event("temporary_buff", "deactivate",
                                    {category = category, upgrade = upgrade})
        end
    ...
    function PlayerManager:count_up_player_minions(...)
        ...
        if self._local_player_minions > 0 then
            if self:has_category_upgrade("player",
                                         "minion_master_speed_multiplier") then
                managers.gameinfo:event("buff", "activate", "partner_in_crime")
            end
            if self:has_category_upgrade("player",
                                         "minion_master_health_multiplier") then
                managers.gameinfo:event("buff", "activate",
    ...
    function PlayerManager:count_down_player_minions(...)
        ...
        if self._local_player_minions <= 0 then
            ...
            managers.gameinfo:event("buff", "deactivate",
                                    "partner_in_crime_aced")
        end
        return result
    end
    ...
    function PlayerManager:update_hostage_situation(num_hostages)
        local hostages = num_hostages or
                             (managers.groupai and
                                 managers.groupai:state():hostage_count()) or 0
        local minions = self:num_local_minions() or 0
        local stack_count = hostages + minions

        if self:has_team_category_upgrade("health", "hostage_multiplier") or
            self:has_team_category_upgrade("stamina", "hostage_multiplier") or
            self:has_team_category_upgrade("damage_dampener",
                                           "hostage_multiplier") then
            if stack_count > 0 then
                local value = self:team_upgrade_value("damage_dampener",
                                                      "hostage_multiplier", 0)
                managers.gameinfo:event("buff", "activate", "hostage_situation")
                managers.gameinfo:event("buff", "set_stack_count",
                                        "hostage_situation",
                                        {stack_count = stack_count})
                if value ~= 0 then
                    managers.gameinfo:event("buff", "set_value",
                                            "hostage_situation", {value = value})
                end
            else
                managers.gameinfo:event("buff", "deactivate",
                                        "hostage_situation")
    ...
    function PlayerManager:set_melee_dmg_multiplier(...)
        ...
        if old_mult ~= self._melee_dmg_mul then
            managers.gameinfo:event("buff", "change_stack_count",
                                    "bloodthirst_basic", {difference = 1})
        end
        managers.gameinfo:event("buff", "set_value", "bloodthirst_basic",
                                {value = self._melee_dmg_mul})
    end
    ...
    function PlayerManager:on_killshot(...)
        ...
        if self._is_sociopath and self._on_killshot_t ~= last_killshot then
            managers.gameinfo:event("timed_buff", "activate",
                                    "sociopath_debuff",
                                    {expire_t = self._on_killshot_t})
        end

        local gain_throwable_per_kill = managers.player:upgrade_value("team",
                                                                      "crew_throwable_regen",
                                                                      0)
        if gain_throwable_per_kill > 0 then
            managers.gameinfo:event("buff", "set_stack_count",
                                    "crew_throwable_regen", {
                stack_count = (gain_throwable_per_kill -
                    (self._throw_regen_kills or 0))
            })
        end
    ...
    function PlayerManager:aquire_team_upgrade(upgrade, ...)
        ...
        local value = self:team_upgrade_value(upgrade.category, upgrade.upgrade,
                                              0)
        managers.gameinfo:event("team_buff", "activate", {
            peer = 0,
    ...
    function PlayerManager:unaquire_team_upgrade(upgrade, ...)
        ...
        managers.gameinfo:event("team_buff", "deactivate", {
            peer = 0,
            category = upgrade.category,
            upgrade = upgrade.upgrade
        })
    ...
    function PlayerManager:peer_dropped_out(peer, ...)
        ...
        for category, data in pairs(
                                  self._global.synced_team_upgrades[peer_id] or
                                      {}) do
            for upgrade, value in pairs(data) do
                managers.gameinfo:event("team_buff", "deactivate", {
                    peer = peer_id,
    ...
    function PlayerManager:on_headshot_dealt(...)
        local t = Application:time()
        if (self._on_headshot_dealt_t or 0) <= t and
            self:has_category_upgrade("player", "headshot_regen_armor_bonus") then
            managers.gameinfo:event("timed_buff", "activate", "bullseye_debuff",
                                    {
                t = t,
                duration = tweak_data.upgrades.on_headshot_dealt_cooldown or 0
            })
        end
    ...
    function PlayerManager:_on_messiah_recharge_event(...)
        ...
        if (self._messiah_charges or 0) > 0 then
            managers.gameinfo:event("buff", "activate", "messiah")
            managers.gameinfo:event("buff", "set_stack_count", "messiah",
                                    {stack_count = self._messiah_charges})
        else
    ...
    function PlayerManager:mul_to_property(name, value, ...)
        ...
        managers.gameinfo:event("buff", "change_stack_count", name,
                                {difference = 1})
        managers.gameinfo:event("buff", "set_value", name,
                                {value = self:get_property(name, 1)})
    end
    ...
    function PlayerManager:set_property(name, value, ...)
        ...
        if name == "revive_damage_reduction" then
            ...
            managers.gameinfo:event("buff", "set_value", "combat_medic_passive",
                                    {value = value})
        end
    end
    ...
    function PlayerManager:chk_wild_kill_counter(...)
        ...
        if alive(player) and
            (managers.player:has_category_upgrade("player", "wild_health_amount") or
                managers.player:has_category_upgrade("player",
                                                     "wild_armor_amount")) then
            local dmg = player:character_damage()
            local missing_health_ratio =
                math.clamp(1 - dmg:health_ratio(), 0, 1)
            local missing_armor_ratio = math.clamp(1 - dmg:armor_ratio(), 0, 1)
            local less_armor_wild_cooldown =
                managers.player:upgrade_value("player",
                                              "less_armor_wild_cooldown", 0)
            local less_health_wild_cooldown =
                managers.player:upgrade_value("player",
                                              "less_health_wild_cooldown", 0)
            local trigger_cooldown = tweak_data.upgrades.wild_trigger_time or 30

            if less_health_wild_cooldown ~= 0 and less_health_wild_cooldown[1] ~=
                0 then
                local missing_health_stacks =
                    math.floor(missing_health_ratio /
                                   less_health_wild_cooldown[1])
                trigger_cooldown = trigger_cooldown -
                                       less_health_wild_cooldown[2] *
                                       missing_health_stacks
            end
            if less_armor_wild_cooldown ~= 0 and less_armor_wild_cooldown[1] ~=
                0 then
                local missing_armor_stacks =
                    math.floor(missing_armor_ratio / less_armor_wild_cooldown[1])
                trigger_cooldown = trigger_cooldown -
                                       less_armor_wild_cooldown[2] *
                                       missing_armor_stacks
            end
        ...
        if self._wild_kill_triggers then
            ...
            for i = 1, #self._wild_kill_triggers, 1 do
                if self._wild_kill_triggers[i] > t then break end
        ...
        if self._wild_kill_triggers and #self._wild_kill_triggers > old_stacks then
            managers.gameinfo:event("timed_stack_buff", "add_stack", "biker",
                                    {t = t, expire_t = expire_t})
        end
    end
    ...
    function PlayerManager:set_synced_cocaine_stacks(...)
        ...
        local ratio = max_stack_data.amount /
                          tweak_data.upgrades.max_total_cocaine_stacks
        if ratio > 0 then
            managers.gameinfo:event("buff", "activate", "maniac")
            managers.gameinfo:event("buff", "set_progress", "maniac",
                                    {progress = ratio})

            local my_id = managers.network:session() and
                              managers.network:session():local_peer():id()
            local mult_level = my_id and
                                   self._global.synced_cocaine_stacks[my_id] and
                                   self._global.synced_cocaine_stacks[my_id]
                                       .power_level or 0
            local damage_absorb = self:_get_cocaine_damage_absorption_from_data(
                                      max_stack_data) *
                                      self:upgrade_value_by_level("player",
                                                                  "cocaine_stack_absorption_multiplier",
                                                                  mult_level or
                                                                      0, 1)
            managers.gameinfo:event("buff", "set_value", "maniac",
                                    {value = damage_absorb * 10})

            if (self._last_stack_decay or 0) <
                (self._damage_dealt_to_cops_decay_t or
                    tweak_data.upgrades.cocaine_stacks_decay_t or 5) then
                managers.gameinfo:event("timed_buff", "activate",
                                        "maniac_debuff", {
                    expire_t = self._damage_dealt_to_cops_decay_t
                })
            end
    ...
    function PlayerManager:add_grenade_amount(...)
        ...
        local gain_throwable_per_kill = managers.player:upgrade_value("team",
                                                                      "crew_throwable_regen",
                                                                      0)
        if gain_throwable_per_kill > 0 and not self:got_ability() and
            not self:got_max_grenades() then
            managers.gameinfo:event("buff", "activate", "crew_throwable_regen")
            managers.gameinfo:event("buff", "set_stack_count",
                                    "crew_throwable_regen", {
                stack_count = (gain_throwable_per_kill -
                    (self._throw_regen_kills or 0) + 1)
            })
        else
    ...
    function PlayerManager:_dodge_shot_gain(gain_value, ...)
        if gain_value then
            if gain_value > 0 then
                ...
                managers.gameinfo:event("buff", "set_value", "sicario_dodge", {
                    value = gain_value *
                        self:upgrade_value("player", "sicario_multiplier", 1)
                })
                managers.gameinfo:event("timed_buff", "activate",
                                        "sicario_dodge_debuff", {
                    duration = tweak_data.upgrades.values.player.dodge_shot_gain[1][2]
                }) -- self:upgrade_value("player", "dodge_shot_gain")[2]
            else
    ...
    function PlayerManager:replenish_grenades(cooldown, ...)
        if not self:has_active_timer("replenish_grenades") then
            ...
            if id then
                ...
                managers.gameinfo:event("buff", "set_duration", id .. "_debuff",
                                        {duration = cooldown})
            end
        end
    ...
    function PlayerManager:speed_up_grenade_cooldown(time, ...)
        if self:has_active_timer("replenish_grenades") then
            ...
            managers.gameinfo:event("timed_buff", "change_expire",
                                    equipped_grenade .. "_debuff",
                                    {difference = -time})
        end
    ...
    function PlayerManager:start_custom_cooldown(category, upgrade, cooldown, ...)
        managers.gameinfo:event("timed_buff", "activate", upgrade .. "_debuff",
                                {duration = cooldown})
    ...
    function PlayerManager:got_ability(grenade_id)
        local equipped_grenade = grenade_id or
...
if string.lower(RequiredScript) == "lib/managers/player/smokescreeneffect" then
    ...
    function SmokeScreenEffect:init(position, normal, time, has_dodge_bonus,
                                    grenade_unit, ...)
        init_original(self, position, normal, time, has_dodge_bonus,
                      grenade_unit, ...)

        self._unit_key = grenade_unit and tostring(grenade_unit:key())
    end

    function SmokeScreenEffect:get_key() return self._unit_key end

    function SmokeScreenEffect:get_time_remaining() return self._timer or 0 end

end

if string.lower(RequiredScript) == "lib/utils/temporarypropertymanager" then

    local activate_property_original =
        TemporaryPropertyManager.activate_property
    ...
    function TemporaryPropertyManager:remove_property(prop, ...)
        managers.gameinfo:event("temporary_buff", "deactivate",
                                {category = "temporary", upgrade = prop})
...
if string.lower(RequiredScript) ==
    "lib/modifiers/boosts/gagemodifiermeleeinvincibility" then

    local OnPlayerManagerKillshot_original =
        GageModifierMeleeInvincibility.OnPlayerManagerKillshot
...
if string.lower(RequiredScript) == "lib/modifiers/boosts/gagemodifierlifesteal" then
    ...
    local OnPlayerManagerKillshot_original =
        GageModifierLifeSteal.OnPlayerManagerKillshot
...
if string.lower(RequiredScript) == "lib/units/beings/player/playermovement" then
    ...
    function PlayerMovement:on_morale_boost(...)
        managers.gameinfo:event("timed_buff", "activate", "inspire", {
            duration = tweak_data.upgrades.morale_boost_time
        })
        ...
        if t > BUFFS_RECHECK_T and alive(self._unit) then
            ...
            for _, smoke_screen in ipairs(managers.player:smoke_screens() or {}) do
                if smoke_screen:alive() and smoke_screen:is_in_smoke(self._unit) then
                    if not longest_smoke or
                        (longest_smoke:get_time_remaining() <
                            smoke_screen:get_time_remaining()) then
            ...
            if longest_smoke and longest_smoke:alive() then
                if not IN_SMOKE_SCREEN or type(IN_SMOKE_SCREEN) == "string" and
                    IN_SMOKE_SCREEN ~= (longest_smoke:get_key() or "") then
                    IN_SMOKE_SCREEN = longest_smoke:get_key() or true
                    managers.gameinfo:event("buff", "activate",
                                            "smoke_screen_grenade")
                    managers.gameinfo:event("buff", "set_duration",
                                            "smoke_screen_grenade", {
                        duration = longest_smoke:get_time_remaining()
                    })
                end
                managers.gameinfo:event("buff", "set_stack_count",
                                        "smoke_screen_grenade", {
                    stack_count = table.size(smoked_units)
                })
                managers.gameinfo:event("buff", "set_value",
                                        "smoke_screen_grenade",
                                        {value = smoke_dodge})
...
if string.lower(RequiredScript) == "lib/units/beings/player/playerinventory" then
    ...
    local _start_feedback_effect_original =
        PlayerInventory._start_feedback_effect
    ...
    function PlayerInventory:_start_jammer_effect(end_time, ...)
        ...
        managers.gameinfo:event("buff", "set_duration", "pocket_ecm_jammer", {
            expire_t = end_time or
                ((self:get_jammer_time() or 0) +
                    (TimerManager:game():time() or 0))
        })
...
if string.lower(RequiredScript) ==
    "lib/units/beings/player/states/playerstandard" then
    ...
    local _interupt_action_melee_original =
        PlayerStandard._interupt_action_melee
    local _do_melee_damage_original = PlayerStandard._do_melee_damage
    local _start_action_interact_original =
        PlayerStandard._start_action_interact
    local _interupt_action_interact_original =
        PlayerStandard._interupt_action_interact
    local _start_action_use_item_original =
        PlayerStandard._start_action_use_item
    local _interupt_action_use_item_original =
        PlayerStandard._interupt_action_use_item
    local _update_use_item_timers_original =
        PlayerStandard._update_use_item_timers
    ...
    local _interupt_action_reload_original =
        PlayerStandard._interupt_action_reload
    local _start_action_charging_weapon_original =
        PlayerStandard._start_action_charging_weapon
    local _end_action_charging_weapon_original =
        PlayerStandard._end_action_charging_weapon
    local _update_equip_weapon_timers_original =
        PlayerStandard._update_equip_weapon_timers

    function PlayerStandard:_do_action_intimidate(t, interact_type, ...)
        if interact_type == "cmd_gogo" or interact_type == "cmd_get_up" then
            local duration = (tweak_data.upgrades.morale_boost_base_cooldown *
                                 managers.player:upgrade_value("player",
                                                               "morale_boost_cooldown_multiplier",
                                                               1)) or 3.5
            managers.gameinfo:event("timed_buff", "activate", "inspire_debuff",
                                    {duration = duration})
        end
    ...
    function PlayerStandard:_start_action_melee(t, input, instant, ...)
        if not instant then
            local duration =
                tweak_data.blackmarket.melee_weapons[managers.blackmarket:equipped_melee_weapon()]
                    .stats.charge_time
            managers.gameinfo:event("player_action", "activate", "melee_charge")
            managers.gameinfo:event("player_action", "set_duration",
                                    "melee_charge", {duration = duration})
        end
    ...
    function PlayerStandard:_interupt_action_melee(...)
        if self._state_data.melee_start_t then
            managers.gameinfo:event("player_action", "deactivate",
                                    "melee_charge")
        end
    ...
    function PlayerStandard:_start_action_interact(t, input, timer,
                                                   interact_object, ...)
        if managers.player:has_category_upgrade("player",
                                                "interacting_damage_multiplier") then
            local value = managers.player:upgrade_value("player",
                                                        "interacting_damage_multiplier",
                                                        0)
            managers.gameinfo:event("buff", "activate", "die_hard")
            managers.gameinfo:event("buff", "set_value", "die_hard",
                                    {value = value})
        end
        ...
        managers.gameinfo:event("player_action", "set_data", "interact", {
            interact_id = interact_object:interaction().tweak_data
        })

        return _start_action_interact_original(self, t, input, timer,
                                               interact_object, ...)
    end
    ...
    function PlayerStandard:_interupt_action_interact(t, input, complete, ...)
        ...
        local value = _interupt_action_interact_original(self, t, input,
                                                         complete, ...)

        if self._equip_weapon_expire_t and not previous_weapon_exire_t then
            local t = Application:time()
            managers.gameinfo:event("player_action", "activate",
                                    "interact_debuff", {
                t = t,
                expire_t = self._equip_weapon_expire_t
            })
        end
    ...
    function PlayerStandard:_interupt_action_use_item(t, input, complete, ...)
        ...
        local previous_weapon_exire_t = self._equip_weapon_expire_t
        local value = _interupt_action_use_item_original(self, t, input,
                                                         complete, ...)
    ...
    function PlayerStandard:_update_use_item_timers(...)
        ...
        if self._use_item_expire_t then
            local valid =
                managers.player:check_selected_equipment_placement_valid(
                    self._unit)
            managers.gameinfo:event("player_action", "set_data", "interact",
                                    {invalid = not valid})
        end
    ...
    function PlayerStandard:_start_action_reload(t, ...)
        ...
        if self._state_data.reload_expire_t then
            managers.gameinfo:event("player_action", "activate", "reload", {
                expire_t = self._state_data.reload_expire_t
            })
        end
    end
    ...
    function PlayerStandard:_start_action_charging_weapon(t, ...)
        ...
        managers.gameinfo:event("player_action", "set_duration",
                                "weapon_charge", {
            duration = self._equipped_unit:base():charge_max_t()
        })
    ...
    function PlayerStandard:_update_omniscience(t, dt)
        local action_forbidden = not managers.player:has_category_upgrade(
                                     "player", "standstill_omniscience") or
                                     managers.player:current_state() ==
                                     "civilian" or self:_interacting() or
                                     self._ext_movement:has_carry_restriction() or
                                     self:is_deploying() or
                                     self:_changing_weapon() or
                                     self:_is_throwing_projectile() or
                                     self:_is_meleeing() or self:_on_zipline() or
                                     self._moving or self:running() or
                                     self:_is_reloading() or self:in_air() or
                                     self:in_steelsight() or self:is_equipping() or
                                     self:shooting() or
        ...
        if not self._state_data.omniscience_t then
            ...
            managers.gameinfo:event("buff", "set_duration", "sixth_sense", {
                duration = tweak_data.player.omniscience.start_t
            })
            managers.gameinfo:event("buff", "set_stack_count", "sixth_sense",
                                    {stack_count = nil})
        end

        self._state_data.omniscience_t =
            self._state_data.omniscience_t or t +
                tweak_data.player.omniscience.start_t
        if t >= self._state_data.omniscience_t then
            local sensed_targets = World:find_units_quick("sphere",
                                                          self._unit:movement()
                                                              :m_pos(),
                                                          tweak_data.player
                                                              .omniscience
                                                              .sense_radius,
                                                          managers.slot:get_mask(
                                                              "trip_mine_targets"))
            managers.gameinfo:event("buff", "set_stack_count", "sixth_sense",
                                    {stack_count = #sensed_targets})
            ...
            for _, unit in ipairs(sensed_targets) do
                if alive(unit) and not unit:base():char_tweak().is_escort then
                    self._state_data.omniscience_units_detected =
                        self._state_data.omniscience_units_detected or {}
                    if not self._state_data.omniscience_units_detected[unit:key()] or
                        t >=
                        self._state_data.omniscience_units_detected[unit:key()] then
                        self._state_data.omniscience_units_detected[unit:key()] =
                            t + tweak_data.player.omniscience.target_resense_t
            ...
            self._state_data.omniscience_t =
                t + tweak_data.player.omniscience.interval_t
            managers.gameinfo:event("buff", "set_duration", "sixth_sense", {
                duration = tweak_data.player.omniscience.interval_t
            })
        end
    end
    ...
    function PlayerStandard:_check_damage_stack_skill(t, category)
        ...
        if stack then
            ...
            if stack[2] > 0 then
                local value = managers.player:upgrade_value(category,
                                                            "stacking_hit_damage_multiplier",
                                                            0)
                managers.gameinfo:event("timed_buff", "activate", buff_id,
                                        {expire_t = stack[1]})
                managers.gameinfo:event("buff", "set_stack_count", buff_id,
                                        {stack_count = stack[2]})
                managers.gameinfo:event("buff", "set_value", buff_id,
                                        {value = 1 + stack[2] * value})
            else
...
if string.lower(RequiredScript) ==
    "lib/units/beings/player/states/playermaskoff" then
    local _start_action_state_standard_original =
        PlayerMaskOff._start_action_state_standard
    local _interupt_action_start_standard_original =
        PlayerMaskOff._interupt_action_start_standard
    ...
    function PlayerMaskOff:_interupt_action_start_standard(t, input, complete, ...)
        ...
        return _interupt_action_start_standard_original(self, t, input,
                                                        complete, ...)
    end
end
...
if string.lower(RequiredScript) == "lib/units/beings/player/playerdamage" then
    ...
    local _start_regen_on_the_side_original =
        PlayerDamage._start_regen_on_the_side
    ...
    local _on_damage_armor_grinding_original =
        PlayerDamage._on_damage_armor_grinding
    local change_regenerate_speed_original =
        PlayerDamage.change_regenerate_speed
    ...
    local HEALTH_RATIO_BONUSES = {
        ...
        damage_health_ratio_multiplier = {
            category = "damage",
            buff_id = "berserker_aced"
        },
        armor_regen_damage_health_ratio_multiplier = {
            category = "armor_regen",
            buff_id = "yakuza_recovery"
        },
        movement_speed_damage_health_ratio_multiplier = {
            category = "movement_speed",
            buff_id = "yakuza_speed"
        }
    }
    ...
    local PASSIVE_HEALTH_REGEN = {
        ...
        hostage_taker = {
            ...
            check_clbk = function()
                return managers.player._HAS_HOSTAGES
            end
        }
    }
    ...
    function PlayerDamage:add_damage_to_hot(...)
        ...
        if num_new_stacks > num_old_stacks then
            local stack_duration = ((self._doh_data.total_ticks or 1) +
                                       managers.player:upgrade_value("player",
                                                                     "damage_to_hot_extra_ticks",
                                                                     0)) *
                                       (self._doh_data.tick_time or 1)
            managers.gameinfo:event("timed_buff", "activate", "grinder_debuff",
                                    {
                duration = tweak_data.upgrades.damage_to_hot_data
                    .stacking_cooldown
            })
            managers.gameinfo:event("timed_stack_buff", "add_stack", "grinder",
        ...
        if health_ratio ~= LAST_HEALTH_RATIO then
            ...
            for upgrade, data in pairs(HEALTH_RATIO_BONUSES) do
                if managers.player:has_category_upgrade("player", upgrade) then
                    local bonus_ratio = managers.player:get_damage_health_ratio(
                                            health_ratio, data.category)
            ...
            if managers.player:has_category_upgrade("player",
                                                    "passive_damage_reduction") then
                local threshold = managers.player:upgrade_value("player",
                                                                "passive_damage_reduction")
                local value = managers.player:team_upgrade_value(
                                  "damage_dampener", "team_damage_reduction")
                if health_ratio < threshold then
                    value = 2 * value - 1
                end
                managers.gameinfo:event("buff", "set_value", "crew_chief_1",
                                        {value = value})
            end
        end

        for buff_id, data in pairs(PASSIVE_HEALTH_REGEN) do
            if not self:full_health() and
                managers.player:has_category_upgrade(data.category, data.upgrade) and
                (not data.check_clbk or data.check_clbk()) then
                managers.gameinfo:event("buff", "activate", buff_id)
                managers.gameinfo:event("buff", "set_value", buff_id, {
                    value = managers.player:upgrade_value(data.category,
                                                          data.upgrade, 0)
                })
            else
                managers.gameinfo:event("buff", "deactivate", buff_id)
    ...
    function PlayerDamage:_upd_health_regen(t, ...)
        ...
        if self._health_regen_update_timer then
            if self._health_regen_update_timer > (old_timer or 0) --[[and not self:full_health()]] then
                for buff_id, data in pairs(PASSIVE_HEALTH_REGEN) do
                    managers.gameinfo:event("buff", "set_duration", buff_id, {
                        duration = self._health_regen_update_timer
                    })
                end
            end
        end
    ...
    function PlayerDamage:_start_regen_on_the_side(time, ...)
        if not self._regen_on_the_side and time > 0 then
            managers.gameinfo:event("timed_buff", "activate", "tooth_and_claw",
                                    {duration = time})
        end
    ...
    function PlayerDamage:_update_armor_grinding(t, ...)
        ...
        if self._armor_grinding.elapsed == 0 and ARMOR_GRIND_ACTIVE then
            managers.gameinfo:event("player_action", "set_duration",
                ...
                duration = self._armor_grinding.target_tick
            })
        end
    end
    ...
    function PlayerDamage:set_armor(armor, ...)
        ...
        elseif self._armor_grinding then
            if not ARMOR_GRIND_ACTIVE then
                ...
                managers.gameinfo:event("player_action", "activate",
                                        "anarchist_armor_regeneration")
                managers.gameinfo:event("player_action", "set_value",
                                        "anarchist_armor_regeneration", {
                    value = self._armor_grinding.armor_value
                })
                managers.gameinfo:event("player_action", "set_duration",
                                        "anarchist_armor_regeneration",
                                        {t = t_start, expire_t = expire_t})
            end
        end
    ...
    function PlayerDamage:_check_bleed_out(...)
        ...
        if (self._uppers_elapsed or 0) > last_uppers then
            managers.gameinfo:event("timed_buff", "activate", "uppers_debuff",
                                    {duration = self._UPPERS_COOLDOWN})
        end
    end

    local REGEN_EXPIRE_T = 0
    function PlayerDamage:_check_armor_regen_timer(reset)
        if not self._armor_grinding and not self:is_downed() and
            self._regenerate_timer and self:get_real_armor() < self:_max_armor() then
            local t = managers.player:player_timer():time()
            local armor_regen_delay = self._regenerate_timer /
                                          (self._regenerate_speed or 1)
            local suppression_delay = 0

            if self._supperssion_data.decay_start_t and
                self._supperssion_data.value ==
                tweak_data.player.suppression.max_value then
            ...
            if expire_t ~= REGEN_EXPIRE_T then
                ...
                managers.gameinfo:event("player_action", "activate",
                                        "standard_armor_regeneration")
                managers.gameinfo:event("player_action", reset and
                                            "set_duration" or "set_expire",
                                        "standard_armor_regeneration", {
                    duration = armor_regen_delay + suppression_delay
                })
            end
        end
    end
    ...
    function PlayerDamage:_update_delayed_damage(t, ...)
        ...
        if self._delayed_damage.next_tick then
            managers.gameinfo:event("buff", "set_value", "delayed_damage", {
                value = math.round((self:remaining_delayed_damage()) * 10)
            })
        end
    ...
    local DELAYED_DAMAGE_DURATION = math.round(
                                        1 /
                                            (tweak_data.upgrades.values.player
                                                .damage_control_passive[1][2] *
                                                0.01))
    function PlayerDamage:delay_damage(damage, seconds, ...)
        local has_calm = managers.player:has_category_upgrade("player",
                                                              "damage_control_auto_shrug")
        ...
        if has_calm then
            managers.gameinfo:event("buff", "set_duration", "delayed_damage", {
                t = t,
                duration = managers.player:upgrade_value("player",
                                                         "damage_control_auto_shrug")
            })
        end

        managers.gameinfo:event("buff", "set_duration", "delayed_damage_debuff",
                                {t = t, duration = DELAYED_DAMAGE_DURATION})
        managers.gameinfo:event("buff", "set_value", "delayed_damage", {
            value = math.round((self:remaining_delayed_damage() + damage) * 10)
        })
...
if string.lower(RequiredScript) ==
    "lib/player_actions/skills/playeractionbloodthirstbase" then
...
if string.lower(RequiredScript) ==
    "lib/player_actions/skills/playeractionexperthandling" then

    local experthandling_original = PlayerAction.ExpertHandling.Function

    function PlayerAction.ExpertHandling.Function(player_manager,
                                                  accuracy_bonus, max_stacks,
                                                  max_time, ...)
        managers.gameinfo:event("buff", "activate", "desperado")
        managers.gameinfo:event("buff", "set_duration", "desperado",
                                {expire_t = max_time})
        experthandling_original(player_manager, accuracy_bonus, max_stacks,
                                max_time, ...)
...
if string.lower(RequiredScript) ==
    "lib/player_actions/skills/playeractionshockandawe" then

    local shockandawe_original = PlayerAction.ShockAndAwe.Function

    function PlayerAction.ShockAndAwe.Function(player_manager, target_enemies,
                                               max_reload_increase,
                                               min_reload_increase, penalty,
                                               min_bullets, ...)
        local kill_count = 1
        ...
        local function on_enemy_killed(weapon_unit, variant)
            if alive(weapon_unit) then
                ...
                if kill_count >= target_enemies then
                    local min_threshold =
                        min_bullets +
                            (weapon_unit:base():is_category("smg",
                                                            "assault_rifle",
                                                            "lmg") and
                                player_manager:upgrade_value("player",
                                                             "automatic_mag_increase",
                                                             0) or 0)
                    local max_threshold =
                        math.floor(min_threshold +
                                       math.log(
                                           min_reload_increase /
                                               max_reload_increase) /
                                       math.log(penalty))
                    local ammo = weapon_unit:base():get_ammo_max_per_clip()
                    local bonus = math.clamp(
                                      max_reload_increase *
                                          math.pow(penalty, ammo - min_threshold),
                                      min_reload_increase, max_reload_increase)
                    managers.gameinfo:event("buff", "activate", "lock_n_load")
                    managers.gameinfo:event("buff", "set_value", "lock_n_load",
                                            {value = bonus})
                    managers.player:unregister_message(Message.OnEnemyKilled,
                                                       "lock_n_load_buff_listener")
                end
            end
        end
        ...
        managers.player:register_message(Message.OnEnemyKilled,
                                         "lock_n_load_buff_listener",
                                         on_enemy_killed)
        shockandawe_original(player_manager, target_enemies,
                             max_reload_increase, min_reload_increase, penalty,
                             min_bullets, ...)
...
if string.lower(RequiredScript) ==
    "lib/player_actions/skills/playeractiondireneed" then
...
if string.lower(RequiredScript) ==
    "lib/player_actions/skills/playeractionunseenstrike" then
    ...
    function PlayerAction.UnseenStrike.Function(player_manager, min_time, ...)
        ...
        player_manager:register_message(Message.OnPlayerDamage,
                                        "unseen_strike_debuff_listener",
                                        on_damage_taken)
        unseenstrike_original(player_manager, min_time, ...)
        player_manager:unregister_message(Message.OnPlayerDamage,
                                          "unseen_strike_debuff_listener")
    end
...
if string.lower(RequiredScript) ==
    "lib/player_actions/skills/playeractionammoefficiency" then

    local ammo_efficieny_original = PlayerAction.AmmoEfficiency.Function

    function PlayerAction.AmmoEfficiency.Function(player_manager,
                                                  target_headshots,
                                                  bullet_refund, target_time, ...)
        local headshots = 1
        ...
        local function on_headshot()
            ...
            if headshots < target_headshots then
                managers.gameinfo:event("buff", "set_stack_count",
                                        "ammo_efficiency", {
                    stack_count = target_headshots - headshots
                })
            end
        end
        ...
        ammo_efficieny_original(player_manager, target_headshots, bullet_refund,
                                target_time, ...)

        player_manager:unregister_message(Message.OnHeadShot,
                                          "ammo_efficiency_buff_listener")
...
if string.lower(RequiredScript) ==
    "lib/player_actions/skills/playeractiontriggerhappy" then

    local trigger_happy_original = PlayerAction.TriggerHappy.Function

    function PlayerAction.TriggerHappy.Function(player_manager, damage_bonus,
                                                max_stacks, max_time, ...)
        managers.gameinfo:event("buff", "activate", "trigger_happy")
        managers.gameinfo:event("buff", "set_duration", "trigger_happy",
                                {expire_t = max_time})
        trigger_happy_original(player_manager, damage_bonus, max_stacks,
                               max_time, ...)
...
if string.lower(RequiredScript) ==
    "lib/player_actions/skills/playeractiontagteam" then
    ...
    local function GetUnitName(unit)
        ...
        if unit:in_slot(3) or unit:in_slot(5) then -- Human criminal (mask off/on)
            local peer = managers.network:session() and
                             managers.network:session():peer_by_unit(unit)
        ...
        elseif unit:in_slot(16) or unit:in_slot(24) then -- Bot (mask on/off)/Joker
            ...
            if managers.gameinfo:get_minions(key) then -- Joker
                ...
                tweak_id =
                    CriminalsManager.convert_new_to_old_character_workname(
                        managers.criminals:character_name_by_unit(unit))
            end
    ...
    function PlayerAction.TagTeam.Function(tagged, owner, ...)
        local on_dmg_listener_key = string.format(
                                        "gameinfo_tag_team_on_damage_listener_%s",
                                        tostring(owner:key()))
        local tagged_name = GetUnitName(tagged)
        local base_values = managers.player:upgrade_value("player",
                                                          "tag_team_base")
        local duration = base_values.duration or 0
        managers.gameinfo:event("timed_buff", "activate", "tag_team",
                                {duration = duration})
        managers.gameinfo:event("buff", "set_value", "tag_team",
                                {value = tagged_name})

        CopDamage.register_listener(on_dmg_listener_key, {"on_damage"},
                                    function(damage_info)
            local was_killed = damage_info.result.type == "death"
            local valid_player = damage_info.attacker_unit == owner or
                                     damage_info.attacker_unit == tagged

            if was_killed and valid_player then
                managers.gameinfo:event("timed_buff", "change_expire",
                                        "tag_team", {
                    difference = base_values.kill_extension
                })
            end
        end)
    ...
    function PlayerAction.TagTeamTagged.Function(tagged, owner, ...)
        local on_dmg_listener_key = string.format(
                                        "gameinfo_tag_team_tagged_on_damage_listener_%s",
                                        tostring(owner:key()))
