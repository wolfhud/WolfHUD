if RequiredScript == "lib/units/enemies/cop/copdamage" then
        ...
        if attacker then
            ...
            if alive(killer) and alive(self._unit) then
                ...
                local special_unit_ids =
                    managers.statistics and managers.statistics.special_unit_ids or
                        {}
                local is_special = managers.groupai:state():is_enemy_special(
                                       self._unit) or
                                       table.contains(special_unit_ids, tweak_id)
                local body = data.col_ray and data.col_ray.body or
                                 self._sync_ibody_killcount and
                                 self._unit:body(self._sync_ibody_killcount)
                local headshot = body and self.is_head and self:is_head(body) or
                                     false

                if killer:in_slot(2) then
                    managers.hud:increment_teammate_kill_count(
                        HUDManager.PLAYER_PANEL, is_special, headshot)

                    local current_player_state =
                        managers.player and managers.player:get_current_state()
                    local weapon_base = current_player_state and
                                            current_player_state._equipped_unit:base()
                    local projectile_name = "bullet"
                    if weapon_base._projectile_type_index then
                        projectile_name =
                            tweak_data and
                                tweak_data:get_raw_value("blackmarket",
                                                         "projectiles",
                                                         "_projectiles_index",
                                                         weapon_base._projectile_type_index)
                    end
                    if projectile_name == (data.variant or "") then
                        local weapon_id = weapon_base:get_name_id()
                        local weapon_tweak =
                            weapon_base and weapon_base:weapon_tweak_data()
                        local weapon_type = weapon_tweak.category
                        local slot = weapon_tweak and weapon_tweak.use_data and
                                         weapon_tweak.use_data.selection_index
                        managers.hud:increment_teammate_kill_count_detailed(
                            HUDManager.PLAYER_PANEL, self._unit, weapon_id,
                            weapon_type, slot)
                    end
                else
                    local crim_data = managers.criminals:character_data_by_unit(
                                          killer)
                    if crim_data and crim_data.panel_id then
                        managers.hud:increment_teammate_kill_count(
                            crim_data.panel_id, is_special, headshot)
                    end
                end
            end
        end
    ...
    function CopDamage:_on_damage_received(data, ...)
        if self._dead then self:_process_kill(data) end
    ...
    function CopDamage:sync_damage_bullet(attacker_unit, damage_percent, i_body, ...)
        if i_body then self._sync_ibody_killcount = i_body end

        return sync_damage_bullet_original(self, attacker_unit, damage_percent,
                                           i_body, ...)
    end

    function CopDamage:sync_damage_melee(attacker_unit, damage_percent,
                                         damage_effect_percent, i_body, ...)
        if i_body then self._sync_ibody_killcount = i_body end

        return sync_damage_melee_original(self, attacker_unit, damage_percent,
                                          damage_effect_percent, i_body, ...)
...
elseif RequiredScript == "lib/managers/statisticsmanager" then
    ...
    function StatisticsManager:shot_fired(data, ...)
        ...
        if slot then -- Exclude throwables like exploding cards mod...
            local weapon_data = name_id and
                                    self._global.session.shots_by_weapon[name_id]
            local weapon_accuracy = 0
            if weapon_data and weapon_data.total > 0 then
                weapon_accuracy = math.floor(
                                      100 * weapon_data.hits / weapon_data.total)
            end
            managers.hud:set_teammate_weapon_accuracy(HUDManager.PLAYER_PANEL,
                                                      slot, weapon_accuracy)
        end

        managers.hud:set_teammate_accuracy(HUDManager.PLAYER_PANEL,
                                           self:session_hit_accuracy())
    end
...
elseif RequiredScript == "lib/managers/hudmanagerpd2" then
    ...
    HUDManager.increment_teammate_kill_count =
        HUDManager.increment_teammate_kill_count or
            function(self, i, is_special, headshot)
                self._teammate_panels[i]:increment_kill_count(is_special,
                                                              headshot)
            end

    HUDManager.reset_teammate_kill_count =
        HUDManager.reset_teammate_kill_count or
            function(self, i)
                self._teammate_panels[i]:reset_kill_count()
            end

    HUDManager.increment_teammate_kill_count_detailed =
        HUDManager.increment_teammate_kill_count_detailed or
            function(self, i, unit, weapon_id, weapon_type, weapon_slot)
                -- TODO: Add call for default HUD  |  No need for that, really...
            end

    HUDManager.set_teammate_accuracy = HUDManager.set_teammate_accuracy or
                                           function(self, i, value)
            self._teammate_panels[i]:set_accuracy(value)
        end

    HUDManager.set_teammate_weapon_accuracy =
        HUDManager.set_teammate_weapon_accuracy or
            function(self, i, slot, value)
                -- TODO
            end

    function HUDManager:teampanels_height()
        return (WolfHUD:getSetting({"CustomHUD", "PLAYER", "SHOW_ACCURACY"},
                                   true) and
                   not WolfHUD:getSetting(
                       {"CustomHUD", "PLAYER", "KILLCOUNTER", "HIDE"}, false)) and
                   140 or 120
    end
    ...
    if not HUDManager.CUSTOM_TEAMMATE_PANELS then -- Custom HUD compatibility
        ...
        function HUDTeammate:_init_killcount()
            self._kills_panel = self._panel:panel(
                                    {
                    name = "kills_panel",
                    visible = not WolfHUD:getSetting(
                        {
                            "CustomHUD", self._setting_prefix, "KILLCOUNTER",
                            "HIDE"
                        }, false),
                    w = 150,
                    h = 20,
            ...
            self._kills_panel:set_rightbottom(player_panel:right(),
                                              (self._main_player or
                                                  WolfHUD:getSetting(
                                                      {
                        "CustomHUD", "TEAMMATE", "INTERACTION", "TEXT"
                    }, true)) and name_label:bottom() or name_label:top())
            local killcount_color = WolfHUD:getColorSetting(
                                        {
                    "CustomHUD", self._setting_prefix, "KILLCOUNTER", "COLOR"
                }, "yellow")
        ...
        function HUDTeammate:init_accuracy()
            ...
            self._accuracy_panel = self._panel:panel(
                                       {
                    name = "accuracy_panel",
                    visible = WolfHUD:getSetting(
                        {"CustomHUD", "PLAYER", "SHOW_ACCURACY"}, true),
                    w = 100,
                    h = 20,
                    x = 0,
                    halign = "right"
            ...
            local player_panel = self._panel:child("player")
            local name_label = self._panel:child("name")
            self._accuracy_panel:set_rightbottom(player_panel:right(),
                                                 self._kills_panel and
                                                     self._kills_panel:visible() and
                                                     self._kills_panel:top() or
                                                     name_label:bottom())
        ...
        function HUDTeammate:increment_kill_count(is_special, headshot)
            ...
            self._kill_count_special = self._kill_count_special +
                                           (is_special and 1 or 0)
        ...
        function HUDTeammate:_update_kill_count_text()
            ...
            if WolfHUD:getSetting({
                ...
                kill_string = kill_string .. "/" ..
                                  tostring(self._kill_count_special)
            end
            if WolfHUD:getSetting({
                "CustomHUD", self._setting_prefix, "KILLCOUNTER",
                "SHOW_HEADSHOT_KILLS"
            ...
            self._kill_icon:set_right(self._kills_panel:w() - w -
                                          self._kill_icon:w() * 0.15)

            if (self._main_player or
                WolfHUD:getSetting({
                    "CustomHUD", "TEAMMATE", "INTERACTION", "TEXT"
                }, true)) and not WolfHUD:getSetting(
                {"CustomHUD", self._setting_prefix, "KILLCOUNTER", "HIDE"},
                false) then
                self._max_name_panel_width =
                    (self._kills_panel:x() + self._kill_icon:x() - 4)
                self:_truncate_name()
            end
        ...
        function HUDTeammate:set_state(...)
            ...
            local visible = not WolfHUD:getSetting(
                                {
                    "CustomHUD", self._setting_prefix, "KILLCOUNTER", "HIDE"
                }, false) and (not self._ai or
                                WolfHUD:getSetting(
                                    {
            ...
            if self._ai then
                self._kills_panel:set_bottom(
                    self._panel:child("player"):bottom())
            else
                local name_label = self._panel:child("name")
                self._kills_panel:set_bottom(
                    (self._main_player or
        ...
        function HUDTeammate:set_accuracy(value)
            ...
            self._accuracy_icon:set_right(
                self._accuracy_panel:w() - w - self._accuracy_icon:w() * 0.15)
            if WolfHUD:getSetting(
                {"CustomHUD", "PLAYER", "KILLCOUNTER", "HIDE"}, false) and
                WolfHUD:getSetting({"CustomHUD", "PLAYER", "SHOW_ACCURACY"},
                                   true) then
                self._max_name_panel_width =
                    (self._accuracy_panel:x() + self._accuracy_icon:x() - 4)
                self:_truncate_name()
            end
        end

        HUDTeammate._truncate_name = HUDTeammate._truncate_name or
                                         function() end
    end
end
