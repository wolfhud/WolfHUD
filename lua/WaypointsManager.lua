...
Last Note: Don't call any functions starting with "_" (or the update-functions) from the outside. Thanks!
    ...
    local function format_time_string(value)
        ...
        if math.floor(value) > 60 then
            frmt_string = string.format("%d:%02d", math.floor(value / 60),
                                        math.floor(value % 60))
    ...
    function WaypointManager:init()
        self._workspace = managers.gui_data and
                              managers.gui_data:create_fullscreen_workspace()
        self._waypoints = {}
    ...
    function WaypointManager:update(t, dt)
        ...
        if alive(self._hud_panel) and alive(self._workspace) and cam then
            ...
            mrotation.y(managers.viewport:get_current_camera_rotation(),
                        cam_forward)
            ...
            for id, wp in pairs(self._waypoints) do
                if wp:is_deleted() then
                    ...
                    wp:update(t, dt, cam, cam_forward, self._hud_panel,
                              self._workspace)
                end
    ...
    function WaypointManager:add_waypoint(id, class, data, overwrite)
        self._hud_panel = self._hud_panel or managers.hud and
                              managers.hud:script(
                                  PlayerBase.PLAYER_INFO_HUD_FULLSCREEN_PD2) and
                              managers.hud:script(
                                  PlayerBase.PLAYER_INFO_HUD_FULLSCREEN_PD2)
                                  .panel

        if not self._hud_panel then
            self._pending_waypoints[id] =
                {class = class, data = data, overwrite = overwrite}
        ...
        elseif not self._waypoints[id] then
            ...
            self._waypoints[id] = item_class and
                                      item_class:new(id, self._hud_panel, data)
        end
    ...
    function WaypointManager:clear_waypoints()
        self._pending_waypoints = {}
        for id, wp in pairs(self._waypoints) do wp:delete() end
    end

    function WaypointManager:add_waypoint_component(id, name, data, order,
                                                    overwrite)
        if self._pending_waypoints[id] then
            ...
            if not pending_data[name] or overwrite then
                ...
                if pending_data.component_order and order then
                    ...
                    pending_data.component_order[i] =
                        pending_data.component_order[i] or {}
        ...
        if self._pending_waypoints[id] then
            ...
            if pending_data[name] then
                ...
                if pending_data.component_order then
                    for i, vertical_order in
                        ipairs(pending_data.component_order) do
    ...
    function WaypointManager:set_waypoint_component_setting(id, component,
                                                            setting, value)
        if self._waypoints[id] then
    ...
    function WaypointManager:get_waypoint(id)
        if self._waypoints[id] then return self._waypoints[id] end
    end
    ...
    function CustomWaypoint:init(id, parent, data)
        ...
        self._slot_mask = data.mask and
                              (type(data.mask) == "string" and
                                  managers.slot:get_mask(data.mask) or data.mask) or
                              (managers.slot:get_mask('bullet_impact_targets',
                                                      'pickups') -
                                  managers.slot:get_mask('criminals'))
            ...
            if type(component_data) == "table" and component_data.type then
                if not self:add_component(component_name, data, nil) then
                    WolfHUD:print_log(string.format(
                                          "Error while creating waypoint: %s",
                                          self._id), "error")
                end
            end
        end
    ...
    function CustomWaypoint:delete() self._deleted = true end

    function CustomWaypoint:is_deleted() return self._deleted end

    function CustomWaypoint:is_on_screen()
        return (self._state == "on_screen" or self._state ==
                   "transition_on_screen")
    end
    ...
    function CustomWaypoint:add_component(name, data, order, overwrite)
        ...
        if type(data[name]) == "table" and data[name].type then
            ...
            if data[name].type == "distance" then
                self:_setup_distance_component(name, data, self._base_size,
                                               self._base_scale)
            elseif data[name].type == "timer" then
                self:_setup_timer_component(name, data, self._base_size,
                                            self._base_scale)
            elseif data[name].type == "duration" then
                self:_setup_duration_component(name, data, self._base_size,
                                               self._base_scale)
            elseif data[name].type == "icon" then
                self:_setup_icon_component(name, data, self._base_size,
                                           self._base_scale)
            elseif data[name].type == "label" then
                self:_setup_label_component(name, data, self._base_size,
                                            self._base_scale)
            else
                return false
            end
    ...
    function CustomWaypoint:remove_component(name)
        ...
        if component then self._panel:remove(component) end
    ...
    function CustomWaypoint:get_component(name) return self._components[name] end
    ...
    function CustomWaypoint:set_timer_pause(name, value)
        if self._settings[name] then self._settings[name].pause = value end
    end
    ...
    function CustomWaypoint:set_setting(setting, value)
        if self._settings then self._settings[setting] = value end
    end
    ...
    function CustomWaypoint:update(t, dt, cam, cam_fwd, hud, workspace)
        ...
        if self._unit and not alive(self._unit) then return self:delete() end

        self._position = self._unit and
                             (self._unit:movement() and
                                 self._unit:movement():m_head_pos() or
                                 self._unit:interaction() and
                                 self._unit:interaction():interact_position() or
                                 self._unit:position()) or self._position

        for name, _ in pairs(self._duration_components) do
            if self._settings[name] and self._settings[name].value then
                self:update_duration(name, math.max(
                                         self._settings[name].value - dt, 0))

                if self._settings[name].fade_duration then
                    local start, stop =
                        self._settings[name].fade_duration.start or 0,
                        self._settings[name].fade_duration.stop or 1
                    local progress = math.clamp(
                                         1 -
                                             (self._settings[name].value /
                                                 self._settings[name]
                                                     .start_value), 0, 1)
                    progress = math.min(math.max(progress - start, 0) /
                                            (stop - start), 1)
                    local fade_offset = self._settings[name].fade_duration
        ...
        for name, _ in pairs(self._timer_components) do
            if self._settings[name] and self._settings[name].auto_update ~= 0 and
                self._settings[name].value then
                if not self._settings[name].pause then
                    self:update_timer(name, self._settings[name].value +
                                          self._settings[name].auto_update * dt)
                end
            end
        end
        ...
        if not self._visible_through_walls then
            ...
            local r = World:raycast("ray", cam:position(), raycast_position,
                                    "slot_mask", self._slot_mask or
                                        managers.slot:get_mask(
                                            'explosion_targets'))
            is_enabled = (not r or not r.unit or type(r.unit) ~= "userdata" or
                             self._unit and (self._unit:key() == r.unit:key()))
        end

        if is_enabled and self._hide_on_uninteractable and
            self._unit:interaction() then
            local player = managers.player:player_unit()
            is_enabled = alive(player) and self._unit:interaction()._active and
                             self._unit:interaction():can_interact(player)
        end
        ...
        if is_enabled then
            ...
            on_screen = dot >= 0 and
                            not hud:outside(mvector3.x(screen_pos),
                                            mvector3.y(screen_pos))

            if not on_screen then
                local radius = self._settings.radius_offscreen
                local panel_center_x, panel_center_y = hud:center()
                mvector3.set_static(dir, screen_pos.x - panel_center_x,
                                    screen_pos.y - panel_center_y, 0)
                mvector3.set(dir_normalized, dir)
                mvector3.normalize(dir_normalized)
                mvector3.set_static(screen_pos, panel_center_x +
                                        mvector3.x(dir_normalized) * radius,
                                    panel_center_y + mvector3.y(dir_normalized) *
                                        radius, 0)
            end
        end

        if is_enabled and self._state ==
            (on_screen and "on_screen" or "off_screen") then
            self:_update_visuals(mvector3.x(screen_pos), mvector3.y(screen_pos),
                                 math.acos(dot), dir_normalized)
        elseif is_enabled and (on_screen or self._settings.show_offscreen) then
            ...
            local x_diff, y_diff = mvector3.x(screen_pos) - x,
                                   mvector3.y(screen_pos) - y
            local x_move, y_move = x_diff / self._transition_time * dt + x,
                                   y_diff / self._transition_time * dt + y
        ...
        if self:_update_component_visuals(self._panel, self._settings, angle) then -- Main panel
            ...
            for name, component in pairs(self._components) do
                if component and self._settings[name] then
                    local visible, need_rearrange =
                        self:_update_component_visuals(component,
                                                       self._settings[name],
                                                       angle)
            ...
            if rearrange then self:_arrange() end

            self._panel:set_center(x, y)

            if self._components.arrow and self._components.arrow:visible() then
                local angle = math.X:angle(arrow_direction) *
                                  math.sign(arrow_direction.y)
                self._components.arrow:set_rotation(angle)
                self._components.arrow:set_center(
                    x + arrow_direction.x * (self._panel:w() + 3),
                    y + arrow_direction.y * (self._panel:h() + 3))
            end
    ...
    function CustomWaypoint:_update_component_visuals(component, data, angle)
        ...
        if component and data.show and
            (self:is_on_screen() or data.show_offscreen) then
            local is_visible = true
            local in_view = true
            local in_range = true

            if data.visible_distance then
                in_range = (not data.visible_distance.min or
                               data.visible_distance.min <= self._distance) and
                               (not data.visible_distance.max or
                                   data.visible_distance.max >= self._distance)
            end

            if data.visible_angle then
                in_view =
                    (not data.visible_angle.min or data.visible_angle.min <=
                        angle) and
                        (not data.visible_angle.max or data.visible_angle.max >=
                            angle)
            end
            ...
            if is_visible then
                if data.fade_duration then
                    ...
                    if not progress then
                        ...
                        for name, _ in pairs(self._duration_components) do
                            if self._settings[name] and
                                self._settings[name].progress then
                                progress =
                                    progress + self._settings[name].progress
                    ...
                    if data.fade_duration.color and
                        data.fade_duration.color.start and
                        data.fade_duration.color.stop then
                        local color = math.lerp(data.fade_duration.color.start,
                                                data.fade_duration.color.stop,
                                                progress)
                        data.color = color
                    end
                end

                if data.rescale_distance and
                    data.rescale_distance.start_distance and
                    data.rescale_distance.end_distance then
                    ...
                    if start < self._distance then
                        local final_scale =
                            data.rescale_distance.final_scale or 0
                        local scale =
                            (self._distance >= stop) and final_scale or
                                math.lerp(1, final_scale, (self._distance -
                                              start) / (stop - start))
                        local new_size =
                            math.round(data.base_values.size * scale)
                        ...
                        if component:h() ~= new_size then
                            ...
                            if data.base_values.font_size then
                                component:set_font_size(
                                    data.base_values.font_size * scale)
                                ...
                                component:set_w(
                                    new_size * data.base_values.w_ratio)
                            end
                        end
                    end
                end

                if data.fade_angle and data.fade_angle.start_angle and
                    data.fade_angle.end_angle then
                    ...
                    if start > angle then
                        ...
                        local scale = (stop >= angle) and final_scale or
                                          math.lerp(final_scale, 1, (angle -
                                                        stop) / (start - stop))
    ...
    function CustomWaypoint:_setup_image_component(name, data, base_size,
                                                   base_scale, base_panel)
        ...
        local w_ratio = data.w_ratio or texture_rect and
                            (texture_rect[3] / texture_rect[4]) or 1
        ...
        self._components[name] = (base_panel or self._panel):bitmap(
                ...
                render_template = data[name].radial_image and
    ...
    function CustomWaypoint:_setup_arrow(data, base_size, base_scale)
        ...
        self:_setup_image_component("arrow", data, base_size, base_scale,
                                    self._panel:parent())

        self._components.arrow:set_rotation(270)
    end

    function CustomWaypoint:_setup_icon_component(name, data, base_size,
                                                  base_scale)
        self:_setup_image_component(name, data, base_size, base_scale,
                                    self._panel)

        if HUDManager.HAS_MINIMAP and data[name].on_minimap then
            managers.hud._hud_minimap:add_entity(HUDMiniMapWaypointEntity,
                                                 self._id, {
                unit = data.unit,
    ...
    function CustomWaypoint:_setup_text_component(name, data, base_size,
                                                  base_scale, base_panel)
        local component_data = data[name] or {}
        local scale = base_scale *
                          (component_data.scale or data.text_scale or 1)
        local color = component_data.color or data.text_color or data.color or
                          Color.white
        local size = base_size * scale
        ...
        self._components[name] = base_panel:text(
                ...
                text = component_data.text and tostring(component_data.text) or
                    "",
                color = color,
                font = component_data.font or data.text_font or
                    tweak_data.hud.medium_font_noshadow,
                font_size = size * 0.95,
                align = "left",
                vertical = "center",
                w = size * 3,
    ...
    function CustomWaypoint:_setup_label_component(name, data, base_size,
                                                   base_scale)
        self:_setup_text_component(name, data, base_size, base_scale,
                                   self._panel)
    end

    function CustomWaypoint:_setup_distance_component(name, data, base_size,
                                                      base_scale)
        self:_setup_text_component(name, data, base_size, base_scale,
                                   self._panel)
    ...
    function CustomWaypoint:_setup_timer_component(name, data, base_size,
                                                   base_scale)
        self:_setup_text_component(name, data, base_size, base_scale,
                                   self._panel)
        self._settings[name].value = data[name] and
                                         (data[name].initial_value or 0)
        self._settings[name].auto_update =
            data[name] and data[name].auto_update or 0
        self._settings[name].pause = data[name] and data[name].pause or false
        self._timer_components[name] = true
    end

    function CustomWaypoint:_setup_duration_component(name, data, base_size,
                                                      base_scale)
        self:_setup_text_component(name, data, base_size, base_scale,
                                   self._panel)
        self._settings[name].start_value =
            data[name] and data[name].initial_value
    ...
    function CustomWaypoint:_arrange()
        ...
        for i, vertical_order in ipairs(self._component_order) do
            for j = 1, #vertical_order do
                ...
                local component = self._components[name]
                if component and component:visible() then
                    local row_w, row_h = w_rows[i] or component:w(),
                                         h_rows[i] or component:h()

                    component:set_x(math.ceil(component:x() + (w - row_w) / 2))
                    component:set_y(math.ceil(
                                        component:y() + (row_h - component:h()) /
                                            2))
                end
            end
        end
