...
if string.lower(RequiredScript) == "lib/managers/hud/hudhitdirection" then
    ...
    function HUDHitDirection:init(...)
        ...
        if alive(self._hud_panel) and alive(self._hit_direction_panel) then
            ...
            self._hit_direction_panel:set_center(
                self._hit_direction_panel:parent():w() * 0.5,
                self._hit_direction_panel:parent():h() * 0.5)
        end
    end

    function HUDHitDirection:_add_hit_indicator(...)
        HUDHitDirection.PANEL_SIZE = WolfHUD:getSetting(
                                         {"DamageIndicator", "SIZE"}, 150)
        if self.indicator_count <
            WolfHUD:getSetting({"DamageIndicator", "MAX_AMOUNT"}, 10) then
    ...
    function HUDHitDirection:_animate(indicator, data, remove_func)
        ...
        while data.t < data.duration do
            ...
            if alive(indicator) then
                ...
                indicator:set_color(self:_get_indicator_color(data.damage_type,
                                                              o))
                ...
                if managers.player:player_unit() then
                    ...
                    if ply_camera then
                        ...
                        local angle = target_vec:to_polar_with_reference(
                                          ply_camera:forward(), math.UP).spin
                        local r = HUDHitDirection.PANEL_SIZE +
                                      (1 - math.pow(o, 0.5)) * (100)
                        if data.fixed_angle ~= nil then
                            angle = data.fixed_angle
                        end
                        indicator:set_rotation(90 - angle)
                        indicator:set_center(
                            self._hit_direction_panel:w() * 0.5 -
                                math.sin(angle + 180) * r,
                            self._hit_direction_panel:h() * 0.5 -
                                math.cos(angle + 180) * r)
                    end
                end
            end
        end
    ...
    function HUDHitDirection:_get_indicator_color(damage_type, t)
        if damage_type == HUDHitDirection.DAMAGE_TYPES.HEALTH then
            return WolfHUD:getColorSetting({"DamageIndicator", "HEALTH_COLOR"},
                                           "red")
        elseif damage_type == HUDHitDirection.DAMAGE_TYPES.ARMOUR then
            return WolfHUD:getColorSetting({"DamageIndicator", "SHIELD_COLOR"},
                                           "white")
        elseif damage_type == HUDHitDirection.DAMAGE_TYPES.VEHICLE then
            return WolfHUD:getColorSetting({"DamageIndicator", "VEHICLE_COLOR"},
                                           "yellow")
        elseif damage_type == HUDHitDirection.DAMAGE_TYPES.CRIT then
            return WolfHUD:getColorSetting({"DamageIndicator", "CRIT_COLOR"},
                                           "purple")
        elseif damage_type == HUDHitDirection.DAMAGE_TYPES.FRIENDLY_FIRE then
            return WolfHUD:getColorSetting(
                       {"DamageIndicator", "FRIENDLY_FIRE_COLOR"}, "orange")
        else
            return Color(1, t, t)
        end
    end
elseif string.lower(RequiredScript) == "lib/units/beings/player/playerdamage" then
    ...
    function PlayerDamage:damage_explosion(attack_data, ...)
        ...
        if alive(self._unit) and
            (attack_data.position or attack_data.col_ray.position) then
            local distance = mvector3.distance(
                                 attack_data.position or
                                     attack_data.col_ray.position,
                                 self._unit:position())
            if self:_chk_can_take_dmg() and distance <= attack_data.range and
                not (self._god_mode or self._invulnerable or
                    self._mission_damage_blockers.invulnerable or
                    self:incapacitated() or self._bleed_out) then
                self:_hit_direction(attack_data.position,
                                    HUDHitDirection.DAMAGE_TYPES.FRIENDLY_FIRE)
            end
        end
        return value
    end
    ...
    function PlayerDamage:_hit_direction(position_vector, damage_type)
        if position_vector then
            local armor_left, low_health = (self:get_real_armor() > 0),
                                           ((self:get_real_health() /
                                               self:_max_health()) <= 0.20)
            local dmg_type = damage_type or armor_left and
                                 HUDHitDirection.DAMAGE_TYPES.ARMOUR or
                                 low_health and
                                 HUDHitDirection.DAMAGE_TYPES.CRIT or
                                 HUDHitDirection.DAMAGE_TYPES.HEALTH
