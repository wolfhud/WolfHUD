if string.lower(RequiredScript) == "lib/managers/menumanager" then
    ...
    function MenuOptionInitiator:modify_controls(...)
        ...
        for i, name in ipairs({
            "camera_sensitivity", "camera_sensitivity_horizontal",
            "camera_sensitivity_vertical", "camera_zoom_sensitivity",
            "camera_zoom_sensitivity_horizontal",
            "camera_zoom_sensitivity_vertical"
        }) do
            item = new_node:item(name)
            if item then
                item:set_step((tweak_data.player.camera.MAX_SENSITIVITY -
                                  tweak_data.player.camera.MIN_SENSITIVITY) *
                                  0.01)
            end
        end
    ...
    Hooks:PostHook(MenuCallbackHandler, "choice_crimenet_lobby_job_plan",
                   "MenuCallbackHandlerPostSaveJobPlan_WolfHUD",
                   function(self, ...) self:save_lobby_settings("job_plan") end)
    Hooks:PostHook(MenuCallbackHandler, "choice_kicking_option",
                   "MenuCallbackHandlerPostSaveKickOption_WolfHUD", function(
        self, ...) self:save_lobby_settings("kick_option") end)
    Hooks:PostHook(MenuCallbackHandler, "choice_crimenet_lobby_permission",
                   "MenuCallbackHandlerPostSavePermission_WolfHUD", function(
        self, ...) self:save_lobby_settings("permission") end)
    Hooks:PostHook(MenuCallbackHandler,
                   "choice_crimenet_lobby_reputation_permission",
                   "MenuCallbackHandlerPostSaveReputationPermission_WolfHUD",
                   function(self, ...)
        self:save_lobby_settings("reputation_permission")
    end)
    Hooks:PostHook(MenuCallbackHandler, "choice_crimenet_drop_in",
                   "MenuCallbackHandlerPostSaveDropInOption_WolfHUD", function(
        self, ...) self:save_lobby_settings("drop_in_option") end)
    Hooks:PostHook(MenuCallbackHandler, "choice_crimenet_team_ai",
                   "MenuCallbackHandlerPostSaveTeamAIOption_WolfHUD",
                   function(self, ...)
        self:save_lobby_settings("team_ai")
        self:save_lobby_settings("team_ai_option")
    end)
    Hooks:PostHook(MenuCallbackHandler, "choice_crimenet_auto_kick",
                   "MenuCallbackHandlerPostSaveAutoKick_WolfHUD",
                   function(self, ...) self:save_lobby_settings("auto_kick") end)
    Hooks:PostHook(MenuCallbackHandler, "change_contract_difficulty",
                   "MenuCallbackHandlerPostSaveDifficulty_WolfHUD",
                   function(self, item, ...)
        self:save_lobby_settings("difficulty",
                                 tweak_data:index_to_difficulty(item:value()))
    end)

    Hooks:PostHook(MenuCallbackHandler, "choice_crimenet_one_down",
                   "MenuCallbackHandlerPostSaveOneDownMod_WolfHUD",
                   function(self, item, ...)
        self:save_lobby_settings("one_down", item:value() == "on")
    end)

    Hooks:PostHook(MenuCallbackHandler, "update_matchmake_attributes",
                   "MenuCallbackHandlerPostUpdateMatchmakeAttributes_WolfHUD",
                   function(self, item, ...) self:save_lobby_settings() end)

    local WOLFHUD_LOBBY_SETTINGS_LOADED = false
    local MenuCrimeNetContractInitiator_modify_node_orig =
        MenuCrimeNetContractInitiator.modify_node
    function MenuCrimeNetContractInitiator:modify_node(original_node, data, ...)
        ...
        if data.customize_contract then
            data.difficulty = WolfHUD:getSetting(
                                  {"LOBBY_SETTINGS", "difficulty"}, "normal")
            data.difficulty_id = tweak_data:difficulty_to_index(data.difficulty)
            data.one_down = WolfHUD:getSetting({"LOBBY_SETTINGS", "one_down"},
                                               false)
        end

        local results = {
            MenuCrimeNetContractInitiator_modify_node_orig(self, original_node,
                                                           data, ...)
        }
        ...
        if data.customize_contract then
            ...
            if diff_item then diff_item:set_value(data.difficulty_id) end
...
elseif string.lower(RequiredScript) == "lib/managers/menu/blackmarketgui" then
    ...
    local populate_weapon_category_new_original =
        BlackMarketGui.populate_weapon_category_new
    function BlackMarketGui:populate_weapon_category_new(data, ...)
        local value = populate_weapon_category_new_original(self, data, ...)
        local show_icons = not WolfHUD:getSetting(
                               {"INVENTORY", "SHOW_WEAPON_MINI_ICONS"}, true)
        for id, w_data in ipairs(data) do
            if tweak_data.weapon[w_data.name] then -- Filter out locked or empty slots
                ...
                local has_silencer = table.contains(categories, "bow") or
                                         table.contains(categories, "crossbow")
                ...
                for id, i_data in pairs(w_data.mini_icons) do -- Needs to handle silent motor saw
                    if i_data.alpha == 1 then -- Icon enabled
                        if i_data.texture ==
                            "guis/textures/pd2/blackmarket/inv_mod_silencer" then
                            has_silencer = true
                        elseif i_data.texture ==
                            "guis/textures/pd2/blackmarket/inv_mod_ammo_explosive" then
                            has_explosive = true
                        end
                    end
                end
        ...
        if data and data.quantity then
            if type(data.quantity) == "table" then
                ...
                for i = 1, #amounts do
                    ...
                    amount_str = amount_str .. (i > 1 and "/x" or "x") ..
                                     tostring(
                                         (amounts[i] or 0) +
                                             managers.player:equiptment_upgrade_value(
    ...
    function BlackMarketGui:populate_deployables(data, ...)
        ...
        for i, equipment in ipairs(data) do
            equipment.name_localized = equipment.name_localized ..
                                           (equipment.unlocked and
                                               getEquipmentAmount(equipment.name) or
                                               "")
        end
    end
    ...
    function BlackMarketGui:populate_grenades(data, ...)
        ...
        for i, throwable in ipairs(data) do
            local has_amount = throwable.unlocked and t_data[throwable.name] or
                                   false
            throwable.name_localized = throwable.name_localized ..
                                           (has_amount and " (x" ..
                                               t_data[throwable.name].max_amount ..
                                               ")" or "")
        end
    end
    ...
    function BlackMarketGuiSlotItem:init(main_panel, data, ...)
        if WolfHUD:getSetting({"INVENTORY", "SHOW_WEAPON_NAMES"}, true) then
            data.custom_name_text =
                data.custom_name_text or not data.empty_slot and
                    data.name_localized
        end
    ...
    local orig_blackmarket_gui_slot_item_set_highlight =
        BlackMarketGuiSlotItem.set_highlight
    function BlackMarketGuiSlotItem:set_highlight(highlight, ...)
        if highlight or self._is_selected or self._data.equipped then
            ...
            if name_text then name_text:set_alpha(1) end
            if self._mini_panel then self._mini_panel:set_alpha(1) end
        else
            local name_text = self._panel:child("custom_name_text")
            if name_text then name_text:set_alpha(0.5) end
            if self._mini_panel then self._mini_panel:set_alpha(0.4) end
        end

        return
            orig_blackmarket_gui_slot_item_set_highlight(self, highlight, ...)
    end

    local orig_blackmarket_gui_set_selected_tab =
        BlackMarketGui.set_selected_tab
    function BlackMarketGui:set_selected_tab(...)
        ...
        local selected_slot = current_tab and
                                  current_tab._slots[current_tab._slot_selected]
        local highlighted_slot = current_tab and
                                     current_tab._slots[current_tab._slot_highlighted]
    ...
    function BlackMarketGui:_setup(is_start_page, component_data)
        ...
        local inv_name_tweak = WolfHUD:getSetting(
                                   {"INVENTORY", "CUSTOM_TAB_NAMES"}, {})
        if inv_name_tweak then
            for i, tab_data in ipairs(component_data) do
                if not tab_data.prev_node_data then
                    ...
                    local custom_tab_name =
                        category_tab_names and category_tab_names[i] or ""
                    for key, subst in pairs(BlackMarketGui._SUB_TABLE) do
                        custom_tab_name =
                            custom_tab_name:upper():gsub(key, subst)
                    end
                    if string.len(custom_tab_name or "") > 0 then
                        tab_data.name_localized =
                            custom_tab_name or tab_data.name_localized
                    end
                    self._renameable_tabs =
                        self._renameable_tabs or category_tab_names and true or
                            false
                end
            end
        end
        ...
        if self._renameable_tabs and self._tabs[1] then
            local first_tab_name = self._tabs[1]._tab_panel and
                                       self._tabs[1]._tab_panel:child("tab_text")
            self._renameable_tabs = first_tab_name:visible()
        end

        if self._renameable_tabs and not component_data.is_loadout and
            alive(self._panel) then
    ...
    function BlackMarketGui:mouse_clicked(...)
        ...
        if not self._enabled or alive(self._context_panel) then return end

        self._mouse_click = self._mouse_click or {}
        self._mouse_click[self._mouse_click_index].selected_tab = self._selected
    end

    local BlackMarketGui_mouse_double_click_original =
        BlackMarketGui.mouse_double_click
    function BlackMarketGui:mouse_double_click(o, button, x, y)
        if self._enabled and not self._data.is_loadout and self._renameable_tabs then
            if self._mouse_click and self._mouse_click[0] and
                self._mouse_click[1] then
                if self._tabs and self._mouse_click[0].selected_tab ==
                    self._mouse_click[1].selected_tab then
                    local current_tab = self._tabs[self._selected]
                    if current_tab and button == Idstring("0") then
                        if self._tab_scroll_panel:inside(x, y) and
                            current_tab:inside(x, y) ~= 1 then
    ...
    function BlackMarketGui:rename_tab_clbk(tab, tab_id)
        ...
        local inv_name_tweak = WolfHUD:getSetting(
                                   {"INVENTORY", "CUSTOM_TAB_NAMES"}, nil)
        if current_tab and tab_data and inv_name_tweak and not self:in_setup() then
            local prev_name = inv_name_tweak[tab_data.category] and
                                  inv_name_tweak[tab_data.category][tab_id or
                                      self._selected] or
                                  current_tab._tab_text_string
            local menu_options = {
                [1] = {
                    ...
                    callback = function(cb_data, button_id, button, text)
                        if self._data and text and text ~= "" then
                            if tab_data and inv_name_tweak then
                                inv_name_tweak[tab_data.category] =
                                    inv_name_tweak[tab_data.category] or {}
                                inv_name_tweak[tab_data.category][tab_id or
                                    self._selected] = text
                                WolfHUD:Save()

                                for key, subst in
                                    pairs(BlackMarketGui._SUB_TABLE) do
                                    text = text:upper():gsub(key, subst)
                                end

                                current_tab._tab_text_string = text
                                local name =
                                    current_tab._tab_panel:child("tab_text")
                                if alive(name) then
                                    name:set_text(text)
                                end
            ...
            QuickInputMenu:new(managers.localization:text(
                                   "wolfhud_dialog_rename_inv_tab"),
                               managers.localization:text(
                                   "wolfhud_dialog_rename_inv_tab_desc"),
                               prev_name, menu_options, true,
                               {w = 420, to_upper = true, max_len = 15})
    ...
    function BlackMarketGui:rearrange_tab_width(start_with_selected)
        ...
        if current_tab then
            ...
            local current_text = current_panel and
                                     current_panel:child("tab_text")
            local current_selection = current_panel and
                                          current_panel:child("tab_select_rect")
            if alive(current_panel) and alive(current_text) and
                alive(current_selection) then
        ...
        local offset = alive(self._tabs[current]._tab_panel) and
                           self._tabs[current]._tab_panel:right() or 0
        for i = start, stop do
...
elseif string.lower(RequiredScript) == "lib/managers/menu/skilltreeguinew" then
    ...
    function NewSkillTreeSkillItem:refresh(...)
        ...
        if alive(self._skill_panel) and
            WolfHUD:getSetting({"INVENTORY", "SHOW_SKILL_NAMES"}, true) then
            local skill_name = self._skill_panel:child("SkillName")
            if skill_name then
                local unlocked = self._skill_id and self._tree and
                                     managers.skilltree and
                                     managers.skilltree:skill_unlocked(
                                         self._tree, self._skill_id) or false
                local step = (self._skilltree:next_skill_step(self._skill_id) or
                                 0)
                local skilled = unlocked and step > 0
                skill_name:set_visible(true)
                skill_name:set_alpha(self._selected and 1 or skilled and 0.6 or
                                         0.4)
            end
        end
    ...
    function NewSkillTreeSkillItem:is_active(...)
        local unlocked =
            self._skill_id and self._tree and managers.skilltree and
                managers.skilltree:skill_unlocked(self._tree, self._skill_id) or
                false
    ...
    local orig_newskilltreetieritem_refresh_points =
        NewSkillTreeTierItem.refresh_points
    local orig_newskilltreetieritem_refresh_tier_text =
        NewSkillTreeTierItem._refresh_tier_text
    function NewSkillTreeTierItem:init(...)
        ...
        if WolfHUD:getSetting({"INVENTORY", "SHOW_SKILL_NAMES"}, true) then
            if self._tier_points_total and self._tier_points_total_zero and
                self._tier_points_total_curr then
    ...
    function NewSkillTreeTierItem:refresh_points(selected, ...)
        ...
        if WolfHUD:getSetting({"INVENTORY", "SHOW_SKILL_NAMES"}, true) then
            if alive(self._tier_points_total) and
                alive(self._tier_points_total_zero) and
                alive(self._tier_points_total_curr) then
            ...
            if alive(self._tier_points_0) and alive(self._tier_points) then
                self._tier_points:set_visible(
                    not self._tier_points_needed:visible())
                self._tier_points_0:set_visible(
                    not self._tier_points_needed:visible())
            end
        end
    end
    function NewSkillTreeTierItem:_refresh_tier_text(selected, ...)
        ...
        if WolfHUD:getSetting({"INVENTORY", "SHOW_SKILL_NAMES"}, true) then
            if selected and alive(self._tier_points_needed) and
                alive(self._tier_points_needed_curr) and
                alive(self._tier_points_needed_zero) then
                self._tier_points_needed_zero:set_left(
                    self._tier_points_0:left())
                self._tier_points_needed_curr:set_left(
                    self._tier_points_needed_zero:right())
                self._tier_points_needed:set_left(
                    self._tier_points_needed_curr:right() + self._text_space)
            end
            if alive(self._tier_points_0) and alive(self._tier_points) then
                self._tier_points:set_visible(
                    not self._tier_points_needed:visible())
...
elseif string.lower(RequiredScript) == "lib/tweak_data/tweakdata" then
    if tweak_data then
        ...
        if Network:is_server() and
            WolfHUD:getSetting({"SkipIt", "INSTANT_RESTART"}, false) then
...
elseif string.lower(RequiredScript) == "lib/tweak_data/guitweakdata" then
    ...
    function GuiTweakData:init(...)
        ...
        self.rename_max_letters = WolfHUD:getTweakEntry(
                                      "MAX_WEAPON_NAME_LENGTH", "number", 30)
        self.rename_skill_set_max_letters =
            WolfHUD:getTweakEntry("MAX_SKILLSET_NAME_LENGTH", "number", 25)
...
elseif string.lower(RequiredScript) == "lib/managers/crimenetmanager" then
    ...
    function CrimeNetGui:check_job_pressed(...)
        if self._jobs["random_contract"] and
            self._jobs["random_contract"].mouse_over == 1 then
            local job_id = tweak_data.narrative._jobs_index[math.random(
                               #tweak_data.narrative._jobs_index)]
            ...
            local data = {
                ...
                server = false,
                special_node = Global.game_settings.single_player and
                    "crimenet_contract_singleplayer" or "crimenet_contract_host"
            }
...
elseif string.lower(RequiredScript) ==
    "core/lib/managers/menu/items/coremenuitemslider" then
    ...
    function ItemSlider:highlight_row_item(node, row_item, mouse_over, ...)
        ...
        row_item.gui_slider_gfx:set_gradient_points(
            {
                0, self:slider_highlighted_color():with_alpha(0.6), 1,
                self:slider_highlighted_color():with_alpha(0.6)
            })
        return val
    end
    ...
    function ItemSlider:set_enabled(...)
        ...
        if self._enabled then
            ...
            self:set_slider_highlighted_color(
...
elseif string.lower(RequiredScript) == "lib/states/ingamewaitingforplayers" then
    local SKIP_BLACKSCREEN = WolfHUD:getSetting({"SkipIt", "SKIP_BLACKSCREEN"},
                                                true)
    local update_original = IngameWaitingForPlayersState.update
    function IngameWaitingForPlayersState:update(...)
        update_original(self, ...)

        if self._skip_promt_shown and SKIP_BLACKSCREEN then self:_skip() end
    end
elseif string.lower(RequiredScript) == "lib/managers/menu/stageendscreengui" then
    ...
    function StageEndScreenGui:init(...)
        ...
        if self._enabled and
            WolfHUD:getSetting({"SkipIt", "STAT_SCREEN_SPEEDUP"}, false) and
            managers.hud then managers.hud:set_speed_up_endscreen_hud(5) end
    end

    local SKIP_STAT_SCREEN_DELAY = WolfHUD:getSetting(
                                       {"SkipIt", "STAT_SCREEN_DELAY"}, 5)
    function StageEndScreenGui:update(t, ...)
        update_original(self, t, ...)
        if not self._button_not_clickable and SKIP_STAT_SCREEN_DELAY > 0 then
            self._auto_continue_t = self._auto_continue_t or
                                        (t + SKIP_STAT_SCREEN_DELAY)
            local gsm = game_state_machine:current_state()
            if gsm and gsm._continue_cb and
                not (gsm._continue_blocked and gsm:_continue_blocked()) and t >=
                self._auto_continue_t then
...
elseif string.lower(RequiredScript) == "lib/managers/menu/lootdropscreengui" then
    local SKIP_LOOT_SCREEN_DELAY = WolfHUD:getSetting(
                                       {"SkipIt", "LOOT_SCREEN_DELAY"}, 3)
    local AUTO_PICK_CARD = WolfHUD:getSetting({"SkipIt", "AUTOPICK_CARD"}, true)
    local AUTO_PICK_SPECIFIC_CARD = WolfHUD:getSetting(
                                        {"SkipIt", "AUTOPICK_CARD_SPECIFIC"}, 1)
    ...
    function LootDropScreenGui:update(t, ...)
        ...
        if not self._button_not_clickable and SKIP_LOOT_SCREEN_DELAY > 0 then
            self._auto_continue_t = self._auto_continue_t or
                                        (t + SKIP_LOOT_SCREEN_DELAY)
            local gsm = game_state_machine:current_state()
            if gsm and not (gsm._continue_blocked and gsm:_continue_blocked()) and
                t >= self._auto_continue_t then
        ...
        if managers.network:session() then
            if managers.network:session():local_peer():id() ~= peer_id then
                ...
                if alive(peer_label) then
                    ...
                    local LPI_offset = LobbyPlayerInfo and
                                           (LobbyPlayerInfo.settings
                                               .show_play_time_mode or 0) > 1 and
                                           LobbyPlayerInfo:GetFontSizeForPlayTime() or
                                           0

                    self._peer_latency = self._peer_latency or {}
                    self._peer_latency[peer_id] =
                        self._peer_latency[peer_id] or self._panel:text(
                            {
                                ...
                                font_size = tweak_data.menu.pd2_medium_font_size *
                                    0.8,
                                layer = 0,
                                color = tweak_data.chat_colors[peer_id] or
                                    Color.white,
                                alpha = 0.8,
                                blend_mode = "add"
                            })
                    self._peer_latency[peer_id]:set_text(
                        latency > 0 and string.format("%.0fms", latency) or
                            "---ms")
...
elseif string.lower(RequiredScript) ==
    "lib/managers/menu/renderers/menunodeskillswitchgui" then
    ...
    function MenuNodeSkillSwitchGui:_create_menu_item(row_item, ...)
        ...
        if row_item.type ~= "divider" and row_item.name ~= "back" then
            ...
            row_item.status_gui:set_text(
                managers.localization:to_upper_text(
                    ("menu_st_spec_%d"):format(
                        managers.skilltree:digest_value(gd.specialization,
                                                        false, 1) or 1)))
            if row_item.skill_points_gui:text() ==
                managers.localization:to_upper_text(
                    "menu_st_points_all_spent_skill_switch") then
                local pts, pt, pp, st, sp = 0, 1, 0, 2, 0
                for i = 1, #gd.trees do
                    pts = Application:digest_value(gd.trees[i].points_spent,
                                                   false)
                    if pts > pp then
                ...
                row_item.skill_points_gui:set_text(
                    managers.localization:to_upper_text(
                        tweak_data.skilltree.trees[pt].name_id) .. " / " ..
                        managers.localization:to_upper_text(
                            tweak_data.skilltree.trees[st].name_id))
            end
        elseif row_item.type == "divider" and row_item.name == "divider_title" then
            if alive(row_item.status_gui) then
                row_item.status_gui:set_text(
                    managers.localization:to_upper_text("menu_specialization",
                                                        {}) .. ":")
            end
        end
    end
    ...
    function ChatManager:_receive_message(channel_id, name, message, ...)
        ...
        for _, pattern in ipairs(ChatManager._BLOCK_PATTERNS) do
            if message2:match("^" .. pattern .. "$") then
                return WolfHUD.DEBUG_MODE and
                           _receive_message_original(self, channel_id, name,
                                                     "Pattern found: " ..
                                                         pattern, ...)
            end
        end
...
elseif string.lower(RequiredScript) == "lib/managers/menumanagerdialogs" then
    ...
    function MenuManager:show_person_joining(id, nick, ...)
        ...
        if peer then
            ...
            nick = "(" ..
                       (peer:rank() > 0 and
                           managers.experience:rank_string(peer:rank()) .. "-" or
                           "") .. peer:level() .. ") " .. nick
        end
    ...
    function MenuManager:update_person_joining(id, progress_percentage, ...)
        if self.peer_join_start_t and self.peer_join_start_t[id] then
            ...
            local result = update_person_joining_original(self, id,
                                                          progress_percentage,
                                                          ...)
            local time_left = (t / progress_percentage) *
                                  (100 - progress_percentage)
            local dialog = managers.system_menu:get_dialog("user_dropin" .. id)
            if dialog and time_left then
                dialog:set_text(managers.localization:text("dialog_wait") ..
                                    string.format(" %d%% (%0.2fs)",
                                                  progress_percentage, time_left))
            end
        end
    end

    function MenuManager:close_person_joining(id, ...)
        if self.peer_join_start_t then self.peer_join_start_t[id] = nil end
        --[[
