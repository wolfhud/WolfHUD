...
if string.lower(RequiredScript) == "lib/managers/hudmanagerpd2" then
    local format_time_string = function(value)
        ...
        if math.floor(value) > 60 and
            not (HUDListManager and HUDListManager.ListOptions.timers_in_seconds) then
            time_str = string.format("%d:%02d", math.floor(value / 60),
                                     math.floor(value % 60))
    ...
    local function get_distance_to_player(unit)
        ...
        if alive(cam) and alive(unit) then
            ...
            rotation = math.floor(vector:to_polar_with_reference(forward,
                                                                 math.UP).spin)
        end
    ...
    local function get_icon_data(icon)
        ...
        elseif icon.perks then
            texture = string.format(
                          "guis/%stextures/pd2/specialization/icons_atlas",
                          icon.texture_bundle_folder and
                              string.format("dlcs/%s/", tostring(
                                                icon.texture_bundle_folder)) or
                              "")
            local x, y = unpack(icon.perks)
            texture_rect = {x * 64, y * 64, 64, 64}
        elseif icon.preplanning then
            texture =
                "guis/dlcs/big_bank/textures/pd2/pre_planning/preplan_icon_types"
            local x, y = unpack(icon.preplanning)
            texture_rect = {x * 48, y * 48, 48, 48}
        elseif icon.hud_tweak then
            texture, texture_rect = tweak_data.hud_icons:get_icon_data(
                                        icon.hud_tweak, texture_rect)
    ...
    local function get_color_from_table(value, max_value, color_table,
                                        default_color)
        ...
        local color = default_color or color_table[#color_table].color or
                          Color.white
        if value and max_value then
            ...
            for i, data in ipairs(color_table) do
                if ratio < data.ratio then
                    ...
                    local scale = (ratio - data.ratio) /
                                      (nxt.ratio - data.ratio)
                    color = Color((data.color.alpha or 1) * (1 - scale) +
                                      (nxt.color.alpha or 1) * scale,
                                  (data.color.red or 0) * (1 - scale) +
                                      (nxt.color.red or 0) * scale, (data.color
                                      .green or 0) * (1 - scale) +
                                      (nxt.color.green or 0) * scale,
                                  (data.color.blue or 0) * (1 - scale) +
                                      (nxt.color.blue or 0) * scale)
                    break
                end
            end
        end
    ...
    local _setup_player_info_hud_pd2_original =
        HUDManager._setup_player_info_hud_pd2
    ...
    function HUDManager:show_stats_screen(...)
        if managers.hudlist then managers.hudlist:fade_lists(0.4) end
        return show_stats_screen_original(self, ...)
    end

    function HUDManager:hide_stats_screen(...)
        if managers.hudlist then managers.hudlist:fade_lists(1) end
    ...
    HUDListManager.ListOptions = {
        ...
        right_list_progress_alpha = WolfHUD:getSetting(
            {"HUDList", "right_list_progress_alpha"}, 0.4),
        left_list_progress_alpha = WolfHUD:getSetting(
            {"HUDList", "left_list_progress_alpha"}, 0.4),
        buff_list_progress_alpha = WolfHUD:getSetting(
            {"HUDList", "buff_list_progress_alpha"}, 1.0),

        timers_in_seconds = WolfHUD:getSetting({"HUDList", "timers_in_seconds"},
                                               false),

        -- Left side list
        show_timers = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_timers"}, true), -- Drills, time locks, hacking etc.
        show_ammo_bags = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_ammo_bags"}, true),
        show_doc_bags = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_doc_bags"}, true),
        show_first_aid_kits = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_first_aid_kits"}, false),
        show_body_bags = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_body_bags"}, true),
        show_grenade_crates = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_grenade_crates"}, true),
        show_sentries = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_sentries"}, true), -- Deployable sentries
        show_ecms = WolfHUD:getSetting({"HUDList", "LEFT_LIST", "show_ecms"},
                                       true), -- Active ECMs
        show_ecm_retrigger = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_ecm_retrigger"}, true), -- Countdown for player owned ECM feedback retrigger delay
        show_minions = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_minions"}, true), -- Converted enemies, type and health
        show_own_minions_only = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_own_minions_only"}, true), -- Only show player-owned minions
        show_pagers = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_pagers"}, true), -- Show currently active pagers
        show_tape_loop = WolfHUD:getSetting(
            {"HUDList", "LEFT_LIST", "show_tape_loop"}, true), -- Show active tape loop duration

        -- Right side list
        show_enemies = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_enemies"}, true), -- Currently spawned enemies
        aggregate_enemies = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "aggregate_enemies"}, false), -- Aggregate all enemies into a single item
        show_turrets = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_turrets"}, true), -- Show active SWAT turrets
        show_civilians = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_civilians"}, true), -- Currently spawned, untied civs
        show_hostages = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_hostages"}, true), -- Currently tied civilian and dominated cops
        aggregate_hostages = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "aggregate_hostages"}, false), -- Aggregate all hostages into a single item
        show_minion_count = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_minion_count"}, true), -- Current number of jokered enemies
        show_pager_count = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_pager_count"}, true), -- Show number of triggered pagers (only counts pagers triggered while you were present)
        show_cam_count = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_cam_count"}, true),
        show_bodybags_count = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_bodybags_count"}, true),
        show_corpse_count = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_corpse_count"}, true),
        show_loot = WolfHUD:getSetting({"HUDList", "RIGHT_LIST", "show_loot"},
                                       true), -- Show spawned and active loot bags/piles (may not be shown if certain mission parameters has not been met)
        aggregate_loot = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "aggregate_loot"}, false), -- Aggregate all loot into a single item
        separate_bagged_loot = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "separate_bagged_loot"}, true), -- Show bagged/unbagged loot as separate values
        show_potential_loot = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_potential_loot"}, false),
        show_special_pickups = WolfHUD:getSetting(
            {"HUDList", "RIGHT_LIST", "show_special_pickups"}, true), -- Show number of special equipment/items

        -- Buff list
        show_buffs = WolfHUD:getSetting({"HUDList", "BUFF_LIST", "show_buffs"},
                                        true), -- Active effects (buffs/debuffs). Also see HUDList.BuffItemBase.IGNORED_BUFFS table to ignore specific buffs that you don't want listed, or enable some of those not shown by default

        list_color = WolfHUD:getColorSetting({"HUDList", "list_color"}, "white"),
        list_color_bg = WolfHUD:getColorSetting({"HUDList", "list_color_bg"},
                                                "black"),
        civilian_color = WolfHUD:getColorSetting({"HUDList", "civilian_color"},
                                                 "white"),
        hostage_color = WolfHUD:getColorSetting({"HUDList", "civilian_color"},
                                                "white"),
        thug_color = WolfHUD:getColorSetting({"HUDList", "thug_color"}, "white"),
        enemy_color = WolfHUD:getColorSetting({"HUDList", "enemy_color"},
                                              "white"),
        guard_color = WolfHUD:getColorSetting({"HUDList", "enemy_color"},
                                              "white"),
        special_color = WolfHUD:getColorSetting({"HUDList", "special_color"},
                                                "white"),
        turret_color = WolfHUD:getColorSetting({"HUDList", "special_color"},
                                               "white")
    }
    ...
    HUDListManager.UNIT_TYPES = {
        ...
        fbi = {
            type_id = "cop",
            category = "enemies",
            long_name = "wolfhud_enemy_fbi"
        },
        swat = {
            type_id = "cop",
            category = "enemies",
            long_name = "wolfhud_enemy_swat"
        },
        heavy_swat = {
            type_id = "cop",
            category = "enemies",
            long_name = "wolfhud_enemy_heavy_swat"
        },
        heavy_swat_sniper = {
            type_id = "cop",
            category = "enemies",
            long_name = "wolfhud_enemy_heavy_swat_sniper"
        },
        fbi_swat = {
            type_id = "cop",
            category = "enemies",
        ...
        city_swat = {
            type_id = "cop",
            category = "enemies",
            long_name = "wolfhud_enemy_city_swat"
        },
        security = {
            type_id = "security",
            category = "enemies",
            long_name = "wolfhud_enemy_security"
        },
        ...
        gensec = {
            type_id = "security",
            category = "enemies",
            long_name = "wolfhud_enemy_gensec"
        },
        bolivian_indoors = {
            type_id = "security",
            category = "enemies",
            long_name = "wolfhud_enemy_bolivian_security"
        },
        bolivian_indoors_mex = {
            type_id = "security",
            category = "enemies",
            long_name = "wolfhud_enemy_bolivian_security_mex"
        },
        bolivian = {
            type_id = "thug",
            category = "enemies",
            long_name = "wolfhud_enemy_bolivian_thug"
        },
        gangster = {
            type_id = "thug",
            category = "enemies",
            long_name = "wolfhud_enemy_gangster"
        },
        mobster = {
            type_id = "thug",
            category = "enemies",
            long_name = "wolfhud_enemy_mobster"
        },
        biker = {
            type_id = "thug",
            category = "enemies",
            long_name = "wolfhud_enemy_biker"
        },
        biker_escape = {
            type_id = "thug",
            category = "enemies",
        ...
        tank = {
            type_id = "tank",
            category = "enemies",
            long_name = "wolfhud_enemy_tank"
        },
        tank_hw = {
            type_id = "tank",
            category = "enemies",
            long_name = "wolfhud_enemy_tank_hw"
        },
        tank_medic = {
            type_id = "tank",
            category = "enemies",
            long_name = "wolfhud_enemy_tank_medic"
        },
        tank_mini = {
            type_id = "tank",
            category = "enemies",
            long_name = "wolfhud_enemy_tank_mini"
        },
        spooc = {
            type_id = "spooc",
            category = "enemies",
            long_name = "wolfhud_enemy_spook"
        },
        shadow_spooc = {
            type_id = "spooc",
            category = "enemies",
            long_name = "wolfhud_enemy_shadow_spook"
        },
        taser = {
            type_id = "taser",
            category = "enemies",
            long_name = "wolfhud_enemy_taser"
        },
        shield = {
            type_id = "shield",
            category = "enemies",
            long_name = "wolfhud_enemy_shield"
        },
        sniper = {
            type_id = "sniper",
            category = "enemies",
            long_name = "wolfhud_enemy_sniper"
        },
        medic = {
            type_id = "medic",
            category = "enemies",
            long_name = "wolfhud_enemy_medic"
        },
        biker_boss = {
            type_id = "thug_boss",
            category = "enemies",
            long_name = "wolfhud_enemy_biker_boss"
        },
        chavez_boss = {
            type_id = "thug_boss",
            category = "enemies",
            long_name = "wolfhud_enemy_chavez_boss"
        },
        drug_lord_boss = {
            type_id = "thug_boss",
            category = "enemies",
            long_name = "wolfhud_enemy_druglord_boss"
        },
        drug_lord_boss_stealth = {
            type_id = "thug_boss",
            category = "enemies",
            long_name = "wolfhud_enemy_druglord_boss_stealth"
        },
        hector_boss = {
            type_id = "thug_boss",
            category = "enemies",
            long_name = "wolfhud_enemy_hector_boss"
        },
        hector_boss_no_armor = {
            type_id = "thug_boss",
            category = "enemies",
            long_name = "wolfhud_enemy_hector_boss_no_armor"
        },
        mobster_boss = {
            type_id = "thug_boss",
            category = "enemies",
            long_name = "wolfhud_enemy_mobster_boss"
        },
        phalanx_vip = {
            type_id = "phalanx",
            category = "enemies",
            long_name = "wolfhud_enemy_phalanx_vip"
        },
        phalanx_minion = {
            type_id = "phalanx",
            category = "enemies",
            long_name = "wolfhud_enemy_phalanx_minion"
        },
        security_mex_no_pager = {
            type_id = "security",
            category = "enemies",
            long_name = "wolfhud_enemy_mex_no_pager"
        },
        civilian = {
            type_id = "civ",
            category = "civilians",
            long_name = "wolfhud_enemy_civilian"
        },
        civilian_female = {
            type_id = "civ",
            category = "civilians",
        ...
        bank_manager = {
            type_id = "civ",
            category = "civilians",
            long_name = "wolfhud_enemy_bank_manager"
        },
        ...
        turret = {
            type_id = "turret",
            category = "turrets",
            long_name = "wolfhud_enemy_swat_van"
        },
        cop_hostage = {
            type_id = "cop_hostage",
            category = "hostages",
            force_update = {"cop", "enemies"}
        },
        sec_hostage = {
            type_id = "cop_hostage",
            category = "hostages",
            force_update = {"security", "enemies"}
        },
        civ_hostage = {
            type_id = "civ_hostage",
            category = "hostages",
            force_update = {"civ", "civilians"}
        },
        cop_minion = {
            type_id = "minion",
            category = "minions",
        ...
        boom = {
            type_id = "grenadier",
            category = "enemies",
            long_name = "wolfhud_enemy_boom"
        },
        omnia_lpf = {
            type_id = "cop",
            category = "enemies",
            long_name = "wolfhud_enemy_omnia_lpf"
        },
        summers = {
            type_id = "phalanx",
            category = "enemies",
            long_name = "wolfhud_enemy_summers"
        },
        boom_summers = {
            type_id = "phalanx",
            category = "enemies",
            long_name = "wolfhud_enemy_boom_summers"
        },
        taser_summers = {
            type_id = "phalanx",
            category = "enemies",
            long_name = "wolfhud_enemy_taser_summers"
        },
        medic_summers = {
            type_id = "phalanx",
            category = "enemies",
            long_name = "wolfhud_enemy_medic_summers"
        },
        spring = {
            type_id = "phalanx",
            category = "enemies",
            long_name = "wolfhud_enemy_spring"
        },
        fbi_vet = {
            type_id = "cop",
            category = "enemies",
            long_name = "wolfhud_enemy_fbi_vet"
        },
        ...
        deathvox_lightar = {
            type_id = "cop",
            category = "enemies",
            long_name = "wolfhud_enemy_deathvox_light"
        },
        deathvox_heavyar = {
            type_id = "cop",
            category = "enemies",
            long_name = "wolfhud_enemy_deathvox_heavy"
        },
        deathvox_lightshot = {
            type_id = "cop",
            category = "enemies",
    ...
    HUDListManager.LOOT_TYPES_CONDITIONS =
        {
            ...
            crate = function(id, data)
                ...
                local disabled_lvls = {
                    ...
                    "election_day_3_skip1", "election_day_3_skip2", "mia_1", -- Hotline Miami Day 1
                    "pal" -- Counterfeit
                }
                return
                    not (level_id and table.contains(disabled_lvls, level_id))
            end,
    ...
    function HUDListManager:init()
        ...
        managers.gameinfo:register_listener("HUDList_whisper_mode_listener",
                                            "whisper_mode", "change", callback(
                                                self, self,
                                                "_whisper_mode_change"))
    end

    function HUDListManager:update(t, dt)
        for _, list in pairs(self._lists) do
            if list:is_active() then list:update(t, dt) end
        end
    end

    function HUDListManager:list(name) return self._lists[name] end
    ...
    function HUDListManager:change_pickup_ignore(category_id, ignore)
        local pickup_list = self:list("right_side_list"):item(
                                "special_pickup_list")
        for _, item in pairs(pickup_list:items()) do
            local pickup_type = item:name()
            local pickup_data = pickup_type and
                                    HUDList.SpecialPickupItem.MAP[pickup_type]
    ...
    function HUDListManager:fade_lists(alpha)
        for _, list in pairs(self._lists) do
            if list:is_active() then list:_fade(alpha) end
        end
    end
    ...
    function HUDListManager:unregister_list(name, instant)
        if self._lists[name] then self._lists[name]:delete(instant) end
    ...
    function HUDListManager:_setup_left_list()
        ...
        local list = self:register_list("left_side_list", HUDList.VerticalList,
                                        {
            align = "left",
            x = x,
            y = y,
        ...
        -- Timers
        local timer_list = list:register_item("timers", HUDList.HorizontalList,
                                              {
            align = "top",
            w = list_width,
            h = 40 * scale,
        ...
        -- Deployables
        local equipment_list = list:register_item("equipment",
                                                  HUDList.HorizontalList, {
            align = "top",
            w = list_width,
            h = 40 * scale,
            left_to_right = true,
        ...
        equipment_list:set_static_item(HUDList.LeftListIcon, 1, 1, {
            {
                skills = HUDList.EquipmentItem.EQUIPMENT_TABLE.ammo_bag.skills,
                h = 0.55,
                w = 0.55,
                valign = "top",
                halign = "right",
                color = HUDListManager.ListOptions.list_color
            }, {
                skills = HUDList.EquipmentItem.EQUIPMENT_TABLE.doc_bag.skills,
                h = 0.55,
                w = 0.55,
                valign = "top",
                halign = "left",
                ...
                skills = HUDList.EquipmentItem.EQUIPMENT_TABLE.sentry.skills,
                h = 0.55,
                w = 0.55,
                valign = "bottom",
                halign = "right",
                ...
                skills = HUDList.EquipmentItem.EQUIPMENT_TABLE.body_bag.skills,
                h = 0.55,
                w = 0.55,
                valign = "bottom",
                halign = "left",
        ...
        -- Minions
        local minion_list = list:register_item("minions",
                                               HUDList.HorizontalList, {
            align = "top",
            w = list_width,
            h = 50 * scale,
        ...
        -- Pagers
        local pager_list = list:register_item("pagers", HUDList.HorizontalList,
                                              {
            align = "top",
            w = list_width,
            h = 40 * scale,
        ...
        -- ECMs
        local ecm_list = list:register_item("ecms", HUDList.HorizontalList, {
            align = "top",
            w = list_width,
            h = 30 * scale,
            left_to_right = true,
        ...
        -- ECM trigger
        local retrigger_list = list:register_item("ecm_retrigger",
                                                  HUDList.HorizontalList, {
            align = "top",
            w = list_width,
            h = 30 * scale,
        ...
        -- Tape loop
        local tape_loop_list = list:register_item("tape_loop",
                                                  HUDList.HorizontalList, {
            align = "top",
            w = list_width,
            h = 30 * scale,
    ...
    function HUDListManager:_setup_right_list()
        ...
        local x =
            managers.hud:script(PlayerBase.PLAYER_INFO_HUD_PD2).panel:right() -
                list_width
        ...
        local unit_count_list = list:register_item("unit_count_list",
                                                   HUDList.HorizontalList, {
            align = "top",
            w = list_width,
            h = 50 * scale,
        ...
        local stealth_list = list:register_item("stealth_list",
                                                HUDList.HorizontalList, {
            align = "top",
            w = list_width,
            h = 50 * scale,
        ...
        local loot_list = list:register_item("loot_list",
                                             HUDList.HorizontalList, {
            align = "top",
            w = list_width,
            h = 50 * scale,
        ...
        local special_equipment_list = list:register_item("special_pickup_list",
                                                          HUDList.HorizontalList,
                                                          {
            align = "top",
            w = list_width,
    ...
    function HUDListManager:_setup_buff_list()
        local hud_panel = managers.hud:script(PlayerBase.PLAYER_INFO_HUD_PD2)
                              .panel
        ...
        if HUDManager.CUSTOM_TEAMMATE_PANELS then
            y = managers.hud._teammate_panels[HUDManager.PLAYER_PANEL]:panel()
                    :top() - (list_height + 10)
        else
            y = hud_panel:bottom() -
                    ((HUDListManager.ListOptions.buff_list_height_offset or 90) +
                        list_height)
        end
        ...
        local buff_list = self:register_list("buff_list",
                                             HUDList.HorizontalList, {
            align = "center",
            x = x,
            y = y,
    ...
    function HUDListManager:_get_buff_items(id)
        ...
        local function register_item(item_id)
            ...
            if item_data then
                ...
                if not item then
                    item = buff_list:register_item(item_id, item_data.class or
                                                       "BuffItemBase", item_data)
                    buff_list:set_item_disabled(item, "setting",
                                                item_data.ignore)
                end
    ...
    function HUDListManager:_update_unit_count_list_items(list, id, members,
                                                          show)
        if show then
            local data = HUDList.UnitCountItem.MAP[id] or {}
            local item = list:register_item(id, data.class or
                                                HUDList.UnitCountItem, id,
                                            members)
        else
    ...
    function HUDListManager:_update_deployable_list_items(type, enabled)
        ...
        for _, event in pairs(events) do
            if enabled then
                managers.gameinfo:register_listener(listener_id, type, event,
                                                    clbk)
            else
    ...
    function HUDListManager:_bag_deployable_event(event, key, data, class,
                                                  bag_type)
    ...
    function HUDListManager:_timer_event(event, key, data)
        ...
        local settings = HUDListManager.TIMER_SETTINGS[level_id] and
                             HUDListManager.TIMER_SETTINGS[level_id][data.id] or
                             HUDListManager.TIMER_SETTINGS[data.device_type] or
                             HUDList.TimerItem.DEVICE_TYPES[data.device_type] or
                             {}
        ...
        if not settings.ignore then
            ...
            if event == "set_active" then
                if data.active then
                    timer_list:register_item(key, settings.class or
                                                 HUDList.TimerItem, data,
                                             settings.params):activate()
                else
    ...
    function HUDListManager:_unit_count_event(event, unit_type, value)
        if HUDListManager.UNIT_TYPES[unit_type] then
            ...
            if item then
                ...
                for _, id in pairs(HUDListManager.UNIT_TYPES[unit_type]
                                       .force_update or {}) do
                    local item = list:item(id)
                    if item then item:change_count(0) end
                end
            end
        end
    end
    ...
    function HUDListManager:_minion_event(event, key, data)
        ...
        if event == "add" then
            local item =
                minion_list:register_item(key, HUDList.MinionItem, data)
    ...
    function HUDListManager:_bodybag_count_event(event, key, data)
        local item = self:list("right_side_list"):item("stealth_list"):item(
                         "BodyBagInv")
        local whisper_mode = managers.groupai:state():whisper_mode()
        if event == "set" and whisper_mode then item:set_count(key) end
    end
    ...
    function HUDListManager:_ecm_retrigger_event(event, key, data)
        ...
        if event == "set_retrigger_active" then
            if data.retrigger_active then
                list:register_item(string.format("%s_retrigger", key),
                                   HUDList.ECMRetriggerItem, data):activate()
            else
                list:unregister_item(string.format("%s_retrigger", key))
            end
        elseif event == "set_feedback_active" then
            if data.feedback_active then
                list:register_item(string.format("%s_feedback", key),
                                   HUDList.ECMFeedbackItem, data):activate()
            else
    ...
    function HUDListManager:_sentry_equipment_event(event, key, data)
        ...
        if event == "set_active" then
            if data.active then
                equipment_list:register_item(key, HUDList.SentryEquipmentItem,
                                             data):activate()
            end
    ...
    function HUDListManager:_buff_event(event, id, data)
        WolfHUD:print_log("(HUDList) _buff_event(%s, %s)", tostring(event),
                          tostring(id), "info")
        ...
        for _, item in ipairs(items) do
            if item[event] then
                ...
                WolfHUD:print_log(
                    "HUDList) _buff_event: No matching function for event %s for buff %s",
                    tostring(event), tostring(id), "warning")
            end
        end

        if HUDListManager.BUFFS.composite_debuffs[id] then
            if event == "activate" or event == "deactivate" or event ==
                "set_duration" then
                local debuff_parent_id =
                    HUDListManager.BUFFS.composite_debuffs[id]
    ...
    function HUDListManager:_ammo_bag_event(event, key, data)
        self:_bag_deployable_event(event, key, data, HUDList.AmmoBagItem,
                                   "ammo_bag")
    end

    function HUDListManager:_doc_bag_event(event, key, data)
        self:_bag_deployable_event(event, key, data, HUDList.BagEquipmentItem,
                                   "doc_bag")
    end
    ...
    function HUDListManager:_body_bag_event(event, key, data)
        self:_bag_deployable_event(event, key, data, HUDList.BodyBagItem,
                                   "body_bag")
    end
    ...
    function HUDListManager:_set_right_list_height_offset()
        local list = self:list("right_side_list")
        if list then
            list:move(list:panel():x(), HUDListManager.ListOptions
                          .right_list_height_offset or 40, false)
        end
    end
    ...
    function HUDListManager:_set_left_list_height_offset()
        local list = self:list("left_side_list")
        if list then
            list:move(list:panel():x(),
                      HUDListManager.ListOptions.left_list_height_offset or 40,
                      false)
        end
    end
        ...
        if list then
            for _, sub_list in pairs(list:items()) do
                for _, item in pairs(sub_list:items()) do
                    item:set_progress_alpha(
                        alpha or
                            HUDListManager.ListOptions.right_list_progress_alpha)
                end
            end
        end
    end
        ...
        if list then
            for _, sub_list in pairs(list:items()) do
                for _, item in pairs(sub_list:items()) do
                    item:set_progress_alpha(
                        alpha or
                            HUDListManager.ListOptions.left_list_progress_alpha)
                end
            end
        end
    end
    ...
    function HUDListManager:_set_buff_list_progress_alpha(alpha)
        ...
        if list then
            for _, item in pairs(list:items()) do
                item:set_progress_alpha(alpha or
                                            HUDListManager.ListOptions
                                                .buff_list_progress_alpha)
            end
        end
    end
        ...
        if list then
            ...
            for _, item in pairs(list:items()) do
                ...
                if map[u_id] and map[u_id].color_id == "civilian_color" then
                    item:set_icon_color(color or
                                            HUDListManager.ListOptions
                                                .civilian_color)
                end
            end
        end
        self:_set_hostage_color(color or
                                    HUDListManager.ListOptions.civilian_color)
    end
    ...
    function HUDListManager:_set_show_timers()
        ...
        for _, event in pairs(events) do
            if HUDListManager.ListOptions.show_timers then
                managers.gameinfo:register_listener(listener_id, "timer", event,
                                                    clbk)
            else
                managers.gameinfo:unregister_listener(listener_id, "timer",
                                                      event)
            end
        end
    ...
    function HUDListManager:_set_show_minions()
        ...
        for _, event in pairs(events) do
            if HUDListManager.ListOptions.show_minions then
                managers.gameinfo:register_listener(listener_id, "minion",
                                                    event, clbk)
            else
                managers.gameinfo:unregister_listener(listener_id, "minion",
                                                      event)
            end
        end

        for key, data in pairs(managers.gameinfo:get_minions()) do
            clbk(HUDListManager.ListOptions.show_minions and "add" or "remove",
                 key, data)
        end
    end
    ...
    function HUDListManager:_set_show_own_minions_only()
        ...
        for name, item in pairs(minion_list:items()) do
            item:set_active(
                not HUDListManager.ListOptions.show_own_minions_only or
                    data.owner == managers.network:session():local_peer():id())
        end
    end
    ...
    function HUDListManager:_set_show_pagers()
        ...
        for _, event in pairs(events) do
            if HUDListManager.ListOptions.show_pagers then
                managers.gameinfo:register_listener(listener_id, "pager", event,
                                                    clbk)
            else
                managers.gameinfo:unregister_listener(listener_id, "pager",
                                                      event)
            end
        end
        ...
        for key, data in pairs(managers.gameinfo:get_pagers()) do
            if HUDListManager.ListOptions.show_pagers then
                if data.active then clbk("add", key, data) end
            else
                list:unregister_item(key)
            end
        end
    ...
    function HUDListManager:_set_show_ecms()
        ...
        for _, event in pairs(events) do
            if HUDListManager.ListOptions.show_ecms then
                managers.gameinfo:register_listener(listener_id, "ecm", event,
                                                    clbk)
            else
    ...
    function HUDListManager:_set_show_ammo_bags()
        self:_update_deployable_list_items("ammo_bag",
                                           HUDListManager.ListOptions
                                               .show_ammo_bags)
    end

    function HUDListManager:_set_show_doc_bags()
        self:_update_deployable_list_items("doc_bag", HUDListManager.ListOptions
                                               .show_doc_bags)
    end

    function HUDListManager:_set_show_first_aid_kits()
        self:_update_deployable_list_items("first_aid_kit",
                                           HUDListManager.ListOptions
                                               .show_first_aid_kits)
    end

    function HUDListManager:_set_show_body_bags()
        self:_update_deployable_list_items("body_bag",
                                           HUDListManager.ListOptions
                                               .show_body_bags)
    end

    function HUDListManager:_set_show_grenade_crates()
        self:_update_deployable_list_items("grenade_crate",
                                           HUDListManager.ListOptions
                                               .show_grenade_crates)
    end
    ...
    function HUDListManager:_set_show_tape_loop()
        ...
        for _, event in pairs(events) do
            if HUDListManager.ListOptions.show_tape_loop then
                managers.gameinfo:register_listener(listener_id, "camera",
                                                    event, clbk)
            else
                managers.gameinfo:unregister_listener(listener_id, "camera",
                                                      event)
            end
        end

        for key, data in pairs(managers.gameinfo:get_cameras()) do
            if data.tape_loop_expire_t and
                HUDListManager.ListOptions.show_tape_loop then
    ...
    function HUDListManager:_set_show_sentries()
        ...
        if HUDListManager.ListOptions.show_sentries then
            ...
            for _, event in pairs(events) do
                managers.gameinfo:register_listener(listener_id, "sentry",
                                                    event, clbk)
            end
        else
            local list = self:list("left_side_list"):item("equipment")
            ...
            for _, event in pairs(events) do
                managers.gameinfo:unregister_listener(listener_id, "sentry",
                                                      event)
            end
    ...
    function HUDListManager:_set_show_enemies()
        ...
        if HUDListManager.ListOptions.aggregate_enemies then
            self:_update_unit_count_list_items(list, "enemies", all_ids,
                                               HUDListManager.ListOptions
                                                   .show_enemies)
        else
            for unit_type, unit_ids in pairs(all_types) do
                self:_update_unit_count_list_items(list, unit_type, unit_ids,
                                                   HUDListManager.ListOptions
                                                       .show_enemies)
            end
        end
    ...
    function HUDListManager:_set_show_hostages()
        ...
        if HUDListManager.ListOptions.aggregate_hostages then
            self:_update_unit_count_list_items(list, "hostages", all_ids,
                                               HUDListManager.ListOptions
                                                   .show_hostages)
        else
            for unit_type, unit_ids in pairs(all_types) do
    ...
    function HUDListManager:_set_aggregate_hostages()
        ...
        for unit_type, unit_ids in pairs(all_types) do
            ...
            if item then
                ...
                for unit_type, unit_ids in pairs(all_types) do
                    self:_update_unit_count_list_items(list, unit_type,
                                                       unit_ids,
                                                       HUDListManager.ListOptions
                                                           .show_hostages)
                end
    ...
    function HUDListManager:_set_show_bodybags_count()
        ...
        if HUDListManager.ListOptions.show_bodybags_count then
            list:register_item("BodyBagInv", HUDList.BodyBagsInvItem,
                               {skills = {5, 11}}, {priority = 3})
        else
    ...
    function HUDListManager:_set_show_corpse_count()
        ...
        if HUDListManager.ListOptions.show_corpse_count then
            list:register_item("CorpseCount", HUDList.CorpseCountItem, {
    ...
    function HUDListManager:_set_show_special_pickups()
        ...
        for pickup_type, members in pairs(all_types) do
            if HUDListManager.ListOptions.show_special_pickups then
                ...
                if pickup_map then
                    local item = list:item(pickup_type) or
                                     list:register_item(pickup_type,
                                                        HUDList.SpecialPickupItem,
                                                        pickup_type, members)
    ...
    function HUDListManager:_set_show_loot()
        ...
        if HUDListManager.ListOptions.aggregate_loot then
            if HUDListManager.ListOptions.show_loot then
                list:register_item("aggregate", HUDList.LootItem, "aggregate",
                                   all_ids)
            else
            ...
            for loot_type, members in pairs(all_types) do
                if HUDListManager.ListOptions.show_loot then
                    list:register_item(loot_type, HUDList.LootItem, loot_type,
                                       members)
                else
    ...
    function HUDListManager:_set_separate_bagged_loot()
        for _, item in pairs(self:list("right_side_list"):item("loot_list")
                                 :items()) do item:update_value() end
    end
        ...
        for src, data in pairs(sources) do
            for _, event in ipairs(data) do
                if HUDListManager.ListOptions.show_buffs then
                    managers.gameinfo:register_listener(listener_id, src, event,
                                                        data.clbk)
                else
                    managers.gameinfo:unregister_listener(listener_id, src,
                                                          event)
                end
            end
        end
    ...
    function HUDList.ItemBase:init(parent_list, name, params)
        ...
        self._scale = params.scale or self._parent_list and
                          self._parent_list:scale() or 1
        self._listener_clbks = {}
        self._disable_reason = {}

        self._panel = (self._parent_list and self._parent_list:panel() or
                          params.native_panel or
                          managers.hud:script(PlayerBase.PLAYER_INFO_HUD_PD2)
                              .panel):panel(
                          {
                name = name,
                visible = true,
    ...
    function HUDList.ItemBase:post_init()
        for i, data in ipairs(self._listener_clbks) do
            for _, event in pairs(data.event) do
                managers.gameinfo:register_listener(data.name, data.source,
                                                    event, data.clbk, data.keys,
                                                    data.data_only)
            end
        end
    end
    ...
    function HUDList.ItemBase:destroy()
        for i, data in ipairs(self._listener_clbks) do
            for _, event in pairs(data.event) do
                managers.gameinfo:unregister_listener(data.name, data.source,
                                                      event)
            end
        end
    end
    ...
    function HUDList.ItemBase:enabled()
    ...
    function HUDList.ItemBase:set_fade_time(time) self._fade_time = time end

    function HUDList.ItemBase:set_move_speed(speed) self._move_speed = speed end
    ...
    function HUDList.ItemBase:_fade(target_alpha, instant, time_override)
        ...
        self._active_fade = {
            instant = instant or self._panel:alpha() == target_alpha,
            alpha = target_alpha,
    ...
    function HUDList.ItemBase:move(x, y, instant, time_override)
        if alive(self._panel) then
            ...
            self._active_move = {
                instant = instant or
                    (self._panel:x() == x and self._panel:y() == y),
                x = x,
                y = y,
                time_override = time_override
            }
    ...
    function HUDList.ItemBase:_animate_item()
        if alive(self._panel) and self._active_fade then
            self._panel:animate(callback(self, self, "_animate_fade"),
                                self._active_fade.alpha,
                                self._active_fade.instant,
                                self._active_fade.time_override)
        end

        if alive(self._panel) and self._active_move then
            self._panel:animate(callback(self, self, "_animate_move"),
                                self._active_move.x, self._active_move.y,
                                self._active_move.instant,
                                self._active_move.time_override)
        end
    end
    ...
    function HUDList.ItemBase:_animate_fade(panel, alpha, instant, time_override)
        if not instant and self._fade_time > 0 then
            ...
            local fade_time = time_override and math.abs(alpha - init_alpha) /
                                  time_override or self._fade_time
            ...
            while alive(panel) and t < T do
                panel:set_alpha(math.clamp(
                                    init_alpha + t * change * 1 / fade_time, 0,
                                    1))
                t = t + coroutine.yield()
            end
        end
        ...
        --	self._parent_list:set_item_visible(self, false)
        -- end
        if self._scheduled_for_deletion then self:_delete() end
    end
    ...
    function HUDList.ItemBase:_animate_move(panel, x, y, instant, time_override)
        if not instant and self._move_speed > 0 then
            ...
            local move_speed = time_override and math.abs(x - init_x) /
                                   time_override or self._move_speed
            local x_change = x > init_x and 1 or x < init_x and -1
            local y_change = y > init_y and 1 or y < init_y and -1
            local T = time_override or
                          math.max(math.abs(x - init_x) / move_speed,
                                   math.abs(y - init_y) / move_speed)
            local t = 0
    ...
    function HUDList.ItemBase:text_rect()
    ...
    function HUDList.ListBase:item(name) return self._items[name] end

    function HUDList.ListBase:items() return self._items end

    function HUDList.ListBase:num_items() return table.size(self._items) end

    function HUDList.ListBase:active_items()
        local count = 0
        for name, item in pairs(self._items) do
            if item:is_active() then count = count + 1 end
        end
        return count
    end

    function HUDList.ListBase:shown_items() return #self._shown_items end
    ...
    function HUDList.ListBase:register_item(name, class, ...)
        if not self._items[name] then
            ...
            if new_item then
                ...
                if self._scale then new_item:rescale(self._scale) end
                new_item:post_init(...)
    ...
    function HUDList.ListBase:unregister_item(name, instant)
        if self._items[name] then self._items[name]:delete(instant) end
    end

    function HUDList.ListBase:set_static_item(class, ...)
        self:delete_static_item()

        if type(class) == "string" then class = _G.HUDList[class] end
    ...
    function HUDList.ListBase:setup_static_item() end
    ...
    function HUDList.ListBase:set_item_visible(item, visible)
        ...
        if visible and not index then
            if #self._shown_items <= 0 then self:activate() end

            local insert_index = #self._shown_items + 1
            if item:priority() then
                for i, list_item in ipairs(self._shown_items) do
                    if not list_item:priority() or
                        (list_item:priority() > item:priority()) then
                        insert_index = i
                        break
                    end
                end
        ...
        elseif not visible and index then
            ...
            if #self._shown_items <= 0 then
                managers.enemy:add_delayed_clbk("visibility_cbk_" .. self._name,
                                                callback(self, self,
                                                         "_cbk_update_visibility"),
                                                Application:time() +
                                                    item:fade_time())
                -- self:deactivate()
            end
    ...
    function HUDList.ListBase:_update_item_positions(insert_item, instant_move,
                                                     move_timer) end

    function HUDList.ListBase:_cbk_update_visibility()
        if #self._shown_items <= 0 then self:deactivate() end
    end
    ...
    function HUDList.ListBase:set_color(color)
        for _, item in pairs(self:items()) do item:set_color(color) end
    end
    function HUDList.ListBase:set_bg_color(color)
        for _, item in pairs(self:items()) do item:set_bg_color(color) end
    end
    ...
    function HUDList.HorizontalList:init(parent, name, params)
        params.align = params.align == "top" and "top" or params.align ==
                           "bottom" and "bottom" or "center"
        ...
        self._centered = params.centered and
                             not (self._right_to_left or self._left_to_right)
    ...
    function HUDList.HorizontalList:set_color(color)
        if self._static_item then self._static_item:set_color(color) end
        for _, item in pairs(self:items()) do item:set_color(color) end
        if self._expansion_indicator then
    ...
    function HUDList.HorizontalList:set_bg_color(color)
        if self._static_item then self._static_item:set_bg_color(color) end
        for _, item in pairs(self:items()) do item:set_bg_color(color) end
        if self._expansion_indicator then
    ...
    function HUDList.HorizontalList:_set_default_item_position(item)
        ...
        local y = item:align() == "top" and 0 or item:align() == "bottom" and
                      offset or offset / 2
    ...
    function HUDList.HorizontalList:setup_static_item()
        ...
        local y = item:align() == "top" and 0 or item:align() == "bottom" and
                      offset or offset / 2
        local x = self._left_to_right and 0 or self._panel:w() -
                      item:panel():w()
    ...
    function HUDList.HorizontalList:setup_expansion_item()
        self._expansion_indicator = HUDList.ExpansionIndicator:new(self,
                                                                   "expansion_indicator",
                                                                   1 / 5, 1, {})
        self._expansion_indicator:set_mirrored(self._right_to_left)
        self._expansion_indicator:set_active(
            self._max_shown_items and self._max_shown_items >=
                self:shown_items())
    end
    ...
    function HUDList.HorizontalList:_update_item_positions(insert_item,
                                                           instant_move,
                                                           move_timer)
        local total_shown_items = 0
        local show_expansion = false
        if self._centered then
            local total_width = self._static_item and
                                    (self._static_item:panel():w() +
                                        self._item_margin) or 0
            local prev_disabled_i = {}
            for i, item in ipairs(self._shown_items) do
                local next_total_width =
                    total_width + item:panel():w() + self._item_margin
                show_expansion = show_expansion or (next_total_width > self:w())
                if self._max_shown_items then
                    show_expansion = show_expansion or
                                         (total_shown_items >=
                                             self._max_shown_items)
                end
            ...
            local left = (self._panel:w() -
                             math.min(total_width, self._panel:w())) / 2
            ...
            for i, item in ipairs(self._shown_items) do
                if item:enabled() then
                    if insert_item and item == insert_item or
                        table.contains(prev_disabled_i, i) then
            ...
            local prev_width = self._static_item and
                                   (self._static_item:panel():w() +
                                       self._item_margin) or 0
            for i, item in ipairs(self._shown_items) do
                local next_width = prev_width + item:panel():w() +
                                       self._item_margin
                ...
                if item:enabled() then
                    ...
                    local new_x = (self._left_to_right and prev_width) or
                                      (self._panel:w() - (width + prev_width))
            ...
            if self._expansion_indicator then
                ...
                local new_x = (self._left_to_right and
                                  math.min(prev_width, self._panel:w() - width)) or
                                  math.max(
                                      self._panel:w() - (width + prev_width), 0)
    ...
    function HUDList.HorizontalList:reapply_item_priorities(update_positions,
                                                            move_time_override)
        ...
        if not self._reorder_in_progress then
            ...
            repeat
                ...
                for i = 2, #self._shown_items, 1 do
                    ...
                    local prev_prio, cur_prio = prev and prev:priority(),
                                                cur and cur:priority()
                    if cur_prio then
                        if not prev_prio or prev_prio > cur_prio then
                            table.insert(self._shown_items, i,
                                         table.remove(self._shown_items, i - 1))
                            swapped = true
                        end
                    end
                end
    ...
    function HUDList.VerticalList:init(parent, name, params)
        params.align = params.align == "left" and "left" or params.align ==
                           "right" and "right" or "center"
    ...
    function HUDList.VerticalList:setup_static_item()
        ...
        local x = item:align() == "left" and 0 or item:align() == "right" and
                      offset or offset / 2
        local y = self._top_to_bottom and 0 or self._panel:h() -
                      item:panel():h()
        item:panel():set_left(x)
        item:panel():set_y(y)
        self:_update_item_positions()
    ...
    function HUDList.VerticalList:_update_item_positions(insert_item,
                                                         instant_move,
                                                         move_timer)
        if self._centered then
            local total_height = self._static_item and
                                     (self._static_item:panel():h() +
                                         self._item_margin) or 0
            for i, item in ipairs(self._shown_items) do
                if item:enabled() then
                    total_height = total_width + item:panel():h() +
                                       self._item_margin
                end
            end
            total_height = total_height - self._item_margin

            local top = (self._panel:h() -
                            math.min(total_height, self._panel:h())) / 2
            ...
            local prev_height = self._static_item and
                                    (self._static_item:panel():h() +
                                        self._item_margin) or 0
            for i, item in ipairs(self._shown_items) do
                if item:enabled() then
                    local height = item:panel():h()
                    local new_y = (self._top_to_bottom and prev_height) or
                                      (self._panel:h() - (height + prev_height))
                    if insert_item and item == insert_item then
    ...
    HUDList.ExpansionIndicator = HUDList.ExpansionIndicator or
                                     class(HUDList.ItemBase)

    function HUDList.ExpansionIndicator:init(parent, name, ratio_w, ratio_h,
                                             params)
        HUDList.ExpansionIndicator.super.init(self, parent, name, {
            align = "center",
            w = parent:panel():h() * (ratio_w or 1),
            h = parent:panel():h() * (ratio_h or 1)
        })
    ...
    function HUDList.ExpansionIndicator:_hide(instant)
        if alive(self._panel) then self:_fade(0, instant) end
    end
    ...
    function HUDList.RightListItem:init(parent, name, icon, params)
        ...
        self._default_text_color = HUDListManager.ListOptions.list_color or
                                       Color.white
        self._default_icon_color = icon.color or icon.color_id and
                                       HUDListManager.ListOptions[icon.color_id]
        ...
        self._progress_bar = PanelFrame:new(self._panel, {
            ...
            bar_alpha = HUDListManager.ListOptions.right_list_progress_alpha or
                0.4,
            add_bg = true
        })
    ...
    function HUDList.RightListItem:rescale(new_scale)
        local enabled, size_mult = HUDList.RightListItem.super.rescale(self,
                                                                       new_scale)

        if enabled then
            self._icon:set_size(self._icon:w() * size_mult,
                                self._icon:h() * size_mult)
            self._icon:set_center(self._panel:w() * 0.5, self._panel:w() * 0.5)
    ...
    function HUDList.RightListItem:set_color(color)
        self._default_text_color = HUDListManager.ListOptions.list_color or
                                       Color.white
        self._icon:set_color(self._default_icon_color or
    ...
    function HUDList.RightListItem:get_count() return self._count or 0 end

    function HUDList.RightListItem:_animate_change(item, diff, progress_bar)
        if self:is_active() and alive(item) and diff ~= 0 then
            local duration = 0.5
            local t = duration
            local color = diff > 0 and self._change_increase_color or
                              self._change_decrease_color

            item:set_color(color)
            while t > 0 do
                t = t - coroutine.yield()
                local ratio = math.clamp(t / duration, 0, 1)
                local new_color = math.lerp(self._default_text_color, color,
                                            ratio)
    ...
    HUDList.UnitCountItem = HUDList.UnitCountItem or
                                class(HUDList.RightListItem)
    do
        ...
        HUDList.UnitCountItem.MAP = {
            enemies = {
                class = "UnitCountItem",
                skills = {6, 1},
                color_id = "enemy_color",
                priority = 1,
                subtract = {"cop_hostage", "sec_hostage", "minions"}
            }, -- Aggregated enemies
            cop = {
                class = "UnitCountItem",
                skills = {0, 5},
                color_id = "enemy_color",
                priority = 5,
                subtract = {"cop_hostage", "cop_minion"}
            }, -- Non-special police
            security = {
                class = "UnitCountItem",
                perks = {1, 4},
                color_id = "guard_color",
                priority = 4,
                subtract = {"sec_hostage", "sec_minion"}
            },
            thug = {
                class = "UnitCountItem",
                skills = {4, 12},
            ...
            turret = {
                class = "UnitCountItem",
                skills = {7, 5},
                color_id = "turret_color",
                priority = 5
            },
            unique = {
                class = "UnitCountItem",
                skills = {3, 8},
                color_id = "civilian_color",
                priority = 3
            },
            cop_hostage = {
                class = "UnitCountItem",
                skills = {2, 8},
                color_id = "hostage_color",
                priority = 2
            },
            civ_hostage = {
                class = "UnitCountItem",
    ...
    function HUDList.UnitCountItem:unit_id() return self._id end
    ...
    HUDList.ShieldCountItem = HUDList.ShieldCountItem or
                                  class(HUDList.UnitCountItem)
    ...
    function HUDList.ShieldCountItem:set_icon_color(color)
        ...
        if self._shield_filler then
            self._shield_filler:set_color(
                self._default_icon_color or self._default_text_color)
        end
    end

    function HUDList.ShieldCountItem:rescale(new_scale)
        local enabled, size_mult = HUDList.ShieldCountItem.super.rescale(self,
                                                                         new_scale)

        if enabled and alive(self._shield_filler) then
            self._shield_filler:set_size(self._icon:w() * 0.4,
                                         self._icon:h() * 0.4)
            self._shield_filler:set_center(self._icon:center())
        end
    ...
    HUDList.UsedPagersItem = HUDList.UsedPagersItem or
                                 class(HUDList.RightListItem)
    ...
    function HUDList.UsedPagersItem:_add_pager(...) self:change_count(1) end
    ...
    function HUDList.UsedPagersItem:set_count(num)
        if managers.groupai:state():whisper_mode() then
            ...
            self._default_text_color = math.lerp(Color(1, 0.2, 0),
                                                 HUDListManager.ListOptions
                                                     .list_color or Color.white,
                                                 tweak and
                                                     tonumber(
                                                         tweak[(num or 0) + 1]) or
                                                     0)
    ...
    function HUDList.CamCountItem:_change_camera_count(count)
        ...
        if diff ~= 0 then self:change_count(diff) end
    end
    ...
    HUDList.BodyBagsInvItem = HUDList.BodyBagsInvItem or
                                  class(HUDList.RightListItem)
    ...
    HUDList.CorpseCountItem = HUDList.CorpseCountItem or
                                  class(HUDList.RightListItem)
    ...
    function HUDList.CorpseCountItem:_change_corpse_count(event, carry_id,
                                                          bagged, value, data)
    ...
    function HUDList.CorpseCountItem:set_count(unbagged, bagged)
        if managers.groupai:state():whisper_mode() then
            ...
            self._text:set_text(self._unbagged_count .. "/" ..
                                    self._bagged_count)
    ...
    HUDList.SpecialPickupItem = HUDList.SpecialPickupItem or
                                    class(HUDList.RightListItem)
    HUDList.SpecialPickupItem.MAP = {
        crowbar = {
            hudpickups = {0, 64, 32, 32},
            priority = 1,
            category = "mission_pickups",
            ignore = not WolfHUD:getSetting(
                {
                    "HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES",
                    "mission_pickups"
                }, true)
        },
        keycard = {
            hudpickups = {32, 0, 32, 32},
            priority = 1,
            category = "mission_pickups",
            ignore = not WolfHUD:getSetting(
                {
        ...
        small_loot = {
            hudpickups = {32, 224, 32, 32},
            priority = 3,
            category = "valuables",
            ignore = not WolfHUD:getSetting(
                {"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "valuables"},
                true)
        },
        briefcase = {
            hudpickups = {96, 224, 32, 32},
            priority = 4,
    ...
    function HUDList.SpecialPickupItem:init(parent, name, id, members)
        ...
        self._listener_clbks = {
            {
                name = string.format("HUDList_%s_special_pickup_count_listener",
                                     id),
                source = "special_equipment_count",
                event = {"change"},
                clbk = callback(self, self,
                                "_change_special_equipment_count_clbk"),
                keys = keys
            }
        }
    ...
    function HUDList.SpecialPickupItem:_change_special_equipment_count_clbk(
        event, interact_id, value, data) self:change_count(value) end
    ...
    HUDList.LootItem.MAP = {
        ...
        crate = {
            text = "wolfhud_hudlist_loot_crate",
            priority = 2,
            no_separate = true
        },
        xmas_present = {
            text = "hud_carry_present",
            priority = 2,
            no_separate = true
        }, -- White Xmas
        shopping_bag = {
            text = "wolfhud_hudlist_loot_bag",
            priority = 2,
            no_separate = true
        }, -- White Xmas
        showcase = {
            text = "wolfhud_hudlist_showcase",
            priority = 2,
            no_separate = true
        } -- Diamond heist + Diamond Museum
    }
    function HUDList.LootItem:init(parent, name, id, members)
        local loot_data = HUDList.LootItem.MAP[id]
        HUDList.LootItem.super.init(self, parent, name, loot_data.icon_data or
                                        {
                hudtabs = {32, 33, 32, 32},
                alpha = 0.75,
                w_ratio = 1.2
        ...
        if loot_data.text then
            local txt = loot_data.no_localize and loot_data.text or
                            managers.localization:text(loot_data.text)
            ...
            self._name_text = self._panel:text(
                    ...
                    w = self._panel:w(),
                    h = self._panel:w(),
                    color = HUDListManager.ListOptions.list_color_bg or
                        Color(0.0, 0.5, 0.0),
                    blend_mode = "normal",
                    font = tweak_data.hud_corner.assault_font,
            ...
            local font_size = math.min(self._name_text:font_size() *
                                           (self._name_text:w() / w) * 0.9,
                                       self._name_text:font_size())
    ...
    function HUDList.LootItem:rescale(new_scale)
        local enabled, size_mult = HUDList.LootItem.super.rescale(self,
                                                                  new_scale)
    ...
    function HUDList.LootItem:update_value()
        ...
        for _, data in pairs(managers.gameinfo:get_loot()) do
            if self._loot_types[data.carry_id] then
                ...
                local condition_clbk =
                    HUDListManager.LOOT_TYPES_CONDITIONS[loot_type]
    ...
    function HUDList.LootItem:set_count(unbagged, bagged)
        ...
        if HUDListManager.ListOptions.separate_bagged_loot and
            not HUDList.LootItem.MAP[self._id].no_separate then
            self._text:set_text(self._unbagged_count .. "/" ..
                                    self._bagged_count)
        else
    ...
    function HUDList.LootItem:_change_loot_count_clbk(event, carry_id, bagged,
                                                      value, data)
        local loot_type = HUDListManager.LOOT_TYPES[carry_id] or
                              HUDListManager.POTENTIAL_LOOT_TYPES[carry_id]
        ...
        if not condition_clbk or condition_clbk(loot_type, data) then
            ...
            if value ~= 0 then
                self._text:stop()
                self._text:animate(callback(self, self, "_animate_change"),
                                   value, self._progress_bar)
            end
        end
    end
    ...
    function HUDList.LeftListIcon:init(parent, name, ratio_w, ratio_h, icons)
        HUDList.LeftListIcon.super.init(self, parent, name, {
            align = "center",
            w = parent:panel():h() * (ratio_w or 1),
            h = parent:panel():h() * (ratio_h or 1)
        })
    ...
    function HUDList.LeftListIcon:rescale(new_scale)
        local enabled, size_mult = HUDList.LeftListIcon.super.rescale(self,
                                                                      new_scale)
    ...
    function HUDList.LeftListIcon:set_color(color)
        ...
        for _, icon in ipairs(self._icons) do icon:set_color(color) end
    end
    ...
    function HUDList.LeftListItem:init(parent, name, params)
        ...
        self._progress_bar = PanelFrame:new(self._panel, {
            ...
            bar_w = 2,
            bar_color = params.progress_color or
                (HUDListManager.ListOptions.list_color or Color.white),
            bg_color = (HUDListManager.ListOptions.list_color_bg or Color.black),
            bar_alpha = params.progress_alpha or
                HUDListManager.ListOptions.left_list_progress_alpha or 0.4,
            add_bg = true
        })
        self._progress_bar:set_ratio(1)
    end
    ...
    function HUDList.LeftListItem:rescale(new_scale)
        local enabled, size_mult = HUDList.LeftListItem.super.rescale(self,
                                                                      new_scale)

        if enabled then
            self._progress_bar:set_size(self._progress_bar:w() * size_mult,
                                        self._progress_bar:h() * size_mult)
        end
    ...
    HUDList.TimerItem.DEVICE_TYPES = {
        ...
        drill = {
            class = "UpgradeableTimerItem",
            name = "wolfhud_hudlist_device_drill"
        },
        drill_noupgrade = {
            class = "TimerItem",
            name = "wolfhud_hudlist_device_drill"
        ...
        saw = {
            class = "UpgradeableTimerItem",
            name = "wolfhud_hudlist_device_saw"
        },
        saw_noupgrade = {
            class = "TimerItem",
            name = "wolfhud_hudlist_device_saw"
        ...
        securitylock = {
            class = "SecurityTimerItem",
            name = "wolfhud_hudlist_device_security"
        }
    }
    function HUDList.TimerItem:init(parent, name, data)
        self.STANDARD_COLOR = HUDListManager.ListOptions.list_color or
                                  Color(1, 1, 1, 1)
        ...
        HUDList.TimerItem.super.init(self, parent, name, {
            align = "left",
            w = data.w or (parent:panel():h() * 4 / 5),
            h = parent:panel():h()
        })

        local txt = HUDList.TimerItem.DEVICE_TYPES[self._device_type] and
                        managers.localization:text(
                            HUDList.TimerItem.DEVICE_TYPES[self._device_type]
                                .name or "N/A") or tostring(self._device_type)
    ...
    function HUDList.TimerItem:rescale(new_scale)
        local enabled, size_mult = HUDList.TimerItem.super.rescale(self,
                                                                   new_scale)

        if enabled then
            self._type_text:set_size(self._panel:w(), self._panel:h() * 0.3)

            self._distance_text:set_size(self._progress_bar:w(),
                                         self._progress_bar:h() - 2)
            self._distance_text:set_y(self._progress_bar:y() + 2)

            self._time_text:set_size(self._progress_bar:w(),
                                     self._progress_bar:h())
    ...
    function HUDList.TimerItem:priority()
        return self._remaining and Utl.round(self._remaining, 1) or
                   self._priority
    end
    ...
    function HUDList.TimerItem:update(t, dt)
        ...
        if self._jammed or not self._powered then
            local new_color = get_color_from_table(
                                  math.sin(t * 360 * self.FLASH_SPEED) * 0.5 +
                                      0.5, 1, self._flash_color_table,
                                  self.STANDARD_COLOR)
    ...
    HUDList.UpgradeableTimerItem = HUDList.UpgradeableTimerItem or
                                       class(HUDList.TimerItem)
    function HUDList.UpgradeableTimerItem:init(parent, name, data)
        ...
        self.UPGRADE_LVL_COLORS = {
            Color(0.3, 1, 1, 1), Color(1, 1, 1, 1), Color(1, 1, 1, 0)
        }
    ...
    function HUDList.UpgradeableTimerItem:rescale(new_scale)
        local enabled, size_mult = HUDList.UpgradeableTimerItem.super.rescale(
                                       self, new_scale)
    ...
    function HUDList.UpgradeableTimerItem:set_color(color)
        ...
        local current_color = self._auto_repair and self.AUTOREPAIR_COLOR or
                                  self._upgradable and self.UPGRADE_COLOR or
                                  self.STANDARD_COLOR
        ...
        if data.upgrades then
            for _, upgrade in ipairs(self._upgrade_types) do
                if data.upgrades[upgrade] and self._upgrade_icons[upgrade] then
                    local upgrade_color =
                        self.UPGRADE_LVL_COLORS[math.clamp(
                            (data.upgrades[upgrade] or 0) + 1, 1,
                            #self.UPGRADE_LVL_COLORS)] or Color.red
    ...
    HUDList.SecurityTimerItem = HUDList.SecurityTimerItem or
                                    class(HUDList.TimerItem)
    function HUDList.SecurityTimerItem:init(parent, name, data)
        ...
        self._bars = {
            current = data.current_bar or 1,
            total = data.total_bars or 3
        }
    ...
    function HUDList.SecurityTimerItem:_update_bar_text()
        local text = managers.localization:text(
                         "wolfhud_hudlist_device_security_bar", {
                CURRENT = self._bars.current or 1,
                TOTAL = self._bars.total or 3
            })
    ...
    HUDList.TemperatureGaugeItem = HUDList.TemperatureGaugeItem or
                                       class(HUDList.TimerItem)
    ...
    function HUDList.TemperatureGaugeItem:update(t, dt)
        ...
        if self._remaining and self._last_update_t then
            local time_left = self._remaining -
                                  math.max(t - self._last_update_t, 0)
    ...
    function HUDList.TemperatureGaugeItem:_update_timer(data)
        if data.timer_value then
            ...
            if dv > 0 then
                self._remaining = math.max(self._goal - data.timer_value, 0) /
                                      dv
                self._last_update_t = Application:time()
            end

            self._distance_text:set_text(
                string.format("%d / %d", data.timer_value, self._goal))
    ...
    function HUDList.EquipmentItem:init(parent, name, data, equipment_type)
        ...
        HUDList.EquipmentItem.super.init(self, parent, name, {
            align = "center",
            w = parent:panel():h() * 4 / 5,
            h = parent:panel():h(),
        ...
        local events = {set_owner = callback(self, self, "_set_owner")}

        for event, clbk in pairs(events) do
            table.insert(self._listener_clbks, {
                name = id,
                source = self._equipment_type,
                event = {event},
                clbk = clbk,
                keys = {self._key},
                data_only = true
        ...
        if not self._defer_activation then self:activate() end
    end

    function HUDList.EquipmentItem:rescale(new_scale)
        local enabled, size_mult = HUDList.EquipmentItem.super.rescale(self,
                                                                       new_scale)
    ...
    function HUDList.EquipmentItem:set_color(color)
        ...
        if not self._owner then self._icon:set_color(color) end
    end
    ...
    function HUDList.EquipmentItem:get_type() return self._equipment_type end

    function HUDList.EquipmentItem:_set_color()
        local color = self._owner and self._owner > 0 and
                          tweak_data.chat_colors[self._owner]:with_alpha(1) or
                          HUDListManager.ListOptions.list_color or Color.white
        self._icon:set_color(color)
    end

    HUDList.BagEquipmentItem = HUDList.BagEquipmentItem or
                                   class(HUDList.EquipmentItem)
    function HUDList.BagEquipmentItem:init(parent, name, data, equipment_type)
        HUDList.BagEquipmentItem.super.init(self, parent, name, data,
                                            equipment_type)
    ...
    function HUDList.BagEquipmentItem:rescale(new_scale)
        local enabled, size_mult = HUDList.BagEquipmentItem.super.rescale(self,
                                                                          new_scale)
    ...
    function HUDList.BagEquipmentItem:priority()
        return HUDList.BagEquipmentItem.super.priority(self) -
                   Utl.round(self:amount() * 0.1, 3)
    end
    ...
    function HUDList.BagEquipmentItem:_update_info_text()
        if self._amount or self._amount_offset then
            ...
            local new_color = get_color_from_table(self:amount(),
                                                   self:max_amount())
    ...
    function HUDList.AmmoBagItem:_update_info_text()
        if self._amount then
            self._info_text:set_text(
                string.format("%.0f%%", self:amount() * 100))
            self._progress_bar:set_ratio(self:amount_ratio())
            local new_color = get_color_from_table(self:amount(),
                                                   self:max_amount())
            self._info_text:set_color(new_color)
    ...
    HUDList.SentryEquipmentItem = HUDList.SentryEquipmentItem or
                                      class(HUDList.EquipmentItem)
    function HUDList.SentryEquipmentItem:init(parent, name, data)
        HUDList.SentryEquipmentItem.super.init(self, parent, name, data,
                                               "sentry")
    ...
    function HUDList.SentryEquipmentItem:rescale(new_scale)
        local enabled, size_mult = HUDList.SentryEquipmentItem.super.rescale(
                                       self, new_scale)
        ...
        if enabled then
            ...
            self._health_bar:set_size(self._health_bar:w() * size_mult,
                                      self._bar_bg:h() * 0.5)
            self._health_bar:set_x(self._bar_bg:x())
            self._health_bar:set_y(self._bar_bg:y())

            self._ammo_bar:set_size(self._ammo_bar:w() * size_mult,
                                    self._bar_bg:h() * 0.5)
    ...
    function HUDList.SentryEquipmentItem:_set_ammo_ratio(data)
        if data.ammo_ratio then
            ...
            if self._ammo_ratio <= 0 then self:_set_inactive(nil) end
        end
    end
    ...
    function HUDList.SentryEquipmentItem:_set_health_ratio(data)
        if data.health_ratio then
            ...
            self._progress_bar:set_color(
                math.lerp(get_color_from_table(self._health_ratio, 1),
                          (HUDListManager.ListOptions.list_color or Color.white),
                          0.4))

            if self._health_ratio <= 0 then self:_set_inactive(nil) end
        end
    end
    ...
    function HUDList.SentryEquipmentItem:_set_inactive(duration)
        if self:is_player_owner() then
            if not self._animating then
                self._icon:animate(callback(self, self, "_animate_inactive"),
                                   Color.red, duration,
                                   callback(self, self, "deactivate"))
            end
        else
            self:deactivate()
        end
    ...
    function HUDList.SentryEquipmentItem:_animate_inactive(icon, flash_color,
        ...
        if expire_clbk then expire_clbk() end
    end
    ...
    function HUDList.MinionItem:init(parent, name, data)
        HUDList.MinionItem.super.init(self, parent, name, {
            align = "center",
            w = parent:panel():h() * 4 / 5,
            h = parent:panel():h()
        })
        ...
        self._unit = data.unit
        local type_table = self._unit:base()._tweak_table and
                               HUDListManager.UNIT_TYPES[self._unit:base()
                                   ._tweak_table] or false
        local type_string = type_table and
                                managers.localization:to_upper_text(
                                    type_table.long_name) or "UNKNOWN"
        ...
        self._health_bar:set_texture_rect(self._health_bar:texture_width(), 0,
                                          -self._health_bar:texture_width(),
                                          self._health_bar:texture_height())
        ...
        self._outline:set_texture_rect(self._outline:texture_width(), 0,
                                       -self._outline:texture_width(),
                                       self._outline:texture_height())
        ...
        self._unit_type = self._panel:text(
                ...
                layer = 3,
                font = tweak_data.hud_corner.assault_font,
                font_size = math.min(8 / string.len(type_string), 1) * 0.25 *
                    self._panel:h()
            })
        ...
        if data.health_ratio then self:_set_health_ratio(data, true) end
        if data.damage_resistance then self:_set_damage_resistance(data) end
        if data.damage_multiplier then self:_set_damage_multiplier(data) end
        ...
        local events = {
            ...
            set_kills = callback(self, self, "_set_kills"),
            set_damage_resistance = callback(self, self,
                                             "_set_damage_resistance"),
            set_damage_multiplier = callback(self, self,
                                             "_set_damage_multiplier")
        }
    ...
    function HUDList.MinionItem:rescale(new_scale)
        local enabled, size_mult = HUDList.MinionItem.super.rescale(self,
                                                                    new_scale)
        ...
        if enabled then
            ...
            self._hit_indicator:set_center(self._health_bar:center())
            self._outline:set_size(self._panel:w() * 0.95,
                                   self._panel:w() * 0.95)
            self._outline:set_center(self._health_bar:center())
    ...
    function HUDList.MinionItem:set_color(color)
        ...
        if not self._owner then self._unit_type:set_color(color) end
    end

    function HUDList.MinionItem:owner() return self._owner end
    ...
    function HUDList.MinionItem:_set_owner(data)
        if data.owner then
            self._owner = data.owner
            self._unit_type:set_color(
                tweak_data.chat_colors[data.owner]:with_alpha(1) or
                    Color(1, 1, 1, 1))

            if HUDListManager.ListOptions.show_own_minions_only then
                self:set_active(data.owner ==
    ...
    function HUDList.MinionItem:_set_damage_resistance(data)
        local max_mult = tweak_data.upgrades.values.player
                             .passive_convert_enemies_health_multiplier[2]
        local alpha = math.clamp((1 - data.damage_resistance) / (1 - max_mult),
                                 0, 1) * 0.7 + 0.3
        self._outline:set_alpha(alpha)
    end

    function HUDList.MinionItem:_set_damage_multiplier(data)
        local min_mult = tweak_data.upgrades.values.player
                             .convert_enemies_damage_multiplier[1] -- 0.65, damage multiplier if player has joker skill
        local max_mult = tweak_data.upgrades.values.player
                             .convert_enemies_damage_multiplier[2] -- 1.00, damage multiplier if player has 35% damage increase skill
        local alpha = math.clamp((data.damage_multiplier - min_mult) /
                                     (max_mult - min_mult), 0, 1) * 0.7 + 0.3
    ...
    function HUDList.PagerItem:init(parent, name, data)
        HUDList.PagerItem.super.init(self, parent, name, {
            align = "left",
            w = parent:panel():h(),
            h = parent:panel():h()
        })
        ...
        self._direction_icon:set_center(self._panel:w() * 0.8,
                                        self._panel:h() * 0.75)
    ...
    function HUDList.PagerItem:rescale(new_scale)
        local enabled, size_mult = HUDList.PagerItem.super.rescale(self,
                                                                   new_scale)

        if enabled then
            self._timer_text:sei_size(self._panel:w(), self._panel:h() * 0.6)

            self._distance_text:sei_size(self._panel:w() * 0.65,
                                         self._panel:h() * 0.4)
            self._distance_text:set_y(self._timer_text:bottom())

            self._direction_icon:set_size(self._panel:h() * 0.3,
                                          self._panel:h() * 0.2)
            self._direction_icon:set_center(self._panel:w() * 0.8,
                                            self._panel:h() * 0.75)
        end
    ...
    function HUDList.PagerItem:update(t, dt)
        if not self._answered then
            ...
            self._timer_text:set_color(get_color_from_table(self._remaining,
                                                            self._duration))
    ...
    function HUDList.ECMItem:init(parent, name, data)
        HUDList.ECMItem.super.init(self, parent, name, {
            align = "right",
            w = parent:panel():h(),
            h = parent:panel():h()
        })
    ...
    function HUDList.ECMItem:rescale(new_scale)
        local enabled, size_mult = HUDList.ECMItem.super
                                       .rescale(self, new_scale)

        if enabled then
            self._text:set_size(self._panel:w(), self._panel:h() * 0.7)
            self._text:set_font_size(self._panel:h() * 0.6)
            self._upgrade_lvl3:set_size(self._panel:w() * 0.4,
                                        self._panel:w() * 0.4)
            self._upgrade_lvl3:set_bottom(self._panel:h() - 2)
            self._level:set_size(self._panel:w() *
                                     (self._blocks_pager and 0.6 or 1),
                                 self._panel:h())
            self._level:set_font_size(self._panel:h() * 0.4)
            self._level:set_x(self._blocks_pager and
                                  (self._upgrade_lvl3:w() - 2) or 0)
        end
    ...
    function HUDList.ECMItem:_animate_battery_low(text, progress_bar)
        ...
        while self._animating_low_battery do
            ...
            if progress_bar then progress_bar:set_color(new_color) end
        end
        text:set_color(self.STANDARD_COLOR or Color.white)
        if progress_bar then
            progress_bar:set_color(HUDListManager.ListOptions.list_color or
                                       Color.white)
        end
    end
    ...
    function HUDList.ECMItem:_set_upgrade_level(data)
        if data.upgrade_level then
            self._max_duration = tweak_data.upgrades
                                     .ecm_jammer_base_battery_life *
                                     ECMJammerBase.battery_life_multiplier[data.upgrade_level]
            ...
            self._level:set_x(self._blocks_pager and
                                  (self._upgrade_lvl3:w() - 2) or 0)
            self._level:set_w(self._panel:w() *
                                  (self._blocks_pager and 0.6 or 1))
        end
    end
    ...
    function HUDList.ECMItem:_set_jammer_battery(data)
        if data.jammer_battery then
            ...
            self._progress_bar:set_ratio(
                data.jammer_battery / self._max_duration)
        end
    end
    ...
    function HUDList.ECMItem:_set_battery_low(data)
        if data.battery_low and not self._animating_low_battery then
            ...
            self._text:animate(callback(self, self, "_animate_battery_low"),
                               self._progress_bar)
    ...
    HUDList.ECMRetriggerItem = HUDList.ECMRetriggerItem or
                                   class(HUDList.LeftListItem)
    function HUDList.ECMRetriggerItem:init(parent, name, data)
        ...
        self._unit = data.unit
        self._max_duration =
            tweak_data.upgrades.ecm_feedback_retrigger_interval or 60
        self._flash_color_table = {
    ...
    function HUDList.ECMRetriggerItem:rescale(new_scale)
        local enabled, size_mult = HUDList.ECMRetriggerItem.super.rescale(self,
                                                                          new_scale)
    ...
    function HUDList.ECMRetriggerItem:_set_retrigger_delay(data)
        if data.retrigger_delay then
            ...
            self._progress_bar:set_ratio(
                data.retrigger_delay / self._max_duration)

            local new_color = get_color_from_table(
                                  self._max_duration - data.retrigger_delay,
                                  self._max_duration, self._flash_color_table,
                                  self.STANDARD_COLOR)
    ...
    HUDList.ECMFeedbackItem = HUDList.ECMFeedbackItem or
                                  class(HUDList.LeftListItem)
    function HUDList.ECMFeedbackItem:init(parent, name, data)
        HUDList.ECMFeedbackItem.super.init(self, parent, name, {
            align = "right",
            w = parent:panel():h(),
            h = parent:panel():h(),
            priority = 0
    ...
    function HUDList.ECMFeedbackItem:rescale(new_scale)
        local enabled, size_mult = HUDList.ECMFeedbackItem.super.rescale(self,
                                                                         new_scale)
    ...
    function HUDList.ECMFeedbackItem:_set_feedback_duration(data)
        if data.feedback_active and
            (data.feedback_duration or data.feedback_expire_t) then
            local t = Application:time()
            self._max_duration = data.feedback_duration or
                                     (data.feedback_expire_t - t) or 15
            self._expire_t = data.feedback_expire_t or
                                 (t + data.feedback_duration) or 0
            self._text:set_font_size(self._panel:h() * 0.6)
        end
    end
    ...
    function HUDList.TapeLoopItem:init(parent, name, data)
        HUDList.TapeLoopItem.super.init(self, parent, name, {
            align = "right",
            w = parent:panel():h(),
            h = parent:panel():h()
        })
        ...
        local events = {
            ...
            set_tape_loop_restart_active = callback(self, self,
                                                    "_set_restart_active")
        }
    ...
    function HUDList.TapeLoopItem:rescale(new_scale)
        local enabled, size_mult = HUDList.TapeLoopItem.super.rescale(self,
                                                                      new_scale)
    ...
    HUDList.BuffItemBase.MAP = {
        ...
        aggressive_reload_aced = {
            ...
            ignore = not WolfHUD:getSetting(
                {
                    "HUDList", "BUFF_LIST", "MASTERMIND_BUFFS",
                    "aggressive_reload_aced"
                }, true)
        },
        ammo_efficiency = {
            skills_new = tweak_data.skilltree.skills.single_shot_ammo_return
                .icon_xy,
            class = "TimedBuffItem",
            priority = 4,
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "ammo_efficiency"},
                true)
        },
        ...
        berserker = {
            ...
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "berserker"}, true)
        },
        biker = {
            perks = {0, 0},
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "biker"}, true)
        },
        bloodthirst_aced = {
            ...
            localized = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "bloodthirst_aced"},
                true)
        },
        bloodthirst_basic = {
            skills_new = tweak_data.skilltree.skills.bloodthirst.icon_xy,
            ...
            localized = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "bloodthirst_basic"},
                false)
        },
        bullet_storm = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "bullet_storm"}, true)
        },
        chico_injector = {
            perks = {0, 0},
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "chico_injector"}, false) and
                (WolfHUD:getSetting({"CustomHUD", "PLAYER", "STATUS"}, true) or
                    WolfHUD:getSetting({"CustomHUD", "ENABLED"}, false))
        },
        close_contact = {
            perks = {5, 4},
            class = "TimedBuffItem",
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "close_contact"}, true)
        },
        combat_medic = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "combat_medic"},
                true)
        },
        ...
        delayed_damage = {
            ...
            show_value = "-%.0f",
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "delayed_damage"}, true)
        },
        desperado = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "desperado"}, true)
        },
        die_hard = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "die_hard"}, false)
        },
        dire_need = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "GHOST_BUFFS", "dire_need"}, true)
        },
        frenzy = {
            ...
            show_value = HUDList.BuffItemBase.VALUE_FUNC.IN_PERCENT,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "frenzy"}, false)
        },
        grinder = {
            perks = {4, 6},
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "grinder"}, true)
        },
        hostage_situation = {
            perks = {0, 1},
            class = "BuffItemBase",
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "hostage_situation"},
                false)
        },
        hostage_taker = {
            ...
            invert_timers = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "hostage_taker"},
                false)
        },
        inspire = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "inspire"}, true)
        },
        lock_n_load = {
            ...
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "TECHNICIAN_BUFFS", "lock_n_load"},
                true)
        },
        maniac = {
            perks = {0, 0},
            ...
            show_value = "-%.1f",
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "maniac"}, false) and
                (WolfHUD:getSetting({"CustomHUD", "PLAYER", "STATUS"}, true) or
                    WolfHUD:getSetting({"CustomHUD", "ENABLED"}, false))
        },
        messiah = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "messiah"}, true)
        },
        melee_stack_damage = {
            perks = {5, 4},
            class = "TimedBuffItem",
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "melee_stack_damage"},
                false)
        },
        muscle_regen = {
            perks = {4, 1},
            ...
            invert_timers = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "muscle_regen"}, false)
        },
        overdog = {
            perks = {6, 4},
            class = "TimedBuffItem",
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "overdog"}, false)
        },
        overkill = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "overkill"}, false)
        },
        painkiller = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "painkiller"},
                false)
        },
        partner_in_crime = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "partner_in_crime"},
                false)
        },
        pocket_ecm_jammer = {
            perks = {0, 0},
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "pocket_ecm_jammer"},
                true)
        },
        pocket_ecm_kill_dodge = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "pocket_ecm_kill_dodge"},
                false)
        },
        running_from_death = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "running_from_death"},
                true)
        },
        quick_fix = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "quick_fix"}, false)
        },
        second_wind = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "GHOST_BUFFS", "second_wind"}, true)
        },
        sicario_dodge = {
            perks = {1, 0},
            ...
            show_value = HUDList.BuffItemBase.VALUE_FUNC.IN_PERCENT,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "sicario_dodge"}, true)
        },
        sixth_sense = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "GHOST_BUFFS", "sixth_sense"}, true)
        },
        smoke_screen_grenade = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "smoke_screen_grenade"},
                true)
        },
        swan_song = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "swan_song"}, false) and
                (WolfHUD:getSetting({"CustomHUD", "PLAYER", "STATUS"}, true) or
                    WolfHUD:getSetting({"CustomHUD", "ENABLED"}, false))
        },
        tag_team = {
            ...
            show_value = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "tag_team"}, true) and
                (WolfHUD:getSetting({"CustomHUD", "PLAYER", "STATUS"}, true) or
                    WolfHUD:getSetting({"CustomHUD", "ENABLED"}, false))
        },
        tooth_and_claw = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "tooth_and_claw"}, true)
        },
        trigger_happy = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "trigger_happy"},
                false)
        },
        underdog = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "underdog"}, false)
        },
        unseen_strike = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "GHOST_BUFFS", "unseen_strike"}, true)
        },
        up_you_go = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "up_you_go"}, false)
        },
        uppers = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "uppers"}, true)
        },
        yakuza = {
            perks = {2, 7},
            class = "BuffItemBase",
            ...
            show_value = HUDList.BuffItemBase.VALUE_FUNC.IN_PERCENT,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "yakuza"}, false)
        },
        ...
        ammo_give_out_debuff = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "ammo_give_out_debuff"},
                true)
        },
        ...
        bullseye_debuff = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "bullseye_debuff"},
                true)
        },
        grinder_debuff = {
            perks = {4, 6},
        ...
        inspire_debuff = {
            ...
            localized = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "inspire_debuff"},
                true)
        },
        ...
        life_drain_debuff = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "life_drain_debuff"},
                true)
        },
        maniac_debuff = {
            perks = {0, 0},
        ...
        sociopath_debuff = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "sociopath_debuff"}, true)
        },
        tag_team_debuff = {
            perks = {0, 0},
            texture_bundle_folder = "ecp",
        ...
        damage_control_debuff = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "damage_control_debuff"},
                false)
        },
        ...
        armorer = {
            ...
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "armorer"}, true)
        },
        bulletproof = { -- TODO: Needs new icon (Faster Team armor recovery)
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.TEAM,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "bulletproof"}, true)
        },
        crew_chief = {
            perks = {2, 0},
            class = "TeamBuffItem",
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.TEAM,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "PERK_BUFFS", "crew_chief"}, true)
        },
        endurance = {
        ...
        invulnerable_buff = {
            ...
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "GAGE_BOOSTS", "invulnerable_buff"},
                true)
        },
        life_steal_debuff = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "GAGE_BOOSTS", "life_steal_debuff"},
                true)
        },
        ...
        crew_inspire_debuff = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "AI_SKILLS", "crew_inspire_debuff"},
                true)
        },
        crew_throwable_regen = {
            ...
            color = HUDList.BuffItemBase.ICON_COLOR.BUFF,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "AI_SKILLS", "crew_throwable_regen"},
                true)
        },
        crew_health_regen = {
            hud_tweak = "skill_5",
            ...
            invert_timers = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "AI_SKILLS", "crew_health_regen"}, true)
        },
        ...
        damage_increase = {
            ...
            localized = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "damage_increase"}, true)
        },
        damage_reduction = {
            ...
            localized = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "damage_reduction"}, true)
        },
        melee_damage_increase = {
            ...
            localized = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "melee_damage_increase"}, true)
        },
        passive_health_regen = {
            ...
            invert_timers = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "passive_health_regen"}, true)
        },
        total_dodge_chance = { -- missing some skills:
            ...
            localized = true,
            ignore = not WolfHUD:getSetting(
                {"HUDList", "BUFF_LIST", "total_dodge_chance"}, true)
        },
        ...
        interact = {
            ...
            priority = 15,
            color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
            ignore = (WolfHUD:getSetting({"INTERACTION", "SHOW_CIRCLE"}, true) or
                WolfHUD:getSetting({"INTERACTION", "SHOW_TIME_REMAINING"}, true))
        },
        interact_debuff = {
            -- skills_new = tweak_data.skilltree.skills.second_chances.icon_xy,
            texture = "guis/textures/pd2/skilltree/drillgui_icon_faster",
    ...
    function HUDList.BuffItemBase:init(parent, name, icon, w, h)
        HUDList.BuffItemBase.super.init(self, parent, name, {
            priority = icon.priority,
            align = "bottom",
            w = w or parent:panel():h() * 0.6,
            h = h or parent:panel():h()
        })

        local texture, texture_rect = get_icon_data(icon)

        self._default_icon_color = icon.color or
                                       HUDList.BuffItemBase.ICON_COLOR.STANDARD or
                                       Color.white
        ...
        self._title = self._panel:text({
            name = "title",
            text = icon.localized and managers.localization:text(icon.title) or
                icon.title or "",
            align = "center",
            vertical = "top",
            w = self._panel:w(),
    ...
    function HUDList.BuffItemBase:rescale(new_scale)
        local enabled, size_mult = HUDList.BuffItemBase.super.rescale(self,
                                                                      new_scale)
        ...
        if enabled then
            ...
            self._bg:set_center(self._icon:center())
            self._title:set_size(self._panel:w(),
                                 (self._panel:h() - icon_size) / 2)
            self._title:set_font_size(0.7 * (self._panel:h() - icon_size) / 2)
            self._value:set_size(self._panel:w(),
                                 (self._panel:h() - icon_size) / 2)
            ...
            self._progress_bar:set_size(self._panel:w() -
                                            (progress_bar_width + 1),
                                        self._panel:w() -
                                            (progress_bar_width + 1))
            self._progress_bar:set_center(self._icon:center())
            self._progress_bar:set_width(progress_bar_width)
            self._progress_bar_inner:set_size(
                self._panel:w() - (progress_bar_width + 1) * 2,
                self._panel:w() - (progress_bar_width + 1) * 2)
            self._progress_bar_inner:set_center(self._icon:center())
    ...
    function HUDList.BuffItemBase:set_duration(id, data)
        ...
        if self._debuff_active and self._debuff_expire_t and self._expire_t <
            self._debuff_expire_t then
            self._icon:set_color(self._default_icon_color)
            self._ace_icon:set_color(self._default_icon_color)
        end
    end
    ...
    function HUDList.BuffItemBase:set_duration_debuff(id, data)
        ...
        if self._buff_active and self._expire_t and self._expire_t <
            self._debuff_expire_t then
            self._icon:set_color(self._default_icon_color)
            self._ace_icon:set_color(self._default_icon_color)
        end
    ...
    function HUDList.BuffItemBase:_update_debuff(t, dt)
        self:_set_progress_debuff((t - self._debuff_start_t) /
                                      (self._debuff_expire_t -
                                          self._debuff_start_t))
    ...
    function HUDList.TimedBuffItem:update(t, dt)
        ...
        if self._buff_active and self._expire_t then
            self:_set_progress((t - self._start_t) /
                                   (self._expire_t - self._start_t))
            ...
            if #time_str > 0 then
                ...
                for i, data in ipairs(time_str) do
                    ...
                    table.insert(color_ranges, {
                        offset, string.len(str),
                        data.color or HUDList.BuffItemBase.ICON_COLOR.STANDARD
                    })
                    if i < #time_str then str = str .. " " end
    ...
    HUDList.TimedStacksBuffItem = HUDList.TimedStacksBuffItem or
                                      class(HUDList.BuffItemBase)
    ...
    function HUDList.TimedStacksBuffItem:update(t, dt)
        ...
        if #self._stacks > 1 then
            ...
            self:_set_progress_inner((stack.expire_t - t) /
                                         (stack.expire_t - stack.t))
        else
            self:_set_progress_inner(0)
        end
        ...
        if not self._has_text then
            if #time_str > 0 then
                ...
                for i, data in ipairs(time_str) do
                    ...
                    table.insert(color_ranges, {
                        offset, string.len(str),
                        data.color or self._default_icon_color or
                            HUDList.BuffItemBase.ICON_COLOR.STANDARD
                    })
                    if i < #time_str then str = str .. " " end
                    offset = offset + string.len(str)
                end
    ...
    HUDList.BikerBuffItem = HUDList.BikerBuffItem or
                                class(HUDList.TimedStacksBuffItem)
    ...
    function HUDList.CompositeBuff:update(t, dt)
        if self._min_expire_buff then
            self:_set_progress_inner((t -
                                         self._member_buffs[self._min_expire_buff]
                                             .start_t) /
                                         (self._member_buffs[self._min_expire_buff]
                                             .expire_t -
                                             self._member_buffs[self._min_expire_buff]
        ...
        if self._max_expire_buff then
            self:_set_progress((t -
                                   self._member_buffs[self._max_expire_buff]
                                       .start_t) /
                                   (self._member_buffs[self._max_expire_buff]
                                       .expire_t -
                                       self._member_buffs[self._max_expire_buff]
    ...
    function HUDList.CompositeBuff:set_stack_count(id, data)
        if self._member_buffs[id] and self._member_buffs[id].stack_count ~=
            data.stack_count then
    ...
    function HUDList.CompositeBuff:set_value(id, data)
        if self._member_buffs[id] and self._member_buffs[id].value ~= data.value then
            WolfHUD:print_log("(HUDList) CompositeBuff:set_value(%s, %s)",
                              tostring(id), tostring(data.value), "info")
    ...
    function HUDList.CompositeBuff:_check_buffs()
        ...
        for id, data in pairs(self._member_buffs) do
            if data.expire_t then
                if not max_expire or data.expire_t >
                    self._member_buffs[max_expire].expire_t then
                    max_expire = id
                end
                if not min_expire or data.expire_t <
                    self._member_buffs[min_expire].expire_t then
                    min_expire = id
                end
            end
        end
        ...
        if not self._min_expire_buff or
            self._member_buffs[self._min_expire_buff].expire_t ==
            self._member_buffs[self._max_expire_buff].expire_t then
    ...
    HUDList.DamageIncreaseBuff = HUDList.DamageIncreaseBuff or
                                     class(HUDList.CompositeBuff)
    function HUDList.DamageIncreaseBuff:init(...)
        HUDList.DamageIncreaseBuff.super.init(self, ...)

        self._buff_weapon_requirements =
            {overkill = {shotgun = true, saw = true}, berserker = {saw = true}}
        ...
        self._buff_effects = {
            berserker = function(value)
                return 1 + (value or 0) *
                           managers.player:upgrade_value("player",
                                                         "melee_damage_health_ratio_multiplier",
                                                         0)
            end,
            berserker_aced = function(value)
                return 1 + (value or 0) *
                           managers.player:upgrade_value("player",
                                                         "damage_health_ratio_multiplier",
                                                         0)
            end
        }
    end
    ...
    function HUDList.DamageIncreaseBuff:update(t, dt)
        ...
        if not alive(self._player_unit) and alive(managers.player:player_unit()) then
            ...
            self._player_unit:inventory():add_listener("DamageIncreaseBuff",
                                                       {"equip"}, callback(self,
                                                                           self,
                                                                           "_on_weapon_equipped"))
            ...
            if self._weapon_tweak.ignore_damage_upgrades then
                ...
                for id, data in pairs(self._member_buffs) do
                    for _, category in ipairs(weapon_categories) do
                        if not self._buff_weapon_requirements[id] or
                            self._buff_weapon_requirements[id][category] then
                            if not (self._buff_weapon_exclusions[id] and
                                self._buff_weapon_exclusions[id][category]) then
                                local clbk = self._buff_effects[id]
                                value = value *
                                            (data.value and
                                                (clbk and clbk(data.value) or
                                                    data.value) or 1)
                                break
                            end
                        end
    ...
    HUDList.MeleeDamageIncreaseBuff = HUDList.MeleeDamageIncreaseBuff or
                                          class(HUDList.CompositeBuff)
    ...
    function HUDList.MeleeDamageIncreaseBuff:_update_value()
        ...
        for id, data in pairs(self._member_buffs) do
            ...
            value = value *
                        (data.value and
                            (clbk and clbk(data.value) or data.value) or 1)
        end

        self:_set_text(string.format("x%.0f", (value - 1)))
    end

    HUDList.DamageReductionBuff = HUDList.DamageReductionBuff or
                                      class(HUDList.CompositeBuff)
    function HUDList.DamageReductionBuff:init(...)
        ...
        self._buff_effects = {
            chico_injector = function(value)
                ...
                local health_ratio = alive(player) and
                                         player:character_damage()
                                             :health_ratio() or 1
                if managers.player:has_category_upgrade("player",
                                                        "chico_injector_low_health_multiplier") then
                    local upg_values = managers.player:upgrade_value("player",
            ...
            frenzy = function(value) return 1 - value end,
            maniac = function(value)
                local new_value = 1
                local player = managers.player:player_unit()
                local player_damage = alive(player) and
                                          player:character_damage()
    ...
    HUDList.PassiveHealthRegenBuff = HUDList.PassiveHealthRegenBuff or
                                         class(HUDList.CompositeBuff)
    ...
    function HUDList.PassiveHealthRegenBuff:_update_value()
        ...
        for id, data in pairs(self._member_buffs) do
            ...
            value = value +
                        (data.value and
                            (clbk and clbk(data.value) or data.value) or 0)
        end

        self:_set_text(string.format("%.1f%%", value * 100))
    end

    HUDList.TotalDodgeChanceBuff = HUDList.TotalDodgeChanceBuff or
                                       class(HUDList.CompositeBuff)
    function HUDList.TotalDodgeChanceBuff:init(...)
        HUDList.TotalDodgeChanceBuff.super.init(self, ...)

        self._member_buffs["base_dodge"] =
            {
                value = (tweak_data.player.damage.DODGE_INIT or 0) +
                    managers.player:body_armor_value("dodge")
            }
        self._member_buffs["crook_dodge"] =
            {
                value = managers.player:upgrade_value("player",
                                                      "passive_dodge_chance", 0) +
                    managers.player:upgrade_value("player", tostring(
                                                      managers.blackmarket:equipped_armor(
                                                          true, true)) ..
                                                      "_dodge_addend", 0) -- Crook Perk
            }
        self._member_buffs["burglar_dodge"] =
            {
                value = managers.player:upgrade_value("player",
                                                      "tier_dodge_chance", 0)
            } -- Burglar Perk
        self._member_buffs["jail_diet"] =
            {
                value = managers.player:get_value_from_risk_upgrade(
                    managers.player:upgrade_value("player",
                                                  "detection_risk_add_dodge_chance"))
            }
        self._member_buffs["henchman_dodge"] =
            {value = managers.player:upgrade_value("team", "crew_add_dodge", 0)}
    ...
    function HUDList.TotalDodgeChanceBuff:_update_value()
        ...
        if self._member_buffs["smoke_screen_grenade"] then
            value = 1 - (1 - value) *
                        (1 -
                            tweak_data.projectiles.smoke_screen_grenade
                                .dodge_chance)
        end
    ...
    function HUDList.TotalDodgeChanceBuff:activate(id)
        if not self._member_buffs[id] then self._member_buffs[id] = {} end
    end
    ...
    HUDList.TimedInteractionItem = HUDList.TimedInteractionItem or
                                       class(HUDList.TimedBuffItem)
    ...
    function PanelFrame:init(parent, settings)
        ...
        self._panel = parent:panel({w = w, h = h, alpha = settings.alpha or 1})
        ...
        self:set_color(settings.bar_color or settings.color or Color.white,
                       settings.alpha or settings.bar_alpha or 1)
        self:reset()
    end

    function PanelFrame:panel() return self._panel end
    ...
    function PanelFrame:set_width(w)
        ...
        self._bar_w = w
        self._stages = {
            0, (pw - 2 * self._bar_w) / total, (pw + ph - self._bar_w) / total,
            (2 * pw + ph - 2 * self._bar_w) / total, 1
        }
        self._top:set_h(w)
        self._top:set_top(0)
    ...
    function PanelFrame:set_bg_color(c, alpha)
        if alive(self._bg) then
            ...
            if alpha then self._bg:set_alpha(alpha) end
        end
    end
    ...
    function PanelFrame:set_bg_alpha(alpha)
        if alive(self._bg) then self._bg:set_alpha(alpha) end
    end
    ...
    function PanelFrame:set_ratio(r)
        ...
        if self._invert_progress then r = 1 - r end

        if r < self._stages[self._current_stage] then self:reset() end
...
if string.lower(RequiredScript) == "lib/managers/objectinteractionmanager" then
    ...
    function ObjectInteractionManager:init(...)
        init_original(self, ...)
        if managers.gameinfo and
            WolfHUD:getSetting(
                {"HUDSuspicion", "REMOVE_ANSWERED_PAGER_CONTOUR"}, true) then
            managers.gameinfo:register_listener("pager_contour_remover",
                                                "pager", "set_answered",
                                                callback(nil, _G,
                                                         "pager_answered_clbk"))
        end
    ...
    function pager_answered_clbk(event, key, data)
        managers.enemy:add_delayed_clbk("contour_remove_" .. key, callback(nil,
                                                                           _G,
                                                                           "remove_answered_pager_contour_clbk",
                                                                           data.unit),
                                        Application:time() + 0.01)
    end

    function remove_answered_pager_contour_clbk(unit)
        if alive(unit) then
            unit:contour():remove(tweak_data.interaction.corpse_alarm_pager
                                      .contour_preset)
        end
    end
end
...
if string.lower(RequiredScript) == "lib/managers/hud/hudassaultcorner" then
    ...
    function HUDAssaultCorner:init(...)
        ...
        if alive(hostages_panel) then hostages_panel:set_alpha(0) end
    end
end
