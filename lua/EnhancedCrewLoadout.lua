...
if string.lower(RequiredScript) == "lib/managers/menu/contractboxgui" then
    ...
    local update_character_menu_state_original =
        ContractBoxGui.update_character_menu_state
    ...
    function ContractBoxGui:init(...)
        ...
        if managers.network:session() then
            ...
            self._loadout_btn = self._loadout_btn or self._panel:text(
                    ...
                    name = "toggle_loadout",
                    text = utf8.to_upper(
                        managers.localization:text("menu_team_loadout") .. ": " ..
                            managers.localization:text("menu_button_show")),
            ...
            self._loadout_data = self._loadout_data or
                                     self._loadout_panel:panel(
                                         {
                        name = "crew_loadout",
            ...
            self._borders = self._borders or
                                BoxGuiObject:new(self._loadout_data,
                                                 {sides = {1, 1, 2, 2}})
            ...
            self:set_loadout_enabled(WolfHUD:getSetting(
                                         {"CrewLoadout", "SHOW_IN_LOBBY"}, true) and
                                         managers.menu:is_pc_controller())
        end
    end
    ...
    function ContractBoxGui:update(t, dt, ...)
        ...
        if self._loadout_visible then
            for _, panel in ipairs(self._peer_loadout) do
                if panel:enabled() then panel:update(t, dt) end
            end
        end
    end
    ...
    function ContractBoxGui:update_bg_state(peer_id, ...)
        ...
        if managers.menu_component and self._loadout_visible then
            if self._peer_loadout[peer_id] and
                self._peer_loadout[peer_id]:local_peer() then
    ...
    function ContractBoxGui:mouse_wheel_up(...)
        if self:can_take_input() and self._enabled and not self._loadout_visible and
            not self._active_move then self:toggle_loadout() end

        mouse_wheel_up_original(self, ...)
    end

    function ContractBoxGui:mouse_wheel_down(...)
        if self:can_take_input() and self._enabled and self._loadout_visible and
            not self._active_move then self:toggle_loadout() end
    ...
    function ContractBoxGui:mouse_pressed(button, x, y, ...)
        if self:can_take_input() and self._enabled and alive(self._loadout_btn) and
            button == Idstring("0") and self._loadout_btn:inside(x, y) and
            not self._active_move then
            self:toggle_loadout()
        ...
        if self._loadout_enabled then
            if self:can_take_input() and alive(self._loadout_btn) and
                ...
                elseif self._loadout_btn_highlighted then
                    self._loadout_btn:set_color(
                        tweak_data.screen_colors.button_stage_3)
    ...
    function ContractBoxGui:set_enabled(...)
        ...
        self:set_loadout_enabled(self._enabled and
                                     WolfHUD:getSetting(
                                         {"CrewLoadout", "SHOW_IN_LOBBY"}, true) and
                                     managers.menu:is_pc_controller())
    end
    ...
    function ContractBoxGui:populate_loadout_panel()
        ...
        for i, peer_id in ipairs({3, 2, 1, 4}) do
            if not self._peer_loadout[peer_id] then
                self._peer_loadout[peer_id] =
                    LoadoutPanel:new(self._loadout_data, self, peer_id, width,
                                     ContractBoxGui._LOADOUT_H + 8, {
                        component_layout = WolfHUD:getTweakEntry(
                            "STD_LOBBY_LOADOUT_LAYOUT", "table", {
                                {"playtime", "ping"}, {"name"}, {"character"},
                                {"skills"}, {"perk"}, {"primary"},
    ...
    function ContractBoxGui:update_loadout_panel(peer_id)
        ...
        if panel then
            ...
            if panel:local_peer() then
                outfit = managers.blackmarket:unpack_outfit_from_string(
    ...
    function ContractBoxGui:toggle_loadout(instant)
        if alive(self._loadout_panel) and alive(self._loadout_btn) then
            ...
            local text_id = self._loadout_visible and
                                managers.localization:text("menu_button_hide") or
                                managers.localization:text("menu_button_show")
            self._loadout_btn:set_text(utf8.to_upper(
            ...
            self._loadout_data:animate(callback(self, self, "_animate_move"),
                                       self._loadout_data:x(), y,
                                       instant or false, callback(self, self,
                                                                  "finish_loadout_animation"))
        end
    end

    function ContractBoxGui:finish_loadout_animation()
        if alive(self._loadout_btn) then
            self._loadout_btn:set_color(self._loadout_btn_highlighted and
    ...
    function ContractBoxGui:_animate_move(panel, x, y, instant, done_clbk)
        ...
        if not instant then
            ...
            local T = math.max(math.abs(x - init_x) / move_speed,
...
elseif string.lower(RequiredScript) ==
    "lib/managers/menu/crimespreedetailsmenucomponent" then
    local populate_tabs_data_original =
        CrimeSpreeDetailsMenuComponent.populate_tabs_data
    ...
    function CrimeSpreeDetailsMenuComponent:populate_tabs_data(tabs_data, ...)
        ...
        if not self:_is_in_preplanning() and
            not Global.game_settings.single_player and
            WolfHUD:getSetting({"CrewLoadout", "SHOW_IN_CS_LOBBY"}, true) then
            table.insert(tabs_data,
                         CrimeSpreeDetailsMenuComponent._LOADOUT_INDEX, {
    ...
    function CrimeSpreeDetailsMenuComponent:update(t, dt, ...)
        ...
        if WolfHUD:getSetting({"CrewLoadout", "SHOW_IN_CS_LOBBY"}, true) then
            for _, data in ipairs(self._tabs) do
                if data.tab._index ==
                    CrimeSpreeDetailsMenuComponent._LOADOUT_INDEX then
    ...
    CrimeSpreeCrewLoadoutPage = CrimeSpreeCrewLoadoutPage or
                                    class(CrimeSpreeDetailsPage)
    ...
    function CrimeSpreeCrewLoadoutPage:populate_loadout_panel()
        ...
        for i, peer_id in ipairs({3, 2, 1, 4}) do
            if not self._peer_loadout[peer_id] then
                self._peer_loadout[peer_id] =
                    LoadoutPanel:new(self._loadout_data, self, peer_id, width,
                                     math.floor(self._loadout_data:h() + 9), {
                        component_layout = WolfHUD:getTweakEntry(
                            "CS_LOBBY_LOADOUT_LAYOUT", "table", {
                                {"playtime", "ping"}, {"name"}, {"skills"},
                        ...
                        name = {
                            font_size = tweak_data.menu.pd2_medium_font_size *
                                0.95,
                            height = tweak_data.menu.pd2_medium_font_size * 1.00,
                            align = "center",
                            margin = 3,
                            use_peer_color = true,
                            alpha = 1
                        ...
                        level = {
                            font_size = tweak_data.menu.pd2_medium_font_size *
                                0.9,
                            height = tweak_data.menu.pd2_medium_font_size * 0.95,
                            align = "right",
                            margin = 3,
                            use_peer_color = true,
                            alpha = 1
                        ...
                        skills = {
                            font_size = tweak_data.menu.pd2_small_font_size *
                                0.7,
                            height = tweak_data.menu.pd2_small_font_size * 0.75,
                            align = "center",
                            use_peer_color = true,
                            alpha = 0.95
                        },
                        perk = {
                            font_size = tweak_data.menu.pd2_medium_font_size *
                                0.7,
                            height = tweak_data.menu.pd2_medium_font_size * 0.75,
                            align = "center",
                            use_peer_color = true,
                            alpha = 0.95
                        },
                        ping = {
                            font_size = tweak_data.menu.pd2_small_font_size *
                                0.75,
                            height = tweak_data.menu.pd2_small_font_size * 0.8,
                            align = "right",
                            alpha = 1
                        },
                        playtime = {
...
elseif string.lower(RequiredScript) == "lib/managers/menu/missionbriefinggui" then
    ...
    function TeamLoadoutCustom:init(panel, text, i)
        ...
        for peer_id = 1, 4 do
            self._player_slots[peer_id] =
                LoadoutPanel:new(self._panel, self, peer_id, quarter_width,
                                 self._panel:h() + 7, {
                    component_layout = WolfHUD:getTweakEntry(
                        "BRIEFING_LOADOUT_LAYOUT", "table", {
                            {"perk"}, {"skills"}, {"primary"}, {"secondary"},
                            {"melee_weapon", "grenade"}, {"armor", "mask"},
                            {"deployable", "secondary_deployable"}
                        }),
                    name = {
                        font_size = tweak_data.menu.pd2_medium_font_size * 0.95,
                        height = tweak_data.menu.pd2_medium_font_size * 1.00,
                        align = "center",
                        margin = 3,
                        use_peer_color = true
                    ...
                    skills = {
                        ...
                        height = tweak_data.menu.pd2_small_font_size * 0.95,
                        align = "center",
                        use_peer_color = true
                    },
                    perk = {
                    ...
                    ping = {
                        font_size = tweak_data.menu.pd2_small_font_size * 0.75,
                        height = tweak_data.menu.pd2_small_font_size * 0.8,
                        align = "right"
                    },
                    playtime = {
                        font_size = tweak_data.menu.pd2_small_font_size * 0.7,
                        height = tweak_data.menu.pd2_small_font_size * 0.75,
                        align = "center"
                    },
            ...
            if kit_menu then
                local kit_slot = kit_menu.renderer:get_player_slot_by_peer_id(
                                     peer_id)
                if kit_slot then
                    local outfit = kit_slot.outfit
                    local character = kit_slot.params and
                                          kit_slot.params.character
        ...
        if type(ptr) == "function" then
            TeamLoadoutCustom[id] = TeamLoadoutCustom[id] or function(self, ...)
                WolfHUD:print_log(string.format(
                                      "TeamLoadoutCustom: Unoverwritten function call: %s",
                                      id), "warning")
            end
        end
    end
    if CoreClass and
        WolfHUD:getSetting({"CrewLoadout", "REPLACE_IN_BRIEFING"}, true) then
...
elseif string.lower(RequiredScript) == "lib/managers/hud/newhudstatsscreen" then
    ...
    function HUDStatsScreen:_create_player_info()
        self._USING_CREW_LOADOUT = WolfHUD:getSetting(
                                       {"CrewLoadout", "SHOW_ON_STATS_PANEL"},
                                       true) and
                                       not (_G.LobbyPlayerInfo and
                                           LobbyPlayerInfo.settings
                                               .show_skills_in_stats_screen)
    ...
    function HUDStatsScreen:_destroy_player_info()
        if self._loadout_data then
            for _, panel in ipairs(self._peer_loadout or {}) do
                if not panel:destroyed() then panel:destroy() end
            end
    ...
    function HUDStatsScreen:update(t, dt, ...)
        if self._USING_CREW_LOADOUT and table.size(self._peer_loadout or {}) > 0 then
            for peer_id = 1, 4 do
                if self._peer_loadout[peer_id] and
                    not self._peer_loadout[peer_id]:destroyed() then
    ...
    function HUDStatsScreen:populate_loadout_panel(parent_panel)
        ...
        if parent_panel then
            ...
            for peer_id = 1, 4 do
                if not self._peer_loadout[peer_id] then
                    self._peer_loadout[peer_id] =
                        LoadoutPanel:new(parent_panel, self, peer_id,
                                         parent_panel:w(),
                                         parent_panel:h() * 0.17, {
                            component_layout = WolfHUD:getTweakEntry(
                                "TAB_LOADOUT_LAYOUT", "table",
                                {{"name", "ping"}, {"skills", "perk"}}),
                            name = {
                                font_size = tweak_data.menu.pd2_medium_font_size *
                                    0.90,
                            ...
                            skills = {
                                font_size = tweak_data.menu.pd2_small_font_size *
                                    1.10,
                                height = tweak_data.menu.pd2_small_font_size *
                                    1.15,
                                align = "left",
                                margin = 3
                            },
    ...
    function HUDStatsScreen:arrange_loadout_panels(parent_panel)
        if parent_panel then
            ...
            parent_panel:set_leftbottom(10, self._right:h() -
                                            (WolfHUD:getSetting(
                                                {"TabStats", "ENABLED"}, true) and
                                                10 or 40))
        end
    end
end
