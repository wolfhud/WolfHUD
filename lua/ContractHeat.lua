...
function ContractBrokerHeistItem:init(...) -- parent_panel, job_data, idx
    ...
    if WolfHUD:getSetting({"INVENTORY", "SHOW_CONTRACTOR_JOB_HEAT"}, true) and
        job_tweak and job_tweak.contact ~= "skirmish" then
        local heat_text, heat_color = self:get_job_heat_text(
...
function ContractBrokerHeistItem:get_job_heat_text(job_id)
    ...
    local exp_multiplier = managers.job:heat_to_experience_multiplier(
    ...
    if exp_percent ~= 0 then
        ...
        heat_color = exp_percent > 0 and
                         tweak_data.screen_colors.heat_warm_color or
