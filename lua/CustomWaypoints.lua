if RequiredScript == "lib/managers/hudmanager" then
    ...
    function HUDManager:add_waypoint(id, data, ...)
        ...
        if wp and wp.bitmap and wp.distance and wp.arrow and data.distance then
            local color = WolfHUD:getColorSetting(
                              {"CustomWaypoints", "WAYPOINTS_COLOR"}, "white")
    ...
    HUDManager.CUSTOM_WAYPOINTS = {
        ...
        EQUIPMENT = {
            ammo_bag = {
                ...
                texture_rect = {1 * 64, 0, 64, 64},
                offset = Vector3(0, 0, 15)
            },
            doc_bag = {
                texture = "guis/textures/pd2/skilltree/icons_atlas",
                texture_rect = {2 * 64, 7 * 64, 64, 64},
                offset = Vector3(0, 0, 25)
            },
            first_aid_kit = {
                texture = "guis/textures/pd2/skilltree/icons_atlas",
                texture_rect = {2 * 64, 11 * 64, 64, 48},
                offset = Vector3(0, 0, 20)
            },
            body_bag = {
                texture = "guis/textures/pd2/skilltree/icons_atlas",
                texture_rect = {5 * 64, 11 * 64, 64, 64},
                offset = Vector3(0, 0, 25)
            },
            grenade_crate = {
                texture = "guis/dlcs/big_bank/textures/pd2/pre_planning/preplan_icon_types",
                texture_rect = {1 * 48, 0, 48, 48},
                offset = Vector3(0, 0, 20)
            },
            UPGRADES = {
                ammo_bag = {
        ...
        SPECIAL_EQUIPMENT = {
            ICON_MAP = {
                ...
                pickup_hotel_room_keycard = {
                    std_icon = "equipment_bank_manager_key"
                },
                ...
                muriatic_acid = {
                    ...
                    texture_rect = {1 * 32, 1 * 32, 32, 32},
                    offset = Vector3(0, 0, 45)
                },
                hydrogen_chloride = {
                    texture = "guis/textures/pd2/hud_pickups",
                    texture_rect = {2 * 32, 1 * 32, 32, 32},
                    offset = Vector3(0, 0, 85)
                },
                caustic_soda = {
                    texture = "guis/textures/pd2/hud_pickups",
                    texture_rect = {3 * 32, 1 * 32, 32, 32},
                ...
                gen_pku_thermite_paste_z_axis = {
                    std_icon = "equipment_thermite"
                },
                c4_consume = {
                    std_icon = "equipment_c4",
                    x_ray = true,
    ...
    function HUDManager:setup_custom_waypoints()
        if managers.gameinfo and managers.waypoints then
            local bag_equip_settings = {
                ["ammo_bag"] = WolfHUD:getSetting(
                    {"CustomWaypoints", "SHOW_AMMO_BAG"}, true),
                ["doc_bag"] = WolfHUD:getSetting(
                    {"CustomWaypoints", "SHOW_DOC_BAG"}, true),
                ["body_bag"] = WolfHUD:getSetting(
                    {"CustomWaypoints", "SHOW_BODY_BAG"}, true),
                ["grenade_crate"] = WolfHUD:getSetting(
                    {"CustomWaypoints", "SHOW_GRENADE_CRATE"}, true),
                ["first_aid_kit"] = WolfHUD:getSetting(
                    {"CustomWaypoints", "SHOW_FIRST_AID_KIT"}, false)
            }
            for equip, enabled in pairs(bag_equip_settings) do
                if enabled then
                    managers.gameinfo:register_listener(
                        equip .. "_waypoint_listener", equip, "set_active",
                        callback(self, self, "custom_waypoint_bag_clbk", equip))
                    managers.gameinfo:register_listener(
                        equip .. "_waypoint_listener", equip, "set_amount",
                        callback(self, self, "custom_waypoint_bag_clbk", equip))
                    managers.gameinfo:register_listener(
                        equip .. "_waypoint_listener", equip,
                        "set_amount_offset",
                        callback(self, self, "custom_waypoint_bag_clbk", equip))
                    managers.gameinfo:register_listener(
                        equip .. "_waypoint_listener", equip, "set_owner",
                        callback(self, self, "custom_waypoint_bag_clbk", equip))
                    managers.gameinfo:register_listener(
                        equip .. "_waypoint_listener", equip, "set_upgrades",
                        callback(self, self, "custom_waypoint_bag_clbk", equip))
                end
            end

            if WolfHUD:getSetting({"CustomWaypoints", "SHOW_SENTRIES"}, true) then
                managers.gameinfo:register_listener("sentry_waypoint_listener",
                                                    "sentry", "set_active",
                                                    callback(self, self,
                                                             "custom_waypoint_sentry_clbk"))
                managers.gameinfo:register_listener("sentry_waypoint_listener",
                                                    "sentry", "set_ammo_ratio",
                                                    callback(self, self,
                                                             "custom_waypoint_sentry_clbk"))
                managers.gameinfo:register_listener("sentry_waypoint_listener",
                                                    "sentry",
            ...
            if WolfHUD:getSetting({"CustomWaypoints", "SHOW_ECMS"}, true) then
                managers.gameinfo:register_listener("ecm_waypoint_listener",
                                                    "ecm", "set_jammer_active",
                                                    callback(self, self,
                                                             "custom_waypoint_ecm_clbk"))
                managers.gameinfo:register_listener("ecm_waypoint_listener",
                                                    "ecm",
                                                    "set_feedback_active",
                                                    callback(self, self,
                ...
                managers.gameinfo:register_listener("ecm_waypoint_listener",
                                                    "ecm",
                                                    "set_feedback_duration",
                                                    callback(self, self,
                                                             "custom_waypoint_ecm_clbk"))
                managers.gameinfo:register_listener("ecm_waypoint_listener",
                                                    "ecm", "set_jammer_battery",
                                                    callback(self, self,
                                                             "custom_waypoint_ecm_clbk"))
                managers.gameinfo:register_listener("ecm_waypoint_listener",
                                                    "ecm", "set_upgrade_level",
                                                    callback(self, self,
                                                             "custom_waypoint_ecm_clbk"))
            end

            if WolfHUD:getSetting({"CustomWaypoints", "SHOW_TIMERS"}, true) then
                managers.gameinfo:register_listener("timer_waypoint_listener",
                                                    "timer", "set_active",
                                                    callback(self, self,
                                                             "custom_waypoint_timer_clbk"))
                managers.gameinfo:register_listener("timer_waypoint_listener",
                ...
                managers.gameinfo:register_listener("timer_waypoint_listener",
                                                    ...
                                                    callback(self, self,
                                                             "custom_waypoint_timer_clbk"))
                managers.gameinfo:register_listener("timer_waypoint_listener",
                                                    "timer", "set_autorepair",
                                                    callback(self, self,
                                                             "custom_waypoint_timer_clbk"))
            end

            if WolfHUD:getSetting({"CustomWaypoints", "SHOW_MINIONS"}, true) then
                managers.gameinfo:register_listener("minion_waypoint_listener",
                                                    "minion", "add", callback(
                                                        self, self,
                                                        "custom_waypoint_minion_clbk"))
                managers.gameinfo:register_listener("minion_waypoint_listener",
                                                    "minion", "remove",
                                                    callback(self, self,
                                                             "custom_waypoint_minion_clbk"))
            ...
            if WolfHUD:getSetting({"CustomWaypoints", "LOOT", "SHOW"}, true) then
                managers.gameinfo:register_listener("loot_waypoint_listener",
                                                    "loot", "add", callback(
                                                        self, self,
                                                        "custom_waypoint_loot_clbk"))
                managers.gameinfo:register_listener("loot_waypoint_listener",
                                                    "loot", "remove", callback(
                                                        self, self,
                                                        "custom_waypoint_loot_clbk"))
                managers.gameinfo:register_listener("loot_waypoint_listener",
                                                    "loot", "interact",
            ...
            if WolfHUD:getSetting({"CustomWaypoints", "SHOW_PAGER"}, true) then
                managers.gameinfo:register_listener("pager_waypoint_listener",
                                                    "pager", "add", callback(
                                                        self, self,
                                                        "custom_waypoint_pager_clbk"))
                managers.gameinfo:register_listener("pager_waypoint_listener",
                                                    "pager", "remove", callback(
                                                        self, self,
                                                        "custom_waypoint_pager_clbk"))
                managers.gameinfo:register_listener("pager_waypoint_listener",
                                                    "pager", "set_answered",
            ...
            if WolfHUD:getSetting({"CustomWaypoints", "SHOW_SPECIAL_EQUIPMENT"},
                                  true) then
                managers.gameinfo:register_listener(
                    "equipment_waypoint_listener", "special_equipment", "add",
                    callback(self, self,
                             "custom_waypoint_special_equipment_clbk"))
                managers.gameinfo:register_listener(
                    "equipment_waypoint_listener", "special_equipment",
                    "remove", callback(self, self,
                                       "custom_waypoint_special_equipment_clbk"))
            end
        end
    end

    function HUDManager:add_custom_equip_waypoint(id, unit, position, texture,
                                                  texture_rect, text, color,
                                                  visible_angle,
                                                  visible_distance, fade_angle,
                                                  offset_position)
        local params = {
            unit = unit,
            position = position,
            ...
            label = {type = "label", show = true, text = text},
            debug_txt = {
                type = "label",
                show = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING,
                text = string.format("Editor ID: %s",
                                     (unit:editor_id() or "N/A"))
            },
        ...
        if event == "set_active" then
            if data.active then
                ...
                if icon_map then
                    ...
                    self:add_custom_equip_waypoint(id, data.unit, data.position,
                                                   icon_map.texture,
                                                   icon_map.texture_rect,
                                                   amount, Color.white,
                                                   {max = 32.5}, {max = 1000},
                                                   {
                        start_angle = 32.5,
                    ...
                    if data.owner then
                        self:custom_waypoint_bag_clbk(type, "set_owner", key,
                                                      data)
                    end
                    if data.upgrades then
                        self:custom_waypoint_bag_clbk(type, "set_upgrades", key,
                                                      data)
                    end
                end
            else
        ...
        elseif managers.waypoints:get_waypoint(id) then
            if event == "set_amount" or event == "set_amount_offset" then
                if data.amount or data.amount_offset then
                    ...
                    managers.waypoints:set_waypoint_label(id, "label",
                                                          tostring(amount))
                end
            elseif event == "set_owner" then
                if data.owner then
                    local peer_color = data.owner > 0 and
                                           tweak_data.chat_colors[data.owner]:with_alpha(
                                               1) or Color.white
                    managers.waypoints:set_waypoint_component_setting(id,
                                                                      "icon",
                                                                      "color",
                                                                      peer_color)
                end
            elseif event == "set_upgrades" then
                if data.upgrades then
                    for name, level in pairs(data.upgrades) do
                        if not managers.waypoints:get_waypoint_component(id,
                                                                         name) then
                            local upgrade_table =
                                HUDManager.CUSTOM_WAYPOINTS.EQUIPMENT.UPGRADES
                            local upgrade_data =
                                upgrade_table and upgrade_table[type] and
                                    upgrade_table[type][name]
                                ...
                                local component =
                                    {
                                        ...
                                        color = HUDManager.CUSTOM_WAYPOINTS
                                            .UPGRADE_COLORS[level] or
                                            Color.white,
                                        visible_distance = {max = 200}
                                    }
                                managers.waypoints:add_waypoint_component(id,
                                                                          name,
                                                                          component,
                                                                          {2, 1})
                            end
                        else
                            managers.waypoints:set_waypoint_component_setting(
                                id, name, "show", level > 0)
                            managers.waypoints:set_waypoint_component_setting(
                                id, name, "color", HUDManager.CUSTOM_WAYPOINTS
                                    .UPGRADE_COLORS[level] or Color.white)
                        end
                    end
                end
            end
    ...
    function HUDManager:custom_waypoint_sentry_clbk(event, key, data)
        ...
        if event == "set_active" or event == "set_owner" then
            if data.active then
                ...
                if peer and data.owner and data.owner == peer:id() then
                    local text = string.format("%.0f%%",
                                               (data.ammo_ratio or 0) * 100)
                    self:add_custom_equip_waypoint(id, data.unit, data.position,
                                                   "guis/textures/pd2/skilltree/icons_atlas",
                                                   {7 * 64, 5 * 64, 64, 64},
                                                   text, Color.green,
                                                   {max = 20}, {max = 2000}, {
                        start_angle = 20,
        ...
        elseif event == "set_ammo_ratio" then
            managers.waypoints:set_waypoint_label(id, "label", string.format(
                                                      "%.0f%%",
                                                      (data.ammo_ratio or 0) *
                                                          100))
        ...
        if event == "set_jammer_active" or event == "set_feedback_active" then
            if data.jammer_active or data.feedback_active then
                local params = {
                    ...
                    distance = {type = "distance", show = true, scale = 0.6},
                    component_order = {
                        {"icon"}, {"jammer_time", "feedback_time"}, {"distance"}
                    }
                ...
                if data.feedback_duration or data.feedback_expire_t then
                    self:custom_waypoint_ecm_clbk("set_feedback_duration", key,
                                                  data)
                end
        ...
        elseif managers.waypoints:get_waypoint(id) then
            if event == "set_jammer_battery" then
                if data.jammer_battery then
                    managers.waypoints:set_waypoint_timer(id, "jammer_time",
                                                          math.max(
                                                              data.jammer_battery,
                                                              0))
                    local max_duration =
                        tweak_data.upgrades.ecm_jammer_base_battery_life *
                            (data.upgrade_level and
                                ECMJammerBase.battery_life_multiplier[data.upgrade_level] or
                                1)
                    local color = math.lerp(Color.red, Color.green,
                                            data.jammer_battery / max_duration)
                    managers.waypoints:set_waypoint_component_setting(id,
                                                                      "jammer_time",
                                                                      "color",
                                                                      color)
            ...
            elseif event == "set_feedback_duration" then
                if data.feedback_duration or data.feedback_expire_t then
                    local duration = data.feedback_expire_t and
                                         (data.feedback_expire_t -
                                             Application:time()) or
                                         data.feedback_duration
                    managers.waypoints:set_waypoint_timer(id, "feedback_time",
                                                          duration, -1)
                end
    ...
    function HUDManager:custom_waypoint_timer_clbk(event, key, data)
        ...
        if event == "set_active" then
            local timer_data =
                HUDManager.CUSTOM_WAYPOINTS.TIMER.OVERRIDE_DATA[data.id] or {}
            if data.active and not timer_data.ignore then
                ...
                local params = {
                    ...
                    debug_txt = {
                        ...
                        show = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING,
                        text = string.format("Editor ID: %s",
                                             (data.unit:editor_id() or "N/A"))
                    },
                    component_order = {
                        {"icon"}, {"timer"},
                        {"speed_upgrade", "noise_upgrade", "restart_upgrade"},
                        {"debug_txt"}
                    }
                }

                managers.waypoints:add_waypoint(id, timer_data.class or
                                                    "CustomWaypoint", params)
        ...
        elseif managers.waypoints:get_waypoint(id) then
            if event == "update" then
                managers.waypoints:set_waypoint_timer(id, "timer",
                                                      data.timer_value)
            elseif event == "set_jammed" then
                if data.jammed then
                    managers.waypoints:set_waypoint_setting(id, "color",
                                                            HUDManager.CUSTOM_WAYPOINTS
                                                                .TIMER
                                                                .BROKEN_COLOR)
                    managers.waypoints:set_waypoint_setting(id,
                                                            "show_offscreen",
                                                            true)
                else
                    managers.waypoints:set_waypoint_setting(id, "color",
            ...
            elseif event == "set_upgrades" then
                if data.upgrades then
                    local speed_upgrade, noise_upgrade, restart_upgrade =
                        data.upgrades.faster or 0, data.upgrades.silent or 0,
                        data.upgrades.restarter or 0
                    managers.waypoints:set_waypoint_component_setting(id,
                                                                      "speed_upgrade",
                                                                      "show",
                                                                      (speed_upgrade >
                    ...
                    managers.waypoints:set_waypoint_component_setting(id,
                                                                      ...
                                                                      "color",
                                                                      HUDManager.CUSTOM_WAYPOINTS
                                                                          .UPGRADE_COLORS[speed_upgrade] or
                                                                          Color.white)
                    managers.waypoints:set_waypoint_component_setting(id,
                                                                      "noise_upgrade",
                                                                      "color",
                                                                      HUDManager.CUSTOM_WAYPOINTS
                                                                          .UPGRADE_COLORS[noise_upgrade] or
                                                                          Color.white)
                    managers.waypoints:set_waypoint_component_setting(id,
                                                                      "restart_upgrade",
                                                                      "color",
                                                                      HUDManager.CUSTOM_WAYPOINTS
                                                                          .UPGRADE_COLORS[restart_upgrade] or
                                                                          Color.white)
                end
            elseif event == "set_autorepair" then
                managers.waypoints:set_waypoint_component_setting(id, "icon",
                                                                  "color",
                                                                  data.auto_repair and
                                                                      HUDManager.CUSTOM_WAYPOINTS
                                                                          .TIMER
                                                                          .AUTOREPAIR_COLOR or
                                                                      Color.white)
    ...
    function HUDManager:custom_waypoint_minion_clbk(event, key, data)
        ...
        if event == "add" then
            local unit_tweak = data.unit:base() and
            ...
            local params = {
                ...
                kills = {
                    type = "label",
                    show = true,
                    text = string.format("%s %d", utf8.char(57364),
                                         data.kills or 0),
                    color = Color.white,
                    alpha = 0.8,
                    scale = 0.7
            ...
            if data.damage_resistance then
                self:custom_waypoint_minion_clbk("set_damage_resistance", key,
                                                 data)
            end
        elseif managers.waypoints:get_waypoint(id) then
                ...
                if data.health_ratio then
                    managers.waypoints:set_waypoint_component_setting(id,
                                                                      ...
                                                                      Color(
                                                                          ...
                                                                          1, 1))
                    local health_dmg =
                        managers.waypoints:get_waypoint_component(id,
                                                                  "health_dmg")
                    if alive(health_dmg) and data.health_ratio < 1 then
                        health_dmg:stop()
                        health_dmg:animate(
                            callback(self, self,
                                     "_animate_custom_waypoint_minion_damage"))
                    end
                end
            elseif event == "set_kills" then
                if data.kills then
                    managers.waypoints:set_waypoint_label(id, "kills",
                                                          string.format("%s %d",
                                                                        utf8.char(
                                                                            57364),
                                                                        data.kills))
            ...
            elseif event == "set_owner" then
                if data.owner then
                    local peer_color = data.owner > 0 and
                                           tweak_data.chat_colors[data.owner or
                                               5]
                    managers.waypoints:set_waypoint_setting(id, "color",
                                                            peer_color)
                end
            elseif event == "set_damage_resistance" then
                local max_mult = tweak_data.upgrades.values.player
                                     .convert_enemies_health_multiplier[1] *
                                     tweak_data.upgrades.values.player
                                         .passive_convert_enemies_health_multiplier[2]
                local alpha = math.clamp(
                                  1 - (data.damage_resistance - max_mult) /
                                      (1 - max_mult), 0, 1) * 0.8 + 0.2
                managers.waypoints:set_waypoint_component_setting(id,
                                                                  "health_shield",
                                                                  "alpha", alpha)
            end
    ...
    function HUDManager:custom_waypoint_loot_clbk(event, key, data)
        ...
        if event == "add" then
            if tweak_entry and not tweak_entry.is_vehicle and
                not tweak_entry.skip_exit_secure and
                (data.carry_id ~= "person" or managers.job:current_level_id() ==
                    "mad" and (data.bagged or data.unit:editor_id() ~= -1)) then
                local angle = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING and 180 or
                                  WolfHUD:getSetting(
                                      {"CustomWaypoints", "LOOT", "ANGLE"}, 25)
                local name_id = data.carry_id and
                                    tweak_data.carry[data.carry_id] and
                                    tweak_data.carry[data.carry_id].name_id
                local bag_name = name_id and
                                     managers.localization:to_upper_text(name_id)
                local count = data.count or 1
                if bag_name then
                    local params = {
                        unit = data.unit,
                        offset = data.bagged and Vector3(0, 0,
                                                         WolfHUD:getSetting(
                                                             {
                                "CustomWaypoints", "LOOT", "BAGGED_OFFSET"
                            }, 30)) or
                            Vector3(0, 0, WolfHUD:getSetting(
                                        {"CustomWaypoints", "LOOT", "OFFSET"},
                                        15)),
                        visible_through_walls = HUDManager.CUSTOM_WAYPOINTS
                            .DEBUGGING or data.bagged,
                        alpha = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING and 1 or
                            0.1,
                        visible_angle = {max = angle},
                        visible_distance = {
                            max = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING and
                                99999 or 2000
                        },
                        fade_angle = {
                            start_angle = angle,
                        ...
                        icon = {
                            type = "icon",
                            show = data.bagged and
                                WolfHUD:getSetting(
                                    {"CustomWaypoints", "LOOT", "ICON"}, true),
                            std_wp = "wp_bag",
                            alpha = 0.5,
                            on_minimap = true
                        },
                        ...
                        label = {type = "label", show = true, text = bag_name},
                        debug_txt = {
                            type = "label",
                            show = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING,
                            text = string.format("Editor ID: %s",
        ...
        elseif managers.waypoints:get_waypoint(id) then
            if event == "interact" then
                local count = data.count or 1
                managers.waypoints:set_waypoint_label(id, "amount",
                                                      string.format("%dx", count))
                managers.waypoints:set_waypoint_component_setting(id, "amount",
                                                                  "show",
                                                                  (count > 1))
    ...
    function HUDManager:custom_waypoint_special_equipment_clbk(event, key, data)
        ...
        if event == "add" then
            local icon_table = HUDManager.CUSTOM_WAYPOINTS.SPECIAL_EQUIPMENT
                                   .ICON_MAP
            ...
            if icon_data then
                local params = {
                    ...
                    hide_on_uninteractable = true,
                    visible_through_walls = HUDManager.CUSTOM_WAYPOINTS
                        .DEBUGGING or icon_data.x_ray or false,
                    scale = 1,
                    ...
                    visible_angle = {
                        max = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING and 180 or
                            35
                    },
                    visible_distance = {
                        max = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING and 99999 or
                            3000
                    },
    ...
    if CustomWaypoint and not MeltdownTemperatureWaypoint then
        MeltdownTemperatureWaypoint = MeltdownTemperatureWaypoint or
                                          class(CustomWaypoint)

        MeltdownTemperatureWaypoint.update_timer =
            function(self, name, value, t, dt)
                if self._settings[name] then
                    self._settings[name].value = value
                    if self._components[name] and self._settings[name].show then
                        self:set_label(name, string.format("%d/50",
                                                           math.floor(value)))
                    end
                end
            end
...
if RequiredScript == "lib/units/vehicles/vehicledrivingext" then
    ...
    function VehicleDrivingExt:add_loot(...)
        ...
        if managers.hud and self._unit:unit_data() and
            not self._unit:unit_data().name_label_id then
            self._unit:unit_data().name_label_id =
                managers.hud:add_vehicle_name_label(
                    {unit = self._unit, name = self._tweak_data.name})
        end
