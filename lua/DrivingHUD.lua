if string.lower(RequiredScript) == "lib/managers/hud/huddriving" then
    ...
    function HUDDriving:init(hud)
        ...
        self._panel = HUDDriving.VerticalListItem:new(self._hud_panel, self,
                                                      "base_panel",
                                                      HUDDriving._WIDTH,
                                                      HUDDriving._FONT_SIZE, {
            margin = HUDDriving._MARGIN
        })

        self._vehicle_name = HUDDriving.LegendItem:new(self._panel:panel(),
                                                       self._panel,
                                                       "vehicle_name",
                                                       HUDDriving._WIDTH,
                                                       HUDDriving._FONT_SIZE, {
        ...
        self._data = HUDDriving.HorizontalListItem:new(self._panel:panel(),
                                                       self._panel,
                                                       "data_panel",
                                                       HUDDriving._WIDTH,
                                                       HUDDriving.WIDTH, {})

        self._vehicle_image = HUDDriving.VehicleImageItem:new(
                                  self._data:panel(), self._data,
                                  "vehicle_image", HUDDriving._WIDTH / 2,
                                  HUDDriving._WIDTH / 2, {})
        self._legend = HUDDriving.VerticalListItem:new(self._data:panel(),
                                                       self._data,
                                                       "legend_panel",
                                                       HUDDriving._WIDTH / 2,
                                                       HUDDriving._WIDTH / 2, {})

        self._legend_health = HUDDriving.LegendHealthItem:new(
                                  self._legend:panel(), self._legend,
                                  "legend_health", HUDDriving._WIDTH / 2,
                                  HUDDriving._FONT_SIZE,
                                  {suffix = "HP", show_decimal_marks = true})
        self._legend_loot = HUDDriving.LegendImageItem:new(self._legend:panel(),
                                                           self._legend,
                                                           "legend_loot",
                                                           HUDDriving._WIDTH / 2,
                                                           HUDDriving._FONT_SIZE,
                                                           {
            texture = "guis/textures/wolfhud/drivinghud/legend",
            text_rect = {0, 0, 128, 128},
            show_max = true
        })
        self._legend_passengers = HUDDriving.LegendImageItem:new(
                                      self._legend:panel(), self._legend,
                                      "legend_passengers",
                                      HUDDriving._WIDTH / 2,
                                      HUDDriving._FONT_SIZE, {
                texture = "guis/textures/wolfhud/drivinghud/legend",
                text_rect = {128, 0, 128, 128},
                show_max = true
            })
        self._legend_gear = HUDDriving.LegendItem:new(self._legend:panel(),
                                                      self._legend,
                                                      "legend_gear",
                                                      HUDDriving._WIDTH / 2,
                                                      HUDDriving._FONT_SIZE, {})
        self._legend_speed = HUDDriving.LegendItem:new(self._legend:panel(),
                                                       self._legend,
                                                       "legend_speed",
                                                       HUDDriving._WIDTH / 2,
                                                       HUDDriving._FONT_SIZE,
                                                       {suffix = "km/h"})
        self._legend_rpm = HUDDriving.LegendItem:new(self._legend:panel(),
                                                     self._legend, "legend_rpm",
                                                     HUDDriving._WIDTH / 2,
                                                     HUDDriving._FONT_SIZE,
                                                     {suffix = "rpm"})

        self._legend:addComponents({
            self._legend_health, self._legend_loot, self._legend_passengers,
            self._legend_gear, self._legend_speed, self._legend_rpm
        })
        ...
        if not HUDManager.CUSTOM_TEAMMATE_PANELS then -- Arrange the position for non-CustomHUD
            local player_hud =
                managers.hud._teammate_panels[HUDManager.PLAYER_PANEL]
    ...
    function HUDDriving:stop()
        self._active = false
        if self._panel:set_enabled("in_vehicle", managers.player:get_vehicle()) and
            managers.hud then
            if HUDManager.CUSTOM_TEAMMATE_PANELS then -- CustomHUD
                managers.hud:arrange_teammate_panels()
            end
    ...
    function HUDDriving:apply_settings()
        ...
        self._legend_speed:set_text_suffix(
            settings.SPEED_IN_MPH and "mph" or "km/h")
        self._vehicle_image:set_enabled("setting",
                                        settings.SHOW_VEHICLE ~= false)
        ...
        self._legend_passengers:set_enabled("setting",
                                            settings.SHOW_PASSENGERS ~= false)
    ...
    function HUDDriving:panel() return self._panel end

    function HUDDriving:active() return self._active end

    function HUDDriving:set_vehicle_state(speed, rpm, gear, people,
                                          people_total, name, seats_table,
                                          loot_current, loot_total,
        ...
        if self._name ~= name then
            self._name = name
            self._vehicle_name:set_value(
                self._name and string.upper(tostring(self._name)))
    ...
    function HUDDriving.BasicItem:init(base_panel, owner, name, width, height,
                                       params)
        self._name = name
    ...
    function HUDDriving.BasicItem:resizable_w() return self._resizable_w end

    function HUDDriving.BasicItem:resizable_h() return self._resizable_h end
    ...
    function HUDDriving.BasicItem:arrange() end
    ...
    function HUDDriving.BasicItem:text_rect()
    ...
    function HUDDriving.BasicItem:set_center(x, y)
    ...
    HUDDriving.HorizontalListItem = HUDDriving.HorizontalListItem or
                                        class(HUDDriving.BasicItem)
    function HUDDriving.HorizontalListItem:init(base_panel, owner, name, width,
                                                height, params)
        HUDDriving.HorizontalListItem.super.init(self, base_panel, owner, name,
                                                 width, height, params)

        self._components = params.components or {}

        if next(self._components) ~= nil then self:arrange() end
    end
    ...
    function HUDDriving.HorizontalListItem:arrange()
        ...
        if self:set_size(w - self._margin, h) then self._owner:arrange() end
    end
    ...
    HUDDriving.VerticalListItem = HUDDriving.VerticalListItem or
                                      class(HUDDriving.BasicItem)
    function HUDDriving.VerticalListItem:init(base_panel, owner, name, width,
                                              height, params)
        HUDDriving.VerticalListItem.super.init(self, base_panel, owner, name,
                                               width, height, params)
    ...
    function HUDDriving.VerticalListItem:arrange()
        ...
        if self:set_size(w, h - self._margin) then self._owner:arrange() end
    end
    ...
    HUDDriving.VehicleImageItem = HUDDriving.VehicleImageItem or
                                      class(HUDDriving.BasicItem)
    function HUDDriving.VehicleImageItem:init(base_panel, owner, name, width,
                                              height, params)

        HUDDriving.VehicleImageItem.super.init(self, base_panel, owner, name,
                                               width, height, params)

        self._passengers = {}
        self._texture = params.texture or
                            "guis/textures/wolfhud/drivinghud/longfellow"
    ...
    function HUDDriving.VehicleImageItem:set_vehicle_name(name)
        if self._image and self._vehicle_name ~= name then
            ...
            if self._vehicle_table then
                self._texture = self._vehicle_table and
                                    self._vehicle_table.texture
                self._texture_rect = self._vehicle_table and
                                         self._vehicle_table.texture_rect
    ...
    function HUDDriving.VehicleImageItem:set_color(color)
        if self._image then self._image:set_color(color) end
    end
        ...
        if self._vehicle_table then
            ...
            for seat_name, seat in pairs(self._vehicle_table) do
                if seat.is_seat then
                    self._passengers[seat_name] =
                        self._passengers[seat_name] or
                            HUDDriving.VehiclePassengerItem:new(self._panel,
                                                                self, seat_name,
                                                                35 *
                                                                    HUDDriving._SCALE,
                                                                35 *
    ...
    function HUDDriving.VehicleImageItem:rescale(factor)
        if HUDDriving.VehicleImageItem.super.rescale(self, factor) then
            for name, passenger in pairs(self._passengers) do
                if passenger then passenger:rescale(factor) end
            end
        end
    end

    HUDDriving.VehiclePassengerItem = HUDDriving.VehiclePassengerItem or
                                          class(HUDDriving.BasicItem)
    function HUDDriving.VehiclePassengerItem:init(base_panel, owner, name,
                                                  width, height, params)

        HUDDriving.VehiclePassengerItem.super.init(self, base_panel, owner,
                                                   name, width, height, params)
    ...
    function HUDDriving.VehiclePassengerItem:set_passenger(unit)
        if unit and (not self._unit or self._unit:key() ~= unit:key()) then
            ...
            local character_data =
                managers.criminals:character_static_data_by_unit(unit)
            local mask_id = outfit and outfit.mask and outfit.mask.mask_id or
                                character_data and character_data.ai_mask_id or
                                "alienware"
            local color_id =
                managers.criminals:character_color_id_by_unit(unit) or 5
    ...
    function HUDDriving.VehiclePassengerItem:_set_passenger()
        if self._mask_id then
            ...
            local texture = tweak_entry and
                                tweak_entry[self._mask_id].custom_texture
            if tweak_entry and not texture then
                local bundle_folder = tweak_entry[self._mask_id] and
                                          tweak_entry[self._mask_id]
                ...
                local guis_catalog = string.format("guis/%s", bundle_folder and
                                                       string.format("dlcs/%s/",
                                                                     tostring(
                                                                         bundle_folder)) or
                                                       "")
                local texture_name = tweak_entry[self._mask_id] and
                                         tweak_entry[self._mask_id].texture_name or
                                         tostring(self._mask_id)
                texture = string.format("%s%s%s", guis_catalog,
                                        "textures/pd2/blackmarket/icons/masks/",
                                        texture_name)
            end
            self._image:set_image(texture)
        end
        if self._color then self._image:set_color(self._color) end
    end
    ...
    function HUDDriving.VehiclePassengerItem:arrange()
        ...
        local x, y = (parent_w / 2) * (self._scale_x + 1),
                     (parent_h / 2) * (self._scale_y + 1)

        self._panel:set_center(math.min(math.max(w2, x), parent_w - w2),
                               math.min(math.max(h2, y), parent_h - h2))
    end

    HUDDriving.LegendItem = HUDDriving.LegendItem or class(HUDDriving.BasicItem)
    function HUDDriving.LegendItem:init(base_panel, owner, name, width, height,
                                        params)

        HUDDriving.LegendItem.super.init(self, base_panel, owner, name, width,
                                         height, params)
        ...
        if self._value and tostring(self._value):len() > 0 then
            ...
            if self._max_value then
                if self._show_max_value then
                    ...
                    if self._show_decimal_marks then
                        max_str = managers.money:add_decimal_marks_to_string(
                                      max_str)
                    end
    ...
    HUDDriving.LegendHealthItem = HUDDriving.LegendHealthItem or
                                      class(HUDDriving.LegendItem)
    function HUDDriving.LegendHealthItem:init(base_panel, owner, name, width,
                                              height, params)
        HUDDriving.LegendHealthItem.super.init(self, base_panel, owner, name,
                                               width, height, params)
    ...
    function HUDDriving.LegendHealthItem:set_size(w, h)
        if HUDDriving.LegendHealthItem.super.set_size(self, w, h) then
            ...
            self._health_bar:set_w((self._health_shield:w() -
                                       (2 * self._health_offset)) *
                                       math.clamp(self._value_ratio, 0, 1))
            self._health_bar:set_h(self._health_shield:h() -
                                       (2 * self._health_offset))
    ...
    function HUDDriving.LegendHealthItem:arrange()
        ...
        if alive(self._health_bar) and alive(self._health_shield) then
            self._health_shield:set_center(self._panel:center())
            self._health_bar:set_left(self._health_shield:left() +
                                          self._health_offset)
    ...
    function HUDDriving.LegendHealthItem:_set_value()
        ...
        if self._value and self._max_value then
            ...
            if alive(self._health_bar) then
                self._health_bar:set_w(self._value_ratio *
                                           (self._health_shield:w() -
                                               (2 * self._health_offset)))
                self._health_bar:set_texture_rect(self._health_texture_rect[1],
                                                  self._health_texture_rect[2],
                                                  self._health_texture_rect[3] *
                                                      self._value_ratio,
                                                  self._health_texture_rect[4])
            end
    ...
    HUDDriving.LegendImageItem = HUDDriving.LegendImageItem or
                                     class(HUDDriving.LegendItem)
    function HUDDriving.LegendImageItem:init(base_panel, owner, name, width,
                                             height, params)
        HUDDriving.LegendImageItem.super.init(self, base_panel, owner, name,
                                              width, height, params)
...
elseif string.lower(RequiredScript) == "lib/states/ingamedriving" then
    ...
    function IngameDriving:_number_in_the_vehicle(vehicle_driving)
        ...
        for _, seat in pairs(vehicle_driving._seats) do
            total = total + 1
            if alive(seat.occupant) then used = used + 1 end
        end
        return used, total
    end
    ...
    function IngameDriving:_update_driving_hud()
        ...
        if vehicle and vehicle_unit then
            ...
            local no_used_seats, no_total_seats =
                self:_number_in_the_vehicle(vehicle_driving)
            local vehicle_name = vehicle_driving._tweak_data.name
            local seats_table = vehicle_driving._seats
            local health_total = math.min(
                                     vehicle_unit:character_damage()._current_max_health,
                                     999999999)
            local health_current = math.clamp(
                                       vehicle_unit:character_damage()._health,
                                       0, health_total)
            ...
            managers.hud:set_driving_vehicle_state(speed, rpm, gear,
                                                   no_used_seats,
                                                   no_total_seats, vehicle_name,
                                                   seats_table, loot_current,
                                                   loot_total,
                                                   math.max(0, health_current),
                                                   health_total)
        end
    end
...
elseif string.lower(RequiredScript) == "lib/managers/hudmanagerpd2" then
    ...
    function HUDManager:set_driving_vehicle_state(speed, rpm, gear, people,
                                                  people_total, name,
                                                  seats_table, loot_current,
                                                  loot_total, health_current,
                                                  health_total)
        self._hud_driving:set_vehicle_state(speed, rpm, gear, people,
                                            people_total, name, seats_table,
                                            loot_current, loot_total,
                                            health_current, health_total)
    end
