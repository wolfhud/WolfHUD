if RequiredScript == "lib/units/enemies/cop/copdamage" then
    ...
    function CopDamage:sync_damage_bullet(attacker_unit, damage_percent, i_body, ...)
        if i_body then self._sync_ibody_popup = i_body end
    ...
    function CopDamage:_process_popup_damage(data)
        CopDamage.DMG_POPUP_SETTING = WolfHUD:getSetting(
                                          {"DamagePopup", "DISPLAY_MODE"}, 2)
        ...
        if attacker and damage >= 0.1 and CopDamage.DMG_POPUP_SETTING > 1 then
            ...
            elseif attacker:in_slot(16) then
                ...
                local minion_data = managers.gameinfo and
                                        managers.gameinfo:get_minions(key)
                if minion_data then
                    -- Joker
                    killer = minion_data.owner and
                                 managers.criminals:character_unit_by_peer_id(
                                     minion_data.owner)
                else
                    -- Bot
                    killer = attacker
            ...
            if alive(killer) and alive(self._unit) then
                local body = data.col_ray and data.col_ray.body or
                                 self._sync_ibody_popup and
                                 self._unit:body(self._sync_ibody_popup)
                ...
                if CopDamage.DMG_POPUP_SETTING == 2 then
                    ...
                    local color_id =
                        managers.criminals:character_color_id_by_unit(killer)
                    if color_id then
                        self:show_popup(damage, self._dead, headshot, false,
                                        color_id)
                    end
                end
            end
    ...
    function CopDamage:show_popup(damage, dead, headshot, critical, color_id)
        if managers.waypoints then
            ...
            local waypoint_color = color_id and ((color_id == 5 and
                                       WolfHUD:getSetting(
                                           {
                    ...
                    }, Color.white) or tweak_data.chat_colors[color_id]) or
                                       WolfHUD:getColorSetting(
                                           {
                        "DamagePopup",
                        critical and "CRITICAL_COLOR" or headshot and
                            "HEADSHOT_COLOR" or "COLOR"
                    }, "yellow")
            waypoint_color = waypoint_color:with_alpha(
                                 WolfHUD:getSetting({"DamagePopup", "ALPHA"}, 1))
            local waypoint_duration = WolfHUD:getSetting(
                                          {"DamagePopup", "DURATION"}, 3)
            if waypoint and not waypoint:is_deleted() then
                managers.waypoints:set_waypoint_duration(id, "duration",
                                                         waypoint_duration)
                managers.waypoints:set_waypoint_label(id, "label",
                                                      self:build_popup_text(
                                                          damage, headshot))
                ...
                local params = {
                    unit = self._unit,
                    offset = Vector3(10, 10, WolfHUD:getSetting(
                                         {"DamagePopup", "HEIGHT"}, 20)),
                    scale = 2 * WolfHUD:getSetting({"DamagePopup", "SCALE"}, 1),
                    color = waypoint_color,
                    visible_distance = {min = 30, max = 10000},
                    rescale_distance = {
                        start_distance = 500,
                        end_distance = 3000,
                        final_scale = 0.5
                    },
                    fade_duration = {start = 0.5, stop = 1, alpha = true},
                    icon = {
                        type = "icon",
                        show = dead,
                        scale = WolfHUD:getSetting(
                            {"DamagePopup", "SKULL_SCALE"}, 1.2),
                    ...
                    component_order = {
                        WolfHUD:getSetting({"DamagePopup", "SKULL_ALIGN"}, 1) ==
                            1 and {"icon", "label"} or {"label", "icon"},
                        {"duration"}
                    }
                }
    ...
    function CopDamage:build_popup_text(damage, headshot, is_new)
        ...
        return math.floor(self._dmg_value) ..
                   ((CopDamage.DMG_POPUP_SETTING == 3 and headshot) and "!" or
                       "")
    end
